...
use but_grammar::ast::{
    EnumDefinition, FunctionDefinition, Identifier, Loc, ModelDefinition, PropertyDefinition,
    SourceUnit, StructDefinition, TypeDefinition, VariableDefinition,
};
...
impl TreeDefinition {
    pub(crate) fn new(
        ...
        let enum_table = build_map("enum", global_enums, diagnostics, |d| (d.name, d.loc));
        let property_table = build_map("property", global_properties, diagnostics, |d| {
            (d.name, d.loc)
        });
        let variable_table = build_map("variable", global_variables, diagnostics, |d| {
            (d.name, d.loc)
        });
        let struct_table = build_map("struct", global_structs, diagnostics, |d| (d.name, d.loc));
        let function_table = build_map("function", global_functions, diagnostics, |d| {
            (d.name, d.loc)
        });
        TreeDefinition {
            models,
...
#[inline]
fn build_map<Definition: Clone, F>(
    build_name: &str,
    values: Vec<Box<Definition>>,
    diagnostics: &mut Vec<Diagnostic>,
    d: F,
) -> HashMap<String, Box<Definition>>
where
    F: Fn(Definition) -> (Option<Identifier>, Loc),
{
        ...
        match name {
            ...
            Some(id) => {
                if table.contains_key(&id.name) {
                    ...
                    diagnostics.push(Diagnostic::error(
                        loc,
                        format!("{} '{}' already defined", build_name, id.name),
                    ));
                } else {
