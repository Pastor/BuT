...
# обязан приниматься инструментом целевой платформы. Соседняя проверка судит
# компиляцию целью `c` и симуляцию, проверка снимков сверяет их - но ни один не
...
# целым, пустая ветвь `match`, вложенный `if`,
...
# тем же приёмом, что у проверки примеров документа.
#
# Использование:
...
# Частота такта из объявления `clock`: без неё цель `c`
