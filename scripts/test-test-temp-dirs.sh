...
# --- Условие 2: общий литеральный каталог  ------------------------------
tree
for f in a b; do
...
# --- Условие 3: общий шаблон без имени потока  --------------------------
tree
for f in a b; do
...
# --- Условие 5: дубль тега в файле без имени потока  --------------------
tree
cat > "$TMP/tree/takt-sim/tests/conformance/a_tests.rs" <<'RS'
fn build_dir(tag: &str) -> std::path::PathBuf {
    std::env::temp_dir()
...
# --- Условие 6: каталог без идентификатора процесса  --------------------
#
...
# --- Условие 9: дерево без тестов - отказ ---------------------------------------
# Дерева Takt без тестовых файлов не бывает, и пустой обход означает не
# отсутствие работы, а сломанный обход: проверка ответила бы успехом, не
# прочитав ни строки.
tree
if run; then fail "дерево без тестов принято за успех"
else ok "дерево без тестов - отказ"; fi
