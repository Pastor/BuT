...
* `C7` - отсылка к решению, требованию, критерию или тесту карточки по коду
  (`A5`, `R4`, `T12`). Код живёт в документе и протухает так же, как номер.
...
Проверяется код: `*.rs`, `*.py`, `*.sh`, `*.js`, `*.mjs`, `*.css`, `*.html`.
Кроме строчных комментариев разбираются блочные и комментарии разметки. Каталоги сборки и
...
SUFFIXES = {".rs", ".py", ".sh", ".js", ".mjs", ".css", ".html"}

# Комментарий: маркер строки, строка блочного комментария или разметки.
...
def check(path: Path) -> list[str]:
    ...
    for no, raw in enumerate(text.splitlines(), 1):
        ...
        if DECISION.search(body):
            found.append(f"C7 {where}: отсылка к решению или требованию")
        if SAMPLE.search(body):
            found.append(f"C8 {where}: отсылка к образцу заимствования")
        if MEASURE.search(body):
            found.append(f"C9 {where}: датированный замер или проработка")
        plain = CODE.sub("", body)
