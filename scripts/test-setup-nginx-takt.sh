...
# Что доказывает:
#
# N1 сниппет несёт преисправление во всех местах (локейшен, редирект, заголовок);
# N2 **завершающей косой в `proxy_pass` нет** - преисправление обязан доехать до
...
# N6 негодный преисправление отвергается;
# N7 скрипт стенда знает про преисправление (`<префикс>/health`);
...
# -- N2 -----------------------------------------------------------------------
# Косая в `proxy_pass` срезает преисправление, а сервер вкладывает под него весь
