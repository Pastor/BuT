...
# E13 адрес модуля в описи сборки несёт отпечаток его содержимого: версия
#       крейта не меняется от выкладки к выкладке, а модуль меняется.
#
...
# -- E13: адрес модуля несёт отпечаток содержимого ----------------------------
# Версия крейта говорит, какой это компилятор, но не говорит, тот же ли это файл:
# между выкладками модуль меняется при неизменной версии, а путь помечен
# `immutable` на год. Тогда у читателя со вчерашней вкладкой новая страница зовёт
# операцию, которой в его кешированном модуле нет, и страница отвечает "модуль не
# загружен". Проверяется зависимость от содержимого, а не наличие запроса: при
# другом модуле обязан быть другой адрес.
WASM_URL="$(sed -n 's/.*"wasm": "\([^"]*\)".*/\1/p' "$STAND/version.json" | head -1)"
WASM_PATH="${WASM_URL%%\?*}"
WASM_TAG="${WASM_URL#*\?}"
VERSION="$(sed -n 's/.*"takt_lang": "\([^"]*\)".*/\1/p' "$STAND/version.json" | head -1)"
MANIFEST_SHA="$(sed -n 's/.*"sha256": "\([^"]*\)".*/\1/p' "$STAND/wasm/$VERSION/manifest.json" | head -1)"
if [[ "$WASM_URL" == "$WASM_PATH" ]]; then
  echo "  ПРОВАЛ: E13 адрес модуля '$WASM_URL' не несёт отпечатка — кеш переживёт смену модуля"
  exit 1
fi
[[ -f "$STAND/$WASM_PATH" ]] || { echo "  ПРОВАЛ: E13 модуля по адресу '$WASM_PATH' на стенде нет"; exit 1; }
STAND_SHA="$(shasum -a 256 "$STAND/$WASM_PATH" | awk '{print $1}')"
if [[ "$MANIFEST_SHA" != "$STAND_SHA" ]]; then
  echo "  ПРОВАЛ: E13 опись модуля называет не ту сумму, что у выложенного файла"
  exit 1
fi
if [[ "$WASM_TAG" != "${STAND_SHA:0:12}" ]]; then
  echo "  ПРОВАЛ: E13 отпечаток адреса '$WASM_TAG' не от содержимого модуля (${STAND_SHA:0:12})"
  exit 1
fi
echo "  OK: E13 адрес модуля несёт отпечаток содержимого ($WASM_TAG)"

# -- E14: модуль экспорта - под тем же правилом, что ядро ---------------------
# Второй модуль грузится по требованию, и адрес его обязан нести отпечаток, а
# подмена под адресом с версией - отвергаться, как у ядра.
EXPORT_URL="$(sed -n 's/.*"export_wasm": "\([^"]*\)".*/\1/p' "$STAND/version.json" | head -1)"
EXPORT_PATH="${EXPORT_URL%%\?*}"
[[ -n "$EXPORT_URL" && -f "$STAND/$EXPORT_PATH" ]] || {
  echo "  ПРОВАЛ: E14 опись не называет модуль экспорта либо его нет на стенде"
  exit 1
}
EXPORT_SHA="$(shasum -a 256 "$STAND/$EXPORT_PATH" | awk '{print $1}')"
[[ "${EXPORT_URL#*\?}" == "${EXPORT_SHA:0:12}" ]] || {
  echo "  ПРОВАЛ: E14 отпечаток адреса модуля экспорта не от его содержимого"
  exit 1
}
grep -q "\"sha256\": \"$EXPORT_SHA\"" "$STAND/wasm/$VERSION/manifest.json" || {
  echo "  ПРОВАЛ: E14 опись версии не несёт сумму модуля экспорта"
  exit 1
}
printf 'подмена' >> "$STAND/$EXPORT_PATH"
if "$ROOT/scripts/deploy-web.sh" -d "$STAND" --dry-run >/dev/null 2>&1; then
  echo "  ПРОВАЛ: E14 подмена модуля экспорта под тем же адресом прошла"
  exit 1
fi
echo "  OK: E14 модуль экспорта: адрес с отпечатком, сумма в описи, подмена отвергнута"
...
echo "  Сторож выкладки: все проверки пройдены (E1…E14)."
