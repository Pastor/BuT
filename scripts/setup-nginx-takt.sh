#!/usr/bin/env bash
#
# Настройка nginx под локейшеном /takt на общем стенде.
#
...
#
# Локейшенов от корня здесь нет. у образца они появились оттого, что его
...
locations_text() {
cat <<CONF
# Локейшены сервиса Takt под префиксом ${PREFIX}.
# Файл создан scripts/setup-nginx-takt.sh - правьте скрипт, не файл.
#
...
# Без завершающей косой в proxy_pass: префикс доезжает до сервера как есть.
