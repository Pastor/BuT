...
Правило записано в `docs/RULE.md` (правило 33); здесь оно проверяется машиной.
...
NUMBER = re.compile(
    r"\b(?:фич\w*|задач\w*|подзадач\w*|правил\w*|инвариант\w*|ADR)"
    r"\s+№?\s*\d{3,4}(?:-\d{2}[a-zа-я]*)?",
    re.I,
)
...
def main() -> int:
    ...
    if errors:
        ...
        print(
            ...
            "прописными и номеров (docs/RULE.md, правило 33).",
            file=sys.stderr,
        )
        return 1
