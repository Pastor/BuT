...
#
# Модуль лежит по адресу с версией (решение A5): публикация открывается
...
# Константа живёт в `takt-lang/src/version.rs` (там же её ищет проверка 0085), а
...
# заходу - лишняя работа (замер: 829 626 Б gzip). `brotli` берётся,
