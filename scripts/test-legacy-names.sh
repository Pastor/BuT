...
# Мутация: вносим старое имя, ждём падения.
#
...
# Исключение этого класса оставляет проверку с одним требованием: чтобы
...
# на любую строку с `q_`. Форма - ровно та, которую печатает цель.
...
# Класс, исключённый как "видит только разработчик инструмента", оставляет в
...
# любое имя с подчёркиванием. Формы - ровно те, что стоят в дереве.
