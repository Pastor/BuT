#!/usr/bin/env bash
# Сторож гейта каталогов сообщений (правило 0315, фича 0532, задача 0532-01).
#
...
#   M6 — согласованное дерево ПРИНИМАЕТСЯ (иначе гейт красен всегда);
#   M7 — вырожденный вход (пустой базовый каталог) даёт отказ, а не молчание.
#
# ⚠️ Проверяются ОБА условия — «плохое ловится» и «хорошее проходит»: гейт,
# который краснеет всегда, так же бесполезен, как молчащий.
#
# ⚠️ Мутации ставятся на КОПИИ дерева (`MSG_ROOT`), рабочие каталоги не
# трогаются.
set -u
...
# Готовит копию дерева: каталоги сообщений и исходники, по которым гейт судит
...
# Прогоняет гейт на копии и сверяет ожидание с кодом возврата.
expect() {
...
# корректном дереве гейт делает бессмысленными все прочие пробы.
...
# M7 — вырожденный вход: пустой базовый каталог обязан давать отказ. Гейт,
# молчащий на пустом файле, ничего не прочёл (урок задачи 0531-07a).
D=$(mktemp -d); prepare "$D"
