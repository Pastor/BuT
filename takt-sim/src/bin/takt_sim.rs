...
fn run(args: Args) -> Result<RunResult, String> {
    ...
    // же, что у целей компиляции: правило одно, и разойтись двум ответам на один вход
    // не по чему. Прежде симулятор такой файл принимал и рапортовал "Завершено: модель
...
fn extract_port_names(model: &takt_lang::semantic::ModelNode) -> PortNames {
    // 0079: сбор рекурсивен (включая под-модели композиции) - вынесен в библиотеку
