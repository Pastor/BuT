...
mod tests {
    ...
    /// 1.5 в q(8, 8) есть представление 384; складываются представления.
    #[test]
    ...
    /// Произведение -1.5 на 2.0 в q(8, 8) даёт -3.0, то есть представление -768.
    /// Округление floor к минус бесконечности проверяется именно на отрицательном: на
    ...
    /// Переполнение `+` даёт перенос, а не насыщение.
    #[test]
