...
//! вариантам перечислений: новый вариант `Value`/`TypeNode`/`BinOp` **ломает сборку**,
//! а не превращается молча в "невычислимо". Корневой причиной была именно ветка `_ =>
//! None`. Линт настроен **точечно** на модуле - как предписывает `docs/CODE.md`
//! (запрещая при этом `#![deny(warnings)]` на крейт). **Снятие этого `deny` (или
//! пометка узла `#[non_exhaustive]`) ловит проверка** `scripts/check-exhaustive-nodes.sh`:
//! оба пути отключили бы инвариант молча, сборка бы прошла. Не снимать.
//!
...
//!   типу. Вызывается на месте присваивания, где известен тип
//!   цели.
...
//! (продвижение операндов до `int`, сужение при присваивании), что и требует критерий
//! A8.
