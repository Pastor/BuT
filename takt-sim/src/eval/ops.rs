...
//! требует знания объявленного типа переменной (S3, S4a, S5, S8). Правила, зависящие от
//! типа назначения (S1, S2, S6, S9), живут в [`crate::eval::coerce_to_type`] и
...
/// приводятся: неявное приведение скрыло бы ошибку типов (S8).
...
fn equality(op: BinOp, lhs: &Value, rhs: &Value, negate: bool) -> Result<Value, EvalError> {
    let equal = match (lhs, rhs) {
        ...
        // Массивы и смешение bool с числом (S8) - ошибка, а не тихое `false`: тихое
