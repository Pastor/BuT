...
///
/// Индекс уже **вычислен** адаптером (у него есть контекст) в `usize` - ядро АСД не
/// знает. Поле - имя, как в структурах 0034. Номер разряда - литерал
...
///
/// Лист приводится к объявленному типу (усечение S9): `p.y := 300` при `y: u8` даёт
/// `44`; `data[0] := 300` при `[u8;4]` - `44` (T22 действует и в элементе).
