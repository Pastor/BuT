...
pub(crate) trait Context {
    ...
    ///
    /// Включает значения родительских контекстов: у параллельных под-моделей родитель
    /// общий, и его переменные - часть их состояния. Константы и локальные переменные не
    /// включаются. Реализация по умолчанию пуста - её достаточно для контекстов без
    ...
    /// Записывает вывод программы: строку встроенной функции `debug`.
    ///
    /// Это вывод модели, а не сообщение инструмента, поэтому канал у него свой -
    /// смешай его с предупреждениями, и отладочная печать автора показывалась бы
    /// как замечание к его же модели.
    ///
    /// Берёт `&self` по той же причине, что и запись состояния: вычисление идёт по
    /// неизменяемой ссылке, и буфер живёт за `RefCell`.
    fn emit_output(&self, _text: &str) {}

    /// Забирает накопленный вывод программы; буфер остаётся пустым.
