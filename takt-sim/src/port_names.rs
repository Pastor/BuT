//! Реестр имён портов и переменных модели (вынесено из `runner.rs`).
//!
//! Отдельный модуль не "чтобы влезло в лимит", а по границе ответственности: здесь -
//! **какие имена есть у модели и её под-моделей**, в `runner.rs` - **как идёт прогон**.
//! Сценарий адресует порты именами, и реестр нужен вне прогона: его зовут бинарник и
//! тесты.
//!
...
impl PortNames {
    ...
    ///
    /// 0079: обход **рекурсивный** - прежде перечислялся только корень, поэтому порты
    /// под-моделей композиции (`Cabin | Motor`) не попадали в драйвер/ дисплей и не
    /// подавались из sim-файла (модель "не реагировала на датчики"). Читать их
    /// симулятор мог (`Unit::get_value` обходит все ветви), а подать вход было нечем.
    /// Одноимённые порты разных под-моделей делят значение (плоское пространство имён;
    /// дедуп по имени).
