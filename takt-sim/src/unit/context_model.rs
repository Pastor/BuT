...
//!
//! Контекст отвечает на вопрос "где живёт значение", тогда как строитель - на вопрос
//! "из чего собран юнит".
...
pub(crate) struct ModelNodeContext {
    ...
    ///
    /// Поле открыто крейту: строитель юнита сбрасывает кэш при записи значения - иначе
    /// следующее чтение вернуло бы старое.
