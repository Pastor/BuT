...
//!
//! Модуль дочерний по отношению к `unit`, поэтому приватные поля `UnitKind::Node`
//! (`state_every` и `every_consumed`) ему видны. Скрытое состояние `every_consumed` -
//! эталон для целей: тело срабатывает, когда прошедшее с входа время догоняет очередной
//! период.
