...
#[path = "clock.rs"]
mod clock;
/// Контекст модели - вынесен из `builder` (предел размера модуля).
mod context_model;
mod every;
mod initial;
...
/// быть напечатанной и забытой (требование R5 ).
...
pub enum TickResult {
    ...
    /// (требование R5): раньше и то и другое давало `false`.
    Failed(String),
}
...
pub(crate) enum UnitKind {
    ...
    Node {
        ...
        /// под-моделей композиции неразличимы, чтение находило первую ветвь, а запись
        /// расходилась по всем. Имя берётся из `ModelNode::name`; у анонимной модели
        /// его нет - тогда квалифицированная адресация к этому узлу невозможна, а голая
        /// работает как прежде.
        ...
        /// поэтому шаги `+` делят переменные между собой и с наблюдателем (эталон -
        /// shared-переменные цели `c`).
        ...
        ///
        /// В жёстком режиме (умолчание, 0044) нарушение останавливает прогон и сюда не
        /// пишется. В мягком (`tick_soft`) - накапливается за такт и сливается
        /// `runner`'ом (`take_invariant_violations`, зеркало `take_last_transitions`),
        /// который тегирует номером шага. Каждый такт поле опустошается сливом.
...
impl Context for Unit {
    ...
    fn get_value(&self, name: &str) -> Option<Value> {
        // Квалифицированное имя `Модель::порт` адресует одну ветвь. Голое имя работает
        // как прежде (первая нашедшаяся ветвь) - правка аддитивна: сценарии, состояния
        // и тесты на голых именах не ломаются.
        if let Some((model, port)) = split_qualified(name) {
            return self.get_qualified(model, port);
        }
        match &self.0 {
            UnitKind::None => None,
            // 0032: единственный источник истины - контекст модели. Собственной карты
            // значений у узла больше нет; читаем оттуда же, куда пишем.
            UnitKind::Node {
                context,
                state,
                state_impls,
            ...
            // шагу: прежде чтение обрывалось на `None`, и значение, доведённое
            // композицией до конца, пропадало из трассы.
    ...
    fn set_value(&mut self, name: &str, value: Value) {
        ...
        match &mut self.0 {
            UnitKind::None => {}
            // 0032: запись идёт в контекст модели тем же путём, что присваивание в теле
            // блока. Shared-переменные уходят по цепочке `parent` в общий родительский
            // контекст - прежний широковещательный путь через собственную карту узла
            // упразднён.
            UnitKind::Node {
                context,
                state,
...
impl Unit {
    ...
    ///
    /// Симметричен [`variable`](Self::variable): раньше запись была доступна только
    /// через трейт `Context`, то есть требовала тащить внутренний трейт в каждый
    /// вызывающий модуль. Имя может быть **квалифицированным** (`Модель::порт`) - тогда
    /// запись идёт ровно в одну ветвь композиции; голое имя рассылается всем ветвям,
    /// как прежде.
