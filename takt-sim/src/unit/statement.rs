...
///
/// ⚠️ **Под `Ok(None)` остаётся ровно ОДНА законная форма** — запись среза
/// (`arr[0:2] := 1`), которую не переводит ни один потребитель (`CC-022`,
/// `RS-011`, `ST-011`, `SV-002`). Запись бита исполняется с фичи 0250, а всё,
/// что не есть место записи вовсе, отвергает семантика (`SE-111`, `SE-112`)
/// ещё до прогона.
fn resolve_place<'a>(
    ...
    match lhs {
        ...
        // Запись одного разряда (фича 0250): `flags.3 := 1`. Сегмент кладётся
        // тем же порядком, что поле и индекс, — правило замены живёт в ядре
        // (`eval/place.rs`), адаптер только раскладывает путь.
    ...
    match expr {
        ExpressionNode::Assign(lhs, rhs) => {
            ...
            let Some(var_rc) = resolve_place(lhs, &mut path, ctx)? else {
                // ⚠️ Текст называет ЕДИНСТВЕННУЮ оставшуюся форму. Прежде он
                // перечислял три законных места и умалчивал о бите, срезе,
                // порте и ячейке — то есть называл автору не ту причину; фича
                // 0249 сузила его до двух форм, 0250 исполнила запись бита.
                // Срез не переводит ни один потребитель, поэтому отказ эталона
                // идёт заодно с целями, а не вместо них.
                return Err(Diagnostic::error(
                    loc_of_assign(lhs),
...
pub(crate) fn call_function(
    ...
    let (params, ret, body, loc, name) = {
        ...
        match &*borrowed {
            ...
            // Встроенные функции (фича 0248). Вычисление живёт в `eval::builtin`
            // и идёт через ТЕ ЖЕ операции, что исполняют `<` и унарный минус в
            // теле модели, — иначе эталон разошёлся бы с целями на q-формате,
            // насыщении и обёртке.
            //
            // ⚠️ `debug` сюда не доходит: его перехватывает вычислитель
            // выражений ДО вычисления аргументов (строковый литерал значением
            // не представим). Остаётся `S(Модель)` в позиции выражения — её не
            // переводит ни одна цель (`CC-022`, `SV-002`, отказ у `rust`),
            // поэтому эталон отказывает тоже: симулятор, «умеющий больше»
            // целей, дал бы ложную уверенность (урок 0034).
            FunctionDefinitionNode::Builtin(name, _, _) => {
                ...
                // ⚠️ Ветвь НЕДОСТИЖИМА из корректной программы, и это проверено
                // (фича 0247): единственная невычислимая встроенная — `S`, а её
                // аргумент-модель отвергает вычислитель выражений раньше
                // (`модель как выражение`); цель `c` тот же вход отвергает
                // `CC-022`. Отказ оставлен страховкой — как `CC-023` у цели `c`:
                // молчаливое значение развело бы трассу эталона с прошивкой.
                //
                // ⚠️ Достижима она была через вызов с НЕВЕРНЫМ числом
                // аргументов (`min(1)`): арность не проверял никто, и текст,
                // говорящий про `S(Модель)`, объяснял автору **чужой** случай
                // (класс 0231). Вход закрыт фичей 0313 (`SE-122`), а сообщение
                // больше не называет причину, которой может не быть.
                return Err(Diagnostic::error(
                    Location::Builtin,
                    format!(
