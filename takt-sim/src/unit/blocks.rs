//! Именованные блоки уровня модели: `always` вне состояния.
//!
//! # Контракт
//!
//! Тело `always` уровня модели исполняется каждый такт до диспетчеризации состояния и
//! безусловно по состоянию; тому же контракту следуют все четыре цели. Исполняет его
//! `Unit::execution("always")`, вызываемый из `tick_body` до `tick_node`,
//! `tick_parallel` и `tick_sequential`, - то есть порядок задан тактом, а не этим
//! модулем.
//!
