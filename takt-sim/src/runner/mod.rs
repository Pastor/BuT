...
pub enum RunResult {
    ...
    ///
    /// Отличает сломанную модель от честно неактивного перехода (R5 ): раньше ошибка
    /// вычисления сводилась к `false` и была неотличима.
...
pub struct SimulationRunner {
    ...
    /// останова. Умолчание - `false` (жёсткий режим 0044).
...
impl SimulationRunner {
    ...
    /// Включает/выключает мягкий режим инвариантов. По умолчанию выключен (жёсткий
    /// режим 0044 - совпадает с `assert()` в C).
    ...
    fn warn_positional_form_once(&self) {
        ...
        // `"XX-NNN"`, и код, вплавленный в сообщение, для него невидим - то есть
        // выпадает и из реестра диагностик. Соседи `SIM-030`...`SIM-032` живут так с
        // 0132 и в реестре отсутствуют (вынесено кандидатом).
    ...
    fn check_port_name(
        ...
        // строковыми литералами `"XX-NNN"`, и вплавленный в текст код для него не
        // существует - то есть выпадает и из реестра диагностик, причём зелёным
        // прогоном (с обеих сторон пусто). Так эти три кода прожили от 0132 до 0255;
        // сегодня возврат к вплавленной форме ловит условие 5 того же проверки.
