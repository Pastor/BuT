//! Сверка эталона с целью `c`: присваивание среза массива.

use std::path::Path;
use std::process::Command;

const FIXTURE: &str = "tests/data/eval/conformance_slice.takt";
const TICKS: usize = 5;
...
fn generated_c_trace(dir: &Path) -> Vec<(i128, i128)> {
    ...
    let harness = format!(
        r#"#include <stdio.h>
#include "conformance_slice.h"

static long last_lo, last_hi;

static void write_numeric(ConformanceSlice_Out_NumericPort port, uint8_t index, int64_t val, void *ud) {{
    (void)index;
    (void)ud;
    if (port == CONFORMANCE_SLICE_PORT_LO) last_lo = (long)val; else last_hi = (long)val;
}}

int main(void) {{
    ConformanceSlice m = {{0}};
    ConformanceSlice_init(&m);
    m.write_numeric = write_numeric;
    m.userdata = 0;
    for (int i = 0; i < {TICKS}; i++) {{
        ConformanceSlice_tick(&m);
        printf("%ld %ld\n", last_lo, last_hi);
    }}
    return 0;
}}
"#
    ...
    let compile = Command::new("cc")
        .args(["-std=c11", "-I"])
        .arg(dir)
        .arg(dir.join("conformance_slice.c"))
...
/// Трасса совпадает у эталона и цели `c` при присваивании среза.
#[test]
fn slice_traces_match() {
    if !cc_available() {
        eprintln!("[ПРОПУСК] slice_traces_match: `cc` не найден");
        return;
    }
    let dir = std::env::temp_dir()
        .join(format!("takt_pid{}", std::process::id()))
        .join("takt_0355_slice");
    ...
    assert_eq!(
        reference,
        vec![(3, 13), (5, 23), (8, 33), (12, 43), (12, 43)],
