...
//!
//! Исправлениетура держит **вторую** константу (`BUMP`), используемую и в приведении, и
//! обычным путём: она обязана остаться в выводе - иначе правка читалась бы как
//! "константы под приведением выбрасываются всегда".
//!
//! Наблюдаемые `as u8` грубы, поэтому исправлениетура сверяет ещё и пути: `eq_v` сравнивает
