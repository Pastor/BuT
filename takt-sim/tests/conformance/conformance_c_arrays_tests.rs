...
//!
//! Отдельный файл по предмету: здесь сверка массивов, а помощники приходят
//! CLAUDE.md; границы модулей = границы ответственности). Здесь - только сверка
//! исполнения массивов симулятором с эталоном C.
//!
//! `Array` **сверяется**: эталон исполняет запись в элемент (`data[i] := v`)
//! и список-инициализатор `{...}`, значения совпадают с порождённым C. Тест пришёл на
//! смену тесту `a9_bit_and_array_conformance_gap`, который фиксировал препятствие
...
/// `Array` **сверяется** с порождённым C.
///
/// Без исполнения массивов эталоном сверки нет вовсе, и разрыв покрытия
...
/// 0`) и `[bit;N]` в объём не входят; C сам scalar-init отвергает (CC-017), поэтому
/// эталона у него нет.
///
