//! Потактовая сверка: повторяющийся образец `match`.
...
fn generated_rust_trace(dir: &Path) -> Vec<i128> {
    ...
    let module = dir.join("conformance_duplicate_arm.rs");
    let driver = format!(
        r#"#[path = "{module}"]
mod generated;
use generated::{{ConformanceDuplicateArm, Hal, OutU8Port}};
...
fn main() {{
    ...
    let mut model = ConformanceDuplicateArm::new(Probe {{ reg: Rc::clone(&reg) }});
    model.init();
    for _ in 0..{TICKS} {{
        model.tick();
...
/// Предмет: исполняется первая ветвь, а не вторая с тем же образцом.
#[test]
fn first_matching_arm_wins() {
    let sim = simulator_trace();
    // Числа названы явно: "трассы совпали" ничего не стоит, если обе стороны ошиблись
    // одинаково. Приращения различны намеренно.
    assert_eq!(
        sim,
        vec![1, 101, 101],
        "первая ветвь даёт +1; вторая (та же метка) недостижима, третья даёт +100"
    );
    ...
    let dir = std::env::temp_dir()
        ...
        .join(format!(
            "takt_conformance_duplicate_arm_{}",
            std::thread::current()
                .name()
                .unwrap_or("t")
                .replace(':', "_")
