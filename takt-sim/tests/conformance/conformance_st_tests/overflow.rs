...
//! самостоятельный предмет со своей исправлениетурой, и держать его в общем файле значило лишь
//! набирать строки (правило размера модуля).

use super::*;

/// Потактовая трасса `t` симулятора на исправлениетуре переполнения.
