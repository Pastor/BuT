//! Сверка симулятора с RTL, порождённым `taktc -t sv` (критерий A10 ).
//!
...
//!
//! Имя сигнала - с преисправлением уровня (`conformance_ticks_counter_n`): модуль SV один на
...
/// Исправлениетура потактовой сверки: `n` принимает 1 -> 2 -> 3 по мере переходов.
///
...
///
/// `signals` - имена **сигналов SV** (с преисправлением уровня), а не имена Takt.
///
...
/// `conformance_sv_time_tests` (тот же преисправление) означало бы `remove_dir_all` чужого
...
/// Пишет временную исправлениетуру и возвращает путь к ней.
...
/// нужна как **пиннинг правила**: она исправлениеирует, что эталон-симулятор именно
...
// -- Сверка `+` - Тройная: sim == C == SV (0057, доведена ) ------------
//
...
/// T10/A4 (цель sv, **ради которой фича**): побитовая потактовая сверка Q-арифметики с
...
/// T4/T7/A2/A5 (цель sv, **ради которой **): под `--float-as-q=8.8` `float` -> q(8,8),
...
/// T9/A1 (тест направления): без флага `float` в цели sv остаётся `SV-003`. Это и
...
/// **0063 (A2/A4): `en` проверкаит шаг, но не сброс - доказано симуляцией.**
///
...
"#;
    ...
    // A4: сброс сработал при en=0 - n вернулся к сбросовому 0.
    assert_eq!(
        rst, 0,
