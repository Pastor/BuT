...
//! исключён отображением `bit` -> `int` у цели `c`. В RTL `bit` -> `logic`, то есть
...
/// 110.6 с против 316 с. Помощник один на три вызова: порознь они
...
///
/// Утверждение "тесты идут однопоточно" здесь неверно:
...
///
/// В C-сверке он был исключён: генератор даёт `bit` -> `int`, и
...
/// **Последовательная композиция `A + B` - sim == C == SV.**
///
...
/// **Параллельный шаг `(B | C)` внутри цепочки `+` - sim == C == SV.**
///
...
// -----------------------------------------------------------------------------
// Q-арифметика fixed-point для цели sv
//
...
// доказывают синтезируемость, сверка - верность, включая floor к минус бесконечности.
...
// -----------------------------------------------------------------------------
// Прозрачный float -> q(m, n) для цели sv
//
...
/// **`en` ограничивает шаг, но не сброс - доказано симуляцией.**
///
...
///   одним фронтом: сброс обязан сработать при `en = 0`.
///
/// Утверждения: `unc == en1` **побитово** на каждом такте (умолчание - `1`); `en0 =
...
// Перенос q к ширине формата - подмодулем по предмету, а
