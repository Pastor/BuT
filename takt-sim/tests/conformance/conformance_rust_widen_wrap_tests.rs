//! Потактовая сверка значений: беззнаковое накопление с расширением.
...
///
/// Драйвер собирается без `-O`: проверки переполнения включены именно там, и потерянная
/// обёртка обязана проявиться падением, а не тихой обёрткой релиза.
...
fn widening_accumulation_wraps_like_the_reference() {
    ...
    // Значения названы явно: "трассы совпали" ничего не стоит, если обе стороны считают
    // одинаково неверно.
    assert_eq!(
        sim,
