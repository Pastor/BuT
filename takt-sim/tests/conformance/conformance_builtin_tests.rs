...
//! границы (`min` вместо `max`) видны сразу - именно так и выглядит самая вероятная
//! ошиблась, и поймала это проба, а не чтение.
//!
//! **Q-формат проверяется отдельным входом:** у `q(m, n)` сравнение идёт по
//! представлению, а унарный минус несёт нормировку переноса и насыщения. Считай
//! эталон встроенные своей арифметикой, а не общим слоем,
...
/// **`min`/`max`/`abs`/`clamp` над целыми - эталон == цель `c`.**
#[test]
...
/// **Те же встроенные над `q(8, 8)` - эталон == цель `c`.**
///
...
/// **`debug` не останавливает прогон** (до фичи он давал `SIM-020`).
#[test]
