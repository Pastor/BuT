...
/// mod 256 = 44): переполнение в сверке намеренно, это норма языка.
const TICKS: usize = 6;
...
/// Ожидаемая трасса общей суммы: обе настройки применены, обёртка `u8` - норма языка.
...
/// Наблюдение уходит из HAL общей ячейкой: поле `hal` модели приватно и
