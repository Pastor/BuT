//! Сверка режимов генерации параметров -, (требование R11, критерий A8 анализа).
//!
//! # Что доказывается
//!
...
/// Исправлениетура, в которой **обе** настройки влияют на одно наблюдаемое значение.
///
...
/// `FEATURES.md`; исправлениетура на него не опирается.
...
fn specialized_constants_drive_generated_rust() {
    ...
    // Драйвер принадлежит проверке, а не продукту (решение 0050): поля модели приватны,
...
/// "Локальная копия" (A6) держится в **обоих** режимах: экземпляр меняет свой параметр -
