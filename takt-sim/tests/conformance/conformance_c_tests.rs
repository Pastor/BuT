...
//! прежнему тесту `a9_bit_and_array_conformance_gap`, фиксировавшему
//! препятствие `SIM-017`).
//!
...
fn sim_value(unit: &Unit, name: &str) -> i128 {
    match unit.variable(name) {
        ...
        // вещественному приближению - побитово.
...
/// Порождает C для фикстуры с отрицательным перечислением, собирает и возвращает
...
"#
    ...
    let compile = Command::new("cc")
        // `-Werror=tautological-constant-out-of-range-compare`: без правки `cc`
...
///
/// **Мутация:** вернуть расчёт цели `c` по `max` (беззнаковый) -> C даст `lv =
...
// ----------------------------------------------------------------------------- // сверка по `Rational`
//
// Сужение   ("не для Rational/Bit/Array") снято **частично**: по `Rational`
// сверка теперь возможна, потому что сменила отображение `float` -> `double`, и обе
// стороны считают в IEEE 754 binary64.
//
// Про `Array` - см. `array_element_matches_generated_c`; про
// `[bit;N]` - шапку модуля: он вне сверки, это вопрос семантики.
...
/// Значения `Rational`: симулятор (f64) = порождённый C (`double`).
///
...
/// Значения зафиксированы вручную - страховка от "сверки пустоты".
///
/// Значения **захвачены зондом**, а не угаданы. `sum` и
...
/// стала бы красной. Именно поэтому фикстура построена на них, а не на "круглых"
/// числах.
#[test]
...
// Потактовая сверка трасс
//
// Без выравнивания тактов сверка вынуждена сравнивать только установившиеся значения: порождённый
...
/// Потактовые трассы симулятора и порождённого C совпадают целиком, а не только
/// в установившемся состоянии. Значения также **пришпилены**: если обе стороны
...
// Переполнение беззнакового целого - в подмодуле по предмету, а
...
// Перенос q к ширине формата - тем же приёмом подмодуля по
// размера.
...
/// Чисто структурная обёртка (`model Mid { start M = Counter; }`) не меняет
...
/// Побитовая потактовая сверка Q-арифметики с симулятором -
/// **включая отрицательные** значения и floor к −∞ у `*` (на S2 усечение к
...
/// Переполнение `+` над q - wraparound (перенос), не насыщение.
#[test]
...
/// Под `--float-embedded` `float` -> q(8,8),
...
// conformance_float_modes_tests.rs.
