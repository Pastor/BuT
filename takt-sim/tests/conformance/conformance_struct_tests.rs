...
//!
//! Область сверки - как у `conformance_c_tests.rs` (A8 ) - сужена до
//! полей `Integer` (для `[T;N]`/`bit`/`float` эталон C дефектен).
//! Исправлениетура **без инициализатора структуры**: `var p: Point := {1,2}` порождает
...
/// T21 (A9): финальные значения полей структуры совпадают у симулятора и порождённого
/// C. T22 (A9): значения в диапазоне полей (`x: u8 = 7`, `y: u16 = 300`).
#[test]
