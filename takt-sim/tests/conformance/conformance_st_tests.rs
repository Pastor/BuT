...
/// (генерация 0048 + правила именования iec2c), поэтому для этой исправлениетуры исправлениеируются
/// здесь.
...
/// исправлениетуре одноимённых `fn`: модель `B` обязана дать `wb = 3` (своей функцией
...
/// Потактовая трасса `acc` (repr q(6,6)) симулятора на w12-исправлениетуре.
...
/// Потактовая трасса `acc` порождённого ST на w12-исправлениетуре (через iec2c -> C).
...
fn fixed_point_arithmetic_matches_generated_st() {
    ...
    //
    // Прежняя редакция сверяла `st.ends_with(&sim)` с нулевым преисправлением - то есть
...
/// Корень исправлениетуры времени в C-символах iec2c (верхний регистр от basename).
