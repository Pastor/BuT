...
/// Фикстура переноса к `W = 12` при типе хранения 16 бит (фикс 0061-01).
...
/// Фикс 0061-01 (цель st): перенос идёт к **W**, а не к ширине хранения.
///
...
/// Разряд в позиции числового значения (фича 0335).
mod bit_value;

/// Перечисление внутри функции (фича 0338).
mod enum_in_function;

/// Обёртка беззнакового целого (правило 0127).
mod overflow;

/// Инициализатор массива (фича 0343).
mod array_init;

/// Агрегат в локальном объявлении (фича 0345).
mod local_aggregate;

/// Массив в параметре функции (фича 0348).
mod array_param;
