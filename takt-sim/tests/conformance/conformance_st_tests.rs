...
/// Фикстура исправления: две модели с одноимённой `fn helper` (тела `x+1` и `x+2`),
...
/// (детерминированная генерация и правила именования iec2c), поэтому для этой фикстуры фиксируются
/// здесь.
...
///
/// **Мутация:** вернуть дедупликацию по голому имени в
...
// -----------------------------------------------------------------------------
// Q-арифметика fixed-point для цели st
//
...
/// Фикстура переноса к `W = 12` при типе хранения 16 бит.
...
fn fixed_point_arithmetic_matches_generated_st() {
    ...
    // состояние (вход в стартовое такта не расходует), поэтому трассы обязаны совпадать напрямую.
    //
    // Прежняя редакция сверяла `st.ends_with(&sim)` с нулевым префиксом - то есть
    // **закрепляла бы дефект как ожидаемое поведение**: тест зелен, а
...
// наследует уже проверенную Q-арифметику цели `st` (fixed_point_..._st выше).
// -----------------------------------------------------------------------------
...
//
// Единственная из четырёх целей, чьё поведение при переполнении никем не
...
int main(void) {{
    ...
    for (int i = 0; i < {TIME_TICKS}; i++) {{
        /* 1 мс на такт: модельное время такта i. */
...
/// состояния исполняется со следующего - свойство структуры ST), (2) MatIEC
...
// Насыщение q(m, n) sat - подмодулем по предмету, а
