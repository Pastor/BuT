...
//! доказывает, что вывод собирается, но не что он верен - на этом стояли дефекты целей
//! `sv` и `rust`.
//!
//! # Как наблюдается ячейка
//!
//! У эталона ячейка - синтетический порт, её значение видно как значение
...
//!
//! Адрес - `0x1000_0000_0000` (16 ТиБ), тот же, что у проверки HAL: macOS отвергает
//! низкие адреса, а страничное
