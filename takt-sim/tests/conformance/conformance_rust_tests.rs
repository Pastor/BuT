//! Сверка симулятора с прошивкой, порождённой `taktc -t rust`.
//!
...
//! законный отладочный механизм языка. В Rust аналога нет, и порт - честный способ:
//! ровно так прошивку наблюдает и реальная плата.
//!
//! # Мягкая деградация
//!
//! `rustc` - зависимость проекта (это Rust-репозиторий), поэтому пропуск здесь
//! теоретический: проверка оставлена по образцу `cc_available()`, чтобы тест падал по
//! существу, а не по отсутствию инструмента.
...
///
/// Порождает `.rs` тем же `taktc`, пишет драйвер, компилирует его настоящим `rustc` и
/// запускает. Драйвер реализует `Hal`, запоминая последнее записанное в порт значение,
/// и печатает его после каждого такта - то есть наблюдает прошивку ровно так, как
/// наблюдала бы плата.
///
...
///
/// `root` - имя корневой структуры (CamelCase от имени файла), `variant` - имя варианта
/// порта в `OutU8Port`.
fn rust_trace(
    dir: &Path,
    fixture: &Path,
    ...
    // Наблюдение уходит из HAL общей ячейкой: поле `hal` модели приватно и аксессора
    // не имеет - как и должно быть у прошивки. Ровно так же плата наблюдает не
    // "внутренности прошивки", а состояние регистра, в который та пишет.
    let driver = format!(
        r#"#[path = "{module}"]
mod generated;
...
/// **Вход в стартовое состояние, глубина 1:** тело стартового состояния - на такте 1.
#[test]
...
/// **Глубина 3** - там, где сдвиг цели `c` был максимальным.
#[test]
...
///
/// Сдвиг на такт (если бы он вернулся) сместил бы **всю** трассу, а не только первое
/// значение, - и был бы пойман здесь.
#[test]
fn per_tick_trace_matches_generated_rust() {
    ...
    let sim = simulate_trace(&path, "n");
    // Пиннинг: если трасса симулятора изменится, тест упадёт здесь, а не "подстроится"
    // под прошивку.
    assert_eq!(
        sim,
        vec![1, 2, 3, 3, 3, 3],
...
///
/// **Тест главного дефекта.** Цель `rust` печатала `n += 1`, и на
...
fn unsigned_overflow_wraps_like_generated_rust() {
    ...
    // Счётчик - `var` (выходной порт читать нельзя, SE-027), наружу он зеркалится
    // портом `n`: наблюдение в этой сверке идёт только через HAL.
...
// -----------------------------------------------------------------------------
// Q-арифметика fixed-point для цели rust
//
...
/// (цель rust): перенос идёт к **W**, а не к ширине хранения.
///
/// Формат `q(6, 6)` выбран намеренно: `W = 12`, хранение - 16 бит. При `q(8, 8)` обе
/// границы совпадают, и прежняя сверка расхождения не видела - это была дыра в
/// **покрытии**, а не в проверке.
#[test]
...
/// (RS-016). Значения кратны 2⁻⁶ и точны в f64, поэтому сверка идёт по битам - то есть
/// по представлению q.
///
/// Прижатие идёт к границам формата (`W = 12`), а не хранения (`i16`): `.clamp(-2048,
/// 2047)` против `as i16`, который вернул бы перенос. Формат `q(8, 8)` этого различия
/// не показывает.
#[test]
...
fn fixed_point_arithmetic_matches_generated_rust() {
    ...
    let sim = simulate_f64_trace(fixture, "probe");
    // Пиннинг битов: -3.0, -1.5, -0.0078125, 1.9921875 (все точны в f64) - и дальше
...
// только бит/число), поэтому Q-модель без портов время выполнения-наблюдать нечем. Сверка -
...
// наследует уже проверенную Q-арифметику цели `rust` (fixed_point_..._rust выше).
// -----------------------------------------------------------------------------
...
// -- Модель времени: профиль "часы" через внешний `now_ms` ---------
...
/// Выдержка `after 5ms` в профиле "часы" срабатывает на том же такте у симулятора и у
/// порождённого Rust (внешний `now_ms`, 1 мс на такт).
#[test]
...
/// **Структуры в цели `rust`: поля доезжают до прошивки.**
///
/// Ответ `RS-011` ("поля структур в цели rust пока не
