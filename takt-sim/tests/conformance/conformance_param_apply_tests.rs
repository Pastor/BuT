//! Потактовая сверка применения аргументов инстанцирования ("локальная копия").
//!
//! # Что доказывается
//!
...
///
/// `gain`, а не `step`: `step` занят стандартной библиотекой IEC (`ST-014`), а фикстура
...
/// Два экземпляра накапливают с разной скоростью; обёртка `u8` - норма переполнения.
#[test]
...
/// "Локальная копия": экземпляр меняет свой параметр - сосед не видит.
///
