...
/// `float`: сумма дробных литералов - у эталона и цели одно значение.
#[test]
fn float_sum_agrees() {
    check(
        "f_sum",
...
/// `q(m, n)`: та же сумма - и в целом q-представлении.
///
...
/// **Контроль: свёрнутая запись равна написанной.**
///
...
/// Умножение и вычитание точны так же, как сложение.
#[test]
...
/// Смешение с целым точно так же - отвергать за форму записи не за что.
#[test]
...
/// Деление отвергается `SE-114`: округление задано эталоном.
#[test]
...
/// **Граница: приведение `as` остаётся законным.**
///
/// Отказ пошире отвергал бы эту форму - вход, на котором эталон и
...
/// **Граница: целых правило не касается.**
///
