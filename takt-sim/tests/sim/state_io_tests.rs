...
/// A8: круговой рейс точен и для значения шире `i64`.
///
...
/// A4 (Д2): после загрузки модель **продолжает вычисляться** - присваивание наблюдаемо
/// меняет значение. Замороженной модели не существует. Это и есть ключевая проверка:
/// точечная починка снимка её бы не прошла.
#[test]
...
/// A7 (R7): файл, сохранённый до (`"variables": {}`), читается без ошибки, и модель
...
/// A8 (R6): константы (`const`) в снимок не попадают - их значение задано исходником,
