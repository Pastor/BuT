...
/// Ошибка внутри импорта названа библиотекой, а не импортёром.
///
/// Это и есть предмет: без пути файла оба случая дают дословно одинаковый вывод.
#[test]
...
/// Ошибки разбора печатаются все, каждая со своей позицией.
///
...
/// Формат позиции совпадает с `taktc` - потому что функция одна и та же
