...
/// T1: композиция с переходом `next` исполняется и уходит по `next` по её завершении.
#[test]
...
///
/// Форма из пробы исправления: дефект срабатывал и через вложение, поэтому "починка только
...
/// T3/T4: композиция **без** `next` не проваливает значение и наблюдается после
/// завершения.
///
...
/// немедленно. Если бы починка лишь "переставила" проверку, не дожидаясь завершения,
...
///
/// Различие "ошибка != ложное условие" - требование R5. Проглоти узел ошибку ребёнка,
