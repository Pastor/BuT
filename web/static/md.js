/**
 * Разбор Markdown: подсветка исходника и безопасный рендер. Свой разбор
 * здесь законен:
 *
 ...
 * постановка). Разбор здесь один на обе работы - подсветку и рендер: заведи их
 ...
 * - код, выделение, ссылки. Картинок нет намеренно: показ картинки означает
 ...
 *
 * Узлы строятся, а не собираются строкой: `innerHTML` из авторского текста
 ...
 *
 * Разрешён список, а не запрет: запретить `javascript:` мало - есть `data:`,
