...
function redraw() {
  ...
  // страницы, и отказ выглядел бы отказом модуля (нашлось прогоном страницы).
 ...
 *
 * Дата, коммит и ветка ушли в подсказку: они нужны, когда о сборке
...
function wire() {
  ...
  // (правило референса: `hover` есть не везде).
  ...
  // Несохранённая работа не теряется молча (R13): подтверждение ухода - единственное,
...
function showDiagnostics(items) {
  ...
  for (const item of items) {
    ...
    // единственный носитель состояния (доступность, замер ).
...
function compile() {
  ...
  // Диагностика при этом идёт как прежде: её показывает своя область под моделью. Мост
  // может быть ещё не загружен: панель выбирается до модуля (страница помнит её с
  // прошлого раза), и без этой проверки старт падал целиком - страница показывала
...
/**
 * Красит порождённый файл по правилам его языка (задача, требование R11).
 *
