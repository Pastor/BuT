...
/// Под преисправлением работают оба адреса корня - и с косой, и без.
///
...
async fn both_forms_of_the_prefixed_root_are_served() {
    ...
    // И корень домена сервису не принадлежит: под преисправлением он там не отвечает.
...
async fn user_row_holds_nothing_personal() {
    ...
    // пароля идёт сбросом администратора, адреса нет потому, что этого требует A6.
    // Колонка, заведённая "на будущее", - персональные данные, которых никто не
    // собирался собирать.
