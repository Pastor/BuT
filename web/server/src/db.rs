...
//!
//! У PostgreSQL запись параллельна, и очередь на соединении была бы выдуманной -
//! отсюда пул, а не одно соединение под мьютексом.
//!
//! Полнотекстовый поиск идёт `tsvector` средствами самой базы; словарь `russian`
//! разбирает кириллицу без дополнительных расширений.
//!
...
//!
//! Без шагов база чужой версии отвергается словами, и выкатываться на стенд с уже
//! живой базой становится нечем: отказ при старте сработает как задумано, а сервис
//! останется старым.
//!
//! Цена решения названа: носителей знания о схеме два, и разъехаться они могут молча -
//! база с нуля и база после перехода окажутся разными при одном номере версии. Держит
//! их тест `both_roads_lead_to_the_same_schema`: он строит обе и сравнивает состав
//! колонок и ограничений.
//!
//! # Чего в схеме нет и почему
//!
//! Ни почты, ни имени, ни адреса, ни `User-Agent`, ни часового пояса, ни
//! `last_seen_at`. Восстановление пароля - сброс администратором, поэтому почта не
//! нужна вовсе, а обещание не хранить адрес держится тем, что хранить его негде. На
//! это стоит отдельный тест: колонка, заведённая "на будущее", - это персональные
...
const MIGRATIONS: &[(i64, &str)] = &[
    ...
    // 7 -> 8: задержка между тактами прогона по сценариям.
    (
        7,
...
CREATE TABLE projects (
    ...
    -- Задержка между тактами прогона по сценариям, секунд: объект JSON
    -- "имя сценария - число". Прогон без задержки летит быстрее, чем глаз
    -- успевает за схемой, а показывать работу модели автор хочет в своём темпе.
    -- ⚠️ Ключ - имя файла: переименование и удаление сценария правят и его.
    run_delays       TEXT NOT NULL DEFAULT '{}',
