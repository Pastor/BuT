...
//! проектов заводит задача `09b` - здесь их нет намеренно, и список маршрутов
...
//! Правило продублировано здесь нарочно: без nginx (своя машина, проба стенда)
...
pub struct AppState {
    ...
    pub rate: Window,
    /// Версия модуля, которую сервер считает последней (решение A5): её получает новый
...
pub fn router(state: Arc<AppState>) -> Router {
    ...
    // Преисправление за обратным прокси. `/` - не преисправление, и вкладывать в него ничего не надо:
...
/// сказав ни слова (нашлось прогоном страницы 2026-09-04).
fn page_service(
    state: Arc<AppState>,
...
/// Собирает `index.html`, переписав корень адресов под преисправление.
///
...
/// Тот же тег под преисправлением обратного прокси.
///
...
mod tests {
    ...
    fn a_page_is_a_page_and_a_file_is_a_file() {
        // Предмет - класс, найденный прогоном страницы: пока `index.html` отдавался на
        // Любой неизвестный путь, промах по файлу выглядел успехом. Со страницы
