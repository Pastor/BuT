...
//! скрипте, в тесте - разошлась бы с первой молча: страница
...
//! фотографии: обещание A6 держится тем, что структура ответа объявлена без них, и
//! `serde` остальное отбрасывает. Токены площадки не хранятся: они нужны один раз,
//! внутри обработчика `callback`.
//!
//! Отсюда цена, названная проработкой: при первом входе логин **просит человек**, а не
//! подставляется из имени. Подставить его значило бы прочитать и сохранить то, чего мы
//! обещали не хранить.
//!
