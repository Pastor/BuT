// Поиск текста мимо словаря.
//
// # Предмет
//
// Строка оболочки, написанная в коде, не переводится никогда и не обнаруживается
// ничем: страница выглядит рабочей, а одна подпись остаётся на чужом языке.
// Ловится это машиной, падающей списком.
//
// # Сканер, а не регулярное выражение
//
// Кириллица в `web/static` есть везде: комментарии проекта пишутся по-русски.
// Отличить комментарий от строкового литерала грепом нельзя, а разбирать
// JavaScript зависимостью нельзя тем более: фронтенд обходится без зависимостей.
// Отсюда посимвольный сканер состояний: код -> строка -> шаблон -> комментарий.
// Файлов немного, и они наши.
...
export function nodesWithoutKey(html) {
  ...
  while ((match = tag.exec(withoutCode)) !== null) {
    ...
    // `meta` и `og:*` - предпросмотр ссылки: он на одном языке, и ключа у него нет.
