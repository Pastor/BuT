// Проверки веб-части, которые можно снять без браузера (фича, задача 03).
//
// # Что здесь проверяется и почему именно это
//
// Браузерного проверки в проекте нет и в этой фиче не заводится (замер задачи 07):
...
test("язык: словари полны — паритет ключей и подстановок", async () => {
  // Замер референса 2026-09-04: у него 163 ключа есть только в `ru`, и
...
test("язык: каждый ключ разметки есть в словаре, и мёртвых ключей нет", async () => {
  ...
  // Подписи кнопок площадок приходят от сервера (задача 09f-3): имён
...
test("прокрутка: вложенная область кода не запирает вертикаль", async () => {
  ...
  // этого контроль падал на собственном пояснении (прогон 2026-09-05).
...
test("шапка: две полосы, и каждая отвечает на свой вопрос", async () => {
  // Предмет - состав полос (раскладка заказчика 2026-09-05): верхняя
  ...
  // Переключатель языка стоит наверху, рядом со входом (решение заказчика
...
test("страница проекта: читается активный файл и сценарий рядом", async () => {
  ...
  // Сценариев несколько (задача 09n), и берётся названный проектом, а не
  ...
  // Задача 09p: чужой проект открывается сборкой автора. Пара берётся не
...
test("страница проекта: закрытый отвечает названным отказом", async () => {
  // Отказ поднимается ключом словаря, а не текстом: текст оболочки строит
...
test("черновик v2: цель и ключи перекрывают проект, а прежняя запись их не имеет", () => {
  // Правило задачи 09p: проект задаёт умолчание, черновик перекрывает его
  ...
  // Имя сценария хранится вместе с его текстом (задача 09n): сценариев
...
test("списки: значение выставляет одна точка, и она обновляет надстройку", async () => {
  // Класс нашёлся прогоном страницы (задача 09p): открытый чужой проект
