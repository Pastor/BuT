...
| **behavior** | Выражение композиции автоматов (`A + B` — последовательно, `A \| B` — параллельно, `A` — одна модель) |
...
model Outer {
    state Complex {
        ...
        behavior -> Inner;   // выбор одной модели: Inner
    }
}
```
...
// Глобальное свойство поведения (параллельная компоновка)
...
model M {
    ...
    behavior -> A | B;   // Параллельная компоновка (| = одновременно)
    // behavior -> A + B;   // Последовательная компоновка (+ = по очереди)
    // behavior -> A;       // Одна модель (выбор)
}
```
...
BehaviorExpr ::= BehaviorExpr "+" BehaviorExpr   (* последовательная, лево-ассоц. *)
               | BehaviorExpr "|" BehaviorExpr   (* параллельная, лево-ассоц. *)
               | Identifier                      (* одна модель = выбор *)
```

Свойство `behavior` объявляется на уровне модели и не совмещается с явными состояниями (`state`). Выражение после `->` строится из имён подмоделей и операторов `+` / `|`.

Если нужно запустить **ровно одну** модель, можно написать просто её имя без оператора — это является сокращённой формой записи.

### 19.2 Типы компоновки

| Тип | Оператор | Пример | Семантика |
|---|---|---|---|
| **Последовательная** | `+` | `A + B + C` | A → B → C; следующая запускается после завершения предыдущей |
| **Параллельная** | `\|` | `A \| B \| C` | Все Mi выполняются одновременно; завершение — когда все завершились |
| **Выбор** | *(нет)* | `A` | Запускается одна конкретная модель; при её завершении — вся компоновка завершена |

> **Приоритет:** оператор `+` имеет такой же приоритет, что и `|` (оба лево-ассоциативны). Для явного управления порядком используйте скобки (если поддерживается целевым синтаксисом).
...
/// Оператор + задаёт последовательность выполнения
model Pipeline {
...
/// Оператор | означает параллельное выполнение
model ParallelPrep {
    behavior -> Calibrate | Process;
    end -> { prep_done = 1; }
}
```

#### Три модели параллельно

```but
/// Все три модели работают одновременно
model FullParallel {
    behavior -> Calibrate | Process | Store;
    end -> { all_done = 1; }
}
...
```but
/// Запускает конкретную модель; при её завершении — завершается сам
/// Одиночное имя без оператора — это выбор одной модели
model RunCalibrate {
    behavior -> Calibrate;
    end -> { choice_done = 1; }
}
...
// Последовательная компоновка: + означает "выполнить одно за другим"
model Pipeline {
    behavior -> Calibrate + Process + Store;
    end -> { pipeline_complete = 1; }
}

// Параллельная компоновка: | означает "выполнить одновременно"
model ParallelPrep {
    behavior -> Calibrate | Process;
    end -> { prep_done = 1; }
}

// Выбор одной модели: просто имя без оператора
model RunCalibrate {
    behavior -> Calibrate;
    end -> { choice_done = 1; }
}
...
```but
model Pipeline {
    behavior -> Calibrate + Process + Store;   // оператор + → Sequential
    end -> { pipeline_complete = 1; }
}
```
...
```but
model ParallelPrep {
    behavior -> Calibrate | Process;   // оператор | → Parallel
    end -> { prep_done = 1; }
}
```
...
#### 20.4.3 Выбор одной модели (C)

```but
model RunCalibrate {
    behavior -> Calibrate;   // одно имя без оператора → Choice
    end -> { choice_done = 1; }
}
```
