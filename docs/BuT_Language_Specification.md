...
Псевдоним присваивает новое имя существующему типу. При кодогенерации псевдоним **рекурсивно раскрывается** до базового типа.

```but
type bool  = bit;          // bool как псевдоним bit
type Timer = u32;          // Timer как псевдоним u32

// Цепочка псевдонимов — раскрывается полностью:
type Byte    = u8;         // Byte → u8 → uint8_t (C)
type Counter = Byte;       // Counter → Byte → u8 → uint8_t (C)
```

Таблица разворачивания для основных целевых платформ:

| Псевдоним BuT | Базовый тип | C (`uint*_t`) | ST (IEC 61131-3) |
|---|---|---|---|
| `type A = u8`  | `u8`  | `uint8_t`  | `BYTE`  |
| `type A = u16` | `u16` | `uint16_t` | `WORD`  |
| `type A = u32` | `u32` | `uint32_t` | `DWORD` |
| `type A = u64` | `u64` | `uint64_t` | `LWORD` |
| `type A = bool`| `bool`| `int`      | `BOOL`  |
| `type A = f64` | `f64` | `double`   | `REAL`  |
| `type A = B; type B = u32` | `u32` | `uint32_t` | `DWORD` |
...
### 19.3 Разворачивание псевдонимов типов в кодогенерации
