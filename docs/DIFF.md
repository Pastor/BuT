...
1. **Нет модели времени.** У UPPAAL — временные автоматы, у IEC 61131-3 — блоки
   ...
   свод(е) свода прямо признаёт костылём. **Разбор шести механизмов,
...
- **Целевые платформы.** `c`, `c-hal`, `st`, `st-at` (IEC 61131-3), `rust`
  (`no_std`), `sv` (синтезируемый SystemVerilog), `sv-mmio`.
...
> автора. Это не «недоделка ради простоты»: свод(е) свода **вынуждает**
...
- **T6. Аддитивность.** Ручные счётчики продолжают работать без изменений: весь корпус примеров и книга остаются валидными.
...
Ничего не делать; свод(е) продолжает требовать комментария о частоте.
