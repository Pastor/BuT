...
- **Номер:** 0539
- **Статус:** ГОТОВО (закрыта 2026-09-08; вердикт тестирования — готово)
...
Предмет — пятнадцать требований анализа (R1–R15) и решения, принятые по ходу
разработки. Условие проверки и ожидаемый результат названы у каждой строки;
способ — автоматический (набор `cargo`, тест `node`, тест сервера, проверка
предкоммита) либо ручной прогон страницы в браузере. Язык не менялся,
поэтому примеров и контрпримеров языка план не требует; их место занимают
модели корпуса и лифт заказчика.

| № | Предмет | Условие проверки | Ожидаемый результат | Способ |
|---|---|---|---|---|
| Т1 | R1 граф модели | вызов на каждом `examples/*.takt` и `examples/language/*.takt` | ответ без отказа; узлов столько, сколько состояний по АСД, рёбер — сколько `ref` и `next`; два вызова равны | `layout/tests.rs` |
| Т2 | R2 ярусы | лифт, самопереход, недостижимое, ветвление | `Idle` в ярусе 0, `DoorOpening` — последний; недостижимое ниже всех; порядок по предшественникам, ничья по имени | `layout/tests.rs` |
| Т3 | R3 экспорт модуля | `takt_graph` на лифте и на неразбираемом тексте | два листа, у `Engine` 5 узлов и 7 рёбер, дерево `Middle` из пяти шагов; на `start S {` — отказ с кодом; ответ детерминирован | `takt-wasm/src/graph.rs`, тест `node` |
| Т4 | R4 формат файла | канон на неопрятном входе; круговой рейс; негодные файлы | ключи по алфавиту, целые числа, умолчания и пустые листы опущены; чтение и запись дают тот же текст; негодный файл — названная причина и пустая раскладка | `layout-tests.mjs` |
| Т5 | R5 сверка | переименование, удаление, добавление, лишний лист | неразмещённые, лишние узлы, рёбра и листы названы; `prune` снимает лишнее, размещённое цело; `rename` переносит узел, подпись и ключи рёбер | `layout-tests.mjs` |
| Т6 | R6 род файла | `PUT` `.takt-ui`, архив с раскладкой, страница проекта | род `layout` в ответе, отказ называет `.takt-ui`, раскладка не может быть активным файлом; архив возвращает файл своим родом; страница берёт пару к активной модели | `limits.rs`, `archive.rs`, `tests/projects.rs`, тест `node` |
| Т7 | R7 вкладка «Схема» | панель открыта на лифте в обеих темах | лист, крошки, миникарта, панель, легенда на месте; консоль без ошибок страницы | браузер |
| Т8 | R8 навигация | панорама тягой, колесо, «вписать», пределы масштаба | вид едет за указателем; точка под курсором на месте; предел `0,25×`…`4×`; «вписать» — по тесной оси | `layout-tests.mjs` (геометрия), браузер |
| Т9 | R9 правка | перенос узла, изломы, углы, автораскладка, отмена | узел встаёт на сетку и теряет метку «не размещён»; изломы ставятся и снимаются; запись канонична после каждой операции; Ctrl+Z возвращает прежнее | `layout-tests.mjs`, браузер |
| Т10 | R10 синхронизация | щелчок по узлу; курсор в объявлении; `rename` из редактора | курсор на имени состояния; узел подсвечен; ключи раскладки переименованы | браузер, `layout-tests.mjs` (`rename`) |
| Т11 | R11 композиция | вход в `Middle`, затем в `Engine`; крошки | лист композиции: пять шагов, второй — две ветви в рамке; вход в любой `Engine` — один лист; крошки ведут назад | геометрия в `node`, браузер |
| Т12 | R12 сохранение | ссылка-снимок, черновик двух видов, проект, архив | раскладка переживает каждый рейс; пустая не заводит файла | `layout-tests.mjs`, `archive.rs`, браузер |
| Т13 | R13 доступность | Tab, стрелки, Shift+стрелки, Enter, Escape; контраст пар | узел с фокусом получает выбор; стрелки двигают на шаг сетки; виды выражены и формой; контраст пар ≥ 4,5:1 по замеру дизайнера | браузер, `05-contrast.md` |
| Т14 | R14 без знания о языке | греп ключевых слов Takt в `web/`; строки мимо словаря | находок нет | `check-web.sh` |
| Т15 | R15 подсветка прогона | такт лифта; счётчик с самопереходом | по списку состояний и ожиданий на строку; имена — узлы графа; ожидаемый переход счётчика ведёт в него же | `takt-wasm/src/sim.rs`, `layout-tests.mjs`, браузер |
| Т16 | шаг прогона | два шага кнопкой; изменение текста; прогон после шагов | две строки трассы, сессия одна; смена текста открывает сессию заново и очищает трассу; прогон продолжает сессию | браузер |
| Т17 | оформление | `check-design.py`; витрина в обеих темах и на 375 px | `D1`…`D8` зелёные; образцы читаются | проверка, браузер |
| Т18 | панели области вывода | генерация, симуляция, схема, «ничего» | одновременно видна одна панель либо ни одной | браузер |
| Т19 | узкий экран | 375 px, режим «Схема» | полка с четырьмя целями; в панели две кнопки; миникарты нет; холст занимает экран | браузер |
| Т20 | предкоммит | `./scripts/precheck.sh` | зелёный целиком | проверка |

## Отчёт о тестировании

**Окружение.** macOS 26.6.2, `rustc 1.97.1` (пин толчейна), `node 26.8.1`,
модуль `takt_wasm.wasm` 3347 КиБ профиля `wasm`; PostgreSQL 16 временным
кластером в каталоге сессии (для проверок хранилища и HTTP сервера). Дата
прогонов — 2026-09-08; коммиты `3a056d61` и `38256ece`.

**Сводка прогонов.**

| Прогон | Результат |
|---|---|
| `./scripts/precheck.sh` (105 проверок, включая `cargo test`, `clippy`, проверки целей, веб-часть, сервер, документ) | зелёный, четыре раза на трёх состояниях дерева; последний — перед коммитом `38256ece` |
| `takt-lang`: набор модуля `layout` | 10 из 10 |
| `takt-wasm`: тесты моста | 43 из 43 |
| `takt-sim`: тесты эталона после поля `states` и взгляда вперёд | 278 из 278 |
| `node web/tests/web-tests.mjs` (с набором `layout-tests.mjs`) | 80 из 80 |
| сервер проектов без базы | 54 из 54 |
| сервер проектов с базой (роды файлов, миграция, архив, права, витрина, хранилище) | все наборы зелёные на временной PostgreSQL |
| `check-design.py` | 306 правил, 8 пар, 27 контролов, расхождений нет |
| `check-web.sh` и самопроверка `test-check-web.sh` | зелёные, `B1`…`B8` |

**Сверка с тест-планом.**

| № | Итог | Примечание |
|---|---|---|
| Т1–Т6 | выполнено | автоматические наборы, см. сводку |
| Т7 | выполнено частично | светлая тема снята прогоном страницы; тёмная тема и снимки витрины сняты дизайнером (`screens/`), на живой странице глазом не проверялись — панель браузера стенда была скрыта во второй половине прогона |
| Т8 | выполнено | панорама на 80 px, масштаб, «вписать» по тесной оси; геометрия в `node` |
| Т9 | выполнено | перенос `Idle` снял метку и записал координаты в черновик; отмена вернула прежнее; изломы и знак — по тестам канона |
| Т10 | выполнено частично | щелчок по узлу и `rename` проверены; подсветка узла от курсора в тексте — кодом (`selectionchange`), прогоном не снята |
| Т11 | выполнено | `Корень › Middle › Engine`, пять шагов, две ветви, семь рёбер и шесть знаков у `Engine` |
| Т12 | выполнено | рейсы ссылки, двух черновиков и архива — тестами; черновик с раскладкой и местом легенды прочитан из хранилища браузера |
| Т13 | выполнено частично | выбор, стрелки (шаг и Shift), отмена и повтор — синтетическими событиями клавиатуры: настоящий фокус в скрытой панели стенда не доставляется; контраст — по таблице дизайнера |
| Т14 | выполнено | |
| Т15 | выполнено | подсветка `Main` и «возможен переход» у `Middle` после двух шагов |
| Т16 | выполнено | |
| Т17 | выполнено частично | проверка зелёная; витрина на 375 px и в тёмной теме — по снимкам дизайнера |
| Т18 | выполнено | после исправления запросов панелей: четыре переключения дают ровно одну панель либо ни одной |
| Т19 | выполнено | полка с четырьмя целями, две кнопки панели, миникарты нет, холст 430 px из 812 |
| Т20 | выполнено | |

**Найденные дефекты.** Все найдены прогоном страницы и тестами по ходу
разработки и исправлены до закрытия — отдельных карточек `docs/fixes/` нет,
каждый описан в разделе своей задачи: канон писал ключи верхнего уровня в
порядке вставки; переименование не пересортировывало ключи рёбер; лист
вписывался в скрытую панель на минимальный масштаб; первый щелчок
перестраивал SVG, и двойной щелчок не наступал; узел левее нуля обрезался
краем листа; три панели области вывода показывались разом (дефект старше
схемы, невидимый при двух панелях); отрицательный ноль в проверке сетки.

**Что не проверено и почему.** Касание (долгое нажатие, щипок) — стенда
с сенсорным экраном нет, эмуляция ширины даёт раскладку, но не жесты.
Тёмная тема на живой странице и подсветка узла от курсора в тексте — панель
браузера стенда скрыта, снимки не снимаются; код обеих ветвей проверен
тестами и синтетическими событиями. Раскладка импортированной модели живёт
в файле её модели (решение вопроса 6) — на странице импорт не работает
вовсе, и ветвь недостижима.

**Вердикт: готово.** Требования R1–R15 выполнены, все проверки предкоммита
зелёные, три частично выполненных пункта плана закрыты тестами и снимками
дизайнера; ручные пункты, невыполнимые на стенде, названы.

## Итог (что сделано)

**Предмет.** У модели Takt появилось графическое представление: вкладка
«Схема» онлайн-редактора показывает состояния и переходы по ответу компилятора,
а раскладку — центры узлов, изломы рёбер, подписи автора, место знака условия и
легенды — хранит отдельным файлом рядом с моделью (`elevator.takt` →
`elevator.takt-ui`). Язык не менялся, версия `0.18.0` прежняя.

**Что сделано** (подробно — в разделе «Разработка», задачи 01–09):

- `takt-lang`: модуль `layout` — граф модели по АСД (листы по моделям, узлы с
  видом и позициями, рёбра с цитатой условия, ярусы без случайности); правила
  «конец» и формы реализации — те же носители, что у семантики.
- `takt-wasm`: экспорт `takt_graph`; ответ такта несёт активные состояния и
  ожидаемые переходы (`Unit::peek_transitions` в `takt-sim` — взгляд вперёд по
  нынешним значениям условий, ничего не исполняя).
- Сервер проектов: четвёртый род файла `layout`, схема базы 7, архив.
- Страница: холст на SVG (`scheme.js`, `scheme-geometry.js`, `legend.js`,
  `layout.js`), бесконечное поле с сеткой, крошки, миникарта, навигатор,
  легенда со знаками `S` и `K` и подписями автора, правка раскладки с отменой,
  композиция квадратом с миниатюрой и листом выражения, синхронизация с
  текстом, сохранение в черновике, ссылке-снимке, проекте и архиве, три вида
  подсветки прогона, шаг прогона и кнопки-значки; три панели области вывода
  взаимоисключающие.
- Оформление принято по выходным требованиям анализа: витрина, книга, токены
  схемы; решения заказчика (знаки вместо имён, легенда, мостики на
  пересечениях, вариант значков и цветов) вошли в первую редакцию.
- Документ: подраздел «Схема состояний» в описании онлайн-редактора; README.

**Ключевые решения.** Граф по АСД, а не по дереву семантики (схема отвечает и
на модели с ошибками); файл — подсказка, ключ — имя (расхождение показывается,
не ломает); формат JSON канонической записью без умолчаний и пустых листов;
лист композиции строится из выражения и не хранится; выбор без перестроения
листа (иначе двойной щелчок не наступает); запросы панелей ограничены рабочей
областью (`data-panel` стоит и на `body`).

**Проверки.** Все автоматические наборы зелёные, предкоммит зелёный (см.
«Отчёт о тестировании»); ручные пункты, невыполнимые на стенде (касание, тёмная
тема на живой странице), названы. Кандидаты, оставшиеся вне объёма:
подсветка активных экземпляров при нескольких копиях одной модели на листе
композиции; раскладка листа композиции (вопрос 6 ADR); панель схемы в
`webview` плагинов.
