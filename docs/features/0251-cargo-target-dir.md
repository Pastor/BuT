...
> вынесенного при работе над 0234; далее проходит жизненный цикл по своду.
...
Целевой поток (схема нужна: меняется адресат записи у трёх
потребителей):
...
- **Язык не затронут**: версия языка прежняя. Код крейтов не
  меняется — версия крейта тоже прежняя. Это настройка среды сборки.
- **(редакторский слой):** ни LSP, ни плагины не правятся; фича
