...
0061) — в [ADR](0097-pid-regulator-example.md#архитектура-adr).
...
2. **Anti-windup обязателен** (решение почти предопределено): арифметика 0061 —
   **wraparound**, а не насыщение (правило 3 ADR). Интеграл `I += e`
...
(wraparound), а не насыщение (правило 3 ADR; насыщение — отложенный кандидат
