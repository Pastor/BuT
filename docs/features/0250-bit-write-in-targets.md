...
- Установка бита — базовая операция промышленного управления (цель проекта). Отнимать её из-за отставания двух печатников — решение наоборот.
...
- Берёт **чужую** работу: структуры в `rust`/`sv` и массивы в `sv` не
  поддержаны и без битов. Это отдельные фичи, каждая со своим ADR.
...
- **Язык меняется**: нормируется наблюдаемое поведение (что попадает
  в бит) и добавляется диагностика `RS-025`. Версия `0.10.0` уже поднята циклом  — подъёма и тега не требуется.
- **Обратная совместимость**: вывод целей `c` и `sv` не меняется
...
- **свод**: раздел о типах получает правило о младшем бите; приложение об
  ошибках — `RS-025`.
- **свод**: токенов и узлов АСД не прибавляется; ответ фиксируется в
  анализе и отчёте.
...
по своду изменения вносятся в неё, **без подъёма и без тега**.
...
Фича меняет **наблюдаемое поведение целей**, поэтому по своду план несёт
примеры и контрпримеры; они же уезжают в документ.
...
| цель `st` | — | — | ✅ | — | — | ✅ |
| цель `sv` | — | — | — | ✅ | — | ✅ |
| LSP и плагины | — | — | — | — | — | ✅ (правок не требуется — токенов и узлов АСД не прибавилось; `RS-025` — диагностика цели, редактор целей не запускает) |
...
| LSP и плагины | ✅ | правок не требуется: токенов и узлов АСД не прибавилось, `RS-025` — диагностика цели, а редактор целей не запускает |
| форматтер | — | узел `BitAccess` печатается с начала |
| семантика | — | правило места записи закрыто фичей |
