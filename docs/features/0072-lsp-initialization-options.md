...
4. **Аддитивность.** Где импорт уже разрешался (каталог документа),
...
- **R5. Аддитивность.** Пустой `searchPaths` (или его отсутствие) →
...
Полная обратная совместимость: изменение **аддитивно**. Клиент, не
...
- **Аддитивно**: пустой `searchPaths` ≡ прежнее поведение (только
...
- **Проверка:** 8 юнит-тестов разбора + 4 интеграционных (`lsp72_init_options`,
  фикстуры `tests/data/lsp72/` — пример/контрпример своду). Детали — в
