...
свод(е) свода уже **вынуждает** автора сообщать частоту комментарием — то
...
как требует свод. Правки существующих разделов остаются за этой фичей в
...
свод(е) свода пересматривается: оно описывает обходной путь, который эта
фича снимает. Версия языка растёт.
...
свод(е) свода. Сравнительный анализ ([`docs/DIFF.md`](../DIFF.md), разделы
...
2. **Механизм включается только при использовании** (решение заказчика; свод).
...
- Ничего не проверяется; свод(е) продолжает требовать комментария вместо
...
#### свод. Механизм включается по факту использования
...
#### свод. Источник времени — внешний и подменяемый
...
| симулятор | виртуальные часы (детерминированные) | сценарий/флаг |
...
#### свод. Два профиля времени; профиль выбирает наличие частоты
...
длительность — решает профиль.

#### свод. `duration` — отдельный тип; мост к числам — только явный `as`
...
иметь один ответ на весь проект.
...
#### свод. Непредставимая длительность — ошибка компиляции
...
#### свод. Разрядность счётчика выбирает компилятор; сравнение — через разность
...
#### свод. Сверка — внутри профиля, по инжектированному времени
...
#### свод. Точность ограничена тактом, и это документируется
...
«Частые ошибки» (свод свода).

#### свод. Фактически заложенное — печатается
...
#### свод. `after` / `every` — сахар над свод–8
...
#### свод. Флаг частоты — второе осознанное исключение из принципа «CLI не меняет логику»
...
- Цель `sv` конструктивно защищена от `#delay`/`$time`, которые оба её проверки
  пропускают молча (П4).
- свод(е) свода («такт — не единица физической величины») перестаёт быть
...
- **A-4. свод(е) свода пересматривается** — его текст описывает обходной
  путь, который фича снимает.
- **A-5. Документирование — большой объём:** новый языковой раздел
  `book/` (литерал, профили, `after`/`every`, точность, «Частые ошибки») плюс
  записи в приложение «Ошибки и предупреждения» для `SE-063…065`.
- **A-6. Версия языка растёт** — конструкция и тип новые.
...
10. Раздел `book/` написан, примеры проходят проверка компиляции и симуляции, версия языка поднята, свод(е) свода пересмотрено.

## Анализ

> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) · задача: `../development/0134-01-*.md`
>
...
ADR). Разбор единиц в лексере, а не в семантике: иначе `1m30s`
...
3. **Дробное.** `1.5s` — ошибка (свод ADR: дробная длительность выражается
...
свод ADR вводит `clock 1kHz;`. Это **вторая** новая лексема: `Token::Frequency(u64)`
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) (своду–8) · задача: `../development/0134-02-*.md`
>
...
Значение хранится в **наносекундах** (`i64`) — каноническое представление свод ADR; профиль (мс или такты) применяется **позже**, на границе генерации.
...
Приоритет источников частоты (свод ADR): **`clock` в модели < `--tick-hz`**.
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) · задача: `../development/0134-03-*.md`
>
...
**Скрытое состояние обязано быть видно в трассе** (свод ADR, критерий A9).
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) · задача: `../development/0134-04-*.md`
>
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) · задача: `../development/0134-05-*.md`
>
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) (9; пробы П1–П3) · задача: `../development/0134-06-*.md`
>
> Требования **R1**, **R6**, **R9**; критерии **A1**, **A6**.
...
2. **Точность на ПЛК.** `TON` меряет время среды, а состояние опрашивается раз в
   скан-цикл: выдержка срабатывает на **первом цикле после** истечения (свод ADR). Это то же ограничение, что у прочих целей, но на ПЛК оно заметнее —
   должно попасть в раздел документа, а не остаться фольклором.

> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) (6; проба П4) · задача: `../development/0134-07-*.md`
>
...
Почему вход, а не счётчик внутри: свод ADR — источник времени внешний, как и
...
**Сброс не проверяется временем.** свод ADR требует, чтобы ветвь
...
приходится откуда-то получать), но не обязательный: выбор остаётся за сводом
...
> Обзор: [0134-language-time-model.md](0134-language-time-model.md#анализ) · ADR: [0134-language-time-model.md#архитектура-adr](0134-language-time-model.md#архитектура-adr) (13; действия A-2, A-4, A-5, A-6) · задача: `../development/0134-08-*.md`
>
...
(свод ADR). Первое — `--float-as-q`/`--float-embedded` (действие A-7 фичи). Исключение обязано быть **названо** в документации: принцип остаётся в силе
...
[заглушки генератора C: диагностика вместо тихого пропуска](0028-c-generator-stubs.md)-01).
...
свод требует раздел «Документирование» в карточке. Фича правит
...
`takt-sim -n 10`); свод требует **функционального** примера из практики с
...
как требует свод. За этой задачей остаются правки существующих разделов в
той части, что раздел 12 не покрывает.

### 5. Свод правил (A12)

**свод(е) пересматривается** (действие A-4 ADR). Сегодня оно предписывает
...
- сохранить оговорку про точность в один такт (свод ADR).
...
Растёт: конструкция и тип новые. Правятся **синхронно** два места —
...
- **Иных зависимостей нет.** Проверено по контракту и инфраструктуре:
  - `book/` — фича правит **существующие** разделы (02 «Лексическая структура»,
    ...
    «Ошибки и предупреждения»), поэтому («каждый новый раздел — отдельная
...
- **Влияние на порядок разработки:** завершение 0134 разблокирует кандидата
  «таймер-объект» (Option D ADR) и делает осмысленным пересмотр своду(е)
...
- **R1. Аддитивность по факту использования.** Модель без литерала длительности и
  ...
  ни колбэков, ни параметров, ни служебных входов (свод ADR).
- **R2. Литерал.** `3s`, `500ms`, `1m30s`, `2h30m` разбираются; `1.5s`, `1e3ms`,
  `3 s` — отвергаются диагностикой. Единица примыкает без пробела (свод ADR).
...
- **R5. Профиль выбирает наличие частоты.** Нет частоты → «часы»; `clock 1kHz;`
  либо `--tick-hz=N` → «такты», флаг переопределяет объявление (свод ADR).
- **R6. Внешний источник времени по целям.** `c` — колбэк `now_ms` в структуре;
  ...
  `time_ms`; симулятор — виртуальные часы (свод ADR).
...
- **R8. Разрядность выбирает компилятор; сравнение — разностью** в беззнаковой
  арифметике (свод ADR, опора на 0127).
...
- **R10. Печать заложенного.** `taktc` печатает, во что превратилась каждая
  длительность; `--quiet` глушит (свод ADR).
...
- **R12. `after`/`every` — сахар без своей семантики**, а его скрытое состояние
  видно в трассе симулятора (свод ADR).
...
| A3 | `SE-063`/`SE-064`/`SE-065` возникают на своих входах | фикстуры-контрпримеры (свод свода) |
...
| A12 | Версия языка поднята, свод(е) свода пересмотрено | `check-language-version.sh` + ревью свода |
...
**Фича аддитивна**. Доказательство — не намерение, а факт: проба П5
...
счётчиком тактов, а частота живёт в комментарии (свод(е) свода это прямо
...
| Аддитивность | вывод корпуса байт-в-байт прежний, проверки зелёные |
...
5. **Версия языка не поднята** — подъём делается при закрытии фичи (версия накапливается по циклам).
...
6. **`every` вынесен в задачу-преемника** `0134-09`: он требует своего поля в
   ...
   часы, значения и `after`. Декомпозиция — свод свода.
...
- **Колбэк `now_ms`** — `uint64_t (*now_ms)(void *userdata)` в **корневой**
  ...
  (модель без времени — вывод байт-в-байт прежний, свод ADR).
...
Это **разворот** прежнего свод ADR («флаг переопределяет объявление») в
...
`every Nms { … }` — сахар над механизмом времени (свод ADR): тело
