...
`rust` и в симуляторе) — версия языка не растёт. Правка **аддитивна**: конструкция, бывшая ошибкой, становится поддержанной; существующие
...
> `extend_complex.sv` не порождался); далее проходит жизненный цикл по своду.
...
- **Не зависит** от изменений языка: `+` уже разбирается
...
**Статус по обратной функциональности:** затрагивается только
...
**Статус по обратной функциональности:** меняется ровно ветка
...
автомат стоит. свод запрещает такую тишину в SV.
...
**Статус по обратной функциональности:** только цель `sv`
...
**Статус по обратной функциональности:** расширяются
...
Язык и версия не менялись. Детали, задачи…04,
