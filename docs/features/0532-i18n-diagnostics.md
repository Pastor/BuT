...
2026-09-06 и записаны разделом «Ответы заказчика». Все стадии — **разделы этой карточки** (правило 32): «Архитектура (ADR)»,
...
уезжают в `Diagnostic.message: String`. Замер 2026-09-04 (счёт по конструкторам
...
**Что уже сторожит тексты и что с этим станет:**

| Гейт | Что судит | Со вторым языком |
|---|---|---|
...
- **D1.** Русский вывод остаётся байт в байт: 1059 тестов, 2995 сообщений
  гейта, 88 пар CLI (методика 0531-02), снимки `book/`.
- **D2.** Код диагностики рождается в коде литералом — иначе слепнет гейт 0077
...
синтаксис FTL — второй язык, которого не знает ни один гейт. Покупаем свойство,
...
**дословно** (сторож: корпус до и после, `diff` пуст); D2 — `with_code`
остаётся литералом; D3 — носитель текста один; D4 — паритет ключей и параметров
проверяет гейт без ратчета, а `build.rs` строит константы из `ru`; D5 — ни одной
...
- **2995 сообщений гейта качества** не меняются: гейт гоняет `taktc` без
  ...
  корпуса отвергнут: удвоил бы время гейта ради того, что каталожная проверка
  доказывает дешевле.
- **Справочник документа** остаётся русским; в разделах «Диагностики» и
  «Инструментарий» появляется абзац о выборе языка. ⚠️ Гейт 0527 требует
...
- **Гейт заглушек** читает и каталог.
...
2. Под `en` — ни одного сообщения с кириллицей (смоук по префиксам) и в
   каталоге (гейт).
3. Гейт паритета падает списком: ключ без перевода, лишний ключ, разный набор
   параметров, неиспользуемый ключ; неизвестный язык — отказ с перечислением.
4. Греп-сторож: в исходниках нет текста с кириллицей в конструкторах диагностик
   и в печати вне тестов; список исключений пуст.
5. `precheck.sh` зелёный в обоих режимах; гейт 0467 без изменений; 0217 видит
   каталог.
...
инструмента, русский вывод не изменился ни на байт, а гейты текстов сохранили
предмет. Замер — в ADR.

### Замер (правило 30, дата 2026-09-04)

Предмет здесь не расхождение потребителей, а устройство, поэтому замер снят
**счётом по исходнику** (таблица ADR). ⚠️ Число подстановок вида `{:?}` внутри
...
| № | Требование | Проверка |
|---|---|---|
| R1 | Текст строится из каталога по ключу с именованными подстановками | греп-сторож «нет текста с кириллицей в конструкторах» |
...
| R5 | Каталоги полны: паритет ключей и параметров, без ратчета; мёртвый ключ — ошибка | `scripts/check-messages.py` + сторож |
| R6 | Код остаётся литералом; `with_code` обязателен | гейт 0077 без правок |
...
| R10 | Документ: абзац о выборе языка; тексты приложения русские | гейты 0527, 0521, 0290, 0520 |
| R11 | Гейт заглушек читает каталог | сторож 0217 на копии |

### План разработки: задачи и порядок выполнения

| Порядок | Задача | Предмет | Зависит от | Проверка |
|---|---|---|---|---|
| 1 | `0532-01` | носитель языка, формат и загрузчик каталога, `build.rs` с константами, `msg()`, гейт паритета со сторожем, ключ и переменная у обоих CLI, подписи печатников, **один** пробный код с контролем, абзац в документе | — | критерии 1, 3, 4; тест «пробный код под `en` без кириллицы» |
...
| 5 | `0532-05` | мост и LSP | 02–04 | тесты моста и LSP; гейт тождественности 0531-02 |
| 6 | `0532-06` | тексты CLI (≈ 165 мест); `--help` — по решению заказчика | 01 | сверка вывода под `ru`; смоук под `en` |
| 7 | `0532-07` | закрытие: реестр, гейт 0217 на каталог, смоук по префиксам, контекст, журнал, версии | 02–06 | `precheck.sh` в обоих режимах |
...
машина сторожит полноту и отсутствие кириллицы, не смысл.
...
- **Гейт 0217 слепнет** до задачи 07 — потому она обязательна.
- **Трассы эталона** содержат русские слова: если они окажутся **данными** для
  сверок и гейта 0522, перевод изменит формат — вопрос заказчику.
...
1. **Языки — `ru` + `en`, но список ОТКРЫТЫЙ.** ⚠️ Это отменяет `enum Lang { Ru,
   ...
   нельзя: он разошёлся бы с файлами молча (класс 0084).
...
3. **Системная локаль игнорируется**, читается только `TAKT_LANG` —
   ...
   потактовые сверки и гейты корпуса.
4. **Tier** — прочерк (как у 0531).
5. **Строка шага трассы — ДАННЫЕ, сводка прогона — текст.** Строку шага язык не
   меняет: на неё опираются потактовые сверки, `examples/simulations/` и гейт
...
### Задача 0532-01 — носитель языка, каталог и гейт паритета
...
ключей из базового каталога. ⚠️ Константы существуют ради одного: ключ,
...
⚠️ Паритет `build.rs` **не** проверяет: сборка, падающая на неполном переводе,
...
текста. ⚠️ Пробный код выбран так, чтобы показать главное: ключ ≠ код
...
**Гейт паритета** — `scripts/check-messages.py` (`M1`…`M5`) и сторож
`scripts/test-check-messages.sh` (`M1`…`M7`), оба в предкоммите, сторож первым.
⚠️ Ратчета у паритета нет **намеренно**: он позволил бы «перевести потом», а
`render` в этом случае молча отдаёт базовый текст — английский вывод оказался
бы наполовину русским при зелёном гейте.
...
| гейт паритета | 7 ключей × 2 языка; сторож — 7 проб, каждая проверена снятием |

⚠️ Тест «системная локаль не читается» — **текстовый**: он грепает исходник
