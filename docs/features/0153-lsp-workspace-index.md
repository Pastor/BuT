...
Целевой поток (схема нужна: меняется источник данных для двух
запросов):
...
- **Влияние на порядок разработки:** закрытие 0153 **разблокирует по существу**
  ...
  пересмотреть в пользу 0154 (критерий «необходимость»).
...
- **Язык не меняется**: версия языка прежняя. Меняется публичный
  API крейта (`references_at`), значит версия крейта поднимается минорно.
- **(редакторский слой).** Затронут именно LSP — это его фича.
...
| T15 | Полный предкоммит | — | `./scripts/precheck.sh` зелёный | свод |
...
3. **Порядок разработки стоит пересмотреть**: закрытие 0153 по
