...
- вход без `q` не затрагивается: понижение срабатывает только при формате
  `q(m, n)` у типа места;
- версия языка не поднимается: синтаксис и семантика прежние,
...
Статус по функциональности: правка целиком в **семантике**
...
слой — «н/п» (обоснование в анализе).
...
- Пробы замера (вне репозитория, чек-лист своду): поле, элемент, сравнение,
...
ребра. Взяв запись из бэклога, замер сняли заново — и это оказалось
не формальностью.
