...
## Замер (правило 30, пересъём 2026-08-23, `scripts/probe.sh`)
...
отвергает `rustc` при **нулевом** коде возврата `taktc`. Урок правила 30
подтверждён в третий раз подряд.
...
цикле; правило 22 (накопление) — подъёма не требуется, изменение вносится в
уже поднятую версию.
...
Замер **пересняли** (правило 30), и он опроверг запись кандидата **трижды**:
