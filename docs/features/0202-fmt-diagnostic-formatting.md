...
- свод: раздел «Инструментарий» документа получает пример отказа `fmt`
  (сегодня показан только случай «требуется форматирование»).
- свод: путь LSP (`formatting_edits`) правится вместе с CLI.
...
Лексика, синтаксис и семантика **не меняются**, поэтому проверка свод здесь короткая; но провести её обязательно — изменение публичного типа,
...
(примеры и контрпримеры) применимо в вырожденном виде: фича не
...
5. **Найдено сверх объёма, записано кандидатами**:
