...
- Р1 лечится как в 0033: диспетчеризовать `INIT` **до** `CASE`, не расходуя
  скан. Меняется **форма вывода** цели `st` — требуется ADR (меняется наблюдаемое поведение → диаграмма).
...
5. **свод.** Форма вывода цели `st` меняется — дифф корпуса
...
что было верно на момент написания. Читателю тех документов следует
