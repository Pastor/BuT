...
Версия языка **`0.10.0`** уже поднята циклом  — по своду
...
записи. Выносится фичей **0250** (крупное декомпозировать).
...
- **Обратная совместимость**: формально ломающее изменение,
  ...
  может.
- **свод**: приложение об ошибках (`book/src/appendix-errors/`) получает
  `SE-111` и `SE-112`; раздел о присваивании — оговорку о том, что стоит слева.
- **свод**: изменение семантики без новых токенов и узлов АСД —
...
принимались). Версия языка **`0.10.0` уже поднята** циклом — [переопределение типа не проверяется: встроенный тип можно затенить молча](0243-type-redefinition-diagnostic.md); по своду подъём
...
- **Не чинит запись бита.** Замер-18: `b.2 := 1;` верно печатают
  ...
  списке мест остаётся; починка — **фича**. Её объём измерен и
...
<> 16#00 := 1;`), и починка целей — отдельная работа.
...
| LSP и плагины | **н/п**: токенов и узлов АСД не прибавилось, диагностика идёт общим `validate_model_all` |
...
Фича меняет **семантику языка**, поэтому по своду план обязан нести
**примеры и контрпримеры**; они же уезжают в документ (`book/`).
...
| LSP и плагины редакторов | — | — | — | — | ✅ (правок не требуется, диагностика идёт общим `validate_model_all`) |
...
| LSP и плагины | ✅ | правок не требуется: токенов и узлов АСД не прибавилось, диагностика идёт общим `validate_model_all`, который зовут **оба** входа — `pipeline::collect_compile_diagnostics` и `lsp/diagnostics.rs` |
...
- заглушка `SIM-017` **не снята**, а передана фиче
  ...
  (`rustc`: E0070), `st` — `(USINT_TO_BYTE(b) AND 16#04) <> 16#00 := 1;`
  (`iec2c`: invalid statement). Починка целей — отдельная работа.
...
**Документ:** `book/src/05-expressions/` — оговорка о том, что
стоит слева, со списком мест и контрпримерами; `book/src/appendix-errors/` —
разборы `SE-111` и `SE-112`. Версия языка `0.10.0` уже поднята циклом  — по своду подъёма и тега не требуется.
