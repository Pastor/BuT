...
Примеры `examples/*.lam` — **часть документации по языку**:
`precheck.sh` генерирует из них C и собирает его, форматтер обязан держать их в
каноне, а свод прямо требует, чтобы проверенные примеры попадали в
...
Профиль проекта — промышленные системы управления. Контроллер
...
- Починка `examples/comprehensive.lam`: модель должна проходить заявленный
  ...
  управления и сохраняя покрытие конструкций языка, заявленное в
  её шапке.
...
- **Приёмочный тест прогоном симулятора**: сценарий проверяется
...
4. **по своду п.4** аналитик ставит фичу **выше** прочих кандидатов по
...
- **Обратная совместимость:** язык **не** меняется — версия языка не
  растёт. На публичный API, грамматику и семантику влияния нет.
...
в документацию по языку, собираются в C через `precheck.sh` и
...
1. **Пример — документация по языку.** Он обязан быть
   корректным: то, что он объявляет, должно исполняться. Мёртвое ребро в витрине
   языка — антипример.
2. **Правдоподобие модели управления.** Профиль проекта —
...
4. **Язык не меняется.** Правка — в модели-примере. Ни
...
- **Язык не меняется**: версия языка не растёт. Правится
...
- Заявленный сценарий становится **проверяемым машиной**, а не
  предметом веры. Класс дефекта, стоивший ошибки в критерии A6 фичи, ловится
  автоматически.
- Пример начинает соответствовать профилю проекта: термоциклирование
...
- **Меняются ожидания зафиксированных проверок.** Тест-план 0025 содержит
  ...
  переписываются** (они — история решения); расхождение снимается
...
- **Action item:** после реализации перенести пример (или его ключевой фрагмент)
  в документацию по языку — свод требует, чтобы проверенные примеры
  попадали в README.
...
7. Язык не изменён: версия языка не растёт; грамматика, семантика и
...
Основание для работы — (примеры суть документация по языку и
обязаны проверяться), (профиль — промышленные системы управления:
контроллер нагрева обязан быть правдоподобным). Язык не меняется, поэтому
свод не срабатывает: версия языка остаётся **0.2.0**.
...
- **Влияние на порядок разработки:** фича **никого не разблокирует** (жёстких
  ...
  момент. по своду п.4 аналитик ставит её **выше** прочих открытых кандидатов
...
- **R4. Модель правдоподобна как система управления.** Автомат
...
- **R5. Сценарий закреплён машиной, а не текстом.** Проверка —
...
- **Язык (`.lam`).** Обратная совместимость **не затрагивается**: правится
  ...
  `.lam`-файл не перестанет компилироваться. Версия языка не растёт,
  остаётся **0.2.0**.
...
- **Сломанная обратная функциональность — одна, и намеренно.** Контрпример **T19a**
  ...
  становится историей. Это **цель фичи**, а не регрессия. Артефакты закрытой фичи **не переписываются** (они фиксируют историю решения);
...
- **`CLAUDE.md`.** Содержит ловушку «`examples/comprehensive.lam` дефектен
  ...
  `CLAUDE.md` занимается координатор при закрытии).
- **`FEATURES.md`.** Кандидат «`examples/comprehensive.lam` заявляет недостижимый
  сценарий» удаляется из блока кандидатов при заведении фичи —
...
Аналитика **не декомпозируется**: объём фичи мал — этого документа
...
| Язык (`grammar.lalrpop`, `lexer.rs`, AST) | **н/п** | язык не меняется; версия остаётся **0.2.0** |
...
**Отдельно зафиксировать в отчёте** («сломанная обратная
...
историей. Артефакты закрытой фичи **не переписываются**;
...
и — главное — всё это закреплено **машиной**, а не документом.
...
> `comprehensive.lam`. по своду пример из T8 после проверки годится для
...
| Тест-план/отчёт фичи | **да, намеренно** — T19a («`Heating` — тупик») перестаёт воспроизводиться; артефакты 0025 **не** переписываются, расхождение снимается ссылкой из отчёта 0030 | T1, T4 | ✅ |
...
**Намеренно сломанная обратная функциональность.** Контрпример
...
переписаны** (они фиксируют историю решения); расхождение снимается
