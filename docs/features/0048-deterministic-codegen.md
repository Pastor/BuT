...
> своду: архитектура (ADR) → анализ → разработка.
...
поэтому версия языка **не растёт**. Однако **порождённый C
...
- **ABI меняется однократно** — номера портов и состояний станут другими для
  всех входов. Это смена ABI, но смягчающее обстоятельство решающее:
...
- **Версия языка не растёт**: синтаксис и семантика не меняются,
...
- **Пересечения по файлам — не зависимости** (критерий 1 неприменим):
...
**Язык не меняется — версия не растёт**. Ни синтаксис, ни семантика
...
старым заголовком, разойдётся с новым. Обоснование необходимости (свод требует его при нарушении совместимости):
...
- `cargo build --all-features --all-targets` — сборка.
...
- Живая проверка (A5): `precheck.sh` целиком зелен — `cc` собирает C, `iec2c`
  принимает ST.
- `python3 scripts/check-links.py` — ссылки целы.
...
Язык не меняется — синтаксических примеров/контрпримеров нет
(свод неприменимо): фича не вводит и не удаляет ни одной конструкции.
