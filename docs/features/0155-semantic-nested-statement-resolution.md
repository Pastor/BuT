...
1. **Молчаливая потеря кода — худший класс дефекта.** Программа принимается, а
   исполняется не та, что написана. Это прямое нарушение цели проекта: язык синтеза автоматов промышленных систем управления не имеет
...
2. **Диагностика не должна зависеть от глубины вложенности.** Один и тот же
   ...
   артефакт реализации.
3. **Обратная совместимость.** Замер: 276 файлов `.takt`
...
файлах). Подъёма версии языка свод здесь не требует: множество
...
- **A-3.** (редакторский слой): новых токенов, форм объявления и
...
формулы**; это отдельный дефект, заведён кандидатом в `FEATURES.md`
...
**Версия языка не поднимается**: синтаксис и семантика не меняются,
...
### 7. Зависимости и связи с другими фичами
...
- **доставка диагностики в редактор** — сервер и `taktc` ходят через общий вход
  ...
  LSP тем же путём. Это **проверяется тестом**, а не рассуждением: свод заведено ровно потому, что «должно доехать» и «доезжает» — разные утверждения.
...
| [семантическое разрешение тел вложенных операторов](0155-semantic-nested-statement-resolution.md#разработка) | Документ `book/` и актуализация `CLAUDE.md`/`CHANGES.md`/`README.md` |
...
**Функциональность.** Языковые конструкции не менялись — «н/п» для
...
**Фикстуры** — `takt-lang/tests/data/nested0155/` (примеры и
...
| `nested_diagnostic_reaches_lsp` | свод: диагностика доходит до редактора |
...
**`book/src/06-control-flow/index.typ`** (меняется наблюдаемое
...
- блок **«Частые ошибки»** дополнен `SE-003` и `SE-062` — оба уже
  разобраны в приложении «Ошибки и предупреждения», дублирования нет.

Номеров фич, ссылок на ADR/отчёты и деталей реализации в текст не попало: раздел говорит о **языке** — что проверяется и что видно, — а не о
...
- `./scripts/precheck.sh` — код возврата `0`; проверка примеров `book/` (правило «Пример раздела: функциональный, из практики, с разбором-выносками»)
  прошёл: 18 примеров компилируются и симулируются, 1 библиотечный.
- Ссылки Markdown правленых файлов проверены.
...
| T14 | Диагностика доходит до LSP | `if_then_unknown.takt` через `collect_diagnostics_at` | `SE-003` в списке | A3 ADR, свод |
...
| LSP | ✅ | T14 |
| Плагины редакторов | — | новых токенов/форм/узлов нет (зафиксировано в анализе) |
...
`book/` перенесены не сами фикстуры, а правило, которое они
...
| LSP | ✅ | T14 |
| Плагины редакторов | — | правок не требуют (обоснование в анализе §8) |
