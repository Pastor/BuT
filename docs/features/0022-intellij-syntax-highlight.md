...
удобное чтение и редактирование спецификаций автоматов Lam, не
...
4. **Целевая аудитория** — инженеры асу тп на IntelliJ; ценят
   офлайн-подсветку «из коробки», настраиваемые цвета и работу в Community.
5. **Обратная совместимость** — фича **аддитивна**: новый
   подпроект `extensions/intellij-lam/`, язык и компилятор не затрагиваются,
   версия языка не меняется (свод не применяется).
...
4. **Аддитивность** — не трогать язык, компилятор, версию языка.
...
- Аддитивно: язык/компилятор/версия языка не затрагиваются (свод неприменим).
...
- README проекта дополняется разделом об оснастке IDE (Zed + IntelliJ).
...
6. Язык, компилятор и версия языка не изменены (аддитивность).
...
- **R7. Аддитивность.** Крейты `grammar`/`simulation`, синтаксис,
  семантика и **версия языка** не изменяются (свод неприменим — меняется
...
незаметно. Версия языка не растёт (свод неприменим).
...
Функциональность по стекам: язык/компилятор — **н/п** (аддитивно, не
...
Функциональность по стекам: язык/компилятор — **н/п** (аддитивно).
...
- **Документация:** создан `extensions/intellij-lam/README.md` (возможности,
  ...
  лексики); корневой `README.md` (§8) дополнен; `CHANGES.md`.
...
от **лексики** языка (набор ключевых слов и операторы 0021), поэтому по своду
...
Фича аддитивна — обратная функциональность языка не задета. Матрица
...
версия языка не затронуты (свод неприменим); дефектов не выявлено, исправления не
требуются.
...
Фича аддитивна — обратная функциональность языка/компилятора не
...
`simulation`, синтаксис/семантика и **версия языка** не тронуты (свод неприменим).
...
- **0022-03 — цвета/эргономика/доки:** `LamColorSettingsPage`, `LamCommenter`
  ...
  `L*/R*`); README подпроекта + раздел в корневом README.
