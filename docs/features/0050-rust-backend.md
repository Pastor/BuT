...
- **Крейт:** `grammar` (минорный рост версии крейта; **версия языка не меняется**
  — синтаксис и семантика `.lam` не тронуты)
...
> жизненный цикл по своду. Проработана до стадии **РАЗРАБОТКА** (ADR принят,
...
| **0050-08** | Примеры, контрпримеры, документация |
...
[генератор генерации в Rust](0050-rust-backend.md)-02 завернул бы в `#![no_std]`-крейт и `main`-ы, которые печатают.
...
1. **Профильная цель.** Прошивки промышленных систем управления —
...
**Pros:**
- Прямое попадание в профиль DSL — тот же контур, что у `c`.
...
- **A6. Ловушки стандартной библиотеки нет** (в отличие от 0041, где `Concat`
  ...
  задача не заводится.
- **A7. Версия языка не растёт**: синтаксис не тронут, цель
  аддитивна (прецеденты `lamc fmt` 0024, `lamc verify` 0049).
- **A8. Кандидаты-расширения** (вносит координатор): (1) MMIO-режим
...
- **R1. Аддитивность.** `Language::Rust` + `generator/rust/` + цель `-t rust` +
  ...
  неизменен; `grammar.lalrpop` и версия языка не тронуты.
...
> **Реализовано.** (2026-07-16). План по ADR.
...
1. **README — раздел «Генерация Rust (`no_std`)»** по образцу раздела ST:
   ...
   - как подключить порождённый `.rs` (`mod`, реализация трейта `Hal`) —
     **работающий пример целиком**, прогнанный пробой;
...
5. Версия языка **не растёт**: `grammar.lalrpop` и `Cargo.toml`
   языка не трогаются.

#### Проверки

- **свод:** каждый пример README прогнан (`lamc compile -t rust` → `rustc`)
...
**Фикстуры** `grammar/tests/data/rust/` — примеры и контрпримеры парами: `self_name.lam` (T15), `keyword_name.lam` (T14), `prelude_name.lam`
