...
- **Приоритет:** средний (**Tier 3**). Выше косметики: симулятор **молча теряет
  ...
  (Tier 1): те ломают **основную** цель проекта — синтез C, тогда как
...
фичи. Язык при этом **не меняется** — версия языка не растёт.
...
3. **Достоверность относительно C**. Эталон C для структур с полями
...
4. **Язык не меняется**. Реализуется уже специфицированное
...
5. Неверная арность инициализатора, неизвестное поле, `Member::Number` по
   ...
   ненулевой код возврата CLI (контрпримеры).
...
10. Язык не изменился: грамматика, АСД и `.lam`-корпус не тронуты; версия языка не
    выросла; тест `syntax_simple` зелёный.
...
[починка вычислителя выражений симулятора](0025-simulator-expression-eval.md) объединила пути **условий** и **тел блоков**, но путь **инициализаторов**
...
- **Влияние на порядок разработки:** фича независима и никого не разблокирует —
  ...
  (Tier 2). Поднимать её вперёд оснований нет (критерий 4): корпус
...
| A7 | Контрпримеры: арность, неизвестное поле, `p.0`, `p = q` → диагностика с позицией + ненулевой код CLI | контрпримеры T11–T15 |
...
**свод.** Обратная совместимость **сохраняется полностью**; обоснования
слома не требуется.

| Слой | Влияние | Обоснование |
|---|---|---|
| **Язык `.lam`** | **нет** | Грамматика и АСД не трогаются. Реализуется уже специфицированное поведение (структуры, `ГОТОВО`). Версия языка **не растёт**. |
...
pub(crate) fn coerce_to_type(
    value: Value,
    ty: &TypeNode,
    structs: &dyn StructRegistry,) -> Result<Value, EvalError>
```
...
свод обязательно: ниже — и **примеры** (что обязано работать), и
