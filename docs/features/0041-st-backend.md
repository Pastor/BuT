...
- **Приоритет / Tier:** **Tier 1 (высокий)** — профильная фича проекта («язык задания и синтеза автоматных моделей **промышленных систем управления**»).
...
- **Крейт:** `grammar` (крейт `0.2.0` → `0.3.0` по SemVer; **версия языка не
  меняется** — синтаксис и семантика `.lam` не тронуты)
...
| **0041-07** | Примеры (`elevator.lam`, `stacker.lam`), контрпримеры, документация |
...
- **`generator/mod.rs:14`** — перечисление `Language` ровно с двумя вариантами:
  ...
  не должно ломать обратную совместимость». Точка расширения
...
1. **Профиль проекта.** ST — целевой язык ПЛК. Без него `lamc`
   синтезирует «во что угодно, кроме промышленного контроллера».
2. **Аддитивность.** `Language` уже `#[non_exhaustive]`; добавление
   варианта не должно менять ни байта вывода целей `c`/`c-hal`/`plantuml` и не
   должно трогать `.lam`. Версия языка не растёт.
...
- **Семантика не совпадает.** SFC-шаг имеет действия с квалификаторами
  ...
  пришлось бы либо менять семантику языка (свод → версия языка растёт;
...
**EBNF порождаемого подмножества ST** (генерируется код на другом
...
**Cons:** **меняет язык** → версия языка растёт, фича перестаёт быть
...
- **Профиль проекта закрыт:** `lamc` синтезирует в стандартный язык
...
8. Примеры и контрпримеры попали в документацию по языку.
...
[генератор генерации в Structured Text (IEC 61131-3)](0041-st-backend.md)-06 до начала 0041-03.** Это самый рискованный пункт задачи: неверный выбор
...
[адрес порта — размещение и потребление (карта адресов)](0020-port-address-decl.md)-03 уже есть).
...
другой сессией (контекст для сессий на другом компьютере).
...
- **R10.3.** Решения по T13 (перечисления), О1 (`VAR_IN_OUT`), РИ5 (`extern fn`),
  ...
  предположению.
- **R10.4.** При провале пробы ограничение фиксируется **явно** в отчёте («итоговый вердикт»), а фича не выдаётся за проверенную.
...
| A9.4 | При провале — ограничение записано в отчёт открытым текстом | Ревью отчёта |

### Заметка на будущее (в бэклог)
...
чтобы `lamc` синтезировал в стандартный язык ПЛК. Это прямое попадание в («язык задания и синтеза автоматных моделей **промышленных систем
...
- **Зависит от:** **`0020`** (Адрес порта: размещение + потребление) — **статус
  ...
  **Зависимость закрыта → статус `ЗАБЛОКИРОВАНА` не ставится** (снимается, когда все зависимости в `ГОТОВО`). Фичу можно брать в работу сразу.
...
- **Фича не блокируется** (единственная зависимость 0020 — `ГОТОВО`) и по своду может быть взята **сразу**: критерий 1 (зависимости) удовлетворён, критерий 2
  (проработанность) — после принятия этих артефактов фича в `РАЗРАБОТКА`, критерий
  3 (приоритет) — **Tier 1**, профильная.
- **Предложение аналитика (свод.4, «необходимость»).** 0041 — самая крупная
...
- **R12. Версионирование.** Синтаксис и семантика `.lam` **не
...
| A12 | Примеры и контрпримеры — в документации по языку | Ревью `README.md`; `lamc fmt --check examples/` зелёный |
...
- **Публичный API крейта расширяется аддитивно.** Ключевой вопрос — «расширение
  ...
  вариантов не должно ломать обратную совместимость». Следствие:
...
| РИ6 | **Дефекты примеров всплывут в новом генераторе.** `comprehensive.lam` заявляет недостижимый сценарий; функции не могут вызывать функции (0031) | Низкая | 0041 их **не чинит** и не обязана: ST-генератор транслирует то, что есть. Если пример не транслируется — фиксируется как находка в отчёте и уходит в бэклог |
...
| **0041-07** | Примеры, контрпримеры, документация | R12, свод | [dev](0041-st-backend.md#разработка) |

**Порядок:** 0041-01 → **0041-06 (проверка)** → 0041-02 → **0041-04 → 0041-03** →
[генератор генерации в Structured Text (IEC 61131-3)](0041-st-backend.md)-05 → 0041-07. Обоснование выноса 0041-06 вперёд — см. «Влияние на порядок
разработки» выше.
...
| Язык (`lexer`, `grammar.lalrpop`, `parser/ast.rs`) | **н/п** | свод: язык не меняется |
...
[генератор генерации в Structured Text (IEC 61131-3)](0041-st-backend.md)-02 обязана эмитить хотя бы `VAR … END_VAR`, иначе вывод остаётся
...
**Вывод:** критерий приёмки в исходной формулировке
...
[генератор генерации в Structured Text (IEC 61131-3)](0041-st-backend.md)-04, ждёт под `#![allow(dead_code)]`; разрешение снимет часть 2.
...
[генератор генерации в Structured Text (IEC 61131-3)](0041-st-backend.md)-03 отображает `ref T: cond;` в `IF <cond> THEN`, а `enter { … }` — в
...
| Язык | **н/п** | свод; `AT %` **не** вводится в `.lam` (Option B ADR отвергнут) |
...
1. **Проверка нельзя включить в `precheck.sh` сейчас** — он валил бы предкоммит до
   ...
   открытым** и это записано честно, а не замаскировано.
...
| **R10.4** (честный отчёт) | ✅ | Красные пробы (П4, П8, П9), незакрытый R10.2 и невалидность нынешнего вывода записаны прямо |
...
`README.md` («детально описывает проект… содержит, в том числе,
...
examples/`) — они **являются документацией по языку**. Фикстуры
...
###### 5. `README.md` — раздел «Генерация Structured Text»
...
- **Контрпримеры**: что **не** транслируется и с какой диагностикой.
...
- **`CHANGES.md`** — раздел `[Не выпущено]`: аддитивная фича; крейт `grammar`
  `0.2.0` → `0.3.0`; **версия языка не растёт** — по образцу записей
  0024/0025 («Язык не менялся → версия языка не растёт»).
- **`CLAUDE.md`** — краткий контекст ST-генератора (**без** хронологии) —
...
- **Карточка фичи** — раздел «Итог (что сделано)» при закрытии.

##### Статус по функциональности

| Функциональность | Работа | Обоснование |
|---|---|---|
| `README.md` | **Требуется** | свод |
| `CHANGES.md` | **Требуется** | свод |
| `CLAUDE.md` | **Требуется** | свод: краткий контекст |
...
| Язык | **н/п** | свод: версия языка остаётся 0.2.0 |
...
| Требование | Проверка | Как |
|---|---|---|
| **свод** (примеры/контрпримеры) | T12–T43 | Все фикстуры покрыты тестами **до** попадания в документацию |
| **свод** (README) | T-ревью / A12 | Раздел «Генерация Structured Text» с примером кода и **порождённого ST** |
| **свод** (версии) | T6 / A11 | `README.md:1007` — «Версия языка: 0.2.0» **не меняется**; `grammar/Cargo.toml` = `0.3.0`; запись в `CHANGES.md` |
| **свод** (ссылки) | — | `scripts/check-links.py` зелёный (подключён в `precheck.sh`) |
...
работает и **проверено** («после реализации и проверки примеры должны
...
`R1..R12` и критерии `A1..A12` анализа. Фича **не меняет язык** (версия языка остаётся 0.2.0), но **меняет то, во что язык компилируется**, —
поэтому свод применяется в полном объёме: тесты корректности компилятора +
...
неавтоматизируема и неповторяема. Допустима как разовая проверка в
отчёте.

### Проверки (условие → ожидаемый результат)

#### Группа 1. Обратная совместимость
...
| T51 | Провал проверки задокументирован | — | Ограничение записано в отчёт **открытым текстом** | R10.4 / A9.4 |

### Примеры и контрпримеры

После проверки **обязаны попасть в документацию по языку** (`README.md`) — раздел «Целевые платформы» / «Генерация ST».
...
5. Примеры и контрпримеры — в `README.md`.
...
`.lam` не тронуты. Цели `c`/`c-hal`/`plantuml` не изменены ни на
строку.
...
языка не менялась** — `.lam` не тронут. Цели `c`/`c-hal`/`plantuml`
