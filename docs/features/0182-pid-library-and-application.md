...
> своду. Проработка (стадии 2–3) выполнена сразу, разработка — по отдельному
...
раздел** документа `book/` («каждый новый раздел — отдельная фича»),
...
вывод (исключение своду). Номера фич, ссылки на ADR и сведения о процессе в
...
3. **Пример — это документация языка**. Регулятор без
...
типичные ошибки по своду), затем применение (объект, замыкание контура,
прогон симулятором, наблюдение сходимости). Номера фич, ссылки на ADR и
сведения о процессе в документ не попадают.
...
порты, `float`, композиция). Версия языка не растёт.
...
- **Влияние на порядок разработки:** ничего не разблокирует и ничем не
  ...
  правок; конкретная позиция — за пересмотром свод при взятии в работу.
...
автоматическая, поэтому номер каталога — единственное, что задаёт
порядок.
...
- **R5. Раздел документа самодостаточен.** Описывает регулятор и его
  применение, содержит блок типичных ошибок и не содержит номеров
  фич, ссылок на ADR и сведений о процессе.
...
`ЗАБЛОКИРОВАНА` с зависимостью от неё (критерий 1). Раскладка файлов
...
внутреннее устройство), полный текст регулятора с разбором по выноскам, применение с объектом и замыканием контура, снятый с настоящего
прогона след сходимости, границы механизма (протокол объявляет одна сторона;
параметризации моделей нет) и блок «Частые ошибки».
...
**свод(е) — такт не единица физической величины.** Модель объекта
...
Язык фича не меняет: используются готовые конструкции. свод применяется в части примеров; свод: ответ в отчёте.
...
| LSP и плагины | язык не меняется | ✅ |
...
| LSP и плагины | ✅ правок не требуется: фича не меняет ни лексику, ни синтаксис, ни семантику — только состав корпуса и документа; новых диагностик не вводит |
