...
до синтеза в C (цель проекта — язык и тулы для промышленных систем
...
5. **Тесты** — сверять **вычисленные значения** переменных, а не только факт
   ...
   своду; `comprehensive.lam` и `stacker.lam` (`eta`) — приёмочные сценарии.
...
специфицированного поведения, версия языка **не** повышается.
...
> проходит жизненный цикл по своду: архитектура (ADR) → анализ → разработка.
...
Симуляция — способ проверить автоматную модель **до** синтеза в C.
...
2. **Достоверность симуляции.** Наблюдаемое поведение симулятора обязано совпадать с
   поведением синтезированного C — иначе проверка модели бессмысленна.
...
4. **Язык не меняется.** свод: чинится исполнение уже
...
- **Результаты симуляции изменятся** — в сторону спецификации. Пересмотреть
  ...
  эталона.
- **Объём больше Option A** → декомпозировать на задачи `0025-YY`;
...
5. Невычислимое выражение приводит к **диагностике** с позицией в исходнике и
   ненулевому коду возврата CLI — проверяется контрпримером.
...
исполнимого плана: зафиксировать зависимости, проверяемые требования,
...
| [крейт simulation — симуляция моделей](0012-simulation-core.md) «Крейт simulation» (`ГОТОВО`) | Фича закрыта; 0025 чинит её дефекты. Закрытая фича зависимостью быть не может. | **не зависимость** |
...
- **Влияние на порядок разработки:** порядок не меняется — 0025 остаётся **первой**
  (подтверждаю решение стадии 2 по критерию 4 своду: дефект, из-за которого
...
(вносятся в бэклог кандидатов). До тех пор симулятор для этих типов
...
| A10 | Невычислимое выражение → диагностика + ненулевой код возврата CLI | контрпример: модель со структурной переменной / делением на ноль |
...
параллельно разработке), поэтому задача 05 переопределена на **канал
...
| `0025-08` | Перенос примеров и контрпримеров в `README.md` | — | 06 |
...
> **Задача 02 разделена при реализации** — исходная оценка анализа
...
- **Язык не меняется.** Синтаксис и семантика `.lam` не затронуты; версия языка
  **не** повышается. Чинится исполнение уже специфицированного
  поведения.
...
- **Результаты симуляции изменятся** — в сторону спецификации; текущее поведение
  дефектно. Это не слом совместимости, а исправление.
...
- **свод / `docs/CODE.md`:** чистые функции (`apply_*`, `coerce_to_type` — без
...
- **свод:** 01 не тянет за собой адаптеры — только ядро.
...
Задача разделена (декомпозировать крупные изменения):
...
примеры являются документацией по языку и обязаны быть верными.
...
«сохранял пустоту» из пустоты. Внесено в бэклог кандидатов.
...
- **`0025-08`** — перенос примеров и контрпримеров в `README.md`.
...
Это существенно для профиля проекта: симуляция служит проверкой
...
свод требует: «после реализации и проверки примеры должны попасть в
документацию по языку программирования»; свод: README описывает синтаксис,
...
тесты корректности языка и его интерпретатора, **примеры и контрпримеры**; после реализации примеры переносятся в документацию по языку (раздел «Перенос примеров в документацию»).
...
#### Группа 4. Контрпримеры — ожидается **отказ с диагностикой**
...
| Язык `.lam` | Синтаксис/семантика не меняются; версия языка не растёт | T36: `cargo test --all-features` — грамматика/семантика без изменений | ⬜ |
...
- **Известное окружение:** при смене nightly возможен `E0514` (устаревшие
  артефакты) — снимается `cargo clean`; к фиче отношения не имеет. свод требует сборки с очисткой — прогон приёмки выполнять после `cargo clean`.
...
сборка с очисткой чистая. Примеры и контрпримеры перенесены в `README.md` и **сверены прогоном**.
...
3. **Примеры не перенесены в документацию** (`README.md`) — свод требуют этого явно.
...
|---|---|---|
| Полная сборка с очисткой | `cargo clean && cargo build --all-features --all-targets` | **0 errors** |
...
**Перенос в документацию (`README.md`) не выполнен** — свод требуют,
чтобы примеры после проверки попадали в документацию по языку. Пункт вынесен в
доработку.

### Найденные дефекты

Все — **вне объёма 0025**, внесены в бэклог кандидатов `FEATURES.md`.
...
2. **`0025-07` — сверочный тест с `lamc -t c`** (критерий A8/T21) вместо ручной
   проверки.
3. **`0025-08` — перенос примеров и контрпримеров в `README.md`**.
...
> свод: краткая сводка; детали — в артефактах стадий, «Итог» их не
...
контрпримеры перенесены в `README.md` и сверены прогоном.
...
версия языка не повышалась. Версия крейта `simulation`: `0.0.1` →
`0.1.0` (minor).
