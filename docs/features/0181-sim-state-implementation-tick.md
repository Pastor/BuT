...
прежние, версия языка не растёт.
...
[симулятор исполняет реализацию состояния с переходом next](0181-sim-state-implementation-tick.md)-03 правят именно такт и добавляют строки, то есть упёрлись бы в проверка
...
Обратная функциональность: затронут только внутренний состав
...
[симулятор исполняет реализацию состояния с переходом next](0181-sim-state-implementation-tick.md)-03 есть у обоих.
...
Обратная функциональность: затронут только крейт `takt-sim`; цели
...
Обратная функциональность: затронут только `takt-sim`. Цели
генерации не изменялись — **н/п**.
...
Обратная функциональность: затронуты только тесты крейта
