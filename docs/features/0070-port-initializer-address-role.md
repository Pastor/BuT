...
- **Обратно совместимо**: множество принятых программ только
...
- **0070-01:** в `validate/enums.rs::validate_bit_values` вывести
  ...
  `Simple`/`Const`). Тесты — примеры/контрпримеры.
...
- **Прецедент 0098:** добавление диагностики `SE-060` (диапазон бита) закрыто
  **без** подъёма версии — в проекте изменение набора диагностик не считается
  ...
  тоже без подъёма.
- Изменение **обратно совместимо**: множество принятых программ лишь
...
- **R2. Не-порт проверку сохраняет.** `var`/`const` типа `bit` с `:= N` (N∉{0,1})
  по-прежнему дают `SE-035` (контрпример — свод).
...
функциональности сводится к A5/A6 (вывод и корпус неизменны) + A2
...
**Обратная совместимость**: множество принятых программ расширяется
...
- **A5/A6/A7:** `./scripts/precheck.sh` зелёный; `git diff --stat
...
- **Примеры/контрпримеры**: `tests/data/semantic/valid/port_bit_address.lam`
...
(примеры/контрпримеры) в **новом файле**
...
- **0070-01** (`grammar/src/semantic/validate/enums.rs`): `VariableNode::Port`
  ...
  адрес».
- **Тесты** — новый файл `grammar/tests/port_initializer_tests.rs`
...
- **README** — раздел «Адресные литералы (MMIO)» переписан: порт-инициализатор =
  адрес, форма `addr:бит`, контрпример `var f: bit := 5;`.
...
**Обратно совместимо**: принимаемых программ больше, ни одна прежняя
