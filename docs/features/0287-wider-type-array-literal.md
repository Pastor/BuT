...
**Замер снят заново, а не унаследован**: исходный замер
...
**Наблюдаемое поведение меняется сознательно**:
...
**Функциональность:** язык — да (выведенный тип и наблюдаемое
значение, см. анализ); LSP и плагины — н/п (новых токенов, форм
...
| Эталон (`takt-sim`) | ✅ | T10 |
| LSP и плагины редакторов | — | свод: новых токенов, форм объявления, узлов АСД и диагностик нет (обосновано в анализе) |
...
| Эталон `takt-sim` | ✅ | T10 |
| LSP и плагины | — | новых токенов, форм объявления, узлов АСД и диагностик нет; проверка не требуется, ответ зафиксирован в анализе |
| Документ `book/` | ✅ | раздел «Вывод типов» дополнен; сборка и проверка глифов зелены |
| Форматтер, `semantic::usages`, стиль | — | узлов АСД не добавлено |

### Выводы и дальнейшие шаги

- Исправления не нужны; фича закрывается.
- **Кандидат заведён**: локальное объявление **в теле блока** типа
...
- **Кандидатов, называющих 0287, не осталось**: запись блока 2 была
...
**Замер снят заново** и показал, что формулировка кандидата
...
примером. **Версия языка не поднята** (цикл `0.10.0`, накопление по своду);
крейт `takt-lang`: `0.44.0` → `0.45.0`.

**Кандидат заведён**: локальное объявление **в теле блока** типа не
