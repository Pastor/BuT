...
- **Неосуществимо:** это проверка прозы на истинность. Ровно та причина, по
  которой свод разделило роли README и `book/`, а не завело «сверку
  текстов».
...
- **A-1. Содержательный дрейф остаётся на человеке**. Проверка проверяет
...
своду, `concat.takt` — гипотетический пример ловушки имён IEC,
...
- **Истинность утверждений о поведении кода.** Проверка не скажет, что «`Bit` →
  ...
  причина, по которой свод разделило роли README и `book/`, а не завело
...
- **свод: не применимо** — ни лексики, ни синтаксиса, ни семантики.
...
- свод: кандидатов, называющих 0149, в блоке 2 не было — запись была
