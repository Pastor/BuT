...
**Версия языка:** язык меняется, но подъёма **нет** — (решение
...
поднятую** версию. Обязанность обосновать изменение языка сводом сохраняется
и исполнена этим ADR.

**свод:** фича меняет синтаксис → EBNF-диаграмма обязательна.
...
- **A-3. Ловушка C11 6.5.7p5** — контролем обязан быть тест, а не вера в
  clang.
...
- **A-5. Насыщение (saturation) — вне объёма** (wraparound). Для
...
4. **A4.** Побитовая потактовая сверка `q`-арифметики: симулятор = `c` = `rust` =
   ...
   `*`-округление к −∞.
5. **A5.** Цель `c` не содержит `>>` над знаковым отрицательным —
   проверяется грепом порождённого кода **и** тестом на значениях.
6. **A6.** Неявное смешение `q` с `float`/целым → ошибка.
...
9. **A9.** Пример с реальной дробной моделью заведён и проходит сценарный проверка
   достижимости.
10. **A10.** Версия языка **не поднята** (цикл 0.3.0 не выпущен);
    README документирует `q(m,n)` с примерами.
...
ADR принял **Option A** с **нормативной** численной семантикой (своду–8).
...
- **Влияние на порядок разработки:** никого не разблокирует. **Аналитик
  рекомендует держать её низко** (критерий 4 своду): фича добавляет
...
| `c` | ловушка C11 6.5.7p5 (свод ADR) |
...
- **R10. Версия языка не поднимается** (цикл 0.3.0 **не выпущен**).
- **R11. README документирует тип с примерами**.
...
**Правка строго аддитивна**: добавляется **новый** тип; `float`,
...
выпущена**, поэтому 0061 вносит изменения в уже поднятую версию (решение заказчика 2026-07-15). Обязанность обосновать изменение языка — исполнена
в ADR.
...
- **R-1. Тихое расхождение арифметики** (главный риск; ровно то, из-за чего 0045
  ...
  своду–8 ADR; (2) **побитовая** сверка A4 как основной критерий; (3) образец
...
- **R-2. C11 6.5.7p5.** Проба на clang «работает» — и именно поэтому опасна:
  ...
  свод ADR + A5 (греп **и** тест на значениях).
...
- **R-6. Насыщение попросят сразу.** Для регуляторов wraparound редко то, что
  нужно. Снижение: свод фиксирует wraparound **явно**, насыщение — кандидат;
...
по своду ADR: `q(m, n)` — знаковый, `m` целых бит **включая знак**, `n`
...
- Версия языка **не поднимается** (цикл 0.3.0 не выпущен) — T18.
...
свод–5 ADR, **побитово совпадающая** с симулятором (0061-02).

**Цель `c` — ловушка C11 6.5.7p5** (свод ADR): `>>` **знакового
...
`q(m, n)` → `logic signed [W-1:0]`; арифметика по своду–5 ADR.

- `SV-003` для `q` **не** срабатывает; для `float` — **остаётся** (свод ADR).
...
1. **Реальная дробная модель** (регулятор — типовой потребитель fixed-point на
   ...
   **Зачем именно пример, а не фикстура:** фикстуры пишет разработчик — они
   ...
   заранее.
2. **README**: описание `q(m,n)`, нормативная арифметика
...
3. **`CLAUDE.md`** — записать ловушки, снятые проработкой: C11 6.5.7p5 (`>>`
   ...
   числами в ST.
4. **Версия языка не поднимается** (0.3.0 уже поднята фичей и
...
Фича **меняет синтаксис и семантику языка**, поэтому свод обязывает: тесты
корректности языка и компилятора + **примеры и контрпримеры**, а после реализации —
их перенос в документацию.
...
| T6 | **Контрпример: неявное смешение** | `var a: q(8,8); var b: u8; a := a + b;` | **Ошибка** (свод ADR) | R5 / A6 |
...
| T18 | Версия языка | — | **Не поднята** (цикл 0.3.0 не выпущен); README документирует `q(m,n)` с примерами | R10, R11 / A10 |
| T19 | Переполнение `+` — wraparound | `q(8,8)` на границе | Перенос, как у целых (**не** насыщение — свод ADR) | R3 / A4 |
...
- **Примеры и контрпримеры** — в `grammar/tests/data/semantic/{valid,invalid}/`;
  после реализации переносятся в README.
...
- **Насыщение (saturation)** — свод ADR фиксирует **wraparound**; насыщение
  вынесено кандидатом.
...
- **Контрпример:** `var t: float` → `SV-003`; `x as float` при `x : q(8,8)` →
  `SV-003` (в синтезируемом RTL плавающей точки нет — свод ADR).
...
| — | README | ✅ | таблица типов + раздел «Fixed-point `q(m, n)`»: арифметика, примеры, контрпримеры |
...
### Примеры и контрпримеры (в README)
...
**Версия языка не поднимается** (решение заказчика 2026-07-15): `0.3.0`
