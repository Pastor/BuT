...
- **Статус:** **ОТМЕНА** (2026-07-15, решение заказчика — см. «Итог (отмена)»
  в конце карточки; своду). До отмены: РАЗРАБОТКА · ЗАБЛОКИРОВАНА.
...
(свод неприменим), растёт лишь версия плагина.
...
> 0038 (отмена — решение заказчика). Детали — в
...
> 2–3 жизненного цикла и отменена на стадии 4 — см. ниже.
...
6. **Отсутствие изменений языка.** Фича редакторная: синтаксис/семантика и версия
   языка не трогаются (свод неприменим; схема слоёв приведена по своду
...
3. **Аналитик выносит заказчику предложение об `ОТМЕНЕ` фичи** (свод —
...
- **Фича `ЗАБЛОКИРОВАНА` до закрытия 0038** — работа не начинается.
- **Контрольная точка после 0038 (обязательна):** переоценить остаток R1/R3/R5 в
  ...
  об `ОТМЕНЕ` 0040** (решение заказчика).
...
- **Обратная совместимость 0023**: `LamParser` перестаёт быть
...
- Язык не меняется: версия языка не растёт (свод неприменим); растёт
...
- **Влияние на порядок разработки:** 0040 ставится в таблице
  `FEATURES.md` **после 0038** со статусом `ЗАБЛОКИРОВАНА` и приоритетом Tier 3
  (свод п.1 — зависимость выполняется первой, даже если менее приоритетна;
...
- **Контрольная точка (обязательна).** После закрытия 0038 аналитик
  **переоценивает** остаток R1/R3/R5 в живой проверке и фиксирует решение здесь:
  ...
  `ОТМЕНЕ`** (отмена — решение заказчика; аналитик вправе лишь
...
- **R8. Обратная совместимость 0022/0023** **. Переход от плоского
...
свод: совместимость **полная**, но требует внимания в одной точке:

- **Язык не затронут.** Синтаксис, семантика, `grammar`/`simulation`, генераторы,
  форматтер, версия языка — без изменений (свод неприменим). Фича живёт
...
- **Пользовательская совместимость:** возможности **аддитивны** — новых
  ...
  минорно (SemVer; свод: к плагину, не к языку).
...
> [семантическая подсветка Lam в IntelliJ через lam-lsp](0038-intellij-semantic-tokens.md): работы не
...
- `grammar`/`simulation`, синтаксис/семантика/версия языка — **н/п**: фича
  редакторная, языковой слой не затрагивается (свод неприменимы).
...
- (полная сборка с очисткой) выполняется `clean buildPlugin test`.
...
- `grammar`/`simulation`, синтаксис/семантика/версия языка — **н/п**: rename
  правит **текст пользователя**, а не язык (свод неприменимы).
...
4. **Контрольная точка (анализ, A1) — фиксируется здесь по факту:** если
   ...
   заказчику **предложение об `ОТМЕНЕ`** фичи (решение
...
свод **неприменимо**: фича не меняет синтаксис/семантику языка (редакторная
оснастка), поэтому тестов компилятора/интерпретатора и примеров/контрпримеров
языка не требуется. **Но** ключевую проверку свод всё же диктует по духу:
...
- `grammar`/`simulation`, синтаксис/семантика/версия языка: **—** (не затронуты;
  фича редакторная, свод неприменимы)
...
разработки. Артефакты стадий сохранены как история решения: ADR,
...
(свод требует проверки: зависящие фичи не разблокируются автоматически).
...
[действие Reformat Code в плагине IntelliJ](0039-intellij-reformat.md) не меняет.
