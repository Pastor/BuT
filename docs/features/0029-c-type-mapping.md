...
- **Статус:** **ГОТОВО** (закрыта 2026-07-15). Оба вопроса, блокировавшие
  ...
  фича, правило 22); [заглушки генератора C: диагностика вместо тихого пропуска](0028-c-generator-stubs.md) берёт **`CC-018`**.
  Снята с витрины `FEATURES.md` (правило 10). См. «Итог» ниже и
...
обоснование по правилу 11 в [анализе](0029-c-type-mapping.md#анализ).
...
| [генератор C: отображение типов Array/Bit/Rational](0029-c-type-mapping.md#разработка) | Расширение сверки с симулятором (A8 фичи); примеры в документации языка (правило 16) | 01, 02, 03 |
...
1. **Версия языка: 0.3.0.** Открытый вопрос 2
   ...
   закрытая из пяти заявивших (правило 22); остальные (0031, 0033, 0042, 0044)
   вносят изменения в уже поднятую версию. Правки `.lam` не требуется,
   автомиграция не нужна. Коммит подъёма помечен тегом **`v0.3.0`** — по этому
   же решению правило 22 дополнено требованием тега.
