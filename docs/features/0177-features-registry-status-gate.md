...
> Фича зарегистрирована из процессного бэклога (отобрана заказчиком 2026-07-28); далее проходит жизненный цикл по своду.
...
2. **Ложное срабатывание убивает проверка** (урок: 73 ложных отказа из 105
   ...
   немедленно.
3. **Правило уже есть, проверки нет.** свод задают и набор
...
#### Option C. Сверка по **токену статуса** + проверка свод Из обеих ячеек извлекается известный статус (`ГОТОВО`, `СОЗДАНА`, …);
...
- («терминальный статус ⇒ фичи нет в витрине») становится проверяемым.
...
4. **свод**: терминальный статус (`ГОТОВО`/`ОТМЕНА`) ⇒ фичи **нет** в
...
`ТЕСТИРОВАНИЕ`, `ИСПРАВЛЕНИЕ`, `ЗАБЛОКИРОВАНА`, `ГОТОВО`, `ОТМЕНА`
плюс **`ЗАМОРОЖЕНА`**.

**`ЗАМОРОЖЕНА` в своду отсутствует.** Она введена решением заказчика для
...
- свод впервые проверяется машиной, а не памятью.
...
- **A-2. Список статусов дублирует свод.** Смягчение: в коде проверки стоит
...
4. Нарушение (закрытая фича осталась в витрине) валит проверка.
...
### 3. Третье отношение: свод Статус связан не только с карточкой, но и с витриной: свод требуют
...
Проверка обязан знать набор статусов. свод перечисляет: `СОЗДАНА`,
...
- **Р-1. Список статусов в коде проверки дублирует свод.** Смягчение: ссылка
...
| свод | закрытая фича осталась в витрине; открытой в витрине нет |
...
**`ЗАМОРОЖЕНА` принимается, хотя свод её не знает** — она введена
...
Проверяется, что проверка ловит расхождение статуса и нарушение своду, **не**
...
статус), A4, A5 (чистое дерево, ноль ложных), A6 (взведённость).
...
| T4 | свод | закрытая фича дописана в таблицу `FEATURES.md` | код `1` с указанием правила | A4 |
...
| T4 | свод | ✅ | `статус ГОТОВО, но фича осталась в таблице FEATURES.md` |
...
[заглушки генератора C: диагностика вместо тихого пропуска](0028-c-generator-stubs.md) — `` ГОТОВО (CC-018; вскрыт дефект трансляции `S(Модель) = Состояние`) `` —
...
Проверке нужен набор статусов. свод перечисляет девять; в реестре и витрине
...
[проверка согласованности статуса в реестре и в карточке фичи](0177-features-registry-status-gate.md) попутно считает существование в обе стороны, поэтому 0164 **дешевеет**, но
её область шире на шесть реестров.

### Выводы и дальнейшие шаги

- Фича закрывается; исправления не требуются.
- **свод:** запись процессного бэклога о синхронизации реестра исчерпана —
  часть (1) была исполнена аудитом, часть (2) закрыта этой фичей.
- **Заведён кандидат:** `ЗАМОРОЖЕНА` не описана в своду свода.
...
**совпадение токенов** статуса в карточке и реестре и **свод**
...
но в своду свода отсутствует — проверка его принимает, несоответствие заведено
