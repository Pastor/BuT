...
[анонимные порты](0189-anonymous-ports.md) анонимные порты). Ни у одного из них нет машинной сверки с кодом: проверки
...
закрытых фич, которые сводом не переписываются.
...
1. **свод свода — дубль снимают разделением ролей, а не сверкой прозы.**
...
2. **Нормативность уже назначена.** Приложение книги само объявляет себя
   нормативным источником, а README по своду отсылает к книге за описанием
...
4. **Контроль обязателен, но покрывает не всё.** Машинно сверить можно **лексику**
   ...
   навсегда, а P1/P2/P4…P12 — разовая правка под ревью.
5. **Цена сопровождения.** Каждый лишний описатель грамматики — работа при каждой
   языковой фиче (свод уже требует стадии «Документирование»).
...
**Pros:**
- Снимает дубль по образцу свод: источник по грамматике ровно один.
...
**Cons:**
- Оставляет два описания одного предмета — прямое противоречие своду и
...
и по своду принадлежит документу, а не README. Приложение приводится к
...
**свод: диаграмма.** ADR синтаксис и семантику языка **не меняет**
...
**Обратная совместимость.** Язык не затрагивается: ни одна
...
- **A2.** Проверка `check-book-keywords.py` покрывает **лексику**, а не правила: если
  ...
  (стадия «Документирование» у каждой языковой фичи) и свод.
...
1. Файла `Takt.ebnf` в репозитории нет; три ссылки `README.md` ведут на
   приложение «Грамматика» книги; проверка ссылок свод зелёный.
...
- **Зависит от:** нет. Проверено по контракту (фича не трогает код компилятора),
  ...
  правится) и по процессу (свод не задето: язык не меняется).
...
историческая проза (`CHANGES.md`, артефакты стадий закрытых фич) по своду не
...
Ссылка из README на файл **внутри** `book/src` законна: свод требует,
...
`./scripts/precheck.sh` проходит (включая новая проверка и свод о ссылках).
...
|---|---|---|
| A1 | `Takt.ebnf` отсутствует; ссылок на него в живых документах нет | `git ls-files Takt.ebnf` пуст; `grep -rn "Takt\.ebnf" README.md` пуст; проверка ссылок свод в `precheck.sh` зелёный |
| A2 | Приложение описывает P2, P4…P12 | Ревью против `grammar.lalrpop` + прогон примеров из приложения настоящим `taktc` (тест-план) |
...
Язык не затрагивается — обратная совместимость программ полная.
...
что приложение книги объявлено нормативным, а свод свода прямо снимает
...
- Три ссылки `README.md` перенаправлены на приложение
  ...
  `book/`), раздел «Расширение языка» — там же названы свод и новая проверка
  лексики, чтобы читатель, добавляющий конструкцию, видел обязанность документа.

**Историческая проза не переписывается**: вхождения `Takt.ebnf` в
`CHANGES.md`, в артефактах стадий закрытых фич (0100, 0116, 0140, 0201) и в
формулировке свод описывают состояние на свой момент и остаются как есть.
Проверка ссылок свод их не задевает — это текст, а не ссылки на файл.

#### Проверки

- `git ls-files Takt.ebnf` — пусто.
- `grep -rn "Takt\.ebnf" README.md` — пусто.
- `scripts/check-links.py` в предкоммите — битых ссылок нет:
...
- **истинность правил EBNF** машине недоступна (это проза о форме) — проверяется
  прогоном настоящего `taktc` на примерах и контрпримерах;
...
| T13 | Ссылки документа целы | свод | `check-links.py` — битых ссылок нет | R1 / A1 |
...
свод: единые условия прогоняются против каждой задеваемой обратной
...
| Предкоммит и CI | новая проверка не валит существующие прогоны; под `PRECHECK_STRICT=1` поведение то же (проверка не зависит от внешних инструментов — только `python3`, уже требуемый сводом) | ✅ (T14) |
...
свод и свод действующими, а не объявляет предмет закрытым машиной.
...
**В документ примеры не переносятся, и это осознанно.** свод требует,
...
- **Машина сверяет лексику, не правила.** Конструкция без нового ключевого
  ...
  свод и ревью. Правила приложения проверены прогоном настоящего `taktc`:
