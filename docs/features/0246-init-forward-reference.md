...
Отсутствие тега `v0.10.0` на коммите подъёма — отступление от правила 22,
замеченное при этой фиче; ретроспективная простановка тега — решение заказчика
(прецедент — [пропущенные метки версий языка и контроль правила 22](0162-git-tag-v040.md)), поэтому здесь
только фиксируется.
