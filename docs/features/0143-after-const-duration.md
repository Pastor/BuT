...
выдержку в него подставить нельзя. Между тем свод(е) свода само учит
...
- Вскрыто при написании раздела `book/src/12-time/`: пример,
  оформленный по своду(е), не скомпилировался.
...
Между тем свод(е) свода само учит выносить длительность в константу
...
1. **Свод и язык обязаны говорить одно.** свод(е) требует именованных
...
- Урок прямо предупреждает о цене: там арифметика **разъехалась** по двум
  ...
  ошибки.
- Повод фичи (свод(е)) полностью закрывается именем; выражение — рост
...
- свод(е) свода становится исполнимым: `const DWELL := 3s; ref Idle: after
...
Повод — расхождение свода и языка: свод(е) требует выносить длительность в
...
- **Влияние на порядок разработки:** ничего не разблокирует и не блокирует.
  Смежная работа — раздел `book/src/12-time/`, внутри этой же фичи.
...
- **R12. Диагностика `SE-072` описана в документе.** Приложение «Ошибки и
  предупреждения» + блок «Частые ошибки» раздела времени.
...
- **Версия языка** — свод: фича меняет **синтаксис**, поэтому обязана быть
...
- **Риск: тактовая константа выглядит поддержанной, а не поддержана.**
  ...
  кандидатом. Опасность здесь в документе: раздел обязан сказать это
...
- **LSP** (`lsp/keywords.rs`): текст hover-подсказки `after` перечисляет новые
  ...
  уже есть (свод; полный ответ по редакторскому слою — в анализе).
...
22 теста, примеры и контрпримеры:
...
языка: пример, оформленный по своду(е) через константу, не компилировался.
...
- **Пример раздела приведён к своду(е)**: `book/src/12-time/examples/fan.takt`
...
- **Блок «Частые ошибки»** раздела получил строку `SE-072`.

- **Приложение «Ошибки и предупреждения»**: строка в сводной таблице
...
проекта — исправление дефекта **закрытой** работы (стадия 7), а здесь
...
Единые условия прогоняются против каждой задеваемой обратной функциональности; фиксируется статус.
...
| Плагин IntelliJ | ✅ (чтением кода) | `after` не входит в `SIMPLE_DECL_KEYWORDS` сканера, а прочий идентификатор парсер оборачивает в `NAME_REF` и резолвит по имени → переход и переименование работают без правок. Gradle-сборка вне предкоммита, поэтому машиной не подтверждено |
...
- **README не менялся** осознанно: описание языка живёт только в `book/`, а нового раздела документа фича не заводит — дополняет
  существующий.
- **Версия языка `0.7.0` без подъёма** (цикл открыт фичей, тег
...
`after 3m` / `after 3t`. Расхождение свода и языка (свод(е) требует
...
приведён к своду(е), `SE-072` — в «Частых ошибках» и приложении с разбором.
Версия языка `0.7.0` (без подъёма); крейт `takt-lang` `0.21.0`.
