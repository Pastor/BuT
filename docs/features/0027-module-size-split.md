...
- **Приоритет:** Tier 2 (средний) — внутреннее качество кода, пользовательской
  функциональности не добавляет. **Но по своду (критерий 4 «необходимость»)
...
2. **Разделение бесплатно только для внутренних модулей.** Внешние контракты
   ...
   - `grammar::lsp` — **публичный API крейта**: `pub mod lsp;` под
     ...
     `grammar/tests/lsp_tests.rs`. Здесь действует свод: ломать нельзя.
...
2. **Сохранение внешнего контракта.** Публичный API `grammar::lsp::*`
...
- Реэкспорт из `mod.rs` делает внешний контракт неизменным — свод выполняется конструктивно, а не проверками постфактум.
...
- Отмена правила из-за его нарушения — плохой прецедент для процесса
  (свод требует обратного: заводить фичу на исправление).
...
- Версия языка **не растёт**: синтаксис и семантика не затронуты.
...
своду — файл **живого контекста**, а не свод правил кода; правила Rust-кода
по своду живут в `docs/CODE.md`. Правило о размере модуля лежит не в том
...
> пересмотреть последовательность разработки.
...
- **Влияние на порядок разработки:** формально 0027 **никого не разблокирует**
  (статус `ЗАБЛОКИРОВАНА` никому не ставится). по своду, критерий 4
...
- **R3. Публичный API крейта не меняется.** Все 16 `pub`-элементов
...
- **R8. Версии.** Версия языка **не растёт**: синтаксис и семантика
...
свод требует рассматривать каждую фичу с позиции обратной совместимости.
...
|---|---|---|
| **Язык `.lam`** (синтаксис, семантика) | не затронут | Фича не касается `grammar.lalrpop`, `lexer.rs`, `parser/ast.rs`. Версия языка не растёт |
...
Аналитика **не декомпозируется** на `0027-YY-*`: объём умещается в
...
(свод требует того же от генератора фич). Запускается из любого каталога,
...
[разделение переросших модулей (validate.rs, lsp.rs, c_expr.rs)](0027-module-size-split.md)-02…04 записи `validate.rs`, `lsp.rs`, `c_expr.rs` удаляются, и к закрытию
...
вопрос о переносе правила в `docs/CODE.md` (правила Rust-кода живут
там, а `CLAUDE.md` по своду — живой контекст) вынести в процессный бэклог.
...
|---|---|---|
| Язык `.lam` | **н/п** | Задача не касается компилятора; версия языка не растёт |
...
| свод | `./scripts/precheck.sh` | Проходит целиком, включая новый шаг |

**Критерии приёмки задачи:** A6, A7 анализа (A8 закрывается окончательно после
[разделение переросших модулей (validate.rs, lsp.rs, c_expr.rs)](0027-module-size-split.md)-02…04, когда из baseline уходят три целевых модуля).
...
| Функциональность | Работа | Обоснование |
|---|---|---|
| Язык `.lam` | **н/п** | Грамматика/лексер/AST не правятся; версия языка не растёт |
...
| свод | `cargo clean && cargo build --all-features --all-targets`; `./scripts/precheck.sh` | Успешно |
...
>
> **свод выполнено конструктивно:** `pub use` в `mod.rs` держит все
...
>
> **Единственная задача фичи, задевающая публичный API крейта**.
...
| Функциональность | Работа | Обоснование |
|---|---|---|
| Язык `.lam` | **н/п** | Не затрагивается; версия языка не растёт |
...
| свод | `cargo clean && cargo build --all-features --all-targets`; `./scripts/precheck.sh` | Успешно (в `precheck.sh` уже есть `cargo build --features lsp --bin lam-lsp`) |
...
| свод | `cargo clean && cargo build --all-features --all-targets`; `./scripts/precheck.sh` | Успешно, включая сборку порождённого C через `cmake`/`ninja` |

**Критерии приёмки задачи:** A1, A3, A4, A5, A9, A10 анализа.

**Осторожно — известные дефекты рядом.** `c_expr.rs` соседствует с кандидатами
[заглушки генератора C: диагностика вместо тихого пропуска](0028-c-generator-stubs.md) (заглушки `//FIXME` в `c_model.rs:769`, `//TODO` в `c_model.rs:316`) и 0029
...
заводится кандидатом в `FEATURES.md`, а не правится здесь: иначе
...
[разделение переросших модулей (validate.rs, lsp.rs, c_expr.rs)](0027-module-size-split.md) — **рефакторинг без смены поведения**: код только перекладывается по
...
изменились**. (тесты синтаксиса/семантики, примеры и контрпримеры)
...
| T16 | Встройка в конвейер | 0027-01 выполнена | `grep -n check-module-size scripts/precheck.sh .github/workflows/ci.yml` → совпадения в обоих файлах; `./scripts/precheck.sh` проходит целиком | R6 / A7 |
...
#### Блок 6. Полная сборка

| # | Проверка | Предусловие | Ожидаемый результат | Ссылка на R/A |
|---|---|---|---|---|
| T19 | Сборка с очисткой | Все задачи выполнены | `cargo clean && cargo build --all-features --all-targets` — успешно (как в CI) | свод |
| T20 | Полный precheck | Все задачи выполнены | `./scripts/precheck.sh` проходит целиком, включая `lamc fmt --check examples/`, генерацию C и сборку через `cmake`/`ninja` | свод |
