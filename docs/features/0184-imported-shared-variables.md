...
- **П4. Импорт файла целиком: обратная картина.** `import "pidlib.takt";` +
  ...
  found`. Сообщение к тому же на английском и бессодержательное.
...
`CC-004: Model with name ' ()' not found` (сообщение к тому же на английском). Симулятор ту же модель исполняет.
...
4. **Не расширять язык.** Синтаксис `import` менять не требуется — речь о том,
   чтобы уже описанное работало (версия языка не растёт).
...
- раздел документа `book/src/09-imports/` обязан описать границу видимости
  честно — сегодня он о ней умалчивает.
...
языка не растёт. Наблюдаемое поведение меняется — там, где сегодня
...
- **Влияние на порядок разработки:** **разблокирует**
  ...
  блока: Tier 1 и разблокировка другой фичи (критерии 3 и 4).
...
| `book/src/09-imports/index.typ` | описание границы видимости |
...
- **О7. (редакторский слой).** Фича не вводит ни токена, ни узла АСД,
...
Блок «Частые ошибки» раздела дополнен `SE-074` и `SE-005`.
...
конструкций нет. (примеры и контрпримеры) применяется в части
диагностик; (редакторский слой) — ответ в отчёте.
...
| LSP и плагины | новых токенов/узлов нет; новая диагностика идёт общим входом | ✅ |
...
| LSP и плагины | — | правок не требуется, обоснование ниже | ✅ |

**(редакторский слой) — ответ:** правок не требуется, и вот почему.
...
2. **Дефект пережил все проверки, потому что импортирующего входа не было ни в
   ...
   сверка значений.
3. **Найдено попутно и записано кандидатами** (свод; в объём 0184 не
