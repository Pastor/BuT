...
- **Номер:** 0035
- **Статус:** **ГОТОВО**
- **Зависит от:** `0010`, `0024` (обе `ГОТОВО` → фича **не блокируется**)
- **Связанные issue (анализ):** новая фича (из блока «Кандидаты в фичи» `FEATURES.md`)
- **Крейт:** `grammar` (версия языка **0.2.0, роста нет** — см. анализ)
...
[lTL-формулы в блоках кода: разбор вместо тихой потери](0035-ltl-in-blocks.md) **не входит** — обоснование в [ADR](0035-ltl-in-blocks.md#архитектура-adr), работа
...
Синтаксис конструкции (EBNF — фактический, вычитан из
...
2. **Стоимость против ценности.** Разбор в блоке стоит одну строку
   ...
   проверка пустоты, CLI). Смешивать их в одной фиче нельзя.
...
4. **Обратная совместимость.** Синтаксис уже принимается; корпус LTL
...
- Не устраняет дефект **сейчас**: до конца работ `Vec::new()` продолжает молча
  терять формулы.
- Смешивает исправление дефекта с новой возможностью → нарушает свод.
...
**Cons:**
- **Слом обратной совместимости** ради устранения `TODO` — цена
  несоразмерна.
- Конструкция **документирована** (`README.md:194`, `:1051-1054`) и закреплена
  ...
  закрытой фичи.
- Удаление потребовало бы роста версии языка и обесценило бы уже
  работающие уровни модели/состояния и печать в форматтере.
- Противоречит цели проекта: формальная спецификация свойств —
...
- **`README.md:1054` документирует неразбираемый пример** (`active()` в атоме).
  Action: исправить в 0035-04 — пример заменяется на
...
6. `TODO`-комментарий и `Vec::new()` отсутствуют в `semantic/statement.rs`
   (проверяется `grep`).
7. Версия языка остаётся **0.2.0**: синтаксис не изменён; изменение
...
- **Пересечение с фичей «Юнит-конструкции языка для симуляции
  ...
  **Рекомендация аналитика (критерий «необходимость»):** ставить
  **0035 перед 0044**. 0035 дешевле, уже проработана до `РАЗРАБОТКА`, снимает
  ...
  «Зависит от» в карточке 0044 проставляет он, настоящий документ
...
- **Порождённые кандидаты (вне границ 0035; регистрирует координатор):**
...
- **R7. Версия языка не растёт.** Синтаксис `grammar.lalrpop` не
...
- **R8. Примеры и контрпримеры.** Сформированы валидные и невалидные
...
**Обратная совместимость сохраняется полностью; обоснование слома не
требуется.**
...
| Версия языка | **нет** (0.2.0) | свод неприменимо: синтаксис не менялся; правится неспецифицированное поведение (прецедент ADR) |
...
декомпозиция самой аналитики на `0035-YY-*.md` не требуется.
...
| Версия языка | **н/п** | Роста нет — 0.2.0 (свод; прецедент ADR) |
...
```lam: [LTL] G(active() -> F done()), X next_state ;
```
...
**Суть.** Завести примеры и контрпримеры, закрепить их фикстурами и
синхронизировать документацию языка проверенным материалом.
...
> **после** прохождения примеров («после реализации и проверки
...
| Функциональность | Работа | Обоснование |
|---|---|---|
| Фикстуры `grammar/tests/data/` | **да** | Примеры и контрпримеры |
...
**Контроль своду:** каждый пример, попавший в README, обязан быть
...
Проверяется корректность работы языка и компилятора (**свод**: фича
...
| Версия языка | T24 | 0.2.0, роста нет | ⬜ |
...
не тронут — версия языка не растёт.
