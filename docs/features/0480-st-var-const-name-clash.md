...
> 0407); далее проходит жизненный цикл по правилу 17.
...
- приложение «Ошибки и предупреждения» — новая запись `ST-025` с разбором и
  примером (правило 25);
- раздел «Цели генерации», блок «Частые ошибки» (правило 26) — строка про
...
Замер снят прогоном (правило 30): `scripts/probe.sh -n 2` плюс прямые пробы
...
**Контроли** (обязательны, правило 30) — всё принято `iec2c`:
...
переснят — правило 30, «одна причина за раз».
