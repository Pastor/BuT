...
**Язык не меняется** — версия языка не растёт; правка касается только
печати цели `rust`.
...
- **A-4. Вторая находка пробы — вне объёма 0058.** Проба формы 2 показала, что
  ...
  не в коде, а в покрытии. Заведено **кандидатом** в `FEATURES.md`, в
...
4. **A4.** Смешанный случай (одна ветка завершается `return`, другая — нет) не
   сворачивается; вывод как сегодня — контроль.
...
| `StatementNode::If { cond, then_, else_ }` | `grammar/src/semantic/mod.rs:779` | `else_: Option<Box<StatementNode>>` — **отсутствие** `else` выразимо, и это ключ к своду ADR |
...
- **Влияние на порядок разработки:** фича **никого не разблокирует** и **никем не
  ...
  `rust_func`/`rust_stmt`. Место в очереди (хвост таблицы, критерий 2 своду)
...
**Правка аддитивна; обратная совместимость не нарушается — и это
...
Заведено **кандидатом** в `FEATURES.md`; в объём 0058 не берётся —
...
`let-else`. Для свод ADR («всё или ничего») нужно уметь ответить на вопрос
...
Статус по обратной функциональности: предикат **ничего не печатает**,
...
Статус по обратной функциональности: меняется вывод цели `rust`
...
| Язык (синтаксис/семантика) | **не меняется** → версия не растёт | — (н/п: правка в печатнике) | — |
...
**Обратная совместимость доказуема, а не декларативна**: разворот
...
детерминизм закрытой 0048). Язык не меняется → версия не растёт.

**Побочная находка проработки, вне объёма** (заведена кандидатом в `FEATURES.md`): минимальная модель **без портов** проверка не проходит и без всякого
...
Язык не менялся → версия не росла. Побочная находка
