...
| A8 | Документ описывает статус формы | раздел о сценариях |

### Особенности по обратной функциональности

- **Язык не затронут**: сценарий — вспомогательный файл симулятора,
  не программа на Takt. Версия языка прежняя; версия крейта `takt-sim` растёт
  (новая наблюдаемая диагностика).
- **свод:** LSP и плагины сценарии не читают — правок нет.
...
| T11 | Предкоммит | `./scripts/precheck.sh` | зелёный | свод |
