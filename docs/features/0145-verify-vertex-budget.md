...
- **Related issues:** [Фича](0145-verify-vertex-budget.md);
  потолок и его нынешняя метрика — [ADR](0068-verify-data-properties.md#архитектура-adr) (Option D); абстракция управления и гарантия `Holds` —
...
построения; превышение → `Unsupported`, а не долгий счёт» (свод ADR).
...
Целевой поток (схема нужна, потому что решение меняет **место и
...
- **Сужение.** Входы с `E > 10⁶` (например 200 состояний × один `u8`:
  ...
  секунд. Это заявленная цель, но её нужно **назвать** в документе
  и в тексте отказа.
...
- **R1. Потолок считается до построения.** Величина
  ...
  единой аллокации графа. Это исходное обещание свод ADR — фича его
...
- **Язык не меняется**: ни лексика, ни синтаксис, ни семантика
...
- **Редакторский слой:** LSP верификацию не вызывает (греп:
...
`E = E_упр × D²` — квадратично (свод ADR: управляющее ребро
...
`verify_model`/`build_data_kripke`), свод отработано в анализе.
...
| T13 | Полный предкоммит | `./scripts/precheck.sh` | зелёный | свод |
...
| Документ `book/` | ✅ | раздел о границах |
| Симулятор `takt-sim` | — | верификацию не вызывает |
| Генераторы целей | — | не затронуты |
| LSP и плагины | — | верификацию не вызывают (ответ в анализе) |
