...
блок «Частые ошибки» того же раздела и в приложение «Ошибки и
предупреждения» — сводной строкой и отдельным подразделом с примером и
фактическим выводом инструмента.
...
структура выносится кандидатом, чтобы не расширять объём фичи
задним числом.
...
- Диагностика попадает в общий вход `collect_compile_diagnostics` — значит
  доезжает и до **LSP**, и накапливается по правилу
...
- Правило доезжает до **редактора**: `SE-105` идёт общим входом
  `collect_compile_diagnostics`, которым пользуется LSP.
...
- **Версия языка.** Правило языка меняется (диагностика — часть контракта), но
  ...
  `[Не выпущено]` в `CHANGES.md`), и по своду фича вносится в уже поднятую
  версию.
...
- `SE-105` вносится в приложение «Ошибки и предупреждения»; раздел
  «Типы» дополняется правилом и блоком «Частые ошибки».
- Кандидат «структура без полей» заводится в `FEATURES.md`.
...
- **R6. Диагностика доезжает до редактора**: LSP отдаёт `SE-105`
...
**Версия языка не поднимается**: правило языка уточняется, но
...
- **Ветви целей не тронуты**: `enum_facts` → `None` и трактовки
  `c`/`st`/`rust`/`sv` остались как есть (ADR). Изменилась
...
| `editor_shows_se105` | R6 (LSP отдаёт `SE-105`) |
...
- **`book/src/03-types/index.typ`**: подраздел «Перечисления»
...
- **Блок «Частые ошибки»** того же раздела пополнен строкой
  `` `SE-105` — перечисление объявлено без вариантов ``, в принятом формате
  «сначала код, затем описание».
- **`book/src/appendix-errors/index.typ`**: строка в сводной таблице
...
Номеров фич, ссылок на ADR и деталей реализации в документе нет.

**`README.md` правки не требует** (свод при разделении ролей своду):
...
семантики (`SE-105`) вместо отказа разбора (`SY-002`). по своду тест-план
...
| Плагины редакторов | — | — | — | — | — (новых токенов и форм объявления нет) |
...
| Языковой сервер `takt-lsp` | ✅ | T8 |
| Цели `c`/`st`/`rust`/`sv` | — | не задеты: вход отсекается семантикой; вывод корпуса байт-в-байт (T10) |
| Симулятор `takt-sim` | — | та же причина |
| Плагины редакторов (`extensions/`) | — | свод: новых ключевых слов, токенов, узлов АСД и форм объявления нет; правка не требуется |
...
- **Кандидат заведён**: структура без полей принимается языком и даёт
...
пропускает; вынесена кандидатом в `FEATURES.md`. Версия языка не поднимается: цикл `0.9.0` не выпущен.
