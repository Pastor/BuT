...
⚠️ Замер относится к палитре, действовавшей на момент 07a. Решением заказчика
...
«какие глифы нужны», которому нужен свой сторож). Лицензии SIL OFL 1.1 лежат
...
пределам, двойной щелчок возвращает во всю ширину). ⚠️ Оболочка стоит по
...
⚠️ Замер нашёл **три** нарушения сразу после переноса палитры: `--alarm` и
...
5. **Мутация сторожа перестала мутировать.** Случай `B6` дописывал роль после
   строки `--tok-comment: #6b7280;`, а палитра сменилась — `sed` не совпал, и
   сторож доложил об успехе, ничего не проверив. Мутация переведена на «после
...
⚠️ **Отменено решением заказчика 2026-09-04: хранилище — PostgreSQL, а не
...
| 18 | `0531-09r` | **переход схемы базы между версиями** (по факту выкатки 2026-09-06): база стенда осталась версии 4, сервер ушёл на 6, и подъём свежего кода уронил работавший сервис. Шаги `MIGRATIONS` + сторож «обе дороги дают одну схему» | 09a | переход применён к КОПИИ реальной схемы стенда; сторож проверен снятием обеих половин |
| 17 | `0531-09q` | **держатели на разделителях областей** (просьба заказчика 2026-09-05): насечка посреди полосы — появляется при наведении и фокусе, на касании видна всегда, область захвата расширена | 08 | ⚠️ Проверяется на КАСАНИИ: `:hover` там не наступает никогда, и правило, годное на указателе, оставляет палец без цели |
...
| 13 | `0531-09j` | **постраничность витрины и подметание осиротевших каталогов** (закрытие двух названных кандидатов, правило 28): кнопка «Ещё» и лента `showcase.js`; обход `sweep_orphans` с выдержкой | 09h, 09i | в `node` — курсор и слово едут вместе; с базой — сирота убирается, молодой каталог и соседи целы; сторожа проверены снятием |
...
⚠️ **Главная цена — проверяемость.** Половина предмета гейта (хранилище и
...
- Гейт `scripts/check-web-server.sh` и сторож `test-check-web-server.sh`
  (`S0`…`S4`): сторож ломает сборку, гасит проверку окна частоты, **дописывает
...
- **Ручки проектов и файлов**: создание, список, чтение с составом файлов,
  ...
  маршрутов, и все перечислены в `ROUTES` — тест списка их и сторожит.
...
⚠️ Предел «сто проектов» набивается **мимо ручки**, прямым `INSERT`: сотня
...
- **Токен стал необязательным** (`routes::optional_user`): открытый образец
  открыт и для того, у кого записи нет вовсе. ⚠️ Испорченный токен — это
...
⚠️ **Класс, невидимый всем тестам разом.** Открыв `/p/<id>` в браузере, увидели
...
3. **Поле `language` описей сборки было ПУСТЫМ.** `build-web.sh` искал
   ...
   непустоту обеих версий проверяет гейт веба.
...
- **Страница, в `node`**: разбор адреса вместе с префиксом; чтение активного
  ...
  словаря**. Плюс сторож полноты списка модулей страницы: забытый в нём модуль
...
⚠️ **Цена, названная замером: поиск зависит от ЛОКАЛИ базы.** На кластере,
...
Стенд обязан жить с `UTF-8`-локалью; сторож на это есть — набор поиска на такой
...
- **`public` даёт `fork`, а не `view`** (проработка §4): текст уже в браузере
  ...
  выкладывает. ⚠️ Решение обратимо и названо в коде.
...
- **Ручки прав** `GET/PUT/DELETE /api/projects/{id}/grants/{login}` — владелец.
  ...
  регистра. ⚠️ Неизвестный логин при выдаче — **отказ**: это единственное
...
- **`403` вместо `404` там, где проект ВИДЕН.** Отсутствие доступа вовсе
  ...
  чтобы попросить право. ⚠️ Правка чужого открытого проекта отвечала `404` с
...
⚠️ Ревизия в матрице спрашивается **перед каждой записью**: удавшаяся запись её
...
- **Текст отказа сервера показывается как есть**, под одним ключом словаря:
  ...
  (граница названа задачей 10a, снимает её фича 0532). Ключ на каждый код
...
1. **Забытое имя в списке узлов.** `cache()` перечисляет имена разметки, и
   ...
   загружен» — при загруженном модуле. Сторож: каждый `id` разметки обязан
   быть найден страницей, тест падает списком.
2. **`display: flex` сильнее `hidden`** — второй раз за фичу (первый в задаче
   03). Обе половины панели входа показывались разом. Сторож: класс, которым
   помечен скрываемый узел и который задаёт `display`, обязан иметь правило
   `[hidden]`; проверено снятием правила — гейт краснеет.
...
- **В `node`**: круговой рейс черновика `v2` (одно имя файла в двух проектах —
  ...
  **одно** обновление пары на три параллельных запроса; отказ приходит кодом и
  числами, а не разобранным текстом. Плюс два новых сторожа разметки (список
  узлов, `[hidden]`).
...
⚠️ **Перед началом требуется решение заказчика: где собирается архив.** Вопрос
...
- **Метаданные — часть архива, а не украшение.** Без них загрузка не
  ...
  архива, отметка времени. ⚠️ Видимость, права, число копий и владелец в
...
⚠️ Открытым остаётся прежний вопрос — **где собирается архив** (таблица выше);
...
1. **Архив собирается и разбирается НА СЕРВЕРЕ.** Расхождения с браузером не
   ...
   (решение A5). ⚠️ Линковка сервера с `takt-lang` отвергнута: это была бы
...
- **Метаданные `takt-project.json`** с **версией формата**: имя, описание,
  ...
  генерации. ⚠️ Версия формата обязательна: читатель, встретивший бо́льшую,
...
- ⚠️ Половина набора требует **собранной статики** (`TAKT_WEB_TEST_STATIC`):
...
- **Подраздел «Онлайн-редактор» в главе «Инструментарий»** документа (правило 24
  допускает прикладной раздел; нового раздела не заводится — язык не меняется):
...
- **Раздел README** с командами сборки, выкладки, стенда и копии — они
  проверяются гейтом команд README (0275).

⚠️ Границы названы в обоих текстах **словами**, а не умолчанием: читатель,
...
- **`web/deploy/docker-compose.yml`** — PostgreSQL и сервер. ⚠️ Локаль базы —
...
- **`scripts/stand.sh`** — четыре действия (`up`, `down`, `status`, `restart`)
  и ни одним больше. ⚠️ Томов скрипт **не удаляет**: `down -v` уносит базу и
  ...
  ⚠️ Подъём, о котором нельзя сказать, удался ли он, считается отказом —
...
Сторож `scripts/test-deploy-web.sh` (`E1`…`E6`): раскладка полна; **подмена
...
⚠️ **Docker сторожем не поднимается**: предмет — раскладка и скрипты, а подъём
...
1. **`mapfile` и `stat -f` непереносимы**: первого нет в bash 3.2 (системный на
   ...
   через `ls -td`.
2. **Сторож ловил собственное объяснение**: греп по всему файлу находил `down
...
Сторож `scripts/test-setup-nginx-takt.sh` (`N1`…`N8`): префикс во всех местах;
...
⚠️ Сторож работает **без nginx и без прав root**: скрипт умеет печатать
...
- **Стенд не поднимался**: скрипты проверены сторожем и чтением, но не
...
- **Копия и восстановление**: `scripts/dump-web.sh` (база `pg_dump` + архив
  исходников) и `scripts/restore-web.sh`. ⚠️ Копия базы **без** файлов
...
- **Сторож копии** `scripts/test-dump-web.sh` (`D1`…`D4`): снятая копия
  **разворачивается** и данные в ней те же; копия без исходников отвергается;
  восстановление без копии отказывает; политика пропуска. ⚠️ Он и есть предмет:
...
- **Осиротевшие каталоги не подметаются**: проект, чья строка исчезла мимо
  ...
  кандидат, и оно названо здесь, а не забыто. ⚠️ **Снято задачей `09j`**
  (правило 28): обход `sweep_orphans` убирает их, не раньше чем через час
...
⚠️ **Задача выполнена** 2026-09-05: решения заказчика, состав работ и проверки
...
⚠️ Отсюда сегодняшнее поведение: открыв чужой проект, читатель видит его код
...
1. **Свойство ПРОЕКТА, а не файла.** Пара «цель + ключи» одна на проект.
   ⚠️ Названная граница: проект с библиотекой и прошивкой собирается одной
   целью.
...
3. **Сервер РАЗБИРАЕТ ключи и отвергает негодные при записи.** Исполнитель
   назвал риск (второй носитель правила «ключ × цель» — класс 0084/0466);
...
- **Сервер спрашивает модуль** (`Modules::check_flags`) — тем же круговым
  рейсом через `wasmtime`, каким собирает архив (09g). ⚠️ Модуля нет — отказ,
...
- **Архив** — версия формата `2`; `manifest_of` принимает проект целиком
  ...
  загрузка. ⚠️ Пара из архива **проверяется** модулем; не прошла либо
...
- **Подпись списка расходилась с его значением.** Открытый проект ставил цель
  ...
  ⚠️ Класс НЕ про 09p — он был и у черновика, и у ссылки-снимка, и у списка
  ...
  одной точкой `setPick`. ⚠️ Слушатель на `change` в `pick.js` уже был, но
  ...
  к одному. Сторож — текстовая проверка в `node`, проверенная снятием обеих
  половин.
...
⚠️ **Род файла придётся хранить явно.** Угадывать по расширению — значит
...
- ⚠️ **Подсветка не может прийти от модуля**: `takt_tokens` знает Takt, а не
...
- ⚠️ **Рендер — это разметка из данных.** Текст пишет автор проекта, а
...
- ⚠️ **Черновик и ссылка** (`09e`, задача 03) хранят один текст: обе формы
  придётся расширить, а старые записи — читать по-прежнему.
- ⚠️ **Пределы проекта** (`09b`) считают файлы и байты; новые роды файлов
...
1. **«Файл прогона» — это сценарий входов И ПРОВЕРОК**, и их несколько.
   ⚠️ Замер при взятии в работу: формат сценария проверки УЖЕ несёт (`guard`
...
⚠️ Проработка утверждала, что род файла придётся **хранить явно**, иначе
«переименование молча сменит род». Замер 2026-09-05 это опроверг: род уже
...
- **`web/static/md.js`** — один разбор на две работы: отрезки подсветки и
  ...
  текстом. ⚠️ Картинок нет намеренно: показ картинки — обращение к чужому
...
- **Файлы проекта вынесены в свой модуль** `web/server/src/files.rs`: с третьим
  родом `projects.rs` перешагнул тысячу строк, и гейт размера отказал.
...
полосу. ⚠️ На касании не работало ничего: `:hover` там не наступает **никогда**,
...
- **Область захвата расширена** двойным зазором в обе стороны (около 27
  пикселей вместе с полосой — подушечка пальца). ⚠️ Расширяется ЗАХВАТ, а не
...
| Что | Где |
|---|---|
| держатель описан в книге дизайна | `web/design/BOOK.md`, гейт `check-design.py` |
...
⚠️ Главная находка замера **меняет состав работы**: трёх независимых
...
**Что не проверено и чем проверяется** (правило 30: неверный параметр в плане
дороже пропуска):
...
- **Список маршрутов** `ROUTES` растёт на пять строк, и тест
  `routes_are_listed_and_unique` меняет ожидаемое число — он и есть сторож
...
пришли. ⚠️ Если проба покажет, что `user_id` при `provider=mail_ru` из другого
...
хранить персональных данных (A6, решение 2026-09-04). Три исхода:
...
Сторож — тест схемы «нет колонок `email`, `name`, `avatar`, `token`»
...
**только по нему**; своего списка площадок у страницы нет (сторож — греп
...
Вторая копия (в странице, в скрипте) запрещена — класс 0084/0466.
...
замер холодной сборки — в отчёт задачи (`measure-precheck.sh`, урок 0272).
...
**Сторож подделки.** Подделка сама обязана ловить, и это проверяется
...
зелёная подделка доказывает лишь, что она соглашается на всё (условие правила
0315: «хорошее проходит» **и** «плохое ловится»).
...
**Человеку — чек-лист с датой в отчёте** (сети у гейта нет и не будет):
...
2. VK ID: то же; обмен с адреса стенда проходит, **с другого адреса —
   отказывает** (контрольный вход правила 30);
...
- **i18n**: ≈ 20 ключей в `ru.json`/`en.json` под тем же гейтом паритета;
...
гейтом i18n): что хранится (логин, хеш пароля, идентификатор внешней учётной
...
Правило 2 — польза каждой ступени: после `09f-1` вход площадкой проверяем
...
- **Правило 29 не задето**, крейты `takt-lang`/`takt-sim` не трогаются;
  `takt-wasm` тоже.
- **Гейта на браузер нет**: cookie, фрагмент, `beforeunload` и окно площадки
  — человек с датой.
- **Каждый гейт со сторожем (0315)**: подделка проверяется на отказ; тест
...
- **Отвязка из сессии** `DELETE /api/me/identities/{provider}`. ⚠️ Отвязка
...
- **Команда `takt-web-server unlink <логин> <площадка>`** для администратора:
  ...
  — войти нечем. ⚠️ Правило «последний способ» действует **и на команду**:
...
пароля и отвязка после него, повторное задание пароля (`409`). ⚠️ Вход паролем
...
- **Кнопки строятся по ответу сервера**: имён площадок в коде страницы нет —
  ...
  от сервера, текст даёт словарь. Сторож грепает страницу на имена площадок и
...
- **Реестр подписей сверяется ДВУМЯ сторонами**: сервер объявляет ключ, словарь
  ...
  бы служебный ключ, и увидел бы это читатель, а не гейт.
...
Гейт веба (42 проверки) плюс два новых сторожа: **имён площадок в коде
...
гейт. Прогон страницы: три кнопки в порядке сервера, узкий экран.

⚠️ Сторож гейта веба пришлось дополнить: он копирует дерево частями, а сверка
...
⚠️ Правило `contain` заведено для ВНЕШНИХ областей (жест в конце списка не
...
⚠️ Класс невидим замерам скорости: прокрутка программным `scrollTop`
работала (я так и мерил), а мёртвым было именно **колесо**. Нашёл его вопрос
заказчика, а не гейт; сторож заведён (`web/tests`), проверен возвратом
дефекта.
...
⚠️ Половина класса — в nginx: `proxy_pass` с завершающей косой СРЕЗАЕТ
...
Сторожа: HTTP-проверка «обе формы корня под префиксом отдаются, а промах по
...
из тела убраны, сторож `N10` требует, чтобы печать конфигурации была
...
передаётся скрипту; сторож `E7` требует именно этого.

**Бинарник сервера искался не там, где собран.** `.cargo/config.toml` корня
переносит каталог сборки в `target/precheck` (фича 0251), и конфигурация
...
в `/var/lib/postgresql/data` отказывается работать вовсе. Сторож — `E9`.

⚠️ Класс дожил до стенда потому, что **дома стек не поднимали ни разу**:
проверки шли на сервере, запущенном напрямую, а гейты файл стека только
грепали (имя проекта, отсутствие публикации порта базы) — то есть читали его
глазами, а не инструментом. Заведён сторож `E6`: `docker compose config -q`
...
⚠️ Опасность не в моменте выкладки: `setup-nginx-takt.sh` делает `nginx -t ||
...
(он и должен действовать только на наши адреса), а на класс заведён сторож
`N9`: в файле локейшенов не должно быть ни одной директивы вне `location`.
Проверен снятием.

##### Задача `0531-09i` — витрина, архив и профиль НА СТРАНИЦЕ

Задача заведена закрытием (правило 28): плановые задачи исчерпаны, но три
...
- **Архив**: «Скачать архивом» и «Загрузить архив». ⚠️ Скачивание идёт
...
- **Раздел профиля**: связанные площадки (строка — кнопка отвязки) и «задать
  пароль». ⚠️ Предложение задать пароль показывается **только** тому, у кого
...
- **Постраничности витрины на странице нет**: курсор сервер отдаёт, страница
  показывает первую страницу. Кнопка «ещё» — кандидат. ⚠️ **Снято задачей
  `09j`** (правило 28): кнопка есть, лента живёт носителем `showcase.js`.
...
Задача заведена закрытием двух кандидатов, названных границами соседних задач
(правило 28): «кнопка ещё» из `09i` и «осиротевшие каталоги не подметаются» из
...
`web/static/showcase.js`. ⚠️ Своей арифметики страниц у страницы по-прежнему
нет: курсор уходит обратно как есть. ⚠️ **Слово поиска едет со следующей
...
⚠️ **Выдержка обязательна** (`ORPHAN_GRACE`, час): запись идёт «сперва база,
...
⚠️ **Чужое в корне хранилища пропускается, а не удаляется**: наши каталоги
...
- **Сторожа проверены снятием** (правило 0315): без выдержки падает проверка
...
- **Разделители панелей ТЯНУТСЯ.** Прежде рабочая область была поделена
  ...
  разделитель рядов (редактор ↔ диагностики). ⚠️ Ручка правил **одна на обе
...
- **Контролы вдвое крупнее** (`--control-scale: 2`): кегль и высота множатся
  ...
  кнопку. ⚠️ Код множителю не подчиняется: крупнее набранный код означал бы
  меньше строк на экране.
- **Номера строк в панелях кода** печатает счётчик CSS. ⚠️ Не разметка: строки
...
1. **Незаданная доля читалась НУЛЁМ.** `Number(null)` — ноль, ноль — законная
   ...
   этом нет: страница просто открывается не такой, как задумана. Сторож
...
2. **Перенос строк — настройка UI, СВОЯ у каждой области кода**: сузив область
   ...
   переключателем, а не горизонтальной прокруткой. ⚠️ Класс ставится ОБЛАСТИ,
   а не строкам: строки перестраивает покраска на каждую правку, и настройка
   исчезала бы с первым же нажатием клавиши. ⚠️ Умолчание — без переноса.
...
3. **Кегль страницы выбирается кнопками ±1** (просьба заказчика): меняется
   ...
   выбирать в каждом правиле. ⚠️ Обе границы названы (10…28 px): ниже нижней
...
4. **Сценарий — область КОДА, а не поле ввода** (просьба заказчика): у
   ...
   правилом. ⚠️ Подсветки у него нет: это JSON, а не Takt, и красить его
...
⚠️ **Значок площадки объявляет СЕРВЕР** — именем файла (`icon`), ровно как
подпись ключом словаря (09f-3): своего списка площадок у страницы нет, и
сторож сверяет объявленное с файлами статики **в обе стороны**.
...
⚠️ Адрес значка считается от МОДУЛЯ (`import.meta.url`), а не от документа:
...
⚠️ `display: flex` сильнее `hidden` — четвёртый случай за фичу: блок «первый
вход через площадку» остался виден в окне. Поймал сторож разметки.
...
⚠️ **Второй дефект нашёлся рядом**: контейнер числился `unhealthy` с самого
...
⚠️ **Настоящая длительность сборки тоже названа**: дерево копируется целиком и
...
⚠️ Сторожа E10…E12 проверены снятием: правка, возвращающая любую из трёх
...
⚠️ **Симуляция перестала быть вкладкой.** Вкладка, оставшаяся в ряду,
показывала бы панель в обход кнопки — и обе оказались бы открыты в области,
рассчитанной на одну. Сторож требует, чтобы `data-tab="trace"` в разметке не
было вовсе.

⚠️ **Скрытая генерация не выполняется** — но и звать её до загрузки модуля
...
загрузился» при живом модуле. Нашёл прогон страницы; в стороже теперь оба
условия.

⚠️ **Область уходит вместе со своим разделителем**: правило смотрит на
...
употреблялась раньше, чем была объявлена. ⚠️ Неизвестную переменную браузер не
...
⚠️ **Класс молчаливый и виден только глазом** — поэтому заведено правило гейта
...
объявления предусмотрено (величины ставит `shell.js`). Сторож дополнен
мутацией G7c.
...
запись») — справа и главной. ⚠️ Разная ширина у пары читалась бы как разная
важность, которой между ними нет. Заголовок «Вход» — по центру и жирным.

⚠️ Главная кнопка потребовала своей пары `--surface-primary`: акцент к тому
...
⚠️ **Список ключей и значений — ИЗ РАЗБОРА аргументов**
...
⚠️ **Ключ с недопустимым для цели значением в строку не идёт** — иначе UI
...
2. **Ручной ввод остаётся.** Поле «Ключи сборки» никуда не делось, и строка —
   **единственная величина**: конструктор её собирает, поле принимает как
   есть, обе стороны читают одну и ту же строку. ⚠️ Заведи конструктору своё
   состояние — ручная правка строки начала бы теряться.
3. **В черновик и ссылку едет прежняя строка** — форма состояния не менялась.

⚠️ **Неизвестный ключ едет обратно как есть** (`rest`): он мог появиться в
компиляторе раньше, чем в описи, и терять его страница не вправе.

⚠️ **Ключ, чьё значение цель не принимает, из строки уходит** при смене цели —
...
⚠️ **Символом значок набрать нельзя.** Замер 2026-09-05 по снимку `cmap`
...
⚠️ **`display` сильнее атрибута `hidden`** — третий случай за фичу. Правило
`.icon-btn[hidden] { display: none }` обязательно: без него скрытые
«Сохранить» и «Скачать архивом» остались бы на экране. Дефект поймал сторож
разметки, а не глаз.
...
⚠️ **Предупреждение о наборе B было ошибочным** и снято: «Сохранить» в нём —
...
⚠️ **Граница узнаваемости:** стрелка в поддон у «Сохранить» читается и как
...
⚠️ **Что в хранилище НЕ идёт:** текст модели, сценарий и выбранная цель. Они
...
⚠️ Дело было не в контрасте: у оливкового `#61684F` он к полю ввода 4.88 —
«сливается» тут значит «того же тона, что палитра страницы», а она
зеленоватая. Итог — нейтральный `#8A8D88`, 3.30 к полю. ⚠️ Это НИЖЕ порога AA
...
⚠️ **Вход и выход — одна кнопка** со сменным значком (внутрь / наружу):
состояние у них одно, и две кнопки, из которых всегда видна одна, читателю
ничего не добавляют. ⚠️ Кнопка учётной записи при этом НЕ исчезла, а переехала
...
⚠️ Значок гасится атрибутом (`.icon[hidden]`) — то же правило, что у кнопки:
...
1. **`title` перехватывается и снимается** при первом показе. В разметке его
   нет вовсе (сторож грепом), но узел с `title` может прийти из кода — и рядом
...
⚠️ Что от референса не взято: **тень**. Теней в этой системе нет вовсе, и одна
на подсказке завела бы вторую шкалу.

⚠️ Имени элементу подсказка не даёт: `aria-describedby` заставил бы диктора
...
2026-09-05 15:18». ⚠️ Версия модуля из РАБОТЫ не ушла: её несёт ссылка
«поделиться» и окно обновления — убран её показ, а не сама величина.
⚠️ Неразобранная метка даёт ПУСТО, а не «Invalid Date»: опись прежней выкладки
поля не несёт, и страница переживает это молча.

⚠️ Сторож ключей словаря пришлось **уточнить**: эвристика «тернарник над двумя
...
исходник и сгенерированные»). ⚠️ Прежде их было две, по одной на область:
...
⚠️ Признак нажатия у кнопки был всегда (`aria-pressed` ставил `attachWrap`), но
на экране не показывался: залипание — это ВИД, а не состояние в разметке.

⚠️ Внутренний узел вывода (`.file-text`) несёт СВОЁ `white-space`, и правило
...
проём (проём справа), у «Выйти» — выходит (проём слева). ⚠️ В первой редакции
...
⚠️ **При обычном вводе дефект невидим**: обе стороны (снятие и восстановление)
...
⚠️ Правило считали по месту **трижды** (каретка, переход к объявлению,
...
вверх и вниз по строкам. ⚠️ Синтетические нажатия в стенде прогона не
...
участвует в её геометрии. ⚠️ Замер дефекта на нём не показал — правка снимает
...
| Что проверено | Как |
|---|---|
| ключи не пересекаются и лежат в пространстве `takt.` | сторож перебирает все девять |
| каждая настройка и пишется, и читается страницей | сторож грепает `app.js` по именам ключей |
...
| скрытая кнопка-значок действительно скрыта | сторож разметки (нашёл дефект) |
...
| состав полос шапки | сторож перечисляет узлы обеих и падает списком (проверен снятием: кнопка входа, уехавшая вниз, ловится) |
...
⚠️ **Оставшееся — действия заказчика, и они не заменяются проверками:**
...
| VK ID: обмен с ЧУЖОГО адреса | отказывает — контрольный вход правила 30 |
...
⚠️ **`pkce` Яндекса стоит `Optional` по документации, а не по пробе**, и это
...
⚠️ Число относится к **своему** workspace сервера: предкоммит корня его не
...
- **Сторож подделки**: её спрашивают **прямыми** запросами, минуя сервер —
...
⚠️ **Отказ сработал ровно как задумано.** В `db.rs` было записано: «Схема — одна
функция, а не каталог миграций… пока выпуска не было, миграция — лишний
носитель того же знания (класс 0084). Появится выложенный стенд с данными —
...
- Шаги идемпотентны по построению (`IF NOT EXISTS`, `DROP CONSTRAINT IF
  ...
  версии.
- ⚠️ `CHECK` **заменяется целиком**: в PostgreSQL его нельзя дополнить, а без
...
отвергнутой записью у читателя. Сторож — `both_roads_lead_to_the_same_schema`:
строит обе базы и сравнивает состав колонок (имя, тип, обязательность,
умолчание) и тексты ограничений `CHECK`.

##### Проверки

- Сторож **проверен снятием обеих половин**: убранная колонка из шага даёт
