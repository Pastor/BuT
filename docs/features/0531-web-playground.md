...
Предмет — **критерии приёмки ADR**, а не отдельные задачи: каждая из них
проверена в своём разделе разработки, а здесь спрашивается фича целиком.
Проверяется то, что ломается молча: тождественность вывода браузера и
инструментов, отсутствие второго знания о языке в вебе, версия модуля у
опубликованной ссылки, остановка прогона по бюджету, ключи сборки и зелёный
предкоммит.

### Проверки (условие → ожидаемый результат)

| Условие | Ожидаемый результат | Чем проверяется |
|---|---|---|
| вывод восьми целей в браузере против `taktc` | байт в байт | `check-wasm.sh` |
| трасса прогона в браузере против `takt-sim` | байт в байт | там же |
| поиск списка ключевых слов Takt в `web/` | не находится | `check-web.sh`, шаг 4 |
| две выложенные версии модуля | старая ссылка открывается своей | набор сборки в `web-tests.mjs` |
| прогон сверх бюджета | останавливается с названной причиной, страница отвечает | набор прогона |
| ключи сборки страницы против разбора аргументов | совпадают, отказ цели назван | набор ключей |
| `./scripts/precheck.sh` | зелёный; без `node` шаги веба — «пропуск» | прогон предкоммита |

### Разбивка проверок по функциональности

Редактор и языковой слой — наборы подсветки, координат, ссылки и редакторского
слоя; проекты и сервер — наборы `web/server/tests` (без базы часть пропускается
названным ослаблением); оформление — `check-design.py`; собранная статика —
наборы сборки; схема — `layout-tests.mjs`.

### Тестовые данные и окружение

Корпус `examples/*.takt` и `examples/simulations/`; собранный модуль
`takt_wasm.wasm`; `node`; для наборов сервера — PostgreSQL по
`TAKT_WEB_TEST_DB` (без неё они пропускаются, и это названное ослабление).

## Отчёт о тестировании

### Резюме

Прогон 2026-09-09 на полном `./scripts/precheck.sh` — **зелёный** (код возврата
ноль, ни одной строки отказа). Проверки сервера, пропускаемые без базы, добраны
отдельным прогоном с поднятой PostgreSQL. Все шесть критериев приёмки
подтверждены прогоном, а не чтением кода.

### Сводка прогонов и окружение

| Прогон | Факт |
|---|---|
| `cargo test --all-features` | 4222 пройдено, 7 пропущено (16 наборов, 196 с) |
| тождественность модуля | 88 компиляций корпуса под целями, 6 трасс, 1032 редакторских входа |
| веб-часть в `node` | 101 тест; собранная статика — 28 модулей, 2 словаря |
| дизайн-система | 418 правил оформления, 8 пар реестра, 30 контролов |
| сервер проектов без базы | 54 модульных теста |
| сервер проектов с базой | 64 теста в семи наборах (архив, права, HTTP, площадки, проекты, хранение, витрина) |
| цели генерации | `cc`, `iec2c` (+ сборка его C), `rustc` с `clippy`, `verilator`, `yosys`; пять тестбенчей SystemVerilog прогнаны |
| документ | 33 примера, 13 трасс, 6 снимков порождённого кода |

Окружение: рабочая станция разработчика, толчейн из пина; PostgreSQL 16 в
контейнере для проверок сервера.

### Сверка с тест-планом

| Критерий | Результат | Чем подтверждён |
|---|---|---|
| 1. вывод целей и трасса в браузере равны инструментам | **выполнен** | тождественность модуля: 88 компиляций и 6 трасс сверены байт в байт |
| 2. языковой слой один, списка ключевых слов в вебе нет | **выполнен** | шаг проверки веб-части; его контроль проверен снятием (`B3`) |
| 3. ссылка грузит модуль своей версии | **выполнен** | наборы сборки: отпечаток бандла, опись версий, контрольная сумма модуля |
| 4. прогон сверх бюджета останавливается с причиной | **выполнен** | набор прогона: предупреждения и вывод модели доезжают до страницы |
| 5. ключи сборки — те же строки, отказ цели назван | **выполнен** | набор ключей сверяет опись страницы с разбором аргументов компилятора |
| 6. `precheck.sh` зелёный, шаги веба пропускаются словом | **выполнен** | прогон 2026-09-09, код возврата ноль; мягкий пропуск проверен контролями `W5`, `B5` |

### Результаты по функциональности

**Редактор и языковой слой.** Подсветка, диагностики, наведение, переход,
использования, переименование и форматирование идут через `takt_lang::lsp`;
1032 редакторских входа сверены с ответами инструментов.

**Генерация.** Восемь целей; вывод браузера равен выводу `taktc` байт в байт.
Порождённый код принимают инструменты целей, пять тестбенчей SystemVerilog
прогнаны и наблюдают заявленное поведение.

**Прогон.** Трассы браузера равны трассам эталона; бюджет останавливает прогон с
названной причиной, страница остаётся отзывчивой (прогон в отдельном потоке).

**Проекты и сервер.** Учётные записи, вход паролем и через площадки, проекты и
файлы с пределами и ревизией, права трёх уровней, копирование, витрина с
поиском, архив круговым рейсом, свёртывание неиспользуемых проектов и
подметание сирот — 64 теста с базой.

**Оформление и статика.** Дизайн-система сверяется тремя сторонами (вёрстка,
книга, витрина); собранная статика проверяется на отпечаток, описи и предсжатие.

### Найденные дефекты

Дефекты, найденные по ходу разработки, закрыты в своих задачах и перечислены в
их разделах; отдельных исправлений после закрытия — одно
([`0531-01`](../fixes/0531-01-wasm-cache-fingerprint.md), отпечаток модуля в
кеше). На момент отчёта незакрытых дефектов нет.

### Названные ослабления

- **Проверки сервера пропускаются без базы**: без `TAKT_WEB_TEST_DB` шаг
  печатает «пропуск» и не проваливает предкоммит. Это политика внешнего
  арбитра, а не оплошность; для отчёта они прогнаны с базой.
- **Браузерной проверки в проекте нет**: вёрстку, фокус и отрисовку смотрит
  человек. Прогон страницы за время фичи нашёл классы, невидимые тестам,— и
  каждый закрыт своей проверкой.
- **Язык сообщений инструментов** остаётся русским: оболочка переводится, а
  диагностики модуля ждут фичу [язык сообщений компилятора и симулятора (i18n)](0532-i18n-diagnostics.md). Граница названа строкой на
  самой странице, а не спрятана.

### Итоговый вердикт

**Готово.** Критерии приёмки выполнены, предкоммит зелёный.

## Итог (что сделано)

Онлайн-редактор Takt работает: модель пишут, проверяют, собирают под все цели и
прогоняют в браузере, а работой делятся ссылкой-снимком или живой страницей
проекта.

**Второго компилятора не появилось.** Страница держит модуль WebAssembly
(`takt-wasm`), собранный из тех же крейтов, что `taktc`, `takt-lsp` и
`takt-sim`; тождественность вывода проверяет шаг предкоммита — 88 компиляций
корпуса под целями и 6 трасс сверяются байт в байт. Знания о языке в `web/` нет:
подсветка красится по ответу модуля, ключи сборки берутся у разбора аргументов
компилятора, и на оба правила стоят проверки.

**Что сделано по частям.** Печать генераторов в строки и подготовка эталона к
работе без диска и консоли; крейт привязок; страница без зависимостей (ни npm,
ни бандлера) с редактором, диагностиками, восемью целями, прогоном, схемой
состояний и адаптивной раскладкой; дизайн-система с книгой, витриной и
проверкой; оснастка статики (адрес задаёт срок жизни в кеше); сервер проектов на
`axum` и PostgreSQL — учётные записи, вход паролем и через площадки, проекты и
файлы, права, витрина с поиском, архив, свёртывание и подметание; оболочка на
двух языках; выкладка на общий стенд под локейшеном.

**Чем держится.** Пятнадцать проверок предкоммита: тождественность модуля,
веб-часть в `node` (101 тест), дизайн-система (418 правил), сервер проектов (54
теста без базы и 64 с ней), выкладка статики, схема и раскладка. Каждая
проверена снятием — контроль идёт первым.

**Что вынесено.** Язык сообщений инструментов — фича
[язык сообщений компилятора и симулятора (i18n)](0532-i18n-diagnostics.md); панель схемы в редакторах — фича
[панель схемы в webview плагинов редакторов](0543-scheme-in-editor-webview.md). Задача «сервер публикаций» отменена как
самостоятельная: её содержимое стало остовом сервера проектов, а анонимная
публикация с токеном владельца не делается — проект с владельцем делает её
лишней.

Крейты `takt-lang` и `takt-sim` изменились только там, где библиотека обязана
работать без диска и консоли; язык не менялся.
