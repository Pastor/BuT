...
- **Номер:** 0183
- **Статус:** ГОТОВО
- **Зависит от:** нет (проставляет аналитик на стадии анализа)
- **Tier:** 2
...
1. **Симулятор и цели обязаны совпадать потактово.** Разрыв «в отладке работает, в
   прошивке нет» — худший для языка синтеза (свод свода). Сверка, а не
...
2. **Арифметика времени живёт в одном месте.** `semantic::duration` — единственный
   слой пересчёта (свод ADR); новое представление не имеет права
   завести второй.
...
- **R2. Литерал понижается в миллисекунды один раз.** Пересчёт «наносекунды →
  миллисекунды» живёт в `semantic::duration` (свод ADR); ни один
...
- **Версия языка** — свод: меняется наблюдаемое поведение целей и
...
- **Риск объёма:** шесть целей + выдержка + сверки — работа декомпозирована ниже; задачи независимы по целям, поэтому провал одной не блокирует
  остальные.
...
- **Общий слой** `semantic::duration` (свод ADR — единственное место
...
- Блок «Частые ошибки» получил `SE-073`; формулировка `SE-072`
  уточнена — переменная и порт больше не в списке отказов.
- **Приложение «Ошибки и предупреждения»**: `SE-073` и `ST-016` в
...
- **Версия языка `0.7.0` без подъёма** (цикл открыт фичей);
...
[модель времени в языке: литерал длительности и частота такта](0134-language-time-model.md)), поэтому граница «длительность ↔ число» **не порождает арифметики** и
