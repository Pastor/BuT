...
Main_Controller_Detect_Line_tick(struct Main *main) {
  ...
  case MAIN_CONTROLLER_DETECT_LINE_START: {
    if (on_line || point > 0) {
      main->controller.detect_line.state = MAIN_CONTROLLER_DETECT_LINE_END;
               ...
               distance_left >= 0 || distance_right >= 0) {
  ...
  case MAIN_CONTROLLER_DETECT_LINE_UP: {
    ...
    if (on_line || point > 0) {
      ...
      main->controller.detect_line.it = 0;
      write_bit_port(main->port, 0, PORT_ADDRESS_MACHINE_GAS, false);
    } else if (main->controller.detect_line.it >=
               main->controller.detect_line.step) {
...
Main_Command_tick(struct Main *main) {
  ...
  case MAIN_COMMAND_SAVING: {
    ...
    {
      main->command.code =
          read_int_port(main->port, PORT_ADDRESS_COMMAND_CODE, 0);
      write_bit_port(main->port, 0, PORT_ADDRESS_COMMAND_EXECUTING, true);
      main->command.next_point =
          read_int_port(main->port, PORT_ADDRESS_COMMAND_POINT, 0);
    }
    //
    if (true) {
  ...
  case MAIN_COMMAND_DECODE: {
    ...
    if (main->command.code == 0) {
      ...
      if (main->command.next_angl_up >= 360) {
        main->command.next_angl_up = 0;
      } else if (main->command.next_angl_up < 0) {
        main->command.next_angl_up = 360;
      }
      if (main->command.next_angl_down >= 360) {
        main->command.next_angl_down = 0;
      } else if (main->command.next_angl_down < 0) {
        main->command.next_angl_down = 360;
      }
      ...
      write_bit_port(main->port, 0, PORT_ADDRESS_COMMAND_RESET, true);
      // Before
      main->command.next_level =
          read_int_port(main->port, PORT_ADDRESS_COMMAND_LEVEL, -1);
