...
// Каркас плагина: FileType, Language, регистрация `.takt`.
// Лексер с подсветкой и настройка цветов добавляются поверх.
plugins {
    id("java")
...
dependencies {
    intellijPlatform {
        ...
        // лексический слой, семантический включается лишь при её наличии.
...
// ── Пусковой JDK: проверка перед сборкой ──────────────────────────
//
...
//
// причина отказа названа неверно. Комментарий
// винил таблицу версий в `kotlin-gradle-plugin`; стектрейс показывает
...
//
// границы ниже сняты прогоном `./gradlew test`, а не догадкой:
