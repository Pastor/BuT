/**
 * Подсказка при наведении - своя, поверх страницы.
 *
 * Нативную подсказку (`title`) браузер рисует сам: своим шрифтом, своим
 * фоном, со своей задержкой и мимо темы страницы. У кнопок-значков подпись
 * ушла в подсказку целиком, и чужой её вид стал заметен - просьба постановка
 * 2026-09-05 "почему тултипы нативные? измени везде".
 *
 * **Устройство взято у образца** (`tamagotchi`, его `web/static/app.js`,
 ...
 *    его нет вовсе (контроль грепом), но узел с `title` может прийти из кода -
 ...
 *
 * Своё имя элементу подсказка не даёт: `aria-describedby` заставил бы
