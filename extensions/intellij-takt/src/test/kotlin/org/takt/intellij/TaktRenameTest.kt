 ...
 *
 * Свой rename плагина здесь не проверяется: декларация была бы
 ...
 * есть области видимости и вся рабочая область, тогда как здесь
...
class TaktRenameTest : BasePlatformTestCase() {
    ...
    /**
     * главный контроль: наш символ нативным рефакторингом **не переименовывается**
     ...
     * **файл**. Контроль на пустоту списка проверял бы наличие файла, а не наше
    ...
    fun testDeclarationIsNotNamedElement() {
        ...
        // здесь же контроль смотрит на фактический класс узла и переживёт, если
     ...
     * разрешается в декларацию того же файла: это тихая деградация.
     */
