...
impl Loc {
    ...
    pub fn begin_range(&self) -> Self {
        match self {
            Loc::Source(file_no, start, _) => Loc::Source(*file_no, *start, *start),
            loc => *loc,
        }
    }
    ...
    pub fn end_range(&self) -> Self {
        match self {
            Loc::Source(file_no, _, end) => Loc::Source(*file_no, *end, *end),
            loc => *loc,
        }
    }
    ...
    pub fn file_no(&self) -> usize {
        match self {
            Loc::Source(file_no, _, _) => *file_no,
            _ => not_a_file(),
        }
    }
    ...
    pub fn try_file_no(&self) -> Option<usize> {
        match self {
            Loc::Source(file_no, _, _) => Some(*file_no),
            _ => None,
        }
    }
    ...
    pub fn use_start_from(&mut self, other: &Loc) {
        match (self, other) {
            (Loc::Source(_, start, _), Loc::Source(_, other_start, _)) => {
                *start = *other_start;
            }
            _ => not_a_file(),
        }
    ...
    pub fn use_end_from(&mut self, other: &Loc) {
        match (self, other) {
            (Loc::Source(_, _, end), Loc::Source(_, _, other_end)) => {
                *end = *other_end;
            }
            _ => not_a_file(),
        }
    ...
    /// Creates a new `Loc::Source` by replacing `start`.
    ///
    /// # Panics
    ///
    /// If this location is not a file.
    ...
    /// Creates a new `Loc::Source` by replacing `end`.
    ///
    /// # Panics
    ///
    /// If this location is not a file.
