...
// model: Detect_Line
__attribute__((always_inline)) static void Main_Controller_Detect_Line_reset(struct Main *main) {
    assert(main != 0);
    main->controller.detect_line.state = MAIN_CONTROLLER_DETECT_LINE_INIT;
}

__attribute__((always_inline)) static void Main_Controller_Detect_Line_init(struct Main *main) {
    assert(main != 0);
    Main_Controller_Detect_Line_reset(main);
}

__attribute__((always_inline)) static void Main_Controller_Detect_Line_tick(struct Main *main) {
    assert(main != 0);
    ...
    const bool machine_gas = read_bit_port(main->port, PORT_ADDRESS_MACHINE_GAS, 0, false);
    const bool machine_back = read_bit_port(main->port, PORT_ADDRESS_MACHINE_BACK, 0, false);
    const bool machine_shift_left = read_bit_port(main->port, PORT_ADDRESS_MACHINE_SHIFT_LEFT, 0, false);
    const bool machine_shift_right = read_bit_port(main->port, PORT_ADDRESS_MACHINE_SHIFT_RIGHT, 0, false);
    ...
    case MAIN_CONTROLLER_DETECT_LINE_START: {
        ...
        if ((((distance_top >= 0 || distance_bottom >= 0) || distance_left >= 0) || distance_right >= 0)) {
...
// model: Detect_Angle
__attribute__((always_inline)) static void Main_Controller_Detect_Angle_reset(struct Main *main) {
    assert(main != 0);
    main->controller.detect_angle.state = MAIN_CONTROLLER_DETECT_ANGLE_INIT;
}

__attribute__((always_inline)) static void Main_Controller_Detect_Angle_init(struct Main *main) {
    assert(main != 0);
    Main_Controller_Detect_Angle_reset(main);
}

__attribute__((always_inline)) static void Main_Controller_Detect_Angle_tick(struct Main *main) {
    assert(main != 0);
    const float angle = read_real_port(main->port, PORT_ADDRESS_ANGLE, 0.);
    const float distance_top = read_real_port(main->port, PORT_ADDRESS_DISTANCE_TOP, 0.);
    const float distance_bottom = read_real_port(main->port, PORT_ADDRESS_DISTANCE_BOTTOM, 0.);
    const bool machine_rot_left = read_bit_port(main->port, PORT_ADDRESS_MACHINE_ROT_LEFT, 0, false);
    const bool machine_rot_right = read_bit_port(main->port, PORT_ADDRESS_MACHINE_ROT_RIGHT, 0, false);
    const bool busy = read_bit_port(main->port, PORT_ADDRESS_BUSY, 0, false);
...
// model: Detect_Point
__attribute__((always_inline)) static void Main_Controller_Detect_Point_reset(struct Main *main) {
    assert(main != 0);
    main->controller.detect_point.state = MAIN_CONTROLLER_DETECT_POINT_INIT;
}

__attribute__((always_inline)) static void Main_Controller_Detect_Point_init(struct Main *main) {
    assert(main != 0);
    Main_Controller_Detect_Point_reset(main);
}

__attribute__((always_inline)) static void Main_Controller_Detect_Point_tick(struct Main *main) {
    assert(main != 0);
    const int point = read_int_port(main->port, PORT_ADDRESS_POINT, 0);
    const bool on_line = read_bit_port(main->port, PORT_ADDRESS_ON_LINE, 0, false);
    const bool machine_gas = read_bit_port(main->port, PORT_ADDRESS_MACHINE_GAS, 0, false);
...
// model: Command
__attribute__((always_inline)) static void Main_Command_reset(struct Main *main) {
    assert(main != 0);
    main->command.state = MAIN_COMMAND_INIT;
}
__attribute__((always_inline)) static void Main_Command_init(struct Main *main) {
    assert(main != 0);
    Main_Command_reset(main);
}
__attribute__((always_inline)) static void Main_Command_tick(struct Main *main) {
    assert(main != 0);
    const float angle = read_real_port(main->port, PORT_ADDRESS_ANGLE, 0.);
    const int point = read_int_port(main->port, PORT_ADDRESS_POINT, 0);
    const float distance_top = read_real_port(main->port, PORT_ADDRESS_DISTANCE_TOP, 0.);
    ...
    const bool machine_shift_left = read_bit_port(main->port, PORT_ADDRESS_MACHINE_SHIFT_LEFT, 0, false);
    const bool machine_shift_right = read_bit_port(main->port, PORT_ADDRESS_MACHINE_SHIFT_RIGHT, 0, false);
    const bool push_up = read_bit_port(main->port, PORT_ADDRESS_PUSH_UP, 0, false);
    const bool push_down = read_bit_port(main->port, PORT_ADDRESS_PUSH_DOWN, 0, false);
    ...
    const bool command_reset = read_bit_port(main->port, PORT_ADDRESS_COMMAND_RESET, 0, false);
...
__attribute__((always_inline)) static bool Main_Command_finished(const struct Main *main) {
    assert(main != 0);
    return main->command.state == MAIN_COMMAND_END;
}
// model: Controller
__attribute__((always_inline)) static void Main_Controller_reset(struct Main *main) {
    assert(main != 0);
    main->controller.state = MAIN_CONTROLLER_INIT;
}
__attribute__((always_inline)) static void Main_Controller_init(struct Main *main) {
    assert(main != 0);
    Main_Controller_reset(main);
}
__attribute__((always_inline)) static void Main_Controller_tick(struct Main *main) {
    assert(main != 0);
    const bool busy = read_bit_port(main->port, PORT_ADDRESS_BUSY, 0, false);
    const bool error = read_bit_port(main->port, PORT_ADDRESS_ERROR, 0, false);
    const bool command_executing = read_bit_port(main->port, PORT_ADDRESS_COMMAND_EXECUTING, 0, false);
...
__attribute__((always_inline)) static bool Main_Controller_finished(const struct Main *main) {
    assert(main != 0);
