...
/// Сгенерировать объединённый ST-файл деклараций для всех моделей.
...
/// Сгенерировать объединённый ST-файл программы для всех моделей.
...
/// Сгенерировать объединённый ST-вывод `(decl, prog)` — один файл деклараций и один программы.
...
/// Сгенерировать файл объявлений для одной модели (для прямого использования в тестах).
pub fn generate_st_decl(model: &ModelDefinition, ctx: &CodegenContext) -> String {
    generate_st_model_decl(model, ctx)
}

/// Сгенерировать файл программы для одной модели (для прямого использования в тестах).
...
fn generate_st_model_decl(model: &ModelDefinition, ctx: &CodegenContext) -> String {
    ...
    if let Some(bk) = &behavior {
        // Компоновочная модель: переменная фазы + экземпляры подмоделей
        match bk {
        ...
        // is_done флаг (если есть терминальные состояния)
...
fn generate_st_fsm_prog(model: &ModelDefinition, ctx: &CodegenContext) -> String {
    ...
    for (idx, state_name) in states.iter().enumerate() {
        ...
        if let Some(ModelPart::StateDefinition(sd)) = model.parts.iter().find(|p| {
            ...
            // Терминальное состояние: end handler + is_done
            if is_terminal {
...
fn generate_st_behavior_prog(
    ...
    match bk {
        BehaviorKind::Sequential(_) => {
            out.push_str("(* Последовательная компоновка *)\n");
            out.push_str("CASE phase OF\n");
            for (idx, m) in models.iter().enumerate() {
                let mlo = m.to_lowercase();
                out.push_str(&format!("{}{} : (* Фаза: {} *)\n", i1, idx, m));
                out.push_str(&format!("{}{}();\n", i2, mlo));
        ...
        BehaviorKind::Parallel(_) => {
            out.push_str("(* Параллельная компоновка *)\n");
            for m in models {
                let mlo = m.to_lowercase();
        ...
        BehaviorKind::Choice(_) => {
            out.push_str("(* Компоновка выбора — выполняется первая выбранная модель *)\n");
