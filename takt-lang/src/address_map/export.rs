...
/// Печать имени источника адреса. Перечень **открыт**: потребитель обязан терпеть
...
/// имя. Ключ карты квалифицирован моделью, поэтому одноимённые порты разных под-моделей
/// дают несколько значений с одним `name`; для плоской выгрузки они схлопываются, и
/// побеждает последний - детерминированно, с максимальным квалифицированным ключом, то
/// есть последняя под-модель в порядке обхода. Так круговой рейс `export -> parse ->
/// export` остаётся тождеством. Полный список обоих портов при этом виден цели `c-hal`:
/// данные теряет только плоская выгрузка, а не карта.
///
...
/// [`AddressResolution::map`]). Порт без адреса опускается, а не печатается как `0x0`:
/// в выгрузке нуль неотличим от настоящего адреса. Результат разбирается
...
/// Повторная выгрузка разобранной карты в формат `map` - второе плечо кругового рейса
/// `export -> parse_address_map -> export`. Печатает те же строки, что
...
///
/// Ключ выгрузки плоский: одноимённые порты разных под-моделей делят запись `map`, и
/// побеждает последний. Это граница формата, а не разрешения адресов.
...
/// отсутствия, **не** `0x0`. Адрес - hex-строка (`"0x40000000"`), а не число:
...
fn export_ports_json(ports: &[JsonPort]) -> String {
    ...
    for (i, p) in ports.iter().enumerate() {
        ...
        match p.resolved {
            ...
            None => {
                // Мёртвый порт: адреса нет ни из одного источника.
