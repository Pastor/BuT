...
pub struct ResolvedAddress {
    ...
    ///
    /// Ключ [`AddressResolution::map`] квалифицирован моделью ([`qualified_port_key`]),
    /// чтобы одноимённые порты разных под-моделей не затирали друг друга. Публичные
    /// форматы - регистры `sv-mmio` и выгрузка `map`, `json` - показывают порт
    /// пользовательским именем, и его несёт это поле: квалификация ключа не протекает
    /// в выгрузку.
    pub name: String,
}
...
/// однозначен без разбора (это ключ `HashMap`, разбирать его не нужно). Продюсер и все
/// потребители обязаны строить ключ этой функцией, иначе поиск промахнётся молча.
pub fn qualified_port_key(model_unique: &str, port: &str) -> String {
    format!("{model_unique}\u{1}{port}")
}

/// Метаданные порта без разрешённого адреса - для выгрузки `json`: она перечисляет и
/// мёртвые порты, помечая отсутствие адреса явно, а не нулём. Заполняется тем же
/// обходом, что и [`AddressResolution::map`]: второго прохода по модели нет.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PortMeta {
    /// Имя порта.
    pub name: String,
...
pub fn resolve_addresses(
    ...
    // SE-051: записи карты без соответствующего порта. Ключ карты квалифицирован
...
fn resolve_model(
    ...
    for var in borrowed.variables.values() {
        ...
        match resolved {
            ...
            None => {
                // Порт без адреса записывается для выгрузки `json` явным отсутствием
                // адреса, а не только диагностируется. Тот же обход, второго прохода
                // нет.
