...
///
/// Адрес и бит живут в `i64` - их ширину эта фича не меняла. Литерал шире (`u64`-маска
/// законна как значение, но адресом быть не может), и сужение обязано быть **явным
/// отказом**: молчаливое `as i64` дало бы адрес, которого автор не писал, - ровно тот
/// класс, из-за которого 0042 завела `SE-055`.
