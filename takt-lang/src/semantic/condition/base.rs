...
//! `[T; N]`.
//!
//! Живёт отдельным модулем, потому что `condition/mod.rs` упирается в лимит размера
...
pub(super) fn cond_base_is_array(base: &ConditionNode, model: &ModelNode) -> bool {
    match base {
        ...
        // Неразрешённое имя индексировать нельзя: прежде ветвь искала переменную и, не
        // найдя, отвечала `SE-117` - поведение сохраняется.
