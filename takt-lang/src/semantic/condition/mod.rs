...
pub fn resolve_condition(
    ...
    match cond {
        ...
        // Индексация - постфикс над выражением, а не только над именем: `b.data[1]`
        // разбирается той же ветвью.
...
"#
    ...
    ///
    /// Форма доезжает до целей `c` и `sv` как есть; свёртку в общую переменную делают
    /// только `st` и `rust`, и делают её в своём конвейере - здесь дерево ещё
    /// каноничное.
    #[test]
