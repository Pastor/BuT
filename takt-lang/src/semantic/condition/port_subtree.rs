...
//! ```
//!
//! Приём - тот же, что у подъёма среза и агрегата, а форма своя: объявление с
//! агрегатом-инициализатором принимают инструменты всех восьми целей, тогда как пара
//! "объявление без значения плюс присваивания по полям" валидна у семи - у `rust` её
//! отвергает `clippy` (`field_reassign_with_default`) флагами проверки самой цели. Имя
//! временной берётся у общего носителя `semantic::fresh` - второй счётчик разошёлся бы
//! с первым молча.
//!
//! **Поднимать можно не везде, и граница названа.** Временную объявляют внутри тела
//! блока - там есть куда вставить пролог. В условии ребра,
