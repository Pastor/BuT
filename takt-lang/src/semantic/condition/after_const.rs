...
//!
//! - **вызов функции** (`after (f(1))`) - значение вызова компилятору неизвестно;
...
//!   лишь в такте. Здесь принимаются только константы; вычисляемую выдержку
//!   строит отдельный проход.
//!
...
fn fail(loc: Location, cause: &Cause) -> Diagnostic {
    // Параметр в позиции величины сборки получает свой код `SE-088`: он о режиме
...
fn nanos_of_var(
    ...
    // 1s;`. Арбитром в этих случаях становится значение: сводится к длительности -
    // принимаем, иначе автор получит `NotLiteral`. Отказ "тип _" сообщал бы автору о
