...
//! знают (приём 0143/0192/0397/0400).
//!
...
//! без нужды - тот же довод, что в 0397.
//!
...
pub(crate) fn split_composite_ports(
    ...
    // затенить авторское имя, а второй сборщик разошёлся бы с первым молча (общий
    // носитель - `semantic::fresh`, приём 0400).
...
fn rewrite_state(
    ...
    // Поля берутся у варианта напрямую: методов-доступов у `StateNode` нет, а заводить
    // их значило бы растить `semantic/mod.rs` - он в реестре долга по размеру (тот же
    // довод, что у обхода 0397).
...
fn rewrite_stmt(
    ...
    match stmt {
        ...
        // Условие оператора - тоже место обращения: прежде обходилось только тело, и
        // `if cfg.lo > 1 { ... }` доезжало до цели с обращением к развёрнутому порту.
        StatementNode::If {
            cond, then_, else_, ..
        } => {
...
/// прежде доезжали до целей нетронутыми - `cc` отвечал "use of undeclared identifier"
/// при нулевом коде возврата. Правая часть раздаётся листьям по позициям (агрегат) либо
/// доступом по остатку пути (значение), и вложенность обеим формам безразлична.
