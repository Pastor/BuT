...
//! границей семантики формы не существует - печатники целей о ней не знают.
//!
//! **Номер состояния - свой**, а не тот, что печатает цель: порядок берётся у
//! `model.states` (`BTreeMap`, детерминирован), и он один для записи и сравнения по
//! построению.
//!
//! **Обновление стоит в `enter`**: наблюдатель обязан увидеть переход соседа
//! **на том же такте**. `enter` исполняется в тике соседа, то есть до тика
...
/// Ключ модели - её уникальное имя: путь по `upper`, как у карты адресов.
...
/// стадий, а из конвейера конкретной цели: у `c` и `sv` наблюдение состояния соседа
/// работает напрямую, у `st` и `sv` - составной порт, и общий разворот изменил бы их
/// вывод без нужды.
///
/// Точка входа одна: вызывающему достаточно назвать цель, а какие понижения ей нужны,
/// знает этот модуль.
