...
///
/// Живёт рядом с самим узлом: строит узел перечисления и регистрирует его в модели.
///
/// # Регистрация идёт в две таблицы
///
...
pub(crate) fn build_enum(model_node: &Rc<RefCell<ModelNode>>, e: &ast::EnumDefine) {
    ...
    // Владелец нужен потребителю, чтобы строить имя из места объявления, а не из
    // модели, которую он печатает: иначе одноимённые перечисления разных моделей дают
    // одно имя в выводе.
    //
...
mod facts_tests {
    ...
    fn single_negative_one_needs_two_bits_not_one() {
        // Однобитного знакового не бывает: -1 занимает два бита.
