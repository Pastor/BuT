//! Трансформация `float -> q(m, n)` над семантическим деревом.
//!
//! Делает работу с `float` **прозрачной**: автор пишет вещественный тип, а
//! представление выбирается при компиляции глобальной Q-точностью `(m, n)` (CLI-флаг
//! `--float-as-q=m.n`). Проход заменяет каждый [`TypeNode::Rational`] на
//! [`TypeNode::Fixed`]`{ m, n }` и понижает литерал-инициализатор `float`-переменной в
//! **представление** (`Number(repr)`), переиспользуя всю Q-инфраструктуру
//! ([`lower_fixed_var`], [`lower_fixed_literal`], кодоген
//! `sv_fixed`/`c_expr::fixed`/..., `eval::fixed`).
//!
//! Проход применяется к обеим сторонам потактовой сверки - к модели цели и к модели
//! эталона (`build_unit`), - поэтому сверка ведётся внутри Q-режима.
//!
//! # Переменная существует в двух представлениях
//!
//! [`VariableNode`] лежит owned в [`ModelNode::variables`] (для объявлений -
//! struct-полей, значений сброса) и за `Rc<RefCell<...>>` в
//! [`ExpressionNode::Variable`], [`ConditionNode::Variable`] (для целей с
//! `fixed_format`, читающих `var.borrow().ty()` при выборе Q-арифметики). Проход обязан
//! мутировать оба: иначе объявление станет `q`, а арифметика останется `float` - или
//! наоборот, - и код будет молча неверен.
//!
//! # Что понижается, а что нет
//!
...
//!   ровно как у явного `q(m, n)`: `binary` эталона требует обоих операндов
//!   `Fixed`, а `Fixed + Number` разошёлся бы с целью `sv`, где `Number` - сырое
//!   представление. Оставшийся `Rational` в теле даёт громкую `SV-003`, а не
//!   тихий расчёт. Тела моделей на `float` пишутся на переменных.
//!
//! # Идемпотентность
//!
//! Понижается **только** `Rational` (тип) и литерал-инициализатор. Повторный проход
//! видит `Fixed` и `Number` и ничего не меняет. `Rc`-переменные разделяются между
//! использованиями, поэтому мутация `ty` многократна, но идемпотентна.
//! Разделяемые под-модели (`Rc<RefCell<ModelNode>>` в композиции) обходятся один раз
//! (набор посещённых по указателю).
...
/// Понижает все `float` (`TypeNode::Rational`) в дереве `model` в `q(m, n)`. Рекурсивно
/// по вложенным моделям; разделяемые под-модели обходятся однократно.
///
/// # Ошибки
///
/// Пробрасывает `SE-058` из [`lower_fixed_literal`], если литерал-инициализатор
/// `float`-переменной не представим точно в `q(m, n)` глобальной точности или выходит
/// за диапазон типа.
///
...
fn lower_model(
    ...
    // Снимаем ссылки на вложенные модели до мутирующего borrow_mut: рекурсия в них идёт
    // после закрытия текущего borrow.
...
fn lower_ty(ty: &TypeNode, m: u8, n: u8) -> TypeNode {
    match ty {
        // `sat: false`: флаг `--float-as-q` даёт формат без насыщения. Признак есть
        // часть типа, который автор пишет сам, а ключ сборки логики автомата не меняет.
        // Насыщающий регулятор записывается явным `q(m, n) sat`.
...
/// Понижает объявленную переменную: меняет тип и, если исходный тип был `float`
/// (`Rational`), понижает литерал-инициализатор в `Number(repr)` тем же
/// [`lower_fixed_var`](type_fixed::lower_fixed_var), что и явный `q(m, n)`.
///
/// Проверка "исходный тип - `Rational`" держит идемпотентность: повторный проход видит
/// `Fixed` и инициализатор не трогает. Без неё [`lower_fixed_var`] понизит уже
/// понижённый `Number(repr)` ещё раз, приняв представление за целое значение
/// (`repr * 2^n`). Та же проверка защищает явный `q(m, n)`: его инициализатор понижен на
/// этапе построения, и повторное понижение вывело бы значение за диапазон.
    ...
    match f {
        FunctionDefinitionNode::Local {
            ...
            Ok(FunctionDefinitionNode::Local {
                ...
                ret,
                body,
                // Понижение `float` -> `q` меняет типы разрешённого тела; сырое АСД
                // остаётся исходным - константный вычислитель работает с текстом
                // автора, а не с результатом флага сборки.
                raw,
            })
        }
...
/// Мутирует сигнатуру `Rc`-ячейки функции в узле вызова (для вывода типа возврата на
/// call-site). Тело **не** обходится: эмитируется тело из `functions`-map, а не эта
/// копия.
...
fn lower_stmt(stmt: &mut StatementNode, m: u8, n: u8) -> Result<(), Diagnostic> {
    match stmt {
        ...
        // Локальное объявление: понижается тип; литерал-инициализатор в теле не
        // понижается - как и у явного `q(m, n)`, понижается только карта объявлений.
...
fn lower_expr(expr: &mut ExpressionNode, m: u8, n: u8) -> Result<(), Diagnostic> {
    match expr {
        ...
        // Приведение: понижаем операнд и **целевой тип** (`... as float` -> `... as
        // q`).
        ...
        // База - выражение: обходится тем же понижением.
        ...
        // Тип обращения по адресу задан автором приведением; понижение `float` его не
        // касается - `float` там не выразим.
...
fn lower_cond(cond: &mut ConditionNode, m: u8, n: u8) -> Result<(), Diagnostic> {
    match cond {
        ...
        // Вычисляемая выдержка несёт условие внутри - его переменные подлежат тому же
        // перетипированию, что и все прочие.
...
/// Применяет трансформацию `float -> q(m, n)`, если она включена для цели опциями
/// генерации.
///
/// `embedded_gate` - требуется ли `--float-embedded`: `true` для программных целей
/// `c`/`rust`/`st` (native по умолчанию, Q - только с флагом), `false` для `sv`
/// (нативного `float` там нет, `q` подставляется всегда при заданной точности).
///
/// Без `--float-as-q` не делает ничего (корпус неизменен). Мутирует модель на месте -
/// вызывать **перед** генерацией.
///
/// Живёт здесь, а не в `lib.rs`: тот пришпилен реестром размеров, и место для нового
/// освобождается выносом.
...
mod tests {
    ...
    /// Идемпотентность: второй проход не понижает `Number` повторно (иначе `repr · 2ⁿ` -
    /// двойное понижение) и не трогает уже `Fixed`.
    #[test]
    ...
    /// Разная точность даёт разное представление того же литерала: `1.5` в q(4,4) = 24
    /// (1.5·16), в q(16,16) = 98304 (1.5·65536).
    #[test]
    ...
    /// Rc-ячейка переменной в теле (`b := a + a`) тоже получает тип `Fixed` - иначе
    /// арифметика цели осталась бы `float` при `q`-объявлении (молча неверный код).
    /// Проверяем через `extract_type` тела блока `always`.
    #[test]
    ...
    /// Литерал вне диапазона точности -> `SE-058` (проброс из `lower_fixed_literal`), а
    /// не тихое округление. `200.0` в q(8,8) (max ≈ 127.996) непредставимо.
    #[test]
