...
//! наоборот) - молча неверный код (ровно класс дефекта, ради которого требует сверку).
//!
...
//! видит `Fixed`/`Number` и ничего не меняет. `Rc`-переменные разделяются между
//! использованиями -> `ty`-мутация многократна, но идемпотентна (`Fixed -> Fixed`).
...
fn lower_ty(ty: &TypeNode, m: u8, n: u8) -> TypeNode {
    match ty {
        ...
        // часть типа, который автор пишет сам, а CLI логику автомата не меняет (принцип
        // 0042). Насыщающий регулятор записывается явным `q(m, n) sat`.
...
/// (`Rational`), понижает литерал-инициализатор в `Number(repr)` (переиспользуя
/// [`lower_fixed_var`](type_fixed::lower_fixed_var), путь 0061).
///
/// Проверка "исходный тип - `Rational`" держит **идемпотентность**: повторный проход видит
/// `Fixed` (не `Rational`) и инициализатор **не** трогает. Без него [`lower_fixed_var`]
/// понизила бы уже понижённый `Number(repr)` **ещё раз** (трактуя repr как целое
/// значение -> `repr · 2ⁿ`) - двойное понижение. Тот же проверка защищает **явный** `q(m,
/// n)` 0061: его инициализатор уже понижён на этапе построения, повторное понижение
/// вывело бы значение за диапазон.
...
fn lower_stmt(stmt: &mut StatementNode, m: u8, n: u8) -> Result<(), Diagnostic> {
    match stmt {
        ...
        // Локальное объявление: понижаем **тип**; литерал-инициализатор в теле не
        // понижаем (паритет с 0061 - понижаются лишь `variables`-map).
