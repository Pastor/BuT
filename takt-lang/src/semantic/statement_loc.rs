//! Позиция оператора и равенство операторов.
//!
//! # Позицию несёт не всякий оператор
//!
//! `Block`, `InlineFormula` и `Formula` своей координаты не хранят: у первого её роль
//! играет позиция первого элемента, у остальных - позиция вместилища. Это названная
//! граница, а не пропуск: поле стоило бы правки всех сопоставлений, а печатать перед
//! ними авторский комментарий (задача `0535-04`) незачем - он привязан к оператору
//! внутри тела.
//!
//! # Модуль отдельный - по требованию проверки размера
//!
...
/// Равенство операторов **позицию игнорирует**).
///
/// Войди позиция в равенство - два одинаковых по смыслу оператора из разных мест
/// перестали бы быть равными. Сравнение узлов используют подстановка функций, свёртка и
/// тесты, и расхождение пришло бы не отказом, а другим выводом. Ровно этот класс
/// закреплён для `Extend` и для ветви `match`.
///
/// Поэтому `PartialEq` написан руками, а не выведен: производный сравнивал бы и `loc`.
...
impl StatementNode {
    ...
    ///
    /// Ветвь `Block` отвечает позицией **первого элемента**: своей координаты у блока
    /// нет, а его началом читатель считает первую строку тела.
...
mod tests {
    ...
    ///
    /// Проверяются все формы, получившие координату: одной ветви мало - равенство
    /// написано руками, и забытая ветвь вернула бы производное поведение только для
    /// неё.
    #[test]
    ...
    /// Позиция доезжает до потребителя - иначе перенос комментария не к чему привязать
    /// (задача `0535-04`).
    #[test]
