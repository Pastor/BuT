//! Guard границ массива в порождённом коде - по флагу.
//!
//! #
//!
//! Guard **сообщает наружу** и операцию не выполняет; флаг **выключен по умолчанию**
//! (иначе изменился бы вывод всего корпуса и цена прошивки).
//!
//! # Форма
//!
...
//! ```
//!
//! `bounds_fault` - синтетический **выходной порт** типа `bit`. Это и есть "сообщить
//! наружу": у цели `c` он становится колбэком записи, у `rust` - методом HAL, у `st` и
//! `sv` - выходом модуля. Печатники целей не трогаются вовсе - приём тот же, что у
//! разворота среза и подъёма результата вызова (0431/0432).
//!
//! **Оборачивается оператор, а не выражение.** Разворот каждого доступа во временную
//! дал бы то же поведение, но потребовал бы знать тип элемента в каждой позиции;
//! условие же строится из размера массива, известного у базы.
//!
//! **Оборачивая `return`, ветвь отказа возвращает умолчание типа**: без этого `c`
//! печатал функцию, которая возвращает значение не по всем путям (`cc -Werror`
//! отвергал), `rust` - `else`-ветвь типа `()` (`E0308`), а `sv` и `st` молча отдавали
//! неопределённое значение - при нулевом коде возврата компилятора. Значение строит
//! [`default_value`](crate::semantic::default_value); типу без умолчания `return` не
//! оборачивается вовсе - граница названа, а не забыта.
//!
//! **Объявление оборачивается иначе, чем прочие операторы**: спрятав `var x := d[i];`
//! внутрь `if`, проход унёс бы туда и саму переменную - дальше по телу её уже не видно
//! (`E0425` у `rust`, "undeclared identifier" у `c`). Поэтому объявление остаётся
//! снаружи с умолчанием, а под guard уходит запись:
//!
//! ```text
//! var x: T := d[i];
//! ⇓
...
//! ```
//!
//! **Объявление функции живёт в двух представлениях**, и проход обязан обновить оба:
//! владелец - `ModelNode::functions`, а вызов держит свою
//! `Rc<RefCell<FunctionDefinitionNode>>`. Пока обновлялся один, признак "функции нужен
//! указатель состояния" (`c_needs`/`rust_needs`) отвечал у объявления "да", а у вызова
//! "нет" - и `cc` отвечал "too few arguments", `rustc` - `E0061`, при нулевом коде
//! возврата `taktc`.
//!
//! **Нижняя граница проверяется только у знакового индекса**: у беззнакового `i >= 0`
...
/// **целевых** языков и не сталкиваться с именем автора.
pub(crate) const FAULT_PORT: &str = "bounds_fault";

/// Заметка к диагностике о порте, заведённом этим проходом.
///
/// `None` - порт написан автором, и объяснять нечего.
///
/// Текст один на двух носителей `SE-052` (`validate::ports` и `address_map::resolve`):
/// вторая формулировка разошлась бы молча - /0193/0195. Позиции у заметки нет:
/// объявления в исходнике не существует (тот же довод, что у `SE-119`).
...
///
/// Зовётся конвейером цели при `GenerateOptions::bounds_check`; эталон (`takt-sim`)
/// зовёт его своим флагом. По умолчанию не зовётся никем - вывод корпуса от фичи не
/// меняется.
...
///
/// Тела изымаются на время обхода (`mem::take`): размер массива читается у той же
/// модели, и изменяемое заимствование этого не допускает (приём 0400).
...
///
/// Сверка идёт по имени: у каждого употребления своя ячейка (тот же урок, что у
/// подстановки тела - 0444), и адресовать её иначе нечем.
...
/// Ячейка синтетического порта - одна на модель, чтобы все присваивания ссылались на
/// один узел.
...
/// Обходит оператор; обернуть можно только элемент блока - там есть куда поставить
/// `if`.
fn guard_stmt(
...
fn wrap_item(
    ...
    // Возврат обязан отдать значение и в ветви отказа: иначе вывод целей невалиден либо
    // значение неопределено. Умолчания у типа нет - оператор не трогаем: дыра в guard
    // честнее невалидного вывода.
    let returns = matches!(item, StatementNode::Return(Some(_), _));
    if returns && ret_default.is_none() {
        return;
    }
    // Объявление переменной: значение уходит под guard, само объявление остаётся
    // снаружи - иначе переменная теряется вместе с областью видимости (см. врезку).
    // Умолчания у типа нет - оператор не трогаем.
...
///
/// Обход не исчерпывающий по построению: пропущенная форма даёт **прежнее** поведение
/// (доступ без guard), а не порчу вывода.
...
///
/// `None` - база не массив известного размера либо индекс уже проверен статически
/// (литерал; константу к этому моменту свернула стадия 2).
fn bound_check(
    base: &ExpressionNode,
    ...
    // Нижняя граница - только у знакового индекса: у беззнакового `i >= 0` истинно
    // всегда, и линт цели ответил бы предупреждением, а проверки считают его ошибкой.
