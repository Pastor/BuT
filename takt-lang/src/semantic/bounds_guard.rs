...
//!
//! Guard сообщает наружу и операцию не выполняет; флаг выключен по умолчанию: иначе
//! изменились бы вывод всего корпуса и цена прошивки.
//!
//! # Форма
//!
...
//! `sv` - выходом модуля. Печатники целей не трогаются вовсе - тем же приёмом живут
//! разворот среза и подъём результата вызова.
//!
...
//! `Rc<RefCell<FunctionDefinitionNode>>`. Обнови проход одно представление - и признак
//! "функции нужен указатель состояния" (`c_needs`, `rust_needs`) ответит у объявления
//! "да", а у вызова "нет": `cc` скажет "too few arguments", `rustc` - `E0061`, при
...
/// вторая формулировка разошлась бы с первой молча. Позиции у заметки нет: объявления в
/// исходнике не существует - тот же довод, что у `SE-119`.
...
/// модели, и изменяемое заимствование этого не допускает.
...
///
/// Сверка идёт по имени: у каждого употребления своя ячейка, и адресовать её иначе
/// нечем.
