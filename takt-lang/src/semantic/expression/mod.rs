...
/// Как назвать базу постисправления в диагностике.
///
...
pub fn construct_expression(
    ...
    match expr {
        ...
        // Индексация и срез - постисправления над выражением: `b.data[1]` прежде не
        // разбирался вовсе, тогда как `ps[1].x` работал.
        //
        // Проверки границ (`SE-028`) и "это не массив" (`SE-030`) остались, но тип базы
        // даёт теперь общий носитель `semantic::base_type`: у выражения объявления нет.
