//! Тесты построения узлов выражений.
//!
//! Раскладка обычная для проекта: директория-подмодуль рядом с логикой и `use super::*`.
...
///
/// Площадка выбрана телом блока, а не инициализатором переменной: инициализаторы
/// `var` и `const` сворачиваются в литерал на семантике, и выражения там уже нет - за
/// границей семантики его быть и не должно, иначе потребителям есть по чему разойтись.
/// В теле блока выражение живёт, и построение узлов проверяется на нём.
///
