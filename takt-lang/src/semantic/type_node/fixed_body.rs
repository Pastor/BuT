...
//! приведения к типу приёмника, только вопрос решается в семантике - за её границей
//! формы не существует, и потребителям не по чему расходиться.
//!
//! **Условие ребра доезжает сюда разрешённым.** Правило "условия рёбер `ref` не
//! разрешаются" верно, но относится к одной форме - паттерну `S(Модель) = Состояние`.
//! Стадия 6 разрешает остальные, а `resolve_condition` отдаёт `Unresolved` ровно на
//! неразрешённом имени, где ни литерала, ни сравнения не бывает. Поэтому обхода сырого
//! `ast::Condition` здесь нет: он не срабатывал бы ни на одном входе корпуса, а
//! недостижимую ветвь нельзя ни проверить, ни удержать от расхождения.
//!
//! Носитель представления - `lower_fixed_literal`: второго знания о масштабе 2ⁿ не
//! заводится.
//!
//! # Место записи - не только имя
//!
//! Приёмник - место, а не имя. Считай приёмником голое имя, и формат `q` возьмётся у
//! ячейки ссылки, а поле структуры с элементом массива под правило не подпадут, хотя тип
//! объявлен рядом:
//!
//! | Запись | эталон | цель `c` | `st`, `rust`, `sv` |
//! |---|---|---|---|
//! | `g.kp := 2.0;` при `kp: q(8, 8)` | `2` | `model->g.kp = 2.0;`, то есть 0 | вывод отвергают их инструменты |
