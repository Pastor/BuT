...
//!
//! Вынесено из `tree.rs` (лимит размера модуля) и заодно по смыслу: эти три стадии
//! отличаются от остальных **природой своих элементов**.
//!
...
//! опирается, поэтому там разбор терминален (решение ).
//!
...
/// ошибка в одном не делает следствием другой. Обход продолжается, а диагностики
/// копятся; проба 0152 показала, что вторая из них - самостоятельная причина (`nope1` и
/// `nope2` в телах разных состояний).
///
