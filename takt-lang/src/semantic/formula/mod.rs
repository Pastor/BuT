...
pub enum Formula {
    ...
    /// `invariant Имя = C;`. У анонимной формы `: [Guard] c;` - `None`. Имя несёт
    /// диагностика эталона (`SIM-025`); на вывод цели `c` оно не влияет - генератор его
    /// игнорирует.
    ///
    /// Третье поле - позиция самой формулы. Без неё реестр мест (`formula::sites`)
    /// кладёт в диагностику позицию вместилища, и два `invariant` в одном файле дают два
    /// предупреждения `ST-022` с одинаковой координатой `1:1`.
