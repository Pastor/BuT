...
//! добавлять его нужно здесь, иначе оно молча выпадет **из обоих** потребителей (тот же
//! класс расхождения, от которого уходили 0084, 0193, 0195). Именно фильтр на выходе
//! прежнего обхода и дал дефект 0203: `Guard` выбрасывался одной веткой, и охранная
//! формула с опечаткой молчала.
...
pub struct FormulaSite {
    ...
    ///
    /// Прежде здесь стояла координата вместилища, и разница была наблюдаема: две
    /// формулы уровня модели печатались с одной и той же строкой `1:1` (замер 0168).
    /// Своей позиции у формулы в дереве не было - её завели 0282 (охранная) и 0471
    /// (темпоральная).
    ///
...
fn collect_formula(
    ...
    match formula {
        // Позиция берётся у самой формулы: с 0471 темпоральная её несёт, а прежде сюда
        // шла координата вместилища - три формулы уровня модели печатали `SE-055` с
        // одной и той же строкой `1:1`, и понять, о которой речь, было нельзя. Запасной
        // ход тот же, что у охранной формы: формула, построенная вне разбора, своей
        // позиции не имеет.
