...
///
/// Объявление `clock` - контракт тактирования **всей сборки** (`SE-069`/ `SE-070`
/// требуют от цели `c` совпадающего `--tick-hz`), и граница импорта его не разрывает:
/// библиотека, объявившая частоту, обязывает импортёра так же, как объявление в его
/// собственном файле.
///
/// Замер 2026-08-19: прежде путь импорта не звал [`collect_clock`] вовсе - `clock`
/// подключённого файла **терялся**, профиль оставался "часы", и прошивка собиралась с
/// частотой, отличной от объявленной, без единого слова.
///
/// # Ошибки
///
