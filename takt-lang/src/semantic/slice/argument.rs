...
//! вовсе.
//!
//! **Форма разворота выбрана замером.** Объявление с инициализатором
//! (`var t: [T; N] := src[a:b];`) не годится: срез в инициализаторе локального
//! объявления не переводит ни одна цель. Работает пара "объявление плюс присваивание" -
//! присваивание среза цели переводят.
//!
//! # Имя временной переменной
//!
//! `takt_slice_<n>` - и оно проверяется на занятость: имя обязано быть допустимым
//! идентификатором целевых языков (на имя вида `#...` `cc` отвечает "expected
//! identifier", а `iec2c` - "invalid variable(s) declaration"), а значит написать такое
//! же может и автор. Занятые имена собираются со всей модели - объявления и локальные
//! переменные тел - до обхода; молчаливое затенение чужого имени было бы тем же классом,
//! что `SE-086` у специализации.
//!
...
pub(crate) fn expand_slice_arguments(model: &Rc<RefCell<ModelNode>>) -> Result<(), Diagnostic> {
    ...
    // обслуживает подстановку тела функции: второй сборщик разошёлся бы с первым молча.
...
fn lift_slice(
    ...
    // Длина источника и тип элемента - у общего носителя: второго знания о срезе не
    // заводится.
...
///
/// Без подъёма `pick({1, 2})` при `fn pick(a: Pair)` цель `c` печатает
/// `Fnstruct_pick({1, 2})`, и `cc` отвечает "expected expression" при нулевом коде
/// возврата `taktc`; `st` и `sv` отказывают (`ST-011`, `SV-002`), а эталон и `rust`
/// запись исполняют. Тот же приём, что у среза: за границей семантики агрегата в
...
enum CallLift {
    /// Массив и структура: индексация результата.
    Composite,
