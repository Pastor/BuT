...
pub fn construct_function(
    ...
    if let FunctionDefinitionNode::Unresolved(def) = func {
        ...
        let name = def
            ...
            .name
            .clone();
        // Дубликат имени (`SE-009`) судит точка вставки - проход сбора функций в
        // `tree.rs`. Здесь этой проверки быть не должно: карта модели на время
        // разрешения тел непуста, и `contains_key` срабатывал бы на каждой функции -
        // её имя уже в карте.
        {
            ...
            for (_, param) in def.params.iter() {
                if let Some(param) = param {
                    ...
                    let param_type = match param.clone().ty {
                        ...
                        // хранит выражением, а `q(8, 8)` как выражение есть вызов: `q`
                        // не ключевое слово, и правило `Type` сюда не подставить - оно
                        // даёт конфликт с вызовом функции. Поэтому форму распознаёт
                        // семантика, как и имя `q` (`construct_fixed`).
...
/// функции: `q` - обычный идентификатор, а правило `Type` в позицию параметра не
/// подставить - оно даёт конфликт с вызовом. Поэтому форму опознаёт семантика: два
/// числовых аргумента и имя, которое дальше проверит `construct_fixed` (`SE-057` на
/// чужое имя).
///
...
/// идентификатор за вызовом, и разбор отвергает его `SY-002`. Это названная граница.
fn fixed_from_call(
