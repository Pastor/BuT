...
//! Полагаться на конкретный вид имени не вправе никто, кроме проверки воспроизводимости
//! (п. Занятое имя - `SE-086`, а не молчаливое переименование: пространство имён
//! кодогена проверок не имеет (кандидаты 0182), и молчание здесь стоило бы коллизии в
//! выводе.
...
fn specialize_one(
    ...
    // Специализация модели с собственными вложенными моделями - за границей задачи:
    // `copy` разделил бы под-модели по `Rc`, и их владелец (а с ним уникальное имя
    // пути) разъехался бы с местом в дереве - `CC-004`, урок adopt 0184 ("владельца
    // под-моделей менять нельзя"). Честный отказ.
    ...
    *counter += 1;
    ...
    // Тела копии. У модели, объявленной в **этом** файле, тел ещё нет - их разрешат
    // стадии 2-6 уже на переменные копии. У модели, пришедшей импортом, они разрешены
    // (весь конвейер прошёл внутри стадии 0 импортёра, 0296), и ячейки-снимки в них
    // указывают на исходную модель: без перепривязки цель `c` печатает доступ к чужому
    // полю. Обход безвреден и в первом случае - перепривязывать там нечего.
    ...
    // (`const_eval` + `construct_expression` в /03): стадия 2 разрешать его не будет, а
    // вывод типов возьмёт тип из аннотации.
    for arg in args {
...
/// Уникальное имя модели - путь по цепочке `upper` (как у продюсера карты адресов 0084:
