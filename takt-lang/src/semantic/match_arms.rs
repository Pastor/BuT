//! Признаки ветвей `match` - общий носитель.
//!
//! Автор вправе написать ветвь без операторов: `_ => {}` ("прочие значения ничего не
//! делают") - запись из практики, она стоит в примерах документа. Эталон и цели `c`,
//! `sv` её принимают, а инструменты двух других отвергают напечатанное при нулевом коде
//! возврата `taktc`:
//!
...
//!
//! Пустого оператора в IEC 61131-3 нет вовсе (`;` MatIEC отвергает), поэтому у цели
//! `st` пустая ветвь опускается, а не наполняется заглушкой.
//!
