...
///
/// Переполнение - **обёртка** (`wrapping_*`): норма для беззнаковых величин, и она же
/// прежде действовала в обеих сведённых сюда таблицах. Сужение результата к своему
/// носителю - задача вызывающего.
...
mod tests {
    ...
    /// Целая степень считается таблицей: прежде знак `**` был ей неизвестен, и `const
    /// SPAN: u16 := 2 ** 8;` терял значение - молча у эталона и цели `st`, отказом у
    /// `c` и `sv`.
    #[test]
