...
//! 0042). Проверка "влезает ли в тип параметра" - забота **применения** значения:
//! только там известен целевой тип, а нормы 0127 (беззнаковое - обёртка `mod 2ⁿ`,
//! знаковое - ошибка) сформулированы про тип.
//!
//! ## Третья арифметика в компиляторе
//!
//! Их теперь три: адрес, выдержка `after` и эта. ("арифметика - в одном месте") требует
//! не размножать правила: длительности здесь хранятся в **наносекундах**, как в
//! `after_const`, и пересчёт в миллисекунды делает единственный
//! `semantic::duration::value_millis`. Объединение трёх вычислителей - кандидат в фичи,
//! а не работа этой задачи.
...
fn int_op(op: &str, a: i128, b: i128, loc: Location) -> Result<ConstValue, Diagnostic> {
    ...
    match int_ops::int_binary(op, a, b) {
        ...
        // Та же граница, что у выражения адреса и у нормы 0127.
...
/// узлом - оба пути штатны (та же двойственность, что в 0143). Вычисляет **понижённый**
/// узел выражения (в отличие от [`eval`], который принимает сырое АСД).
///
...
///
/// Возвращает **дробный литерал**, а не готовое представление: дальше он идёт тем же
/// путём, что литерал автора (понижение 0096, свёртка 0192), и второй ветки для "уже
/// понижённого" значения не заводится.
///
...
fn cast_identity(
    ...
    if let ast::Type::Array {
        ...
        // неподдержанных элементов. Первая редакция так и сделала - ветвь не
        // срабатывала ни на одном входе, и проба это показала.
    ...
    if let TypeNode::Integer { bits, signed } = target {
        return match int_cast::integer(*n, bits, signed) {
            ...
            // потребителям - эталон дал бы `0` молча (замер 2026-08-20), цели
