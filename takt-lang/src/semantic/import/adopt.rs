...
struct Adoption {
    ...
    /// импорта не жива, и делить её не с кем. У копии-специализации делить есть с кем:
    /// `ModelNode::copy` клонирует узлы тел, но `Rc`-ячейки внутри них разделяет с
    /// исходной моделью, - правка на месте испортит исходную, и вторая специализация
    /// напечатает доступ через поле первой (`cc`: "no member named 'run'").
    fresh_cells: bool,
}
...
/// моделью, а обращение к переменной промежуточной модели цели генерации не выражают:
/// цель `c` печатает доступ через несуществующее поле (`model->app.pidlib0.z` изнутри
/// вложенной модели). Перенос делает такое объявление обычной переменной корня, видимой
/// всему поддереву.
///
/// # Ошибки
///
...
pub(in crate::semantic) fn adopt_whole_file(
    ...
    }
    {
        ...
        // Позиции переносятся вместе с типами: без них проверка занятости имени не
        // назовёт первое объявление, а `SE-108` без второй позиции - половина сообщения.
...
///
/// Условия рёбер и формулы обходятся наравне с телами блоков. Импорту хватило бы одних
/// `named_blocks`: ячейки условий импортированной модели принадлежат ей самой, а
/// перепривязки требуют лишь пришедшие от корня библиотеки. Специализация же меняет
/// владельца всех ячеек модели, и без этих полей условие `ref Idle: out_value > 0;`
/// копии читает поле исходной модели - `model->tuner.out_value` в теле `TunerP1`.
///
/// `next` - отдельное поле, а не элемент `references`: обход, взявший только список,
/// теряет безусловный переход.
