...
//! константность выводится, а не объявляется. Выразить её можно только там, где на
//! каждую настройку приходится своя модель, - то есть в режиме
//! `--parameters=specialize`: константа цели `c` - макрос на модель, и одно объявление
//! с двумя значениями невыразимо. В умолчании `assign` параметр остаётся полем
//! экземпляра, и проход не применяется.
//!
...
//!
//! **Порядок "до стадии 2" несущий.** Тела разрешаются стадиями 2-6, и ссылка на
//! переменную в теле - своя `Rc`-ячейка, склонированная из карты `model.variables` в
//! момент разрешения (`semantic/expression/mod.rs`). Пока стадия 2 не прошла, ячеек не
//! существует: правки карты достаточно. Смена вида после разрешения тел потребовала бы
//! мутировать оба представления, иначе объявление разойдётся с арифметикой. Второй довод
//! той же силы: `after PARAM` и адрес порта спрашивают у объявления "константа ли", и
//! спрашивают на стадиях 2-3, - опоздав, проход дал бы вычисляемую выдержку вместо
//! константной.
//!
...
//! изменяемым. Это ошибка в безопасную сторону: параметр останется полем - форма вывода
//! проще, чем могла быть, поведение прежнее. Обратная ошибка (изменяемый параметр
//! объявлен константой) сделала бы вывод молча неверным, и ради её невозможности
//! точностью здесь пренебрегаем. По той же причине не
...
struct Scan {
    ...
    /// Разобран ли АСД целиком. `false` - встретился узел ошибки разбора: полноту
    /// обещать нельзя, и все параметры объявляются изменяемыми.
    complete: bool,
}
...
///
/// Сообщение называет режим: в умолчании `assign` параметр - поле экземпляра, то есть
/// величина такта, и молчаливая деградация запрещена. Без отказа автор получил бы
/// вычисляемую выдержку вместо константной, усечённую ширину или потерянный адрес,
/// который диагностируется следствием (`SE-052`), а не причиной.
