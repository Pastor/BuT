...
///
/// Имя файла к этому отношения не имеет: ломается `start Pid = Pid;` в любом файле
/// (замер опроверг первоначальную запись, связывавшую дефект с именем файла). Ломаются
/// **все четыре** цели: карта моделей теряет запись, и `rust` отвечает `RS-012`, `sv` -
/// `SV-002`, `c` печатает несуществующий тип, `iec2c` отвергает вывод `st`.
///
/// **Условие "дочерняя того же владельца" - не перестраховка, а граница дефекта.**
/// Состояние-тёзка **соседней** модели работает и переводится: в эталонной исправлениетуре
/// проекта `model Toggle { state Ping = Ping; ... }` при `model Ping` уровнем выше даёт
/// валидный C (проверено `cc`). Первая редакция этой проверки условия не имела и
/// отвергала законную запись - поймал прогон теста инварианта `S(Ping) = End`, а не
/// чтение.
