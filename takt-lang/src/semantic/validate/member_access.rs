...
//! шириной** (`SE-125`).
//!
//! Доступ `p.field` к несуществующему полю структуры судит семантика: без этого цель `c`
//! печатает `model->p.NOSUCHFIELD` молча, и ошибку ловит лишь `cc` ("no member named
//! ...") - поздно и невнятно, а эталон отвечает `SIM-027` уже на исполнении. Здесь
//! диагностика приходит при компиляции и одинаково у всех целей.
//!
...
//!
//! На `var w: [bit;96]; w.100 := 1;` без этой проверки эталон пишет, `c`, `c-hal` и
//! `rust` печатают вывод, принимаемый их инструментами, `st` отказывает (широких
//! векторов не переводит), а `verilator` вывод `sv` отвергает (`SELRANGE: Selection
//! index out of range: 100:100 outside 95:0`) при нулевом коде возврата `taktc`: цель
//! печатает регистр шириной по объявлению.
//!
...
//! обязан.
//!
//! **Граница - выражение, чей тип не выводится** (результат вызова и подобное):
