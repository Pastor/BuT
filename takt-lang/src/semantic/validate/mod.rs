...
pub fn validate_model_all(model: Rc<RefCell<ModelNode>>) -> Vec<Diagnostic> {
    ...
    // Каждая проверка **накапливает**: прежде все одиннадцать
    ...
    let checks: [Vec<Diagnostic>; 14] = [
        ...
        // использование пустого перечисления давало бессодержательное `SE-043` "...не
        ...
        // `diagnostics::normalize` по позиции в тексте (замер 0172 - перестановка
        // проверок местами ничего не меняет).
    ...
    // Ce16: рекурсивные псевдонимы - проверка отдаёт все циклы сразу. Прежде вызывающий
    // брал из них первую: накопление здесь было написано, но выбрасывалось.
    ...
    //  * SE-026/SE-027: направление порта во всех позициях. Тела блоков и
    ...
    // SE-100/SE-101: столкновения имён, неразрешимые в пространстве имён цели. Отказ
    // переносится из чужих инструментов (`cc`, `iec2c`) в свой компилятор: обе формы не
    // работают ни В одной цели, поэтому запрещается не работающая запись, а уже
    // существующий отказ получает позицию.
