...
//! отвергает свёртка инициализаторов (`semantic/declaration.rs`, и 0306) стадией 2 -
//! там же, где вычислитель называет причину. Место эмиссии `SE-084` одно; вторая копия
//! правила разъехалась бы текстом.
...
fn find_undefined_read(expr: &ExpressionNode) -> Option<UndefinedRead> {
    match expr {
        ...
        // Пока проверка стояла и здесь, у одного кода было два места эмиссии с разными
        // текстами; их расхождение - /0193/0195.
