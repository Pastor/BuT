    ...
    match cond.clone() {
        ...
        ConditionNode::Unresolved(cond) => {
                ...
                && let ast::Condition::Variable(id) = cond.clone()
            {
                ...
                // одна функция на проект: знай судья только `S(Модель)`, тогда как цели
                // `c` и `rust` знают и краткое `Модель`, - `ref X: E != End;` отвергался
                // бы `SE-025` на записи, которую генератор переводит.
            ...
            if let ConditionNode::Unresolved(_) = resolve_condition(&cond, model.clone())? {
                // Цитата - текст исходника, а не `Debug`-дамп узла: дамп дал бы
                // сообщение вида "Неразрешённое условие: Variable(Identifier { loc:
...
pub(super) fn validate_expression(
    ...
    match expr {
        ...
        ExpressionNode::Assign(left, right) => {
            ...
            //
            // Исключение действует на все формы цели записи, а не только на `BitAccess`
            // (`led.0 := 1;`): проверка идёт и по телам блоков, где `led := 1;`
            // достижимо.
            //
