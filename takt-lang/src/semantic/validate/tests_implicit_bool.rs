...
//!
//! Вынесены из `tests.rs`: файл упёрся в предел размера модуля, а это самостоятельная
//! тема, у которой уже есть соседи-прецеденты (`tests_ce4_declarations.rs`,
//! `tests_ce15_array_size.rs`).
//!
//! **Проверяется поведение, а не предикат**. Правило удалено (замер: 433 проверки,
//! запасной путь не сработал ни разу), и вместе с ним ушли тесты, стоявшие на нём.
//! Оставшиеся идут через `check_implicit_bool_conditions` - то есть проверяют вердикт
//! на исходнике, а не устройство внутренней функции.
...
// внутренней функции. Таблица покрывает те же случаи, что прежде юнит-тесты удалённого
// АСД-правила, - но так, как их видит автор программы.
