...
///
/// Тип `bit` принимает только числовые значения `0` или `1`, а также булевы литералы
/// `true` / `false`. Выражения, не являющиеся числовыми литералами (переменные,
/// операции), не проверяются статически.
///
/// # Ошибки
///
...
///
/// **Порт исключён намеренно.** Инициализатор порта - это его адрес, а не значение:
/// `in BTN: bit at 0x00100000;` есть адрес, и `check_bit_variable_value` дал бы ложную
/// `SE-035` "плохое значение бита". Адрес порта проверяет и потребляет слой
/// `address_map/resolve.rs` (`SE-052`, `SE-049` и прочие). Проверка значения бита
/// остаётся только там, где инициализатор - действительно значение (`var` и `const`).
///
/// Рекурсия по вложенным моделям не нужна - [`validate_model`] уже обходит их
/// самостоятельно, вызывая `validate_bit_values` для каждой вложенной модели.
///
/// # Ошибки
///
...
pub(super) fn validate_bit_values(model: Rc<RefCell<ModelNode>>) -> Vec<Diagnostic> {
    ...
    for var in borrowed.variables.values() {
        match var {
            ...
            // Порт: инициализатор - адрес, не значение; не проверяем.
...
/// Проверяет, что у каждого перечисления модели есть хотя бы один вариант (`SE-105`).
///
/// ## Мотивация
///
...
/// принял решение языка: **перечисление обязано иметь хотя бы один
/// вариант**, и высказывает его семантика - на объявлении, а не на первом
/// использовании.
///
/// Отказ стоит именно на **объявлении**. Без него использование пустого перечисления
/// давало бессодержательное `SE-043` "...не является вариантом перечисления (допустимые
/// варианты: )" - про пустой список автору не сообщалось ничего.
///
/// Тип с пустым доменом не имеет представления ни в одной цели: `enum_facts` отдаёт
/// `None`, после чего `c`/`st`/`rust` молча берут 8 бит без знака, а `sv` отвечает
/// `SV-004`. Эта развилка сохраняется - её недостижимость из исходника теперь держит
/// `SE-105`, а не квантификатор грамматики.
///
/// Рекурсия по вложенным моделям не нужна - [`validate_model_all`] обходит их сам (тот
/// же довод, что у [`validate_bit_values`]).
///
/// # Ошибки
///
/// Возвращает по одной [`Diagnostic`] на каждое пустое перечисление (накопление,
/// правило ).
...
/// Проверяет, что все переменные, тип которых - [`TypeNode::Enum`], ссылаются на
/// фактически объявленные перечисления.
///
/// ## Мотивация
///
/// `construct_type` не может проверить существование перечисления на этапе построения
/// дерева, поскольку перечисления и переменные обрабатываются в одном проходе и могут
/// идти в любом порядке. Эта функция выполняется после полного построения дерева, когда
/// `ModelNode::enums` уже заполнена.
///
/// ## Примеры
///
/// ```text
...
pub(super) fn validate_enum_type_declarations(model: Rc<RefCell<ModelNode>>) -> Vec<Diagnostic> {
    ...
    // Накопление по объявлениям: второе объявление с неизвестным перечислением -
    // самостоятельное нарушение, а не следствие первого.
    let mut out = Vec::new();
...
/// Проверяет все переменные модели на корректность начальных значений для enum-типов
/// (NI6).
///
...
///
/// Возвращает `true`, если значение `n` совпадает с числовым значением хотя бы одного
/// варианта перечисления `enum_name` в контексте модели. Если перечисление не найдено -
/// не блокируем (ошибка другой проверки).
...
/// допустимых вариантов перечисления. Проверяются:
///
/// - Инициализаторы переменных объявленных как `var x: Direction = 0;`
/// - Присваивания в именованных блоках `always`, `enter`, `exit`
///
/// Статически проверяются только числовые литералы. Присваивания через переменные или
/// функции не проверяются.
///
/// # Примеры (Takt)
///
/// ```text
...
///
/// Вектор [`Diagnostic`] уровня `Error` с типом
/// [`TypeError`](crate::diagnostics::ErrorType::TypeError). Пустой вектор означает
