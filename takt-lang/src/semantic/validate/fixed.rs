//! Запрет неявного смешения типов в арифметике: `q(m, n)` (T6, `SE-059`) и `duration`.
//!
//! Оба правила живут в **одном** обходе намеренно: арифметическое выражение обходится
//! один раз, и второй экземпляр обхода неизбежно разъехался бы с первым.
//!
//! `q(m, n)` в арифметике сочетается **только** с тем же `q(m, n)`. Смешение с иным
//! `q`, целым, `float` и т. - ошибка `SE-059`, а не молчаливая потеря точности (иначе
//! [`wider_type`] вернул бы `Unsupported`, и расхождение уехало бы в кодоген). Выход -
//! явное приведение `... as q(m, n)` (узел `Cast`), который смешение снимает.
//!
//! Обход повторяет [`unused`](crate::semantic::unused): выражения инициализаторов, тел
//! функций, именованных блоков и блоков состояний. Вложенные модели обходит
//! [`validate_model`](super::validate_model), поэтому здесь - только текущий уровень.
//!
...
pub(super) fn check_fixed_mixing(model: Rc<RefCell<ModelNode>>) -> Vec<Diagnostic> {
    // Снимаем владельческие копии до обхода: `extract_type` не берёт `borrow_mut`, но
    // держать borrow через `?`-возвраты незачем.
    ...
    // Накопление по элементам: объявление, функция, блок и состояние - самостоятельные
    // места; внутри каждого остаётся первая ошибка (дальше по тому же выражению пошли
    // бы следствия).
    let mut out = Vec::new();
    for var in &vars {
...
fn var_init(var: &VariableNode) -> Option<&ExpressionNode> {
    match var {
        ...
        // У порта берётся **начальное значение**, а не адрес: смешение типов
        // fixed-point - свойство значения; адрес - целое число и к арифметике `q(m, n)`
        // отношения не имеет.
...
/// Блоки и рёбра состояния (условия рёбер - отдельная грамматика, здесь не проверяются:
/// смешение - свойство **выражений**).
...
/// проверяет запрет смешения - **после** рекурсии в операнды, чтобы самое вложенное
/// смешение сообщалось первым.
fn check_expr(expr: &ExpressionNode, model: &Rc<RefCell<ModelNode>>) -> Result<(), Diagnostic> {
    match expr {
        // Арифметика: сюда бьёт.
        ...
        // Прочие бинарные (сравнения, логика, битовые, сдвиги, присваивание): только
        // рекурсия - смешение ловит арифметика, а `as` - узел `Cast`.
        ...
        // Унарные и обёртки. `Cast` рекурсирует в операнд, но само приведение смешением
        // не является - оно его и снимает.
        ...
        // Длительность - не fixed-point: смешение `duration` с числом ловит отдельная
        // проверка (SE-065), а не эта.
        | ExpressionNode::Duration(_)
        | ExpressionNode::Rational(_, _)
        | ExpressionNode::String(_)
        | ExpressionNode::Type(_)
        | ExpressionNode::Address(_, _)
        // Тип обращения по адресу задан приведением автора: если это `q(m, n)`,
        // проверку ведут общие правила fixed-арифметики по типу узла.
...
/// если хотя бы один операнд - `q(m, n)`, а типы операндов не совпадают, это смешение ->
/// `SE-059`. Типы, которые вывести не удалось, пропускаются (их ошибки поднимут свои
/// проходы).
fn check_mixing(
    l: &ExpressionNode,
    r: &ExpressionNode,
    ...
    // Время: `duration` сочетается с `duration`. Проверка живёт здесь, а не в своём
    // обходе, намеренно: обход арифметики один, и второй его экземпляр разъехался бы с
    // первым - прецедент известен (два матчера адреса обязаны были давать один
    // результат).
...
/// `SE-065` - смешение `duration` с числом.
///
/// Умножение длительности на целое (`DWELL * 2`) языком **разрешено**, но сегодня
/// отвергается вместе с прочим смешением: масштабирование введёт та задача, которая
/// заведёт для него правило вывода типа. Ошибиться в сторону запрета безопасно (запрет
/// снимается аддитивно), в сторону разрешения - нет.
...
/// `SE-059` - неявное смешение `q(m, n)` с другим типом.
///
/// Модификатор `sat` входит в формат, поэтому `q(8, 8)` и `q(8, 8) sat` - разные типы,
/// и их смешение ловится **этим же** правилом. Отдельный код для него заводить не стали
/// (отступление от, замер ): правило одно - "формат обязан совпасть", - и разделение
/// его надвое дало бы два места, обязанные давать один вердикт. Имена типов в тексте
...
/// Позиция для диагностики: первая переменная/порт/константа в выражении. У
/// `ExpressionNode` собственной позиции нет, поэтому берём её у ближайшего именованного
/// узла.
