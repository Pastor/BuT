//! Запрет неявного смешения типов в арифметике: `q(m, n)` (`SE-059`) и `duration`.
//!
...
//!
//! `q(m, n)` в арифметике сочетается только с тем же `q(m, n)`. Смешение с иным `q`,
//! целым или `float` - ошибка `SE-059`, а не молчаливая потеря точности: без отказа
//! [`wider_type`] вернул бы `Unsupported`, и расхождение уехало бы в порождённый код.
//! Выход - явное приведение `... as q(m, n)` (узел `Cast`), которое смешение снимает.
//!
...
/// : правило одно - "формат обязан совпасть", - и разделение
