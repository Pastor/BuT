...
//! ## Что нормируется
//!
//! (ось 4,) постановил: **чтение порта - выражение, запись - оператор**, а
//! как это отображается, задаёт цель по единой таблице. Правило записано здесь, потому
//! что в языке порт и переменная в этом смысле неразличимы: запись - действие, а не
...
//! Первые две отсекает грамматика (`SY-006`), третью - этот модуль (**`SE-095`**), см.
//! "Роль модуля" ниже.
//!
//! ## Чем платили за отсутствие правила
//!
...
//! диагностику давали чужие инструменты - на порождённом файле и с координатами этого
//! файла, а не исходника.
//!
//! **На обычных переменных цена была хуже, чем на портах.** Вход `seen := (a := 3) +
//! 1;` цель `c` печатает валидным C (`(model->a = 3) + 1`) и считает `4`, тогда как
//! эталон-симулятор ту же модель **не исполняет вовсе** (`SIM-014`). Расхождение
//! эталона и цели - ровно то, что правило снимает.
//!
//! ## Роль модуля после исправления
//!
//! Правило переехало в **грамматику** (`SY-006`): `:=` разбирается ровно в трёх
//! позициях - оператор тела, шаг цикла `for` и **именованный аргумент вызова** (`Pid(kp
//! := 0.25)`, форма инстанцирования 0185).
//!
