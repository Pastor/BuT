...
//! правило оказалось бы в двух местах и разошлось бы.
//!
...
//!
//! Второму судье нужна позиция выражения ([`Position`]): присваивание на верхнем уровне
//! оператора законно, внутри вычисляемого выражения - нет. Обход знает позицию по
//! построению, судья её только принимает. Большую часть форм отсекает грамматика
//! (`SY-006`), и судья остаётся страховкой на достижимом остатке - именованном
//! аргументе вызова.
//!
//! Модуль назван по доставке, а не по направлению порта: судей здесь двое.
//!
//! Накопление идёт по выражениям, одна ошибка на выражение: редактор подчёркивает все
//! нарушения, а не первое.
