...
//!
//! Путь `semantic::ConditionNode` сохранён реэкспортом из `semantic/mod.rs`, поэтому
//! потребители адресуют узел по-прежнему.
...
impl ConditionNode {
    ...
    ///
    /// Единственное место, где принимается это решение. Реши его пять потребителей
    /// порознь - цели `c`, `st`, `rust`, `sv` и эталон, - и они разойдутся: считающий
    /// безусловным ещё и [`ConditionNode::Unresolved`] срабатывает на условном ребре
    /// всегда. Вывод при этом валиден, автомат другой, и расхождение молчаливо.
    ///
    /// **`Unresolved` безусловным не считается.** Неразрешённое условие значит "имя не
    /// найдено", а не "условия нет"; такой вход отсекает `SE-025` до генерации.
    /// Недостижимость эту держит чужая проверка: ослабнет она - и потребитель,
    /// считающий `Unresolved` безусловным, начнёт молча менять автомат. Отданный
    /// печатнику узел даёт громкий отказ (`RS-011`, `SV-002`, `CC-023`, `ST-011`) вместо
    /// тихой подмены.
    #[must_use]
