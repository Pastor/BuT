...
//!
//! Правило живёт здесь, в одном месте: цели `c`, `rust`, `st` и эталон зовут его, чтобы
//! не разъехаться. Цель `sv` бит-вектор упаковывает сама (нативный `logic [N-1:0]`
//! любой ширины - слова ей не нужны), но [`is_bit_vector`] использует и она.
//!
