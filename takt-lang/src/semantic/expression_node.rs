...
//!
//! Вынесен из `semantic/mod.rs` **чистым перемещением** по той же причине и тем же
//! приёмом, что [`ConditionNode`](crate::semantic::ConditionNode): файл пришпилен
//! реестром размеров и расти не имеет права, а узел - самостоятельное знание. Путь
//! `semantic::ExpressionNode` сохранён реэкспортом, поэтому потребители (генераторы,
//! симулятор, LSP) не тронуты.
...
pub enum ExpressionNode {
    ...
    ///
    /// База - **выражение**, а не переменная: `b.data[1]` прежде не разбирался вовсе
    /// (`SY-002`), тогда как обратная цепочка `ps[1].x` работала - постисправлениеные операции
    /// были асимметричны.
