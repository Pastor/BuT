...
//!
//! **Цена ошибки здесь - не "нет диагностики".** Проглоти проход ошибку вложенного
//! тела, оператор останется [`StatementNode::Unresolved`], а цель `c` напечатает его
//! пустотой (`generator/c/c_expr/stmt.rs`), эталон же его пропустит
//! (`takt-sim/src/unit/statement.rs`): `always { if x > 0 { x := неизвестное_имя; } }`
//! соберётся в `if (model->x > 0) { }` - программа принята, а написанный оператор исчез.
//! Тем же механизмом теряется и предел глубины `SE-062`.
//!
//! Законные источники [`StatementNode::Unresolved`] - ветка `_ =>` в
//! [`resolve_ast_statement`] (`Args`, `Error`, `StraySemicolon`): она отдаёт узел через
//! `Ok`, а не `Err`. У `Assembly` и `Formula` свои узлы: законный вход, приходящий целям
//! неразрешённым, они отвергают как дефект.
...
fn resolve_ast_statement(
    ...
    match stmt {
        ...
        ast::Statement::For(loc, init, cond, step, body) => {
            ...
            // Разрешение условия, шага и тела отделено от снятия регистрации: любая из
            // трёх частей может завершиться ошибкой, а переменная цикла обязана быть
            // снята с регистрации в любом исходе. Иначе неудачная сборка оставит `i` в
        ...
        ast::Statement::InlineFormula(inline) => {
            match &**inline {
                ast::InlineFormulaDefine::Guard {
                    ...
                    // спрашивает только модель, и без регистрации `: [Guard] v < 200;` в
                    // теле `fn bump(v: u8)` даёт `SE-025` "неразрешённое условие". Приём
                    // тот же, каким видны локальные переменные блока.
                    ...
                    let resolved: Vec<Formula> = resolved?
                        .iter()
                        // Охранная формула тела несёт позицию своего объявления.
                ...
                ast::InlineFormulaDefine::Ltl { formulas, loc } => {
                    // LTL в блоке разбирается той же функцией, что на уровнях модели и
                    // состояния (`tree.rs`): уровни равноправны.
