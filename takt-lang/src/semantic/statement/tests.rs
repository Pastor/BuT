//! Тесты разрешения семантических операторов.
//!
...
/// ```
///
/// Голым `C;` вторым оператором записать нельзя: выражение в позиции оператора обязано
/// иметь эффект. Предмет теста - разрешение локальной `const`, и её использование
/// записано законной формой.
#[test]
