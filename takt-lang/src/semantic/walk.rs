...
///
/// Порядок значим: вложенный вызов `outer(inner(x))` обязан быть подставлен раньше
/// внешнего, иначе объявления встанут в `prelude` в обратном порядке.
pub(crate) fn walk_expr_mut(expr: &mut ExpressionNode, f: &mut dyn FnMut(&mut ExpressionNode)) {
    match expr {
        ...
        // Именованные аргументы (`NamedFunctionBox`) - узлы **сырого** АСД: на уровне
        // АСД вызов функции и инстанцирование модели неразличимы, и разрешённых
        // подвыражений там нет. Обходится только база.
...
pub(crate) fn walk_stmt_exprs_mut(
    ...
    match stmt {
        ...
        // Вставка для цели - обычные операторы Takt: обход обязан спускаться в неё,
        // иначе выражения тела не увидит ни один проход.
        StatementNode::Assembly { body, .. } => walk_stmt_exprs_mut(body, f),
        // Операторы без выражений. Блок формул адресован внешнему анализатору:
        // выражений Takt в нём нет вовсе.
        StatementNode::None
