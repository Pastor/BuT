...
pub(super) fn construct_model_stage0(
    ...
    for element in model.elements.iter() {
        if let ModelElement::Model(model) = element {
            ...
            // `invariant P = C;` в модели есть сахар над `cond P = C;` и `: [Guard] P;`.
            // Имя `P` регистрируется как условие - оно годится атомом LTL `G(P)` и
            // условием ребра `ref: P`, - а обязательство становится охранной формулой с
            // именем инварианта и проверяется каждый такт до диспетчеризации состояния.
            // АСД не переписывается: форматтер печатает `invariant`.
            let inv_loc = inv
                .name
                .as_ref()
                .map(|id| id.loc)
            ...
            let name = def
                ...
                .name
                .clone();
            // Дубликат имени функции - ошибка `SE-009`: без неё `HashMap::insert` молча
            // перетирает объявление, и побеждает последнее. Проверка живёт здесь, а не в
            // `construct_function`: карта на время разрешения тел непуста, и проверка в
            // точке разрешения ловила бы каждую функцию как "уже определённую".
            ...
            // `claim_type_name` дал бы ложную `SE-108`. Сам узел перечисления строит
            // `enum_node::build_enum`.
...
pub fn construct_model_with_files(
    ...
    // Контракт этого входа - одна диагностика. Стадии построения накапливают внутри
    // себя, но потребителям вроде `takt-sim` нужна первая ошибка, а не список. Кому
        ...
        if let ModelElement::State(def) = element {
            ...
            for element in def.elements.iter() {
                if let StateElement::Reference(_, id, cond) = element {
                    ...
                    // Инвариант состояния есть охранная формула с именем инварианта:
                    // условие проверяется каждый такт, пока автомат в этом состоянии.
