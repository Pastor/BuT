...
///
/// Автовыведённое равенство (редакция ) включало `loc`, и это был дефект: цель `sv`
...
/// модели нет" и идёт переносить состояния на верхний уровень (замер 0211). На деле
...
fn unroll_ast_extend(
    ...
    match expr {
        ...
        // Прочие формы реализацией быть не могут. Диагностика **с кодом и позицией**:
...
/// модель не может быть одновременно композицией и автоматом. Прежде такая
