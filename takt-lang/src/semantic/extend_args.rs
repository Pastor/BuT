...
///
/// Проход зовётся **только** когда специализации не было. При
...
/// понизь мы его здесь тоже, литерал прошёл бы масштабирование
/// **дважды** - замер 0489 дал `SE-058` на всех восьми целях.
///
/// Аргумент - такая же позиция приёмника, как присваивание и вызов (0381, 0382): тип
/// параметра известен, и за границей семантики дробного литерала быть не должно - иначе
/// цели расходятся (`sv` отвечал `SV-002`, прочие печатали 2 вместо 640).
