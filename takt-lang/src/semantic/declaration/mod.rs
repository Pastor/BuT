...
//!
//! Вынесен из `semantic/tree.rs`: файл давно сверх лимита размера
//! (`scripts/check-module-size.sh`), а "построить узел объявления" - самостоятельная
//! ответственность, отделимая от обхода элементов модели.
...
///
/// Вынесено из `tree.rs`: у порта выражений **два** - размещение и начальное значение, -
/// и разбор каждого на месте раздул бы файл, стоящий в реестре размера. Логика прежняя:
/// узел без "сырых" выражений возвращается как есть.
...
pub(crate) fn fold_variable_initializers(
    ...
    for name in order {
        ...
        // доходит - он отсечён строкой выше, и его выход за границы по-прежнему
        // `SE-089`. Ширина выведенного типа берётся у результата, а не у операндов. Тип
        // выводится до свёртки, поэтому `const K := 1 + 255;` получал ширину левого
        // литерала (8 бит), а нормирование 0207 заворачивало вычисленные 256 в ноль -
        // молча и одинаково у всех девяти потребителей. Автор ширины не выбирал: её
        // выбрал операнд.
        //
        ...
        // выражение в теле.
        //
        // Расширяем только когда значение не помещается в выведенный тип, а не при
        // каждой свёртке. Первая редакция переопределяла тип всегда - и сломала вывод
        // из сигнатуры функции: `fn get32() -> [bit;32]` с `var val := get32();` давал
        // `[bit;8]`, потому что вычислитель сворачивает вызов в `0`. Там ширину выбрал
        // не операнд, а объявленный возвращаемый тип, и трогать её нельзя. Поймал тест
        // Ce6, а не чтение.
        //
        ...
        // не "расшириться" до знакового. Второй перебор, пойманный тестом нормирования,
        // а не чтением.
        ...
        // q-представление. Понижение литералов (0096/0061) идёт при выводе типов, то
        // есть до свёртки, и `1.0 + 2.0` ему не литерал - оно проходит мимо. Без этого
        // шага цель `c` печатала `model->s = 3.0;` в поле `int8_t`, то есть **3**, что
        // в q(4, 4) значит 0.1875, тогда как эталон давал 3.0. Понижение делает тот же
        // носитель, что и для написанного автором литерала: своя копия разошлась бы с
        // ним в округлении.
...
///
/// Меняются **оба** представления? Нет: здесь правится только запись в карте
/// объявлений. Ячейка тела (`Rc<RefCell<VariableNode>>`, засада 0096) к этому моменту
/// ещё не построена - тела разрешаются позже, - поэтому второго представления не
/// существует и синхронизировать нечего.
...
fn lower_folded_fixed(
    ...
    // Агрегат понижается поэлементно по типу элемента: прежде правило смотрело только
    // на скалярный тип, и `var gains: [q(8, 8); 2] := {1.5, 2.5};` доезжало до целей
    // дробным литералом - `cc -Werror` отвечал "implicit conversion from 'double' to
    // 'int16_t' changes value from 1.5 to 1", `rustc` - `E0308`, `sv` - `SV-002`, при
    // том что та же запись скаляром работает у всех девяти потребителей.
...
///   верное, а тип - нет.
///
/// Вынесено из `tree.rs`: тот файл стоит в реестре размера
