...
//!
//! Модуль выделен из `declaration.rs` по **границе ответственности**, а не ради строк:
//! свёртка решает, чем стало значение, а эти судьи - законна ли сама запись. Тексты
//! диагностик живут рядом с признаками, которые их порождают.
...
/// неподвижной точки и даёт согласованный результат у эталона и целей - проверено
/// пробой. Запрет сломал бы работающие входы.
///
/// **Прочие невычислимые формы (порт, вызов функции, обращение к полю) остаются
/// законными:** фича не ужесточает язык, а исполняет уже принятое правило. Отвергать
/// всё невычислимое значило бы ломать входы, которых нет в корпусе, - тот же довод, по
/// которому 0192 выбрала расширение, а не запрет.
