...
//! ([`slice::argument`](crate::semantic::slice::argument)) и подстановка тела функции
//! ([`inline`](crate::semantic::inline)).
//!
...
//!
//! Имя временной обязано быть допустимым идентификатором целевых языков (C, IEC, Rust,
//! SystemVerilog): на имя вида `#...` тот же `cc` отвечает "expected identifier". А раз
//! такое имя может написать и автор, занятость проверяется, а не предполагается.
