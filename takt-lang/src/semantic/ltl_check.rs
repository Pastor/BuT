...
//! кода - паритет обеспечен ). **Компиляция её не проверяет**: ни одна цель генерации
//! LTL не эмитит. Чтобы ни один путь LTL не заканчивался тишиной (принцип "громкий
//! отказ", /0024), здесь собираются предупреждения:
//!
//! - **SE-055** - на каждую LTL-формулу: разобрана и сохранена, но при
//!   компиляции не проверяется. дала формулам **потребителя** -
//!   `taktc verify` (управляющие свойства, атом = имя состояния), поэтому текст
//!   предупреждения указывает на него. Само предупреждение осталось: `taktc
...
//! [`unused_variable_warnings`](crate::unused_variable_warnings); `taktc` их пока не
//! печатает (известный дефект в бэклоге), но они доступны через API и тесты.
