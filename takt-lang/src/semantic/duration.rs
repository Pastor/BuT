...
///
/// Сборка обязана печатать, во что превратилась каждая длительность, - иначе автор не
/// видит, столько ли это тактов, сколько он думал. Возвращает предупреждения (канал
/// предупреждений компилятора, глушится `--quiet`), по одному на зависящую от профиля
/// выдержку `after`; такты `after Nt` пересчёта не требуют и здесь не перечисляются.
///
