...
pub enum Type {
    ...
    ///
    /// Последнее поле - **необязательный постисправлениеный модификатор**:
...
pub enum VariableDefine {
    ...
    Port {
        ...
        /// Инициализатор порта. Переходно (0187, ->07->02) здесь по-прежнему **адрес**
        /// (старая форма `:= 0x100`).
        initializer: Option<Expression>,
        /// Направление порта
...
pub struct StringLiteral {
    /// Местоположение в исходном тексте.
    pub loc: Location,
    /// `true` - строка объявлена с преисправлением `unicode`.
    pub unicode: bool,
