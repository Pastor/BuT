...
//!
//! `Token` - выход лексера и вход парсера (lalrpop `extern { enum Token }`); путь
//! `parser::lexer::Token` сохранён реэкспортом в `lexer.rs`.
...
pub enum Token<'input> {
    ...
    ///
    /// Носитель - `i128`, а не `i64`: язык объявляет тип `u64` и упакованный скаляр
    /// `[bit;64]`, верхняя половина диапазона которых в знаковый 64-битный носитель не
    /// влезает, - маску "все единицы" им было бы не записать.
    ...
    ///
    /// Лексема сохранена намеренно, терминал - нет. Пока `==` распознаётся лексером,
    /// диагностика называет его целиком: "нераспознанный токен `==`". Изъятие лексемы
    /// дало бы разбор на `=` и `=` - сообщение о следствии вместо причины, а объявление
    /// терминала добавило бы вторую, каскадную ошибку.
    Equal,
