...
// Сканирование литералов времени - дочерний модуль: работает с приватными полями
// `Lexer`, вынесен из-за лимита размера модуля.
#[path = "lexer_time.rs"]
mod lexer_time;
...
///
/// Реэкспорт держит публичный путь `parser::lexer::LexicalError` неизменным: тип
/// переехал в свой модуль по правилу размера, а не сменил адрес для потребителей.
...
///
/// Заведено ради теста редакторского слоя: список автодополнения LSP обязан покрывать
/// таблицу лексера, и проверять это надо **настоящими данными**, а не грепом исходника
/// (; прежде такой греп нёс на себе тест плагина IntelliJ - второй парсер `phf_map!` в
/// проекте).
