...
//!
//! Модуль дочерний по отношению к [`lexer`](super) по необходимости: методы работают с
//! приватными полями `Lexer` (`input`, `chars`, `file_no`), а приватность в Rust
//! распространяется на потомков.
