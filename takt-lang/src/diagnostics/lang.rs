//! Язык сообщений инструментов: носитель, каталог и подстановка (фича 0532).
//!
//! # Зачем
//!
//! Тексты диагностик и CLI жили строковыми литералами в местах, где рождаются.
//! Пока язык был один, это ничего не стоило; как только языков становится два,
//! литерал означает, что сообщение **нельзя** перевести, не тронув проверку.
//! Здесь текст отделён от места: место называет **ключ**, а строку по ключу
//! отдаёт каталог активного языка.
//!
//! # Устройство
//!
//! - Каталоги — `takt-lang/messages/<код>.txt`, по строке на ключ; в код их
//!   вносит `build.rs` (там же строятся [`keys`] — константы ключей).
//! - **Список языков открыт** (решение заказчика 2026-09-06): третий язык — это
...
//!
//! ⚠️ **Системная локаль не читается** (решение заказчика 2026-09-06):
//! `LANG`/`LC_MESSAGES` на выбор не влияют. Вывод инструмента не должен
//! зависеть от машины — на этом стоят потактовые сверки и гейты корпуса, а
//! пересъём эталонов из-за чужого окружения был бы молчаливым.
//!
//! ⚠️ Носитель **потоковый**: язык, выбранный в одном потоке, в другой не
//! переезжает. Для CLI и моста этого достаточно (вход выбирает язык один раз),
//! а многопоточному потребителю язык передаётся параметром входа.
...
///
/// Обёртка над строкой существует ради одного: ключ, написанный литералом,
/// ошибался бы **молча** — сообщение просто не нашлось бы, а вместо текста
/// поехал бы запасной. Константа делает опечатку отказом компиляции.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Key(pub &'static str);

impl Key {
    /// Строковое имя ключа — для гейтов и сообщений об ошибке.
...
///
/// Перечисление обязательно: список открыт, и автор, промахнувшийся мимо `en`,
/// иначе не узнает, чем дерево располагает.
pub fn parse(code: &str) -> Result<Lang, String> {
    ...
    // Текст отказа берётся из каталога БАЗОВОГО языка: активный язык в этот
    // момент ещё не выбран — его как раз и не удалось разобрать.
    Err(render_in(
        Lang::base(),
        keys::LANG_UNKNOWN,
...
///
/// Живёт в библиотеке, а не в бинарнике, по той же причине, что и разбор
/// подкоманд (прецедент 0043): ключ общий для `taktc` и `takt-sim`, а вторая
/// копия разбора разошлась бы с первой молча.
///
/// ⚠️ Ключ изымается **до** разбора подкоманды и потому действует на все из
/// них: язык — свойство прогона, а не подкоманды. Отказ возвращается словами и
/// перечисляет известные языки; печатает его вызывающий (библиотека, пишущая в
/// `stderr`, лишает вызывающего выбора — фича 0168).
    ...
    while let Some(arg) = iter.next() {
        if let Some(code) = arg.strip_prefix("--lang=") {
            ...
            let Some(code) = iter.next() else {
                // Значения нет — значит `--lang` шёл последним, и в `iter`
                // ничего не осталось: аргументы вызывающего целы, отдаём их
                // обратно и только потом отказываем.
                *args = rest;
...
///
/// ⚠️ Негодное значение `TAKT_LANG` **не отказ**: переменная окружения не место
/// разговора с автором, и падать из-за неё посреди сборки хуже, чем ответить
/// по-русски. Отказ с перечислением даёт ключ `--lang`, где промах виден автору.
pub fn current() -> Lang {
...
///
/// Значения приходят через [`Display`], а не [`std::fmt::Debug`]: дамп узла в
/// тексте диагностики — известный класс (фича 0231), и здесь он невозможен по
/// построению.
...
pub fn render_in(lang: Lang, key: Key, params: &[(&str, &dyn Display)]) -> String {
    let template = lookup(lang, key)
        // Запасной путь: ключ есть в базе, но не в каталоге языка. При зелёном
        // гейте паритета случай недостижим — потому здесь и стоит база, а не
        // паника: сломанный перевод не повод не сообщить о дефекте модели.
...
///
/// Оставить неизвестное имя — намеренно: гейт паритета сверяет наборы имён и
/// ловит расхождение списком, а сообщение с видимым `{name}` читается как
/// дефект каталога, тогда как пустота выглядела бы дефектом модели.
...
mod tests {
    ...
    ///
    /// ⚠️ Половина про умолчание проверяется, только если `TAKT_LANG` не задана:
    /// переменная процесса общая для всех потоков, и тест, требующий её
    /// отсутствия, был бы флаки на машине, где она выставлена.
    #[test]
    ...
    ///
    /// Сторож текстовый: чтение `LANG`/`LC_MESSAGES` сделало бы вывод
    /// инструмента зависимым от машины, а на нём стоят потактовые сверки и
    /// гейты корпуса. Проверяется исходник самого носителя — иначе правило
    /// живёт только в док-строке.
    #[test]
