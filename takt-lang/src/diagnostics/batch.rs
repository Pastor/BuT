//! Пачка диагностик: порядок, уникальность и печать.
//!
//! Пока инструмент сообщал **одну** ошибку за прогон, ни порядок, ни повторы значения
//! не имели. Как только сообщений становится несколько, оба вопроса становятся
//! содержательными:
//!
//! - **порядок** - обход модели идёт по `BTreeMap`, то есть по
...
//!
//! Часть модуля `diagnostics` (деление по логике: `mod.rs` упирается в лимит размера).
...
///
/// Порядок - `(файл, смещение начала)`; сортировка **устойчивая**, поэтому у диагностик
/// с одинаковой позицией сохраняется порядок обхода (а он детерминирован - ).
/// Диагностики без позиции (`Codegen`, `Implicit`, `Builtin`, `CommandLine`) идут **в
/// конец**: они не про место в тексте, и ставить их между привязанными значило бы
/// сбивать чтение сверху вниз.
///
/// Повтором считается полное совпадение тройки "позиция + код + сообщение". Совпадение
/// только текста повтором **не** считается: одна и та же ошибка в двух местах - это две
/// ошибки.
...
///
/// Формат живёт в **библиотеке**, а не в бинарнике, по той же причине, по которой там
/// же живёт [`position_prefix`]: вид диагностики - её собственное свойство. Копия
/// формата в `taktc` уже расходилась однажды (: цели `c`/`st` печатали сообщение без
/// кода, `c-hal`/`st-at` - с кодом, а заметки не печатал никто).
///
...
///
/// **Носитель один на всех потребителей**. Это тот самый класс, о котором предупреждает
/// док-строка выше ("копия формата в `taktc` уже расходилась однажды"), - вторая копия
/// жила в другом крейте.
///
/// заметка печатается **со своей** позицией; у заметки, указывающей в другой файл,
/// координата опускается (чужую строку под своим путём показывать нельзя), и строка
/// остаётся прежней.
...
///
/// Спутник [`format_compile_error`] и живёт по той же причине: вид диагностики - её
/// собственное свойство, а не свойство бинарника. До этот формат был написан прямо в
/// `compile_cli` литералом, и второй потребитель (`taktc fmt`) завёл бы копию - ровно
/// так уже расходился формат ошибки.
