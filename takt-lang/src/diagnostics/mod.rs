...
impl Diagnostic {
    ...
    /// Ставит позицию, если своей у диагностики нет (фича 0467).
    ///
    /// Часть отказов рождается там, где координаты неоткуда взять: реестр
    /// встроенных функций спрашивают по строке (`SE-004`), запись карты
    /// адресов — по имени порта. Такие диагностики несут `Location::Builtin`
    /// и обязаны получить координату у ВЫЗЫВАЮЩЕГО — он знает узел.
    ///
    /// ⚠️ Своя позиция не перетирается: чужая верная координата хуже
    /// отсутствующей (урок 0264).
    #[must_use]
