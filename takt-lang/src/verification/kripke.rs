...
//!
//! Следствия абстракции (исправлениеируются здесь, потому что от них зависит смысл вердикта):
//!
...
/// в `ModelNode::states`), поэтому вердикт воспроизводим побайтно (проверка 0048).
...
pub fn build_kripke(model: &ModelNode) -> Option<Kripke> {
    ...
    for (k, name) in states.iter().enumerate() {
        ...
        let mut targets: BTreeSet<usize> = crate::semantic::validate::reachable_targets(state)
            ...
            .collect();
        // R2/A2: самопетля там, где автомат может задержаться ещё на такт - тупик или
        // состояние, чьи выходы все под guard'ом.
...
mod tests {
    ...
    /// Детерминизм (проверка 0048): два построения дают идентичную структуру.
    #[test]
