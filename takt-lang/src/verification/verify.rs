...
///
/// Лассо в **именах состояний FSM**: конечный преисправление, затем цикл, повторяемый
/// бесконечно.
...
///    видно и не нужно.
/// 2. Хвост преисправления, совпадающий с циклом-самопетлёй, отбрасывается: при
...
fn counterexample(prefix: Vec<String>, cycle: Vec<String>) -> Counterexample {
    ...
    // Последняя вершина преисправления - вход в цикл; в тексте она уже печатается как первая
...
mod tests {
    ...
    fn response_property_violated_when_fault_can_loop_forever() {
        ...
        // Цикл - вечное залипание в Fault. Длина цикла в тактах не исправлениеируется: она
