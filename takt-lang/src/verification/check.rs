...
//!
//! Найденный путь возвращается как **лассо** ([`Lasso`]): конечный преисправление до
...
/// Контрпример: бесконечный прогон вида "преисправление, затем цикл навсегда".
///
...
pub struct Lasso {
    ...
    /// преисправления - первое состояние цикла).
...
impl Lasso {
    /// Имена состояний FSM на преисправлении и на цикле (проекция на разметку Крипке).
    ///
...
/// постпорядке.
///
/// `path` - текущий путь от начального состояния; он же становится преисправлением лассо.
