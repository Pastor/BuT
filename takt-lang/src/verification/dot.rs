...
/// `may_stutter`) видна ребром-петлёй. У пути данных (`labels` непусто) метка вершины
/// несёт набор истинных в ней атомов.
...
/// а бинарник оставался тонким.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GraphKind {
    /// Структура Крипке модели: без `--property` это абстракция управляющего графа.
    Kripke,
