...
//!
//! Это **сохраняет главную гарантию 0049** (): реальный прогон присутствует в
//! абстракции с точными метками (полный недетерминизм включает ход реальных данных),
//! поэтому `Holds` надёжен. Прецизионное вычисление хода данных (доказывающее `G (temp
//! <= 100)`, когда логика ограничивает `temp`) - Option D в полном объёме - сознательно
//! **не** реализовано (тот же класс риска, что стоил проекту года: неверный ход данных
//! сломал бы `Holds`).
//!
//! # Потолок
//!
//! Размер считается **до** построения; превышение [`EDGE_LIMIT`] -> `Unsupported`, а не
//! долгий счёт. `float`/`q` и прочие неперечислимые типы -> `Unsupported` ().
//!
...
//! ```
//!
//! где `D` - произведение доменов отслеживаемых переменных. Квадрат берётся из:
//! управляющее ребро скрещивается с **любой** оценкой, то есть ход данных
//! недетерминирован. Работа (произведение с автоматом Бюхи и nested DFS) идёт по
//! рёбрам, поэтому потолок по вершинам стоимость не предсказывал: замер 0145 дал два
//! входа по 12 288 вершин, различающиеся по времени в 2000 раз, - прежний потолок
//! `1_000_000` **вершин** пропускал входы, на которых инструмент не завершался за
//! минуты.
...
///
/// Замер 0145  даёт 0.35-2.4 мкс на ребро - разброс от
/// размера автомата Бюхи, - то есть этот порог держит проверку в пределах
/// 0.2-2 с. Калибровка: `comprehensive`/`Controller` + `u8` -> 8 x 256² =
/// 524 288; φ над **двумя** `u8` -> >= 8.6 x 10⁹
...
mod tests {
    ...
    /// A1: предикат над `u8` даёт **вершины** (не Unsupported); размер - |состояний| x
    /// 256.
    #[test]
    ...
    /// A4 (главная): гарантия "держится" сохранена. Тавтология над доменом `u8` (`temp
    /// <= 255` истинно при **всех** 256 значениях) -> `Holds`, несмотря на полный
    /// недетерминизм данных. Тест **на вердикт**, а не на форму.
    #[test]
    ...
    /// A4: булева переменная, которую никто не меняет? - нет, данные роумят. Но `G
    /// (Safe | !Safe)` - тавтология - держится (сохранность вердикта).
    #[test]
    ...
    /// A4: свойство, ложное при некоторой достижимой оценке, -> `Violated` (возможно,
    /// ложное - цена сверх-аппроксимации). Здесь `temp` роумит по всему домену, значит
    /// `temp >= 200` достижимо.
    #[test]
    ...
    /// A5 (направление ошибки): переход/значение из абстрагированного источника даёт
    ...
    /// A2/потолок: φ над тремя `u8` (рёбер `1 x (256³)² ≈ 2.8 x 10¹⁴`) -> `Unsupported`
    ...
    /// A2: вход, проходивший **вершинный** потолок и вешавший инструмент, отвергается
    /// рёберным.
    ///
    /// Двенадцать `bit`-переменных при 3 состояниях: вершин `3 x 4096 = 12 288` - в 81
    /// раз ниже прежнего `VERTEX_LIMIT = 1_000_000`, то есть прежний потолок вход
    /// **пропускал**, а рёбер `3 x 4096² ≈ 5 x 10⁷`, и замер 0145 показал: прогон не
    /// заканчивается за 60 секунд.
    ///
    /// Тест обязан быть именно таким входом. На трёх `u8` (соседний тест) отвечают
    /// отказом **обе** метрики, и тест, взявший их, доказывал бы прежнее поведение.
    ///
    ...
    /// A1: потолок считает `max(E_упр, V_упр) x D²`.
    ///
    ...
    /// A3: корпусной вход остаётся проверяемым - 8 x 256² = 524 288 рёбер против порога
    /// 10⁶.
    #[test]
    ...
    ///
    /// Причина - **`PredicateOutsideSubset`, а не `DomainNotEnumerable`** (замер,
    /// 2026-08-19): неперечислимый тип отсекается раньше, на шаге 2, -
    /// `collect_tracked` не пускает такой предикат в отслеживаемые. Тем же путём уходят
    /// `q`, массив и структура; ветвь `domain_size == None` (шаг 3) сегодня
    /// **недостижима** и остаётся защитной.
    #[test]
    ...
    /// A6: `bit`-переменная-атом -> домен 2; |состояний| x 2 вершин.
    #[test]
    ...
    /// A1 на корпусном примере: `cond Overheated = temperature >= MAX_TEMP` (u8 +
    /// const) над моделью `Controller` даёт **вершины** = состояний x 256, а не
    /// `Unsupported`. Реально у `Controller` **4** состояния -> 1024 вершины.
    #[test]
