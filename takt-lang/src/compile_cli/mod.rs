...
//! (`bin/taktc.rs` пришпилен к baseline и расти не может): бинарник держит лишь тонкий
//! диспетчер `args[1] == "compile" -> run_compile`. Прецедент - подкоманды
//! `address-map` (`address_map::export_cli`) и `verify` (`verification::verify_cli`),.
//!
//! Разбор аргументов - тот же ручной `while i < args.len()` + `match`, что и у соседних
...
/// неверный формат - **ошибка CLI**, а не молчаливое умолчание: угадывать точность
/// нельзя (ровно довод, по которому 0045/0061 отвергли автоугадывание формата).
...
/// поэтому такая диагностика останется без преисправления - как прежде, но не соврёт.
///
/// Координата в `loc` при этом была - терялась она на печати.
fn print_warnings(
...
/// импортированного файла остаётся без преисправления - названная граница: цель строит своё
...
pub fn run_compile(args: &[String]) -> i32 {
    ...
    // Применимость флага к цели - один носитель: прежде на один вопрос отвечали трое и
    // по-разному, вплоть до молчаливого приёма `--bus=apb` целью `rust` (рапорт об
    ...
    // **всех** целей (до 0081 CLI не звал ни `SE-036`, ни `SE-037`). Адрес-специфичные
    // (наложение карты, сломанное выражение адреса) - только у целей, адрес не
    // потребляющих: у `c-hal`/`st-at` это ошибки при генерации. Пересчёт длительностей
    // печатается тем же каналом.
    ...
    if let Some((ast, model)) = crate::parse(&source, 0).ok().and_then(|(ast, _)| {
        ...
        // Пересчёт длительностей во что превратилась каждая (R10/A10): best-effort по
