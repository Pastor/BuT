...
//!
//! Бинарник `takt_lsp.rs` до 0072 игнорировал параметры инициализации клиента и
//! передавал в ядро пустой список путей поиска (`&[]`), поэтому импорт из общей
//! библиотеки вне каталога документа в редакторе не находился, хотя `taktc -I lib` его
//! собирает. Здесь - чистая, тестируемая логика извлечения путей из штатного
//! `initializationOptions`; бинарник остаётся тонкой обвязкой.
...
/// перекрывает локальный - поведение ядра 0055). Битые записи (не массив, элемент не
/// строка) молча пропускаются: плохой конфиг не должен ронять сервер.
