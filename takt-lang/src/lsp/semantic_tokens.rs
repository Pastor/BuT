...
//! [`Token`](crate::parser::lexer::Token) здесь нет, и `deny` ниже не даёт её вернуть.
//! Новый токен языка валит сборку - автор обязан назвать его категорию подсветки, а не
//! промолчать. Ветка `_ => continue` оставила бы непокрытый токен без подсветки при
//! зелёной сборке. Токен, который не подсвечивается намеренно (пунктуация),
//! перечисляется явной веткой: "не подсвечиваем" тоже решение.
//!
...
pub fn semantic_tokens(source: &str) -> SemanticTokens {
    ...
    for (start, token, end) in token_results {
        let tt = match token {
                ...
                match type_refs {
                    Some(ref refs) => {
                        if refs.is_type_position(start as u32, end as u32) {
                            ...
                            // Остальные категории разбираются по имени.
            ...
            | Token::MoreEqual
            | Token::PeirceArrow
            // Стрелки и тернарный знак: подсвечиваются наравне с `-->` выше. `->` (тип
            // возврата), `=>` (ветка `match`) и `?` - такие же знаки языка, и
            // разнобой в их окраске был бы случайностью, а не решением.
            | Token::Arrow
            | Token::FatArrow
            | Token::Question
