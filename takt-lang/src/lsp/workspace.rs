...
//! переменные, типы, структуры, перечисления, функции (усыновление, и исправления /04). Такие
//! имена автор пишет голыми, и слой использований помечает их `unresolved`: "имя есть,
//! связать не с чем". Этой пометки и графа импортов достаточно, чтобы найти ссылку.
//!
...
//! `Engine`. Совпадение имени файла с именем модели (исправлениетура `goto56/helper.takt`) -
