...
"#;
    ...
    ///
    /// Проверяются именно ошибки, а не всякая диагностика: исправлениетура законно получает
    /// предупреждения (например, о каноне именования `CS-001`), и прежняя проверка
    /// "диагностик нет вовсе" приравнивала совет к отказу. Прежде исправлениетура объявляла
    /// `type u8 = [bit;8]` - затенение встроенного имени; с это `SE-107`, и объявление
    /// из исправлениетуры убрано.
    #[test]
    ...
    /// `invariant` подсвечивается наравне со структурно равным ему `cond` (критерий A3
    /// ).
    ///
    ...
    /// Операторы LTL и виды формул подсвечиваются как ключевые слова (критерий A4 ).
    ///
    ...
    /// Стрелки `->` и `=>` подсвечиваются как операторы (критерий A5, A-2 ).
    ///
