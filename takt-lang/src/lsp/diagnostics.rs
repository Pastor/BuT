...
pub fn collect_diagnostics_at(
    ...
    // пользуется `taktc compile`.
    //
    // Прежде здесь стоял свой список из двух проверок (`SE-036` и `SE-037` о
    // недетерминизме), а в единой точке их **восемь**: редактор молча не показывал
    // недостижимое состояние, всегда-истинное условие, лишнюю `;`, неизвестное имя
    // блока, предупреждения LTL и записи по адресу. Два списка одного набора расходятся -
    // /0193/0195; здесь он проявлялся как "в консоли предупреждение есть, в редакторе
    // нет".
