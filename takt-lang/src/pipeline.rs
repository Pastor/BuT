...
//!
//! Вынесено из `lib.rs`: файл пришпилен реестром размеров, а логика здесь
//! самостоятельная - "как получить модель (или все ошибки) из текста".
...
pub(crate) fn parse_and_construct(
    ...
    // базы, а печатникам целей о его форме знать не нужно (приём 0400). Место -
    // конвейер, а не стадии: флаг принадлежит вызову цели, и эталон зовёт проход своим
    // (`takt-sim --bounds-check`).
    //
    // Guard идёт после подстановки: защита обязана охранять то, что напечатается, а
    // решение о подстановке не должно зависеть от того, включена ли защита. Обратный
    // порядок стоил двух расхождений: эвристика 0444 мерила тело, уже раздутое
    // обёртками, а ветвь отказа приезжала в место вызова блоком из двух операторов - и
    // цель `rust` печатала отложенное объявление, которое `clippy` отвергает
    // (`needless_late_init`).
