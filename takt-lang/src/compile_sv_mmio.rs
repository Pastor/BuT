...
//!
//! Вынесена из `lib.rs` (тот пришпилен к лимиту размера, реестр долга): в корне крейта
//! остаётся тонкий реэкспорт [`compile_to_sv_mmio`]. Функция соединяет два существующих
//! образца - разрешение адресов `c-hal` (`resolve_addresses` + приоритет источников) и
//! понижение `float -> q` цели `sv`.
