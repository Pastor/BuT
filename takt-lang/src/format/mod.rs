...
//! на семантически некорректных, но синтаксически валидных файлах.
//!
...
pub enum FormatError {
    ...
    ///
    /// Несёт [`Diagnostic`](crate::diagnostics::Diagnostic) - позицию, код и текст:
    /// без позиции узел пришлось бы искать грепом по большому файлу.
    Unsupported(crate::diagnostics::Diagnostic),
}

impl std::fmt::Display for FormatError {
    /// Ветвь `Parse` печатает диагностики **без префикса позиции**: пути к файлу на
...
/// печатаются как ведущие/хвостовые. Ни один не теряется.
...
pub fn format_source_with_warnings(
    ...
    let printed = (|| {
        ...
        // Страховка: ни один комментарий не должен остаться непогашенным.
        debug_assert!(
...
///
/// Устройство в точности повторяет [`print_enum`]: печать через `line` в обход
/// привязки комментариев уводит комментарий обо всей записи
/// (`struct Trip { ... } // структура`) на свою строку за `}`, где его подберёт
/// `leading()` следующего элемента, - комментарий меняет хозяина.
///
/// # Границы
///
...
///   печать перестала бы быть идемпотентной.
...
/// общий путь печатает `struct S {` и `}` двумя строками, а комментарий привязан к `}`
/// наравне с непустой записью.
///
/// Висячей запятой нет по той же причине, что у перечисления: `Comma<T>` её не
/// принимает, и напечатанная запятая сделала бы файл неразбираемым, а `taktc fmt`
/// пишет на месте.
fn print_struct(
    out: &mut Out,
    s: &ast::StructDefine,
...
/// Обе части печатаются вместе, потому что порознь каждая портит исходник: без
/// выражения теряется композиция, а без тела получается `model M = A;`, чего
/// грамматика не принимает.
