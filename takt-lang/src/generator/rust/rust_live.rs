...
//!
//! Это   (вариант "а", не эмитить лишнее), применённое к телу функции.
...
fn verdict_of(name: &str, stmt: &StatementNode) -> Verdict {
    match stmt {
        ...
        //
        // Без этого вердикта печатник, разворачивающий `match` в цепочку `if/else`,
        // даёт на входе с ветвью `_` объявление `let mut t: u8 = 0;` с мёртвым
        // значением - отказ `clippy -D warnings` при нулевом коде возврата `taktc`.
        StatementNode::Match { expr, arms, .. } => {
            if reads_expr(name, expr) {
                return Verdict::Read;
            }
