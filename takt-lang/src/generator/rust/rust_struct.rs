...
//!
//! Отдельный модуль, потому что `rust_expr.rs` пришпилен лимитом размера, а знание "как
//! выглядит агрегат структуры" самостоятельно: оно повторяет правило именования полей
//! из `rust_decl` и обязано меняться вместе с ним.
