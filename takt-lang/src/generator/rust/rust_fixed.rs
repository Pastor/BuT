...
///
/// Приведение `as i{S}` переносит к ширине **хранения**, а требует переноса к `W = m +
/// n`: при `W = 12` это 16 бит против 12, то есть другая граница. Совпадают они лишь
/// при `W ∈ {8, 16, 32, 64}` - таков весь корпус, поэтому расхождение дожило от 0061
/// незамеченным.
///
/// При `W = S` форма печати **прежняя байт-в-байт**: иначе поедут снапшоты
/// `examples/generated`. Перенос делается парой сдвигов (в Rust `<<` не паникует на
/// потере старших бит, `>>` знакового - арифметический), а не маской: так знак
/// восстанавливается тем же выражением. `wide` говорит, что `inner` посчитан в `i128`
/// (произведение, деление, пересчёт `q -> q`): такому выражению нужен явный `as i64`
/// перед сдвигами, а уже-`i64` его давать **нельзя** - `clippy::unnecessary_cast` под
/// `-D warnings`.
