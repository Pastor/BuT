...
pub(crate) fn needs_hal_in_tick(map: &RustMap, name: &Name, seen: &mut BTreeSet<String>) -> bool {
    ...
    if let Ok(model_rc) = map.raw_model_at(name.clone()) {
        ...
        for child in children {
            ...
            // Сперва по уникальному имени, и лишь потом по локальному:
            // одноимённых моделей в карте бывает несколько - файл `helper.takt`
            // вносится как модель `Helper`, а внутри него объявлена модель с тем
            // же именем. Поиск по локальному имени найдёт обёртку, её `unique`
            // уже лежит в `seen`, и признак ответит "HAL не нужен": цель
            // напечатает `self.root.tick(&mut *hal)` в функции без параметра
            // `hal` - `E0425` при нулевом коде возврата `taktc`.
            //
...
///
/// Выбран `impl Default`, а не `#[allow(clippy::new_without_default)]`: цель не
/// печатает того, на что ругается линт, а заглушка атрибутом спрятала бы и будущие
/// срабатывания.
...
fn emit_init(p: &mut Printer, ctx: &ModelEmit) -> Result<(), Diagnostic> {
    ...
    // Инициализация вложенных идёт здесь: чтение поля до первого `tick` не должно
    // давать мусор.
    //
