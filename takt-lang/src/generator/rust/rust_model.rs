    ...
    pub(crate) fn build(map: &RustMap, name: &Name, states: &[Name]) -> Result<Self, Diagnostic> {
        ...
        for state in states {
            if map.state_at(state.clone()).is_none() {
                ...
                // валит `-D warnings` (dead_code). Решение R9, вариант (а).
                continue;
            }
            variants.push((
                state.clone(),
...
/// вариантом (б) решения R9 ("заглушить линт") в маскировке. Состояния придумывает
...
/// `dead_code` поймал бы (решение R9, вариант (а)).
...
pub(crate) fn emit_model(
    ...
    // read` валит проверка (проба 2026-07-16).
...
/// отвечает "unused variable: `hal`" при нулевом коде возврата `taktc` (замер 0450).
///
...
///
/// Блоков `enter` здесь нет **намеренно**: по (R6) в `_init` живёт только память, а
...
///
/// Ещё один случай решения R9, вариант (а): не эмитить то, чего не бывает. Заглушить
