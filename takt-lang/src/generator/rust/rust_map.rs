...
//! [`CMap`](crate::generator::c) и `StMap` - снимок дерева плюс множество используемых
//! имён ([`UsageSet`]).
//!
//! Детерминизм достаётся даром: `Map::elements` - `BTreeMap`, ключ `Name` имеет ручной
...
pub(crate) struct RustMap {
    ...
    guard_enable: bool,
    /// Форма печати автомата: `match` либо таблица переходов.
    ...
    /// Профиль времени: "часы" либо "такты". Разрешается общим слоем
...
impl RustMap {
    ...
    /// Задаёт профиль времени. Умолчание - "часы"; существующие вызовы `new` его не
    /// повторяют (аддитивно).
    ...
    /// Эмитить ли guard-проверки. Задаёт форму печати автомата. Комментарии автора
    /// модели.
    ...
    /// Семантический узел модели по её **уникальному** имени.
    ///
    /// Нужен печати аргументов инстанцирования: тип параметра берётся у целевой модели,
    /// а не угадывается по литералу.
