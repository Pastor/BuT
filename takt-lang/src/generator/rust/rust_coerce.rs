...
pub(crate) fn coerce_to(
    ...
    match (target, value) {
        ...
        // Арифметика печатается в типе приёмника. Печатай операнды как есть - и
        // `r: u16 := a + b;` при `a, b: u8` даст `E0308`, то есть вывод не соберётся
        // вовсе при нулевом коде возврата `taktc`, тогда как эталон и цель `c` с её
        // продвижением до `int` ту же запись считают верно.
        //
        ...
        (
            ...
            //
            // Лишнее приведение снимает общий носитель `rust_type::same_printed_type`
            // в печати `Cast`: второго знания о нужде здесь не заводится.
