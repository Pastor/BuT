...
pub(crate) fn print_match(
    ...
    for arm in arms {
        // Берётся первая ветвь `_`: вторая недостижима, и цепочка, напечатанная по
        // последней, исполняла бы не то, что считает эталон.
        if wildcard.is_none()
            && arm
                .patterns
                .iter()
...
///
/// Границы замерены прогоном `clippy`: на всех трёх соседних формах линт молчит, и
/// трогать их значило бы менять вывод без повода.
///
/// Условие в Takt эффектов не имеет (присваивание - оператор), поэтому конъюнкция
/// вычисляет то же самое; порядок проверок сохранён - `&&` в Rust ленив, как и вложенный
/// `if`.
