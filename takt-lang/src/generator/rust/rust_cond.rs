...
fn state_comparison(
    ...
    let field = scope
        ...
        .map(|(_, f)| f.clone())
        .ok_or_else(|| {
            // Причина названа поимённо: "экземпляр не найден" верно, но автору неясно,
            // что делать. Наблюдать соседа цель `rust` не может по устройству:
...
///
/// Печать "как есть" здесь не годится: `self.s < self.u` при `i8` и `u8` даёт `E0308`
/// при нулевом коде возврата `taktc` - в Rust смешанное сравнение не компилируется
/// вовсе. Операнды приводятся к типу, вмещающему оба; когда такого типа нет (`u64`
/// против знакового), правило раскрывается проверкой знака.
fn cond_compare(
    a: &ConditionNode,
    op: &str,
    b: &ConditionNode,
