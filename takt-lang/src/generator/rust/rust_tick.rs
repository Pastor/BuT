...
//!
//! Вынесено из `rust_model.rs`: чистое перемещение, вывод Rust байт-в-байт неизменен.
//! Здесь - тело `tick` (диспетчеризация состояний, контракт входа 0033), guard-формулы
//! и все виды переходов (простые, составные `= Модель`/`|`/`+`).
...
pub(crate) fn emit_tick(
    ...
    // `-D warnings`, как неиспользуемое поле. Признак - На функцию: HAL, нужный только
    // начальному значению порта, в такте не используется, и `rustc` под `-D warnings`
    ...
    // -- Контракт 0033: вход в стартовое состояние ---------------------------- `if` до
    // `match`, без выхода из такта. В C здесь нет `break`, и тело стартового состояния
    // исполняется в том же такте; здесь того же добивается `match`, читающий
