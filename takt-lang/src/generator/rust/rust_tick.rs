...
fn emit_extend_transition(
    ...
    // наступает вовсе: выхода нет. Проверка стоит ДО печати `exit` — иначе он
    // исполнялся бы каждый такт после завершения композиции, тогда как эталон
    // не исполняет его ни разу.
