...
//! ## Два места, а не одно
//!
//! Записи печатаются и в `new()`, и в `init()`: это разные входы (0185 уже ловил на
//! этом аргументы инстанцирования). `new()` строит модель, `init()` возвращает её в
//! начальное состояние - если выставить значение только в одном, сброс либо не дошёл бы
//! до порта, либо порт получил бы значение только после `init()`.
