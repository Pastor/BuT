...
///
/// Скаляру умолчание не нужно и вредно: там отложенная форма законна, а лишнее значение
/// даёт `unused_assignments` - отказ проверки под `-D warnings`.
///
...
/// печатать обычным путём, и тогда `return` остаётся.
///
...
pub(crate) fn print_statement_ctx(
    ...
    match stmt {
        ...
        StatementNode::If {
            ...
            // Одно условие печатается без внешних скобок (`if (x) {` есть
            // `unused_parens`); слитые - конъюнкцией, где скобки подвыражений обязаны
            // остаться.
        ...
        // `taktc verify`. Ответ `RS-010` "LTL-формула не транслируется" здесь был бы
        // неверен дважды: охранную цель переводит на уровне модели и состояния, а LTL во
        // входе может и не быть. Формула-оператор и формула-элемент печатаются одним
        // носителем `emit_guard`: второй ответ на один вопрос и есть дефект.
        StatementNode::InlineFormula(formulas) => {
            if scope.guard_enable {
                for formula in formulas {
                    // Отказ печати условия не роняет компиляцию: формула -
                    // обязательство, а не поведение, и модель, переводимая без неё,
                    // обязана переводиться и с нею.
                    // Непереводимое условие становится предупреждением с названной
                    // причиной - например обращение к переменной модели из тела функции
...
mod tail_tests {
    ...
    /// Блок сворачивается по своему последнему оператору; операторы до него печатаются
    /// обычным путём.
    #[test]
    ...
    /// `if` без `else` не сворачивается: значения у пропущенной ветви нет.
    #[test]
