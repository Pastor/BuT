...
//! `self` дважды (`E0499`). Поэтому общие переменные передаются параметром, и не по
//! одной, а свёрнутыми в структуру `<Root>Shared`, ретранслируемую вниз по композиции
//! параметром `&mut Shared`: передача по одной упирается в
//! `clippy::too_many_arguments`.
//!
//! `Shared` есть объединение нужд под-моделей: переменная корня, не нужная ни одной
...
/// переменной); печать идёт в порядке объединения, то есть детерминирована.
///
/// **Умолчание строит `default_value` - тот же носитель, что у прямых полей.** Строка
/// `"Default::default()"` была бы вторым носителем знания "чему равно умолчание типа":
/// она работает, лишь пока у структуры выводится `Default`, а выводится он не всегда.
///
/// **Запасная ветвь защитная: достижимого входа у неё нет.** Переменная попадает в
/// объединение из объявлений корня, а `model_fields` корня их и обходит - значит
/// значение в `inits` есть. Ветвь оставлена: молчаливая подмена умолчания была бы хуже,
/// чем лишний путь.
