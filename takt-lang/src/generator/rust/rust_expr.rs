...
///
/// Координата - у **оператора**, который печатается сейчас: своей позиции у выражения
/// нет (решение 0056), а `Location::Codegen` печатается вовсе без преисправления - автор
/// искал место сам.
...
pub(crate) struct Scope<'a> {
    ...
    ///
    /// Образец - `return_type`: та же роль подсказки о приёмнике.
    ...
    /// дойти до печатника тел: прежде он спрашивался только у карты, а тела о нём не
    /// знали.
...
pub(crate) fn write_port(
    ...
    // сразу, `E0499`. Прочие сочетания законны, и это замер, а не отестность: два
...
/// именования, а `RS-026` тестыт поля структуры модели.
...
/// Имя объявления константы - **с преисправлением владельца** (; форма
...
pub(crate) fn print_expression(expr: &ExpressionNode, scope: &Scope) -> Result<String, Diagnostic> {
    match expr {
        ...
        // HAL-трейта, адреса она не знает (решение 4A ).
...
/// печатнику доступен именно он. Приём - суфисправление, а не приведение: `10 as u8`
...
/// **отказ** под `-D warnings`, теми же флагами, что стоят в проверке предкоммита.
///
/// Проверка этого не видел: в корпусе индексируют переменной (`arr[i]`), где
/// приведение необходимо, а литеральных индексов там нет ни одного.
///
/// Отрицательный литерал сюда не доходит - его отсекает `SE-028` в
...
/// правила разъехались бы - /0193/0195.
...
fn call(
    ...
    // Аргумент - позиция приёмника с известным типом: у литерала
    ...
    // `taktc`.
    //
    // Внешняя функция сюда тоже входит. Прежде здесь стояло
    // "внешняя функция типов параметров не объявляет" - утверждение о чужом
    // коде, неверное по факту: у `External` есть то же поле `params`, что у
    // `Local`. Цена умолчания: `sink(2 ** 3)` отвергалось
    // `RS-011`, тогда как эталон и семь остальных целей запись исполняют.
    ...
    match &*borrowed {
        ...
        // `extern fn` -> метод HAL (решение (а) ). Вариант
...
fn builtin(
    ...
    match (name, printed.len()) {
        ...
        // `debug` -> метод HAL (решение (а) ). В no_std нет printf,
