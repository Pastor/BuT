...
impl PortSet {
    ...
    ///
    /// Модель без портов не печатает ни трейта, ни параметра типа `H`. Ограничивает
    /// печать не неиспользуемый параметр типа, а `dead_code` на поле: цель не печатает
    /// того, на что ругается линт.
