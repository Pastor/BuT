...
mod rust_modulo;
mod rust_name;
// Носитель "что функции нужно сверх параметров" общий с целью `c`: вопрос
// семантический, и второго знания о нём быть не должно.
mod rust_match;
...
mod tests {
    ...
    /// Хвостовой `if/else` со сворачиваемыми ветвями печатается выражением.
    ///
    /// `if a > b { return a; } else { return b; }` печатается как `if a > b { a } else
    /// { b }`: `return` исчезает, и `needless_return` не возникает.
    #[test]
    ...
    /// Цепочка `if / else if / else` с `return` сворачивается.
    #[test]
    ...
    /// Несворачиваемый хвост сохраняет `return`.
    ///
    /// `if` без `else` не в хвостовой позиции и завершающий `return`: сворачивается
    /// только последний, ранний выход из `if` свой `return` сохраняет.
    #[test]
    ...
    /// Переменная корня, не нужная ни одной под-модели, в `Shared` не входит и остаётся
    /// прямым полем корня.
    ///
    ...
    /// Модель без под-моделей структуры `Shared` не получает: иначе `dead_code` на
    /// неиспользуемом типе.
    #[test]
    ...
    /// Такт под-модели не превышает трёх параметров: `self`, `&mut Shared` и `&mut H`,
    /// последние два - по нужде. Заглушки `#[allow]` в выводе нет.
    #[test]
    ...
    /// Модель без под-моделей получает тип корня.
    ///
    /// У цели `rust` тип корня - обычная `struct`, отдельного объявления не требующая,
    /// поэтому класс, стоивший цели `c` отдельного typedef, здесь не воспроизводится.
    #[test]
    ...
    /// В порождённом коде нет `unsafe`, и следит за этим сам `rustc`.
    #[test]
    ...
    ///
    /// Атрибут допустим только в корне крейта, и в файле, подключаемом через `mod`, он
    /// даёт предупреждение, то есть ломает сборку пользователя под `-D warnings`.
    #[test]
