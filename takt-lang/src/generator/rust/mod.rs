...
//!
//! требовал начинать модуль с `#![no_std]` (R2). Проба при реализации
...
fn generate_program(map: &RustMap) -> Result<(String, Vec<Diagnostic>), Diagnostic> {
    ...
    // Компилятором, а не grep'ом (R10, A12).
...
mod tests {
    ...
    /// **0059 (A7): переменная корня, не нужная ни одной под-модели, в
    ...
    /// **0059 (A6): модель без под-моделей структуры `Shared` не
