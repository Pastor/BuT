...
pub struct GenerateOptions {
    ...
    /// Режим `c-hal`: эмитить таблицу адресов портов и умолчаниеную реализацию HAL
    ...
    /// [`float_embedded`](Self::float_embedded). Границы -.
    ...
    ///
    /// Это **второе осознанное исключение** из принципа "CLI не меняет логику": первое -
    /// `--float-as-q`/`--float-embedded` (действие A-7 ). Исключение уже, чем у 0096: в
    /// профиле "часы" флага нет вовсе.
    ...
    /// поведение (потактовая сверка - тест гибрида, Option E).
    pub specialize: bool,
    ...
    /// Литеральный и константный индекс судит семантика (`SE-028`, и 0434).
    ///
    ...
    /// [`InlinePolicy::Off`].
    ///
    /// Флаг управляет только **эвристикой** ("тело не длиннее пяти операторов и 1...3
    /// вызова"): атрибут `[inline]` автора действует всегда, а `[noinline]` всегда
    /// подавляет. Умолчание `off` -: у эвристики нет адресата, который бы её просил, а
    /// включение изменило бы вывод всего корпуса разом (тот же порядок у
    /// `--bounds-check` 0433 и `--fsm` 0435).
...
pub trait Generator {
    ...
    ///
    /// Код принадлежит **цели** (`CC-010`, `ST-001`, `RS-001`, `SV-001`, `PU-001`),
    /// поэтому общий носитель записи спрашивает его здесь, а не печатает свой:
    /// сообщение об отказе диска обязано быть узнаваемым по коду так же, как прежде.
...
///
/// Каталог создаётся молча - как и прежде: отсутствие права на создание увидит сама
/// запись, а существующий каталог ошибкой не является.
fn write_output(
    files: &[GeneratedFile],
    output_path: &str,
