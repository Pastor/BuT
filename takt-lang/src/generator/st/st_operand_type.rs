...
///
/// Отличается от [`inner_expr_type`] одним случаем - **полем структуры**: `t.hold as
/// u32` прежде отвергалось "тип не определяется статически", хотя тип поля объявлен.
/// Имя функции преобразования IEC строится из обоих типов, и без типа источника
/// печатать нечего.
pub(crate) fn inner_expr_type_in(expr: &ExpressionNode, model: &ModelNode) -> Option<TypeNode> {
    ...
    // живут `SE-061`, `SE-028`/`SE-030` и печать среза - /0193/0195, стоивший четырёх
    // правок в 0371.
