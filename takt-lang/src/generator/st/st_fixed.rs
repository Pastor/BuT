...
///
/// Сужение `LINT_TO_{S}` переносит к ширине хранения, а не к `W = m + n`: при `W = 12`
/// это 16 бит против 12, то есть другая граница. Совпадают они лишь когда `W` есть 8,
/// 16, 32 или 64, - а таков весь корпус.
///
/// Модуль `2^W` передаётся аргументом, а не считается: сдвигов над числами в IEC нет
/// вовсе, а `SHL` определён лишь над битовыми строками. `MOD` в IEC даёт остаток со
/// знаком делимого - отсюда две поправки.
