...
//! MatIEC (2026-07-18). Это существенно: список нельзя пополнять по памяти. `action`
...
//! (склейка одноимённых `fn`) закрывает преисправленияция POU
...
/// нет, а место оператора слой уже знает (0277/0308).
///
/// # Ошибки
...
///
/// Правило снято **прогоном `iec2c`**, а не чтением стандарта (приём 0342): хвостовое
/// подчёркивание и два подряд инструмент отвергает, ведущее - принимает, хотя стандарт
/// запрещает и его. Отказ за то, что арбитр принимает, был бы ложным.
///
/// Прежде такое имя доезжало до вывода как есть: `taktc` возвращал **ноль**, а `iec2c`
/// отвечал "invalid located variable declaration" - сообщением, по которому исходную
/// причину не опознать (тот же обман, что у `ST-023`).
...
/// проверки вывод принимают (замер 2026-08-04).
fn st020(name: &str, ch: char, loc: Location) -> Diagnostic {
    Diagnostic::error(
        loc,
        format!(
            // Строка склеена явно (`\` в конце), а не продолжением литерала:
            // продолжение сохраняет отступ исходника, и в вывод уходили десятки
            // пробелов подряд.
...
mod tests {
    ...
    ///
    /// Первая редакция проверки брала имена у `structs`, `enums` и `types` разом и
    /// отвергала `var mode: Mode;` - вход, который `iec2c` **принимает** (проба
    /// 2026-08-22). Ложный отказ хуже пропуска: он ломает валидную модель.
    #[test]
