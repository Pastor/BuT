    ...
    Diagnostic::error(
        ...
        format!(
            "имя '{}' совпадает (без учёта регистра) со стандартной функцией \
             ...
             'c', 'c-hal', 'rust' и 'sv'. Переименуйте элемент в \
    ...
    Err(Diagnostic::error(
        ...
        format!(
            "имя '{name}': {what} — идентификатор IEC 61131-3 такой формы не \
             ...
             валидной для целей 'c', 'c-hal', 'rust', 'sv' и \
             'sv-mmio'. Переименуйте объявление, если модель нужна для ПЛК"
        ),
    )
    .with_code("ST-026"))
}
    ...
    Diagnostic::error(
        ...
        format!(
            ...
            "имя '{name}' содержит символ '{ch}', недопустимый в идентификаторе \
             ...
             'c-hal' и 'rust'. Переименуйте элемент, если модель \
             нужна для ПЛК"
        ),
    )
    .with_code("ST-020")
    ...
    Err(Diagnostic::error(
        ...
        format!(
            "имя '{name}' совпадает с именем типа '{clash}': идентификаторы \
             ...
             'c', 'c-hal', 'rust', 'sv' и 'sv-mmio'. Переименуйте \
    ...
    Err(Diagnostic::error(
        ...
        format!(
            "имя размещённого порта '{name}' совпадает с именем '{clash}', которое цель \
             ...
             'c-hal', 'st', 'rust', 'sv' и 'sv-mmio'. Переименуйте порт либо \
    ...
    Err(Diagnostic::error(
        ...
        format!(
            "имя '{name}' совпадает с именем '{clash}', которое цель печатает в том же \
             ...
             языка Takt — модель остаётся валидной для целей 'c', 'c-hal', 'rust', \
             'sv' и 'sv-mmio'. Переименуйте одно из объявлений, если модель нужна \
             для ПЛК"
        ),
    )
    .with_code("ST-025"))
