...
//!
//! Вынесено из `st_model.rs` по двум причинам. Формальная: файл упирается в лимит
//! размера модуля. Содержательная: **рёбра печатаются из двух мест** - у обычного
//! состояния и внутри ветви завершения состояния-композиции.
//!
...
///
/// Вынесено из [`emit_edges`]: в табличной форме переходы печатает таблица, а таймеры
...
///
/// Вынесено из [`emit_edges`]: тот же страж служит строке таблицы, и второй его копии
/// быть не должно.
