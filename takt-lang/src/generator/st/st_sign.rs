...
//!
//! Отдельным модулем, потому что `st_expr.rs` упирается в лимит размера (правило
//! `docs/CODE.md`): новое выносится, а не дописывается.
//!
//! Печатников два - условий и выражений: условие ребра приходит `ConditionNode`,
...
pub(super) fn compare_cond(
    ...
    // Имя варианта рядом с целым операндом печатается значением (0508, 0512). У цели
    // `st` мнемонику объявляет `VAR CONSTANT` того же POU, но перечисление, пришедшее
    // из библиотеки вместе С моделью и не названное в списке импорта, до дерева
    // импортёра не доезжает - и `iec2c` отвечал "Ambiguous enumerate value or Variable
    // not declared in this scope" при нулевом коде возврата `taktc`. Значение доезжает
    // всегда: оно лежит в самом узле.
