...
fn generate_program(map: &StMap) -> Result<(String, Vec<Diagnostic>), Diagnostic> {
    ...
    // . Цель `st` обёртки не требует (П2) - цели асимметричны намеренно.
    if map.at_addresses() {
    ...
    // тоже нельзя: без предупреждения охрана исчезает бесследно, и автор об этом не
    // узнаёт.
    //
...
fn emit_configuration(
    ...
    for (model_name, model_rc) in blocks {
        ...
        for name in names {
            ...
            // Порт без адреса при `st-at` - уже ошибка слоя адресов (`SE-052`), и сюда
            // он не доходит; но если карта пуста, размещать нечего. Ключ карты
            // квалифицирован моделью - тот же, что строит продюсер `resolve_model`.
