...
//!
//! Граница модулей - граница ответственности: `st_model.rs` печатает простые модели,
//! этот модуль - композицию.
//!
//! ## Что здесь
//!
...
pub(crate) fn emit_composition(
    ...
    // Тело ветви печатается в буфер, и пустой `IF` не печатается вовсе: состояние с
    // телом автомат не завершает, выхода у него нет, - а пустого оператора MatIEC не
    // знает и отвергает файл.
