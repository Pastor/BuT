...
//!
//! **часть 1: простые модели** (композиция `M1 | M2` / `M1 + M2` - часть 2). Подключает
...
/// в `INIT` без отдельного вызова инициализации (S3).
...
pub(crate) fn emit_body(
    ...
    // значения - расхождение с эталоном (замер: `o = 0` у ST против `3`).
    ...
    for name in &table.ordered {
        ...
        // in case element").
        //
        // Пустой ветвь бывает у двух форм: табличная и состояние, у которого всё тело -
        // `enter` (исполняется на входе) при отсутствии рёбер. Второе появилось с:
        // прежде такое состояние уходило в `END`, и ветвь пустой не оставалась.
...
mod tests {
    ...
    ///
    /// Порядок `enter` -> переход тестытся здесь, а не глазами: при выносе ветви из
