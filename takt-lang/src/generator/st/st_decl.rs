    ...
    for key in names {
        match &model.variables[key] {
            ...
            VariableNode::Port {
                ...
                // первого вызова. Это ровно то "до первого такта", которого требует R5,
                // и запасного пути (запись первым сканом) не нужно - проба показала,
                // что `iec2c` принимает инициализатор и на выходе FB, и на размещённой
...
mod tests {
    ...
    /// Неотображаемый тип **используемой** переменной обязан завалить генерацию, а не
    /// убрать переменную из вывода (R4.3, контрпример дефекту Д1b).
    ///
