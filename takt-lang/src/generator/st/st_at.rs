...
//!
//! `InOut`->`%M` - соглашение: "двунаправленной" локации в IEC нет, `%M` (память) ближе
//! всего по смыслу. В корпусе `InOut`-портов нет, поэтому правило остаётся
//! непроверенным на реальных данных (T36 - синтетическая исправлениетура).
...
mod tests {
    ...
    ///
    /// Прежде здесь стоял `[bit;4]`, и это было неверно: бит-вектор до слова -
    /// упакованный скаляр, и с исправления он размещается наравне с целым. Настоящий
    /// составной тип - массив целых: его цель разворачивает по листам либо отвергает,
    /// но локации у него нет.
    #[test]
