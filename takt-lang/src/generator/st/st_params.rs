...
//!
//! Граница модуля - ответственность: в `st_func` идёт печать функций и вызовов, здесь -
//! три списка, из которых складывается заголовок POU:
//!
...
//!
//! Списки - единый источник истины и для объявления, и для аргументов вызова: разойдись
...
/// возврата `taktc`. У целей `c` и `rust` тот же признак тоже транзитивен.
///
