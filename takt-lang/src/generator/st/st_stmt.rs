...
//!
//! часть 2. Дополняет `st_expr.rs` (часть 1: выражения и условия). Функции
...
pub(crate) fn print_statement(
    ...
    match stmt {
        ...
        // пропуск узла (R4): молчание - семантика блока, а неразрешённый узел
        ...
        StatementNode::If {
            ...
            // возврата `taktc`.
            //
            // Пустого оператора у MatIEC нет: `;` он отвергает (проба 2026-09-01).
...
fn print_match(
    ...
    for (index, arm) in arms.iter().enumerate() {
        ...
        // опущение тестыл признак "образец повторяется ниже" - при дубле пустая ветвь
