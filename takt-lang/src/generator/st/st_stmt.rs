...
pub(crate) fn print_statement(
    ...
    match stmt {
        ...
        StatementNode::Expression(expr, loc) => {
            ...
            // и без него цель печатает `ST-011` вовсе без координаты.
            ...
            // массива в IEC 61131-3 нет вовсе, а эталон, `c` и `rust` такой вход
            // исполняют.
            if let ExpressionNode::Assign(lhs, rhs) = expr.as_ref()
            ...
            if let ExpressionNode::Assign(lhs, rhs) = expr.as_ref()
                ...
                // поднимает во временную свой проход, и разворот здесь дал бы отказ
                // `iec2c` ("';' missing") на `got[0] := pair(k)[0];`.
...
fn print_match(
    ...
    for (index, arm) in arms.iter().enumerate() {
        ...
        // Пустая ветвь не печатается: пустого оператора в IEC нет вовсе. Оговорки
        // "образец не повторяется ниже" здесь не нужно - дубль образца до печати не
        // доходит.
...
mod tests {
    ...
    /// Литерал `bit` в присваивании печатается как `FALSE` либо `TRUE`.
    #[test]
    ...
    /// Значение перечисления в присваивании печатается именем константы.
    ///
    ...
    /// Значение, которому не соответствует ни один вариант, печатается числом.
    ///
    ...
    fn test_inline_formula_warns_st010() {
        ...
        let model = rc.borrow();
        // Формула темпоральная: `ST-010` адресован только ей, а охранная получает
        // `ST-022` и печатается целями `c`, `rust`, `sv`.
