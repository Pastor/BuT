...
//! `abs_diff`, `stacker.takt:100-103`).
//!
//! ## Три синтетические подпорки — и почему они неизбежны
//!
//! Пробы MatIEC (0041-04) вскрыли, что **`extern fn` Takt в стандартном ST
//! невыразим сразу по трём осям**. Все три бьют по `elevator.takt:93-115` (восемь
...
///
/// `unit` Takt → синтетический `USINT`: функции, возвращающие `VOID`, — не
/// стандарт (`iec2c -b`).
...
mod tests {
    ...
    ///
    /// Вход из `elevator.takt:93`: `extern fn motor_up();`. Пустой
