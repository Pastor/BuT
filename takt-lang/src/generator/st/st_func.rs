...
pub(crate) fn emit_functions(
    ...
    // локальные функции разных моделей не склеиваются: их итоговые имена различны.
    // Склейка по голому имени молча теряла бы тело второй функции. Ключ дедупликации -
    // итоговое имя: единственный случай совпадения после префиксации - одноимённые
    // `extern fn` разных моделей (голое имя, как в цели `c`); их заглушки идентичны,
    // поэтому склейка безопасна и лишь не даёт `iec2c` ответить "duplicate" на
    // корректной модели.
...
fn collect_hoisted(
    ...
    // неиспользуемого параметра: вопрос задаётся напечатанному тексту.
...
mod tests {
    ...
    /// отвергается ("Assignment to CONSTANT variables is not allowed"). Порядок
    /// `VAR CONSTANT` перед `VAR` инструмент не принимает, обратный принимает.
    ///
    /// Тест нужен потому, что проверка цели этого класса не видит: функции с локальными
    /// переменными и константой разом в корпусе может не быть, и дефект остаётся
    /// латентным при зелёном `iec2c`.
    #[test]
