...
///
/// **Локальная** функция получает **преисправление уникального имени модели-владельца** -
/// как в цели `c` (`get_function_name`: `Stacker_travel_time`). Пространство имён
/// `FUNCTION`/`FUNCTION_BLOCK` в IEC 61131-3 **плоское**, поэтому без преисправления
...
/// Tier 1). Преисправление берётся из **`upper`** (модель, где функция объявлена), а не
...
/// именует реальный символ платформы, преисправленийать его нельзя.
...
fn print_call_in(
    ...
    // получает преисправление модели-владельца, поэтому вызов и `FUNCTION` совпадут даже если
...
pub(crate) fn emit_functions(
    ...
    // POU именуются с преисправлением модели-владельца (`pou_name`), поэтому одноимённые
    ...
    // случай совпадения после преисправленияции - одноимённые `extern fn` разных
    ...
    for (_, model_rc) in models {
        ...
        for key in crate::generator::call_order::sorted(&model.functions) {
            ...
            // имя, потому что `pou_name` уже несёт преисправление модели-владельца.
            //
...
fn emit_function(
    ...
    // функции имя преисправленийано (`Stacker_travel_time`) и совпасть не может; у `extern`
...
///
/// Границы обязательны: `Mode_Idle` - преисправление `Mode_IdleLong`.
...
mod tests {
    ...
    /// из имени файла, а у анонимной модели теста оно пусто - тогда преисправление POU
    /// выродился бы в `_add1`. Имя делает преисправление осмысленным (`M_add1`) и совпадающим
    ...
    fn test_local_function_emits_function_pou() {
        ...
        // Локальная функция преисправлениеуется именем модели-владельца (`M`) - как в цели
        // `c`: без преисправления одноимённые `fn` разных моделей склеились бы.
        assert!(
    ...
    ///
    /// Тела у внешней функции нет, а IEC требует его от `FUNCTION` (проба П9). Молчание
