...
pub(crate) enum Plan {
    /// Знаковости совпадают либо тип не выводится - печать обычная.
    AsIs,
...
///
/// Правило разное по операторам и по стороне знакового операнда, и копии его в четырёх
/// целях разошлись бы: пропусти одна из них `=` и `!=`, и `sv` сочтёт `-1 == 255`
/// истиной, а вывод `rust` и `st` не соберётся.
///
