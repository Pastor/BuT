...
impl<'a> Printer<'a> {
    ...
    ///
    /// Нужен, когда текст надо получить прежде, чем решить, что печатать перед ним:
    ...
    ///
    /// Лайфтайм приёмника **независим** от лайфтайма исходного печатника: буфер живёт в
    /// вызывающем и переживает копия. Прежняя редакция этого метода (`copy`) связывала
    /// оба лайфтайма, из-за чего буфер было не прочитать - и метод не звал никто (он
    /// стоял под `#[allow(dead_code)]`).
