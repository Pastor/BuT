//! Порядок печати функций - по зависимостям вызова.
//!
//! В IEC 61131-3 опережающих ссылок нет: `FUNCTION`, вызывающая другую, обязана стоять
//! **после** неё. Порядок был алфавитным, и замер 2026-08-20 показал цену: `fn quad`
//! вызывает `fn twice`, а `Loops_quad` печаталась первой - `iec2c` отвечал "';' missing
//! at the end of statement" (сообщение, по которому исходную причину не опознать) при
//! **нулевом** коде возврата `taktc`.
//!
//! # Устойчивость
//!
//! Обход идёт по **алфавитному** списку имён, зависимости поднимаются выше: порядок
//! детерминирован.
...
///
/// Разбор **не** исчерпывающий: пропущенная форма даёт прежний алфавитный порядок, то
/// есть прежнее поведение, а не молчаливую порчу вывода.
