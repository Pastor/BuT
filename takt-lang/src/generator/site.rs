...
//!
//! **Носитель один на все цели и потоковый**.
//!
//! Состояние **потоковое**, и это не украшение: тесты идут параллельно, а генерация
//! внутри потока однопоточна по построению - одна цель, один проход.
//!
//! # Второй слой: Объявление
//!
//! Отказ, рождённый вне операторов, координаты не имел вовсе: печать объявлений
...
//! модели, у которой объявления уже напечатаны, но слой объявления к тому времени снят -
//! а если бы не был, отказ в теле получил бы координату последнего объявления, то есть
//! указал бы не туда.
//!
...
//!
//! **Сброс обязателен на входе в генерацию** ([`reset`]): позиция последнего оператора
//! пережила бы вызов и досталась бы **следующей** генерации в том же потоке - то есть
//! отказ вне операторов получил бы координату из чужого файла. Это ровно та ложная
//! координата, от которой ушли 0264 и 0276.
...
mod tests {
    ...
    /// получил бы координату последнего объявления - та самая ложь, от которой ушли
    /// 0264 и 0276.
    #[test]
