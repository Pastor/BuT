...
pub(super) fn map_c_type(
    ...
    match typ {
        ...
        TypeNode::Enum(enum_name) => {
            ...
            // Знак и ширина берутся из общего факта, а цель лишь отображает его в имя
            // типа C. Возьми она тип по одному `max` - он всегда беззнаков, и
            // отрицательный вариант молча станет `uint8_t` (`-5` даёт `251`, переход
            // `== -5` тождественно ложен, автомат стоит).
...
mod tests {
    ...
    /// Бит-вектор `N > 64` отображается в массив слов `uint64_t name[⌈N/64⌉]`.
    /// Типа без имени, как у скаляра, у него нет, а объявление с именем есть.
    #[test]
