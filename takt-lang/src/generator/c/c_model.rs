...
fn generate_state_transitions(
    ...
    for reference in raw_state.references() {
        ...
        //
        // `Unresolved` сюда не входит: считай проход неразрешённое условие отсутствием
        // условия - и ребро напечатается безусловным переходом. Это опаснее пустой
        // строки в печатнике: вывод валиден, `cc` молчит, а автомат другой - переход
        // срабатывает всегда. Такое ребро идёт в печатник, получает `CC-023` и доезжает
        // до автора обёрткой `CC-018`: позиция ребра плюс причина заметкой. Переход
        // объявляет своё место: условие ребра - не оператор, и отказ в нём иначе идёт
        // без координаты.
        ...
        if has_cond {
            match generate_condition_expr(&reference.cond, map, model) {
                ...
                // Последствия: переход не генерировался, `taktc`
            ...
            //
            // Собственного `break;` у него нет, и цикл по рёбрам на нём заканчивается -
            // ровно как в трёх остальных целях (`rust_tick.rs`, `st_model.rs`,
            // `sv_fsm.rs`): всё, что за безусловным ребром, недостижимо. Напечатай цель
            // и `break;`, и хвост рёбер - ветвь `case` закрывается общим `break;` ниже,
            // и в выводе выйдет пара `break; break;`, а рёбра после безусловного лягут
            // недостижимым кодом. `cc -Wall -Wextra -Wunreachable-code` об этом молчит,
            // то есть цена - шум в коде, который читает человек.
            generate_named_blocks(printer, raw_state, map, model, "exit")?;
            let target_rc = map.raw_state_at(target.clone())?;
            let target_raw = &*target_rc.borrow();
            generate_named_blocks(printer, target_raw, map, model, "enter")?;
...
fn generate_model_tick(
    ...
    // Вход в стартовое состояние не расходует такт. Работа входа - инициализация
    // вложенных, блоки `enter`, установка стартового состояния - диспетчеризуется в `if`
    // до `switch` и не завершается `break`, поэтому тело стартового состояния
    // исполняется в этом же такте, как у эталона. Блоки `enter` остаются внутри `_tick`
    // (они вправе писать в порты), а не выносятся в `_init` вне цикла сканирования.
    // Правило рекурсивно по уровням: вложенный `_tick` делает то же, поэтому сдвиг
    // такта равен нулю на любой глубине.
    ...
    printer
        ...
        .up()
        .nl();
    // Инициализация вложенных живёт в `_init`. Здесь - только поведение входа: блоки
    // `enter` (они вправе писать в порты, поэтому внутри такта) и установка стартового
    // состояния. Без `break` тело исполняется в этом же такте.
    ...
    for state_name in states.iter() {
        ...
        match state {
                    ...
                    StateExtend::Model(name, _) => {
                        ...
                        if !map.fsm_table() {
                            printer
                                ...
                                .up()
                                .nl();
                            // Переход печатает общая функция: своя копия "next либо
                            // END" потеряла бы собственные рёбра состояния.
                            generate_extend_transition(
                                printer,
                                raw_state,
                                map,
            ...
            // Недостижимо по контракту `CMap::state_at`: он отдаёт элемент, только если
            // `element.is_state()`, а тот верен лишь у `Element::State` и
            // `Element::StateExtend`. То есть `Element::Model` сюда не попадает никогда.
            //
            // `unreachable!`, а не `Err`: инвариант обеспечен фильтром по типу, а не
            // входными данными автора - диагностика предлагала бы ему исправить то,
            // чего он не писал.
    ...
    // Вариант `_INIT` до `switch` не доходит - его снял диспетчер выше, - но остаётся
    // значением перечисления, и `default` гасит `-Wswitch`, не пряча при этом настоящий
    // пропуск состояния: у всех достижимых состояний есть `case`.
...
pub(super) fn generate_model_functions(
    ...
    // Указатель на корень печатается по нужде: стой он в сигнатуре всякой под-модели,
    // тело пользовалось бы им не везде, и корпус наполнился бы заглушками `(void)main;`.
    // Признак - общий носитель `c_needs`. Нужда считается на функцию: у `_init` и
    // `_tick` тела разные, а `_is_done` указателем не пользуется никогда.
    let needs = |which| {
        !is_main
