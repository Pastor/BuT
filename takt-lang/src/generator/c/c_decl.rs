        ...
        for var in variables {
            match var {
                ...
                VariableNode::Const {
                    ...
                    // `{...}.field` - невалидный C (фигурный литерал вне объявления), а
                    // `static const Type X = {...};` делает `X.field` корректным. Массив
                    // печатается так же и по той же причине: `#define X {1, 2}` при
                    // `a[0] = X[0];` разворачивается в `{1, 2}[0]`, и `cc` отвечает
                    // "expected expression" при нулевом коде возврата `taktc`. Скаляр
                    // остаётся `#define`.
                    //
...
pub(super) fn generate_functions(printer: &mut Printer, map: &CMap) -> Result<(), Diagnostic> {
    ...
    for model in models {
        ...
        // Опережающие прототипы локальных функций. Композиция `f -> g` делает порядок
        ...
        // алфавитный порядок определений не важен.
        ...
        for ref fun in model.functions.clone().into_values() {
            ...
            match fun {
                FunctionDefinitionNode::Local {
                    ...
                    // Указатель на состояние печатается по нужде: стой он в сигнатуре
                    // всегда, тело пользовалось бы им не везде, и корпус наполнился бы
                    // заглушками `(void)model;`. Признак берётся у общего носителя
