...
fn build_parallel_item(
    ...
    match extend {
        ...
        // Вложенная последовательность внутри параллели. Разложи её плоско, без
        // собственной машины шагов, - и печать тика пропустит её молча, а ветвь `A + B`
        // в прошивке не исполнится вовсе.
        //
    ...
    for var in model.borrow().variables.clone().into_values() {
        match var {
            ...
            VariableNode::Simple { name, ty, loc, .. } => {
                ...
                // Отказ отображения типа несёт свою причину (`CC-014`, `CC-015`), а не
                // `CC-009` "Variable not found": переменная найдена, невыразим её тип.
...
pub fn generate_header(
    ...
    // to refer to type".
    //
    // Ветку `c-hal` с ней слили в один путь: расхождение целей и было
    ...
    // В режиме `c-hal` печатаются таблица адресов портов и умолчательный HAL
    // (`c_hal.rs`).
    if options.hal {
...
mod tests {
    ...
    /// Одиночная модель без под-моделей получает typedef корня.
    ///
    /// Структура печатается тегом (`struct Demo { ... };`), а прототипы - через голое имя
    /// (`void Demo_init(Demo *main);`), поэтому без typedef имя типом не становится.
    #[test]
    ...
    /// Модель с под-моделями typedef корня не теряет и не дублирует.
    #[test]
    ...
    /// Заголовок одиночной модели самодостаточен: голое имя есть тип.
    #[test]
