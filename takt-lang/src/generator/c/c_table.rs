...
//!
//! **Тела состояний остаются в `switch`.** Таблицей выражается переход, а не содержимое
//! такта: блоки `always` и `every`, тик реализации состояния и проверки формул печатает
//! обычный печатник. Иначе завелось бы второе знание о теле.
//!
...
//! шагов при этом остаётся в теле такта: она ведёт переходы внутри состояния, тогда как
//! таблица ведёт выход наружу.
