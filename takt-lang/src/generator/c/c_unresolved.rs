...
//! Второй конструктор разошёлся бы формулировкой с первым - /0193/0195. Образец -
//! `format::unsupported(loc, вид)` -> `FM-001`: **один** код, вид узла назван словом,
...
//! приходит в цель неразрешённой **по инварианту проекта** (проход
//! `resolve_state_references` запрещён). Её разбирает
//! `c_expr::condition::generate_state_comparison` - **до** общего печатника, и до этой
//! воронки она не доходит. Заводя новую форму, проверь порядок: сначала разбор законной
//! формы, потом отказ.
...
mod tests {
    ...
    /// **T6. Тест класса.** Каждый вид узла из [`UnresolvedNode::ALL`] обязан
    /// иметь **своё** название в тексте и отдавать `CC-023`.
    ///
    /// Падает **списком**: новый вид, забытый в `phrase`, называется поимённо.
    /// Проверять "программа падает" здесь нечем - дерево с неразрешённым узлом из
    /// исходника не построить (замер: `construct_model` отвергает вход
    /// `SE-025`/`SE-003`), поэтому мера теста - **ветвь**.
    #[test]
