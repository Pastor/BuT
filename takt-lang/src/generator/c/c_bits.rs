...
/// имени (засада 0204). Для **объявленного** типа он верен; при `Inference` вернётся
/// `None`, то есть печать деградирует в прежнюю, а не в отказ.
...
/// словам.
///
/// Разбор намеренно **не** исчерпывающий: узел, которого здесь нет, ведёт себя как
/// прежде. Это граница отказа, а не пропуск - эталон те же операции не поддерживает
/// вовсе (`SIM-005` в такте), и отказ здесь лишь переносит сообщение из чужого
/// компилятора в свою диагностику.
