...
pub(super) fn emit_zero_init(
    ...
    match ty {
        ...
        // в теле.
        TypeNode::Enum(name) => {
            // Имя константы строится той же функцией, что и объявление `#define`
            // (0167/0195), а владелец берётся у самого узла: перечисление могло быть
            ...
            // Перечисления без вариантов не бывает (`SE-105`, 0172), и владелец у узла
            // есть всегда - ветвь `None` защитная, ноль в ней прежнее поведение.
