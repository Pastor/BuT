...
//! тест T11.
//!
//! Тип операнда выводится общим слоем [`extract_type`]: `SE-059` гарантирует, что оба
//! операнда - один и тот же `q(m, n)`, поэтому формат берётся у любого.
...
///
/// Различие существенно: требует переноса к **W**, а приведение к типу хранения сужает
/// к `S = fixed_storage_bits(W)` - при `W = 12` это 16 бит, то есть перенос происходит
/// на другой границе, чем у эталона. Совпадают они только когда `W ∈ {8, 16, 32, 64}` -
/// а весь корпус ровно таков, поэтому расхождение дожило от 0061 незамеченным.
...
pub(super) fn cast(
    ...
    match (src, target) {
        ...
        //
        // Прежде здесь печатался `floor((2.5) * 256.0)` - вызов libm В Времени выполнения ради
        // константы; тот самый класс, который назвала 0317, но починила только в
        // инициализаторах. Счёт - у общего носителя (`const_eval::fixed_literal` ->
        // `const_eval::fixed_repr`), поэтому значение совпадает с эталоном по
        // построению.
...
///
/// Прежде степень печаталась как `pow((double)a, (double)b)`, и на широких типах это
/// давало **другое число**: у `double` 53 разряда мантиссы, а `3 ** 40` =
/// 12157665459056928801 требует 64 - прогон дал 12157665459056928768, то есть прошивка
/// расходилась с эталоном молча.
///
/// Считается в **беззнаковом**: обёртка `mod 2ⁿ` определена стандартом при любом
/// значении, а переполнение знакового - UB.
...
/// знаковый сдвиг через `min`, а не `unwrap_or`.
///
/// Насыщение знакового вправо даёт знак, и он берётся **сравнением**, а не сдвигом на
/// `w - 1`: `>>` отрицательного в C implementation-defined (тот же довод, что у
/// floor-хелпера 0061), а `v < 0` определено стандартом.
