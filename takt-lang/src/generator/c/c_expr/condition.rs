...
// и тот же C.
//
// Разбор формы - **общий на проект** (`semantic::condition::state_of`): прежде он был
// написан здесь, в цели `rust` и в судье условий порознь, и судья отставал - краткую
// форму он отвергал `SE-025` на записи, которую этот печатник переводит.
//
...
fn generate_state_comparison(
    ...
    // Разбор трёх форм правой части - общий с симулятором: своя копия разошлась бы на
    // форме, которую видит только один из потребителей. Защитная ветвь без
    // `Debug`-дампа: прежде сообщение печатало внутреннее представление узла целиком.
    ...
    // 'entry' in 'struct SrefPong'" (проба). Дефект не проявлялся, потому что
    ...
    let path = if is_same_model {
        ...
        // Прямого пути у владельца нет (модель-сестра, в т.ч. вложенная), но корень
        // доступен всегда и владеет всем по значению - адресуем цепочкой от него.
        //
        // Достижимость ветки `CC-019` не установлена: единственный найденный способ
        // получить `None` - модель, не встроенную ни в одно состояние, - отсекается
        // раньше кодом `CC-012` ("Модель не найдена"), потому что такой модели нет в
        // `using_models` (проба). Диагностика оставлена как отказ по умолчанию:
        // альтернатива - вернуть заведомо неверный путь молча, то есть ровно тот класс
        // дефекта, против которого заведена 0028.
    ...
    match cond {
        ...
        ConditionNode::Equal(l, r) => {
            if let Some(model) = state_of_model(l) {
                ...
                // Смешанная знаковость: на 64 битах C сравнивает беззнаково, и
                // равенство отрицательного с беззнаковым тоже ломается - первая
                // редакция 0359 покрыла лишь `<`/`>`.
                        ...
                        } = &*var
                        {
                            let model_name =
                                if let Some(rc) = upper.as_ref().and_then(|w| w.upgrade()) {
                                    ...
                                    // Воронка `CC-023`: код, вид узла и позиция. Место
                                    // пропущено при закрытии той фичи - найдено замером
                                    // 0276.
                                    return Err(crate::generator::c::c_unresolved::refuse(
                            ...
                            return match cls {
                                ...
                                // Позицию даёт носитель (0308/0468): условие ребра
                                // объявляет своё место, и без этого причина отказа
                                // приезжала заметкой без координаты - "примечание:
                                // причина [CC-001]".
        ...
        ConditionNode::Function(fun_rc, args, _) => {
            ...
            // Локальная функция принимает указатель на состояние первым аргументом - но
            // По нужде: тем же признаком, по которому он попал (или не попал) в
            // сигнатуру.
            //
            // Признак спрашивается у общего носителя, а не считается здесь заново:
            // печатников вызова у цели два (выражения и условия), и прежде второй
            // передавал указатель безусловно - функция без обращения к состоянию
            // объявлялась без него, а из условия ребра звалась с ним, и `cc` отвечал
            // "too many arguments" при нулевом коде возврата `taktc`.
...
/// нормализуются в snake_case без этого преисправления.
...
/// миллисекунды в такты множителем `hertz / 1000`, который обязан быть целым (иначе
/// `SE-073`: округление молча изменило бы выдержку).
...
///
/// На 8/16/32 битах операнды продвигаются до `int`, и печать "как есть" верна. На
/// **64** битах - нет: `int64_t < uint64_t` считается беззнаковым, и `-1 < 200` давало
/// **ложь**, а `cc -Wextra -Werror` отвечал `-Wsign-compare`. Тот же класс, что 0334
/// (сдвиг на 32/64 битах).
///
