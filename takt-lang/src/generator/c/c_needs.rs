...
//! "какие переменные передать", у `c` обе нужды сводятся к одному указателю. Своё знание
//! здесь разошлось бы с тем молча.
//!
//! **Транзитивность бесплатна:** рекурсия в языке запрещена (`SE-053`), граф вызовов
//! ацикличен, и признак считается одним обходом снизу вверх.
...
/// `main` от неё же.
///
/// Признак считается по семантике, а не по напечатанному тексту: сигнатура печатается
/// раньше тела и в трёх местах - прототип, определение, вызов, - и текст к тому моменту
/// ещё не готов.
///
