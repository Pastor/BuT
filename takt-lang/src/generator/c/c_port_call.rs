...
//!
//! Прежде порт `[bit;N]` был числовым, и разряд писался чтением-правкой- записью через
//! `write_numeric`. Запись же индексом (`heater[3] := 1`) печаталась как индексация
//! результата чтения - `(*model->read_numeric)(PORT, ud)[3] = 1;`, - что не собирает ни
//! один компилятор C, при нулевом коде возврата `taktc` (замер 2026-09-04).
