...
pub struct CMap {
    ...
    guard_enable: bool,
    /// Форма печати автомата: `switch` либо таблица переходов.
    fsm: crate::generator::FsmForm,
    /// Ширина вещественного типа: `double` при `W64`, `float` при `W32`.
    float_width: FloatWidth,
    /// Профиль времени: в какие единицы пересчитывается длительность.
    time_profile: crate::semantic::duration::TimeProfile,
    /// Режим `c-hal`: цель знает адреса и вправе обращаться к памяти напрямую.
    ///
    /// Нужен печати анонимного обращения `#0x...`: у цели `c` доступ идёт **только**
    /// через колбэки HAL, поэтому обращение по адресу она отвергает (`CC-021`), а
    /// `c-hal` печатает `*(volatile uintN_t*)`.
    hal: bool,
    /// Предупреждения цели, накопленные за проход.
    ///
    /// Канал у цели `c` существует с (`Generator::generate` возвращает
    /// `Vec<Diagnostic>`), но говорить по нему было нечем. Первое, что по нему поехало, -
    /// `CC-024`: вызов `debug(...)` цель **выбрасывает**, и молчать об этом нельзя
    /// (соседние `st` и `rust` на том же входе отказывают).
    ///
    /// `RefCell`, потому что печатники получают карту по `&self`: заводить `&mut` через
    /// полтора десятка сигнатур ради накопителя - цена выше пользы. Генерация
    /// однопоточна по построению.
    warnings: RefCell<Vec<Diagnostic>>,
}

impl CMap {
    /// Аргумент-указатель на корень для вызова функций модели `child`: `", main"`, `",
    /// model"` либо пусто.
    ///
    /// `caller_is_main` - печатается ли вызов из тела корневой модели: там указатель на
    /// корень зовётся `model`, в под-модели - `main`.
    ///
    /// `which` - какая функция ребёнка вызывается: нужда считается на функцию, и
    /// `X_init` может обойтись без указателя там, где `X_tick` без него не может.
    ///
    /// Признак спрашивается у **одного** носителя (`c_needs`) и здесь, и в сигнатуре:
    /// разъехавшись, они дали бы "too many arguments" - отказ `cc`, то есть громкий, но
    /// всё же отказ.
    ...
    pub(crate) fn raw_state_at(&self, name: Name) -> Result<Rc<RefCell<StateNode>>, Diagnostic> {
        self.map
            ...
            .ok_or_else(|| {
                // Позицию даёт общий носитель: своей у отказа нет - состояния, о котором
                // он говорит, не существует, зато место ссылки на него знает печатник
                // переходов.
                Diagnostic::error(
...
impl CMap {
    /// Запоминает предупреждение цели.
    ...
    /// Задаёт форму печати автомата.
    ///
    /// Отдельным методом по той же причине, что профиль времени и режим HAL: умолчание -
    /// `switch`, и существующие вызовы `new` его не повторяют.
    ...
    /// Включает режим `c-hal`.
    ///
    /// Отдельным методом по той же причине, что профиль времени и ширина вещественного:
    /// умолчание - цель `c`, и существующие вызовы `new` его не повторяют. Умолчание
    /// при этом **безопасное**: не зная адресов, цель отказывает, а не печатает доступ
    /// наугад.
    ...
    /// Задаёт профиль времени: "часы" либо "такты".
    ///
    /// Отдельным методом по той же причине, что и ширина вещественного: умолчание -
    /// профиль "часы", и существующие вызовы `new` его не повторяют.
    pub fn with_time_profile(mut self, profile: crate::semantic::duration::TimeProfile) -> Self {
        self.time_profile = profile;
        self
    }

    /// Профиль времени, выбранный для этой сборки.
    pub(crate) fn time_profile(&self) -> crate::semantic::duration::TimeProfile {
        self.time_profile
    }

    /// Задаёт ширину вещественного типа.
    ///
    /// Отдельным методом, а не параметром `new`: умолчание `W64` - поведение по
    /// умолчанию генератора, и 26 существующих вызовов `new` (в основном тесты) не
    /// должны повторять его дословно.
...
/// Карта цели `c` - источник состояний для общего носителя строк таблицы: вопросы к
/// карте одни у всех целей, а сама карта у каждой своя.
