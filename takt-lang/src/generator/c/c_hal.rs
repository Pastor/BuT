//! Умолчательный HAL цели `c-hal`: таблица адресов портов и реализации `read_*`/`write_*`
//! через `*(volatile T*)addr`.
//!
//! HAL - самостоятельная тема; `c_header::generate_header` вызывает [`generate_hal`] в
//! режиме `options.hal`.
//!
//! ## Ширина доступа к бит-порту - по слову, а не по типу
//!
//! Ширина доступа бит-порта - минимальное слово, содержащее бит
//! ([`word_bytes_for_bit`]), а `read_bit` и `write_bit` выбирают тип по `b.width`
//! (`switch`, как у числового порта). Бит вне [0, 63] отсекает `SE-060` до генерации.
...
///
/// Перечисление исчерпывающее. Ветвь `_ => 4` читала бы всё неузнанное четырьмя байтами
/// молча: битовый порт получал бы 4 байта через `Bit` -> `int`, а порт структурного типа
/// в 2 байта - тоже 4. На железе это доступ за пределы регистра.
///
/// `None` - ширина неизвестна: вызывающий обязан дать `CC-016`, а не выбрать число за
/// пользователя.
...
///
/// / (): ширину доступа бит-порта задаёт **не тип порта** (`bool` -> 1 байт), а бит
/// адреса. Читать один байт и сдвигать на `b.bit` нельзя: бит 8...31 дал бы **молча
/// ноль** (после продвижения до `int` сдвиг за пределы загруженного байта), бит >= 32 -
/// **UB** (`>> >= ширины`). Минимальное слово по биту убирает оба класса конструктивно;
/// бит вне [0, 63] уже отсечён `SE-060`, поэтому здесь всегда 1/2/4/8.
...
/// C-тип порта `(model, name)` через [`get_c_type`](c::get_c_type) (для ширины
/// доступа).
fn port_ctype(map: &CMap, model_name: &Name, port_name: &str) -> Option<String> {
    ...
    // У массива-порта ширина доступа - ширина элемента: порт адресуется индексом, и
    // регистр читается по одному элементу. Она же служит шагом: элемент `i` лежит по
    // `addr + i * width`.
    let scalar = match ty {
...
/// эмитит таблицу адресов портов и принятую по умолчанию реализацию HAL.
///
/// Для каждой пары `(класс, направление)` генерируется `static const`-таблица
/// `{Enum}__ADDR[]`, индексируемая enum-вариантами порта, и умолчательные
/// `read_*`/`write_*` через `*(volatile T*)addr`. Помощник `bind_default_hal` связывает
/// указатели структуры с этими функциями (только для присутствующих
/// классов/направлений).
pub(super) fn generate_hal(
    ...
    // Нужен ли умолчательный источник времени `now_ms` (профиль "часы").
    let needs_time = map
        .root_model_node()
    ...
    for cls in [PortClass::Bit, PortClass::Rational, PortClass::Numeric] {
        for dir in [PortDirection::In, PortDirection::Out] {
            ...
            for (model_name, port_name, declared) in ports {
                ...
                // карта ключуется квалифицированно (модель+порт) - строим тот же ключ
                // хелвером, что и продюсер `resolve_model`. `model_name.unique()` ==
                // `unique_model_name(ModelNode)` (обе из обхода `upper`), поэтому
                // lookup попадает.
                ...
                // `-1` означает "позиции бита нет" и остаётся у числовых и вещественных
                // портов: у слова разряда не выбирают, поле в их таблицах не читается.
                //
                // у **бит-порта** позиция нормирована слоем адресов (`SE-090`
                // подставляет ноль и говорит об этом автору), поэтому в битовых
                // таблицах `-1` появиться не может, и `read_bit`/ `write_bit` сдвигают
                // на `b.bit` без проверки знака - прежняя время выполнения-заплатка `b.bit < 0 ?
                // 0 : b.bit` снята как мёртвая.
                let bit = resolved.and_then(|r| r.bit).unwrap_or(-1);
                // было `.unwrap_or(4)` поверх `_ => 4` - два молчаливых умолчания
                // подряд. Ширина доступа к MMIO угадыванию не подлежит.
                ...
                // Бит-порт: ширина доступа - минимальное слово, содержащее бит, а не
                // ширина типа `bool` (1 байт). Иначе `read_bit`/`write_bit` сдвигали бы
                // за пределы одного байта.
    ...
    // Умолчательные функции чтения/записи (только для присутствующих классов). Бит-порт:
    // тип чтения выбирается по `b.width` (минимальное слово по биту) - иначе `>> b.bit`
    // при бите >= 8 читает не тот бит либо даёт UB.
    ...
    *(volatile float*)(b.addr + (uintptr_t)index * b.width) = val;
    ...
    // Умолчательный источник времени профиля "часы": библиотечные монотонные часы. Тянет
    // `<time.h>` и `clock_gettime` - на голом железе их нет; там штатный путь - профиль
    // "такты" либо свой `now_ms` (см. книгу). `_POSIX_C_SOURCE` объявлен у самого верха
    // заголовка, иначе на строгом glibc `CLOCK_MONOTONIC` скрыт под `-std=c11`.
    if needs_time {
    ...
    // Помощник связывания принятого по умолчаниюо HAL со структурой модели.
    //
    // Помощник объявлен в заголовке и вызывается не каждой единицей трансляции - у
    // голого `static` это даёт `-Wunused-function` в любом файле, который заголовок
    // включил, но помощника не позвал. Неиспользованная `static inline` предупреждения
    // не даёт - проверено пробой на `cc`.
    printer
        .print(&format!(
    ...
    // У модели без адресованных портов и без источника времени связывать нечего: тело
    // пусто, и параметр остаётся неиспользуемым. Помощник - `static inline` в
    // заголовке, то есть предупреждение уедет к пользователю.
...
"#;
    ...
    /// Ширина доступа к MMIO берётся по типу порта.
    ///
    /// Значения сняты с настоящего вывода `taktc -t c-hal`, а не угаданы:
    /// - битовый порт - 1 байт; чтение 4 байтами из однобайтового регистра есть
    ///   доступ за его пределы;
    /// - вещественный порт - 8 байт: это цена умолчания `--float-width=64`.
    #[test]
    ...
    /// `--float-width=32` даёт вещественному порту 4 байта - для платформ, где
    /// 8-байтное чтение недопустимо.
    #[test]
    ...
    /// Композиция с **одноимёнными адресованными портами** двух под-моделей. Тест: до
    /// неё карта ключевалась голым именем `sig`, оба варианта
    /// `COLL_A_PORT_SIG`/`COLL_B_PORT_SIG` брали адрес по `sig` и получали один
    /// (последний, 0x20) - адрес первого порта терялся. С квалифицированным ключом
    /// каждый порт получает **свой** адрес.
...
"#;
    ...
    /// Одноимённые порты под-моделей получают каждый свой адрес в цели `c-hal`.
    #[test]
    fn address_collision_qualified_key_distinct_addresses() {
        ...
        // С голым ключом `sig` оба варианта взяли бы 0x20, и адрес 0x10 потерялся бы.
        assert!(
    ...
    /// Таблица ширин исчерпывающая: неузнанный тип даёт `None`, а не молчаливые 4 байта.
    ///
    /// Ветвь достижима: порт структурного типа `Point` в 2 байта получал бы ширину 4,
    /// то есть доступ за пределы регистра, выданный молча.
    #[test]
    ...
    /// ширина доступа бит-порта - минимальное слово, содержащее бит (границы степеней
    /// двойки). Бит 33 -> 8 байт (uint64), а не 1 (UB).
    #[test]
