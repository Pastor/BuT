...
//!
//! Вынесено из `c_header.rs`: файл упирался в лимит размера модуля, а HAL -
//! самостоятельная тема. `c_header::generate_header` вызывает [`generate_hal`] в режиме
//! `options.hal`.
//!
//! ## Ширина доступа к бит-порту - по слову, а не по типу
//!
//! Оба проверки `cc` принимали это молча (`b.bit` - значение времени выполнения). Теперь
...
///
/// **Перечисление исчерпывающее** (R9). Прежде здесь стояло `_ => 4`,
/// и всё неузнанное читалось четырьмя байтами **молча** - тот же класс дефекта,
/// против которого заведена вся. Пробы показали, что ветка достижима
...
"#;
    ...
    /// **T10 (/03).** Ширина доступа к MMIO по типу порта.
    ///
    ...
    ///   ожидаемая цена умолчания `--float-width=64`,.
    #[test]
    ...
    /// **T11.** `--float-width=32` возвращает вещественному порту 4
...
"#;
    ...
    /// **A1.** Одноимённые порты под-моделей -> каждый свой адрес в c-hal.
    #[test]
    ...
    /// **R9.** Таблица ширин исчерпывающая: неузнанный тип даёт
    /// `None`, а не молчаливые 4 байта.
    ///
    /// Достижимость проверена зондом: порт структурного типа `Point` (2 байта) получал
    /// `width 4` - доступ за пределы регистра, выданный молча.
    #[test]
