...
pub(super) fn generate_model_init(
    ...
    printer
        ...
        .print("_INIT;")
        .nl();
    // Счётчик и метка времени: счётчик тактов равен нулю (на первом такте с входа не
    // прошло ни одного такта), метка профиля "часы" защёлкивает `now_ms`.
    // `takt_prev_state` совпадает с начальным состоянием, поэтому вход в стартовое
    // распознаётся сменой состояния. Правило живёт в `c_time`.
    ...
    // Инициализация вложенных элементов стартового состояния идёт в `_init`, а не в
    // `_tick`: память приводится в определённое состояние до первого такта, и чтение
    // полей между `_init` и `_tick` перестаёт быть неопределённым поведением. Правило
    // рекурсивно по уровням: `_init` вложенной модели инициализирует свои вложенные.
    // Блоки `enter` сюда не входят - они поведение и остаются в `_tick`.
...
/// пределами структуры. Значение выставлено до первого такта, а не в такте 1: там живут
/// блоки `enter`.
...
/// . Выбрать один ответ здесь - значит
