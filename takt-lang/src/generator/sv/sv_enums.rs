//! Перечисления состояний и шагов у цели `sv`.
//!
...
///
/// Значения назначает генератор (от 0 до n-1), поэтому ширина - ⌈log₂(n)⌉, как у
/// перечислений состояний. Порядок задаёт обход `Fsm::build`, то есть он детерминирован.
