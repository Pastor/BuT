...
//! SystemVerilog.
//!
//! Источников блоков два - состояние и сама модель (`always` уровня модели); обе печати
//! идут в `always_comb` над сигналами `_next`.
//!
...
///
/// Объявления локальных переменных идут до операторов: без них вывод невалиден при
/// нулевом коде возврата.
///
/// **Локальная переменная, содержащая структуру, поднимается в начало `always_comb`.**
/// Тело пишет её поля (`tmp.lo = ...`), а внутри ветви `case` yosys такую запись полным
/// присваиванием не считает: "Latch inferred for signal `$unnamed_block$1.tmp.lo`" - при
/// том что verilator модуль принимает, а `taktc` возвращает ноль. Формы измерены
/// прогоном обоих инструментов: именованный блок, `= '0`, `'{N{'0}}` и объявление на
/// уровне модуля без умолчаний yosys отвергает; принимает он объявление в начале
/// процесса с нулевыми умолчаниями по листьям.
///
/// Прочие локальные (скаляр, перечисление, массив скаляров) печатаются на месте: подъём
/// изменил бы вывод корпуса без нужды.
fn emit_block_body(
    p: &mut Printer,
...
///
/// **Форма выбрана пробой, а не вкусом**: `assert (условие);` принимают
