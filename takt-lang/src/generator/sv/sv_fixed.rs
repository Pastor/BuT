...
//!
//! Ширина в SV **не округляется** (в отличие от `c`/`rust`/`st`): `q(m, n)` - `logic
//! signed [W-1:0]`, `W = m + n`. Нормативные правила совпадают с эталоном симулятора
//! (`eval::fixed`) и целями `c`/`rust`/`st` **побитово**:
//!
...
//!   Синтезируется в аппаратный делитель (предупреждение - объём 0064).
//!
...
/// объявления передаются замыканием (приём 0366). Своего разбора цепочки здесь больше
/// нет - он разошёлся бы с тем, которым живут `SE-061`, `SE-028`/`SE-030` и печать
/// среза.
fn field_type(
    base: &ExpressionNode,
    field: &str,
