//! Объявления констант и восстановление литералов перечисления у цели `sv`.
//!
...
///
/// Проверка, принимающая только литералы, отвергла бы
/// `enter { cmd_target_stack := CHARGE_STACK; }` - обычную запись корпуса.
///
/// # Ошибки
...
fn struct_reset(
    ...
    for ((field, field_ty), value) in def.fields.iter().zip(items) {
        ...
        if let Some(text) = packed_field_array(value, field_ty, enums, field, name, loc, scope)? {
            // Форма позиционная, как и у прочих полей: именованную yosys не принимает.
            parts.push(text);
            continue;
        }
        let printed = reset_value(
        ...
        // литерала: `verilator -Wall` отвечает `WIDTHCONCAT` "Unsized numbers not
        // allowed in concatenations" на любое безразмерное число внутри `{...}`.
        //
        // Литерал длительности (`5ms`) - тоже число: он приходит узлом `Duration` и
        // печатается миллисекундами, и безразмерным он даёт `WIDTHCONCAT` на структуре с
        // полем `duration`. Признак берётся у напечатанного текста: он уже приведён к
        // единицам цели.
