...
//!
//! На непокрытом узле - ошибка, а не молчание: у `get_c_type` неотображаемый тип
//! возвращает `None`, и отличить "отфильтровали как неиспользуемое" от "не смогли
//! перевести" снаружи невозможно.
//!
//! ## Ширина перечисления считается по диапазону значений, а не по числу вариантов
//!
//! Формула `W = ⌈log₂(вариантов)⌉` корпуса не выдерживает: у `enum Action { Idle = 670,
//! ... }` два варианта, то есть по ней `logic [0:0]`, - а значение 670 требует десяти
//! бит:
//!
//! ```text
...
//! ```
//!
//! Тот же капкан стоит у соседних целей: у `st` разрядность тоже считается по диапазону
//! вариантов, а не берётся плоским `USINT`, а у `rust` `#[repr(u8)]` не принимает
//! `Idle = 670`.
//!
//! Формула [`enum_width`] - по диапазону - покрывает оба случая одной ветвью: у
//! перечисления состояний, где значения назначает сам генератор (от 0 до n-1), она даёт
...
pub(crate) fn sv_type(ty: &TypeNode, what: &str) -> Result<SvType, Diagnostic> {
    match ty {
        // Идеальное соответствие: один провод и один триггер. Тип `duration` -
        // беззнаковый вектор в миллисекундах, та же единица, что у остальных целей,
        // поэтому потактовая сверка сравнивает одинаковые числа. Ширина -
        // `duration::VALUE_BITS`.
...
mod tests {
    ...
    /// `bit` отображается в `logic`, а не в целое: один провод занимает один бит.
    #[test]
    ...
    /// `bool` отображается в `logic`: для RTL `bit` и `bool` - один провод.
    #[test]
    fn bool_maps_to_logic() {
        assert_eq!(sv_type(&TypeNode::Bool, "тест").unwrap().prefix, "logic");
    }

    /// Разрядность целого буквальна.
    #[test]
    ...
    /// Знаковое целое получает `signed`, и это не косметика.
    ///
    ...
    /// Разрядность, не кратная машинному слову, отображается без округления.
    ///
    /// Записать такой вход исходником `.takt` нельзя: `u12` язык не принимает
    /// (`SE-034`), конструктор типов строит только 8, 16, 32 и 64. Утверждение
    /// "произвольная разрядность бесплатна" относится к отображению, и тест проверяет
    /// его прямо на `TypeNode`.
    #[test]
    ...
    /// `[u8; 4]` отображается в `logic [7:0] data [0:3]`, а не в тип по числу элементов.
    #[test]
    ...
    /// Перечисление отображается в именованный тип с суффиксом `_e`.
    #[test]
    ...
    /// Структура отображается в именованный тип с суффиксом `_t`.
    #[test]
    ...
    /// Пустой тип отображается в `void`: он осмыслен только как тип возврата функции.
    #[test]
    fn unit_maps_to_void() {
        assert_eq!(sv_type(&TypeNode::Unit, "тест").unwrap().prefix, "void");
    }

    /// `float` даёт `SV-003`, а не `real`.
    ///
    /// Молчаливая подстановка `real` дала бы модуль, который симулируется, но не
    /// синтезируется, и `verilator --lint-only` пропустил бы это молча.
    #[test]
    ...
    /// Непокрытый узел даёт `SV-002`, а не молчание.
    ///
    /// Ответ `None` сделал бы "не смогли перевести" неотличимым снаружи от
    /// "отфильтровали как неиспользуемое".
    #[test]
    ...
    /// Массив нулевого размера даёт `SV-004`.
    #[test]
    ...
    /// Целое нулевой разрядности даёт `SV-004`.
    #[test]
