//! Отображение типов Takt в типы SystemVerilog (задача 0045-03).
//!
//! ## Зачем это ядро фичи
//!
...
//!
//! | Takt | C (дефект 0029) | SystemVerilog |
//! |---|---|---|
...
//! = 670, … }` (`elevator.takt:121`) — два варианта, то есть по формуле ADR
...
/// Литерал `value` в размерной форме `<W>'d<value>`, если она нужна (фича 0157).
///
/// `None` — печатать как есть. Так происходит для всего, что укладывается в
/// 32-битное **знаковое** целое (то есть для подавляющего большинства литералов
/// корпуса: их вывод обязан остаться байт-в-байт прежним), а также для
/// отрицательных значений любой величины — проба показала, что их verilator
/// принимает без предупреждений, и знаковое расширение работает верно.
///
/// `None` и тогда, когда у приёмника нет одной скалярной ширины: угадывать её
/// нельзя — форма `32'd…` в 64-битном регистре даёт то же `WIDTHEXPAND`.
...
/// Ширина **скалярного** типа в битах, если она у него есть (фича 0157).
///
/// Нужна печати литерала: нетипизированная десятичная константа в SystemVerilog
/// имеет ширину не меньше 32 бит и **знаковая**, поэтому значение больше
/// `i32::MAX` verilator встречает предупреждением `WIDTHEXPAND` — а гейт цели
/// идёт под `-Wall` и считает его ошибкой. Лечится размерной формой `<W>'d<v>`,
/// где `W` — ширина **приёмника**: собственной ширины литерала недостаточно
/// (проба: `32'd4294967295` в 64-битный регистр — тот же `WIDTHEXPAND`).
///
/// `None` — у типа нет одной скалярной ширины (массив, структура, `real`):
/// печатать размерную форму не по чему, и литерал остаётся как есть.
...
mod tests {
    ...
    /// исходника `.takt` **невыполнима**: проба 2026-07-16 показала, что `u12`
