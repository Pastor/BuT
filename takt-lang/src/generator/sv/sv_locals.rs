//! Локальные переменные тел, содержащие структуру, у цели `sv`.
//!
//! # Форма и как она выбрана
//!
//! Прогон обоих инструментов 2026-08-21 (по **коду возврата**: yosys печатает
//! `путь:строка: ERROR: ...`, и греп `'^ERROR'` объявляет годными негодные формы):
//!
...
//!
//! **Умолчаний два - целиком и по листьям.** Тело вправе писать структуру обоими
//! способами (`whole = make(n);` против `field.lo = ...`), а синтезатор считает
//! незаданной ту форму, которой присваивает тело. Значение одно и то же, поэтому лишнее
//! присваивание нуля смысла не меняет.
//!
//! **Список заполняет сам печатник тел** (`sv_blocks`), а не отдельный обход: второй
//! список того же набора разошёлся бы с первым. Отсюда `RefCell` - тела печатаются по
//! `&Fsm`.
...
/// # Ошибки
///
/// [`SV-002`](sv002), если одно имя объявлено с **разными** типами: молча склеив их,
/// цель подменила бы тип временной. Обход - переименование в исходнике, и текст отказа
/// его называет.
pub(crate) fn hoist(
...
///
/// В SystemVerilog объявления обязаны предшествовать операторам блока: иначе verilator
/// отвечает `syntax error, unexpected automatic`.
pub(crate) fn emit_declarations(p: &mut Printer, hoisted: &HoistedLocals) {
    ...
    for local in hoisted.iter() {
        ...
        // Объявление поглотителя - здесь же: объявления обязаны предшествовать
        // операторам блока.
...
///
/// **Следом печатается поглотитель**: поле структуры, которое тело пишет, но не читает,
/// verilator объявляет неиспользованным (`%Warning-UNUSEDSIGNAL: Bits of signal are not
/// used: 'tmp'[15:8]`), а проверка цели считает предупреждение ошибкой - при нулевом коде
/// возврата `taktc` и при том, что эталон, `c`, `st` и `rust` тот же вход исполняют.
/// Идиома та же, что у неиспользуемого параметра и у сигналов записи обёртки APB:
/// редукция с константой, которую синтезатор выбрасывает сам.
///
/// Поглотитель печатается **безусловно**, и это осознанно: "прочитано ли поле" по
/// напечатанному тексту не спросишь - запись `tmp.hi = ...` упоминает поле ровно так
/// же, как чтение. Признак, ошибающийся в сторону "поглотитель не нужен", давал бы
/// отказ проверки у пользователя, а корпус такого входа не содержит и не поймал бы его.
