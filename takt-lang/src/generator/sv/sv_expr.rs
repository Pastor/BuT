...
mod tests {
    ...
    /// **Уплощение:** преисправленийанный сигнал под-модели получает свою пару `_next`.
    ///
    ...
    /// Вариант перечисления получает преисправление имени перечисления.
    ///
    /// Метки `enum` в SV живут в общем пространстве имён модуля: два перечисления с
    /// вариантом `Idle` без преисправления дали бы повторное объявление.
    #[test]
