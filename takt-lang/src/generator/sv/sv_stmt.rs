...
pub(crate) fn print_statement(
    ...
    match stmt {
        ...
        StatementNode::Variable(name, ty, init, loc) => {
            ...
            if let Some(expr) = init {
                ...
                if let ExpressionNode::Initializer(items) | ExpressionNode::Array(items) = &**expr {
                    // Агрегат раскрывается до листьев общим носителем: прежде раскрытие
                    // было одноуровневым, и `pts := {{1, 2}, {3, 4}};` давало `SV-002`
                    // на записи, которую исполняют эталон, `rust` и `c`.
        ...
        StatementNode::For {
            ...
            // её у инициализатора больше не нужно - заголовок без объявления (`for ; i
            // < 3; i := i + 1`) прежде координаты не получал вовсе.
            crate::generator::site::enter(*loc);
        ...
        // `match` переводится в `case`: в SystemVerilog это прямой аналог, и отказ
        // здесь был пробелом, а не решением - остальные семь потребителей вход
        // исполняли.
        //
        // Ветвь `default` печатается **всегда**: `case` без неё в `always_comb`
        // оставляет сигнал без значения на непокрытом входе, и синтезатор выводит
        // Защёлку - то же, чем обернулась необъявленная переменная цикла в 0321.
        // Молчаливая защёлка хуже отказа.
        ...
        // Формула в теле: Охранная печатается `assert`, темпоральная - предмет `taktc
        // verify`.
        //
        // Прежде ветвь молчала на обеих, хотя охранную цель печатает на уровне модели и
        // состояния с: обязательство автора теряЛОСЬ молча - тот же класс, что закрыла
        // 0035 у семантики. Носитель печати один с формулой-элементом
        // (`sv_blocks::emit_guard`).
        //
        // Отказ печати условия не роняет компиляцию: формула - обязательство, а не
        // поведение, и вход, прежде переводившийся, обязан переводиться.
...
pub(crate) fn loop_variables(stmt: &StatementNode, out: &mut Vec<String>) {
    match stmt {
        ...
        // Обход намеренно не исчерпывающий: пропущенная форма даёт прежний отказ
        // инструмента, а не порчу вывода (приём 0246).
        _ => {}
    }
}

/// Присваивание поглотителя - печатается после объявлений. Имена, которые тело читает
/// Только как индекс массива.
///
...
///
/// Признак по дереву, а не по напечатанному тексту (в отличие от заглушки 0337):
/// объявление поглотителя обязано встать до операторов, то есть до того, как тело
/// напечатано.
///
/// Индекс-Выражение (`a[i + 1]`) под правило не подпадает: там читается всё имя
/// целиком. Граница названа - лишний поглотитель гасил бы честное предупреждение.
...
fn print_expression_statement(
    ...
    match expr {
        ...
        // выразима всегда.
        //
        // Прежняя редакция этого комментария добавляла "...и совпадает с тем, что
        // печатает цель `c`" - замер 0340 это опроверг: цель `c` печатала агрегат как
        // есть, и её вывод не собирался вовсе.
        ExpressionNode::Assign(target, value)
            if matches!(
                value.as_ref(),
...
fn target_type_scalar(target: &ExpressionNode) -> Option<TypeNode> {
    // Элемент массива несёт тип элемента: прежде здесь стоял `None`, и `modes[0] :=
    // Work;` печаталось `modes_next[0] = 1;` - verilator отвечает **ошибкой**
    // `ENUMVALUE` ("Implicit conversion to enum ... Suggest use enum's mnemonic"), при
