...
//! ## Что делает цель
//!
//! Целью `sv` карта адресов не потребляется - MMIO-адрес для RTL бессмыслен. Но
//! осмысленна другая трактовка: породить регистровый файл - модуль с синхронным
//! регистровым интерфейсом, где адрес порта задаёт смещение регистра, а бит - позицию
//! внутри слова. Это парная цель, как `c-hal` парная к `c`.
//!
//! ## Форма интерфейса
//!
//! Интерфейс шинно-агностичный и синхронный: `reg_addr` (адрес), `reg_wdata` (данные
//! записи), `reg_wen` (строб записи), `reg_rdata` (данные чтения, комбинационные).
//! Протокол шины (APB, AXI-Lite, Wishbone) ядро не выбирает: выбор произволен, а цена
//! ошибки высока. Адаптер под конкретный протокол - тонкая обёртка поверх, отдельный
//! файл рядом.
//!
...
//!
//! - бит `out` - регистр автомата, защёлкнутый в его `always_ff`; шина его только
//!   читает, а запись шиной игнорируется, иначе вышел бы конфликт драйверов;
//! - бит `in` - регистр, записываемый шиной и читаемый автоматом; чтение шиной
//!   возвращает записанное.
//!
//! ## Ширина данных - по старшему занятому биту; предел - 64
//!
//! Ширина слова данных ([`Mmio::data_width`]) - максимум `bit + width` по портам модели,
//! а не фиксированные 64: лишние старшие биты `reg_wdata` повисли бы как `UNUSEDSIGNAL`
//! у `verilator -Wall`, а глушить его `lint_off` в проекте запрещено. Жёсткий предел -
//! 64 ([`MAX_REG_WIDTH`]): `SE-060` держит бит в `[0, 63]`, то есть регистр не шире
//! `uint64_t` - того же слова, что читает умолчательный HAL цели `c-hal`. Порт занимает
...
pub(crate) struct Mmio {
    ...
    ports: Vec<MmioPort>,
    /// Ширина `reg_addr` в битах - по максимальному адресу модели.
    addr_width: u32,
...
impl Mmio {
    ...
    ///
    /// Ширина слова считается по всем портам, а `reg_wdata` читают лишь входные: биты
    /// `out` запись игнорируют. Поэтому у модели, где входной порт уже выходного,
    /// старшие биты входа висят, и `verilator -Wall` отвечает `UNUSEDSIGNAL: Bits of
    /// signal are not used` при нулевом коде возврата `taktc`.
    ///
...
fn reset_literal(ty: &TypeNode, enums: &BTreeMap<String, Vec<(String, i128)>>) -> String {
    match ty {
        TypeNode::Enum(name) => enums
            ...
            // Перечисления без вариантов не бывает (`SE-105`): ветвь защитная.
...
pub(crate) fn emit_register_file(p: &mut Printer, mmio: &Mmio) {
    ...
    if !inputs.is_empty() {
        ...
        // биты `in` - биты `out` запись игнорируют.
    ...
    // и `out` (регистр автомата), и `in` (чтение возвращает записанное).
