...
//!
//! Строки собирает общий носитель [`generator::table`](crate::generator::table) - он же
//! обслуживает `c`, `rust` и `st`. Форма же здесь своя, и каждая её черта - вывод из
//! прогона, а не вкус:
//!
...
//!   yosys отвергает синтаксически** - тот самый случай, ради которого проверка
//!   держит два инструмента. Тот же плоский вектор цель уже
//!   печатает у массива в параметре функции;
...
//!
//! **Служебные признаки объявляются сигналами модуля**, а не `automatic` внутри блока:
//! диспетчер под-модели печатается **в глубине** ветви `case` родителя (композиция у
//! цели уплощена), а объявления в SV стоят в начале блока - `automatic` там оказался бы
//! после операторов.
//!
//! **Момент проверки совпадает с формой `unique case`.** Готовность композиции читается
//! по `_next`, готовность цепочки - по регистру шага, который в комбинационном блоке не
//! меняется. Поэтому диспетчер в конце блока видит ровно те значения, при которых
//! переход печатает форма по умолчанию.
...
///
/// Зовётся после перечислений состояний: вектор ссылается на их варианты (порядок
/// разделов файла - ).
...
///
/// Без них yosys объявляет признак **защёлкой**: диспетчер под-модели стоит внутри
/// ветви `case` родителя, и на прочих ветвях присваивания нет. Verilator при этом
/// модуль принимает - снова тот случай, ради которого проверка держит два инструмента.
...
///
/// `ready` - предикаты готовности реализаций состояний, собранные при печати тел: их
/// значение цель вычисляет по ходу инлайна композиции, и второго знания о раскладке её
/// регистров здесь не заводится.
