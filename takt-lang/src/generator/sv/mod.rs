...
/// (суффикс имени файла и его текст).
///
...
fn generate_program(
    ...
    // Режим `sv-mmio`: порт с адресом становится битом регистрового файла, порт без
    // адреса - портом модуля. Карта адресов уже разрешена (`resolve_addresses` с
    // приоритетом inline < `address` < внешняя) и передана в `options.address_map`
    // библиотечной обёрткой `compile_to_sv_mmio` - как у `c-hal`. В режиме `sv` карта
    // пуста, и всё ниже вырождается в вывод без регистрового файла. Анонимные ячейки
    // собираются с корня: в регистровый файл они входят наравне с адресованными портами.
    let anon_cells = map
        .root_model_node()
    ...
    // Пользовательские типы объявляются вне модуля: порт может иметь структурный тип, а
    // шапка модуля печатается раньше тела, и verilator отвечает "Reference to 'pair_t'
    // before declaration". Форма проверена обоими инструментами: `typedef` на уровне
    // файла принимают и verilator, и yosys.
    //
    // Перечисления печатаются перед структурами: поле перечислимого типа делает
    // структуру зависимой от `mode_e`, а обратной зависимости не бывает - варианты
    // перечисления суть литералы. Обратный порядок даёт "Reference to 'mode_e' before
    // declaration" при нулевом коде возврата `taktc`.
    ...
    // Предупреждения генератора - часть результата, а не печать из библиотеки: печать
    // прошла бы мимо `--quiet` и мимо общего формата.
...
mod tests {
    ...
    /// Порты модели попадают в заголовок, и `bit` отображается в `logic`, а не в целое:
    /// один провод занимает один бит.
    #[test]
    ...
    // построения дерева, поэтому переменная печатается без префикса модели и проба
    ...
    /// Вложенная `+` внутри параллельного шага переводится.
    ///
