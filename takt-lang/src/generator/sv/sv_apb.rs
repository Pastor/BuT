...
//! # Что это
//!
//! Ядро `sv-mmio` порождает **шинно-агностичный** регистровый файл: `reg_addr` /
//! `reg_wdata` / `reg_wen` / `reg_rdata`. Протокол в него не зашит намеренно - выбор
//! диктует платформа. Здесь живёт первый адаптер, выбранный: **AMBA APB3**, сторона
//! slave.
//!
//! Адаптер - **отдельный модуль-обёртка** `<name>_apb`, инстанцирующий ядро. Ядро при
...
/// бессмыслен, сигнал приходит на вывод кристалла, а не по адресу.
///
/// Текст **отдельный**, хотя код тот же: причина иная, и сообщение "у модели нет портов
...
///
/// Ширины зависят от модели (`[0:0]` у однобитной шины данных, `[10:0]` у адреса),
/// поэтому колонка имени и колонка комментария вычисляются, а не набиваются пробелами
/// вручную: иначе шапка модуля разъезжается на каждой второй модели.
fn port_line(
    dir: &str,
    range: &str,
    name: &str,
...
pub(crate) fn generate_apb(core: &str, mmio: &Mmio) -> Result<String, Diagnostic> {
    ...
    // Колонки: диапазон - по самой широкой шине, комментарий - по самому длинному
    // объявлению. Считаются, а не задаются константой.
    ...
    //
    // у ядра без записываемых регистров нет входов `reg_wdata` и `reg_wen` - заводить
    // провода к несуществующим выводам нельзя (`PINNOTFOUND`), да и незачем.
    ...
    if writable {
        ...
        // Ядро доступно шине только на чтение: цикл записи APB завершается штатно
        // (`pready` уже 1), но данные никуда не идут. Сигналы шины остаются в
        // интерфейсе - их состав задан протоколом, а не моделью, - и здесь честно
        // поглощаются, чтобы `verilator -Wall` не сообщал о висящем входе (глушить его
        // `lint_off` правило проекта запрещает).
