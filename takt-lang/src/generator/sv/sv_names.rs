...
//
// Граница модуля - ответственность: `sv_fsm` отвечает за автомат, а имя сигнала есть
// правило именования, и оно живёт здесь вместе с прочими (`signal_of`, `const_signal`).
...
/// префикс модели и писала в её сигнал. Список локальных имён снимает двусмысленность
...
/// который один знает про префикса уплощения.
pub(crate) fn signal_of(var: &std::rc::Rc<std::cell::RefCell<VariableNode>>) -> Option<String> {
    match &*var.borrow() {
        // Переменная модели получает префикс владельца, а не "модели, которую сейчас
        ...
        VariableNode::Simple { upper, name, .. } => {
            let Some(owner) = upper.as_ref().and_then(|u| u.upgrade()) else {
                ...
                // функции: у неё нет ни регистра, ни префикса.
                return Some(name.clone());
            };
        ...
        // префикс ей не нужен и только мешал бы читать - **кроме** константы,
...
/// Имя `localparam` константы - **с префиксом владельца**.
///
