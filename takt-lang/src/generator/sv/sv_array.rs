...
/// (замер: verilator по-прежнему отвечал `WIDTHTRUNC`).
///
...
/// Листья типа: суфисправление и тип каждого скалярного места.
///
...
/// умолчании массива целиком объявляет защёлкой ("Latch inferred" - замер 2026-08-21).
...
/// whole-array (проверено прогоном), а структура вне массива - упакованная, и её
...
pub(crate) fn leaf_zero_defaults(
    ...
    // присваиванием не сходится - yosys объявляет защёлку (прогон 2026-08-21). Правило
    ...
    if !needs_leafwise(ty, fields_of) {
        ...
        // тело (прогон 2026-08-21). Значение одно и то же, поэтому лишним присваиванием
        // смысл не меняется.
...
///
/// Пустой вектор - регистр сбрасывается целиком, как прежде. Значения листьев берутся у
...
pub(crate) struct FlatParam {
    ...
    pub(crate) width: u32,
    /// Суфисправление пути к части, её ширина и тип - от младших разрядов к старшим.
    pub(crate) parts: Vec<(String, u32, TypeNode)>,
}

/// Раскладка массива в плоский вектор, если она у типа есть.
///
/// `None` - тип не распакованный массив (бит-вектор 0078 передаётся как есть) либо у
/// какой-то части нет ширины: угадывать её нельзя, и такой параметр печатается прежним
/// путём.
///
/// **Спуск останавливается на упакованном типе, и это замер, а не вкус.** Первая
/// редакция раскладывала структуру по полям, и yosys отвечал "Latch inferred for signal
/// `...a[0].lo`": запись полей элемента внутри `always_comb` он полным присваиванием не
/// считает (тот же класс, что 0367). Присваивание структуры целиком принимают **оба**
/// инструмента - проба 2026-08-21.
