...
//!
//! ## Сброс: вход в стартовое состояние не расходует такт
//!
//! Синтетического `INIT`-состояния нет вовсе - его роль исполняет цепь сброса: ветвь
//! `if (!rst_n)` кладёт в регистры стартовые состояния всех уровней одним фронтом.
//! Поэтому первый же такт после снятия `rst_n` исполняет тело стартового состояния, и
//! сдвиг равен нулю на любой глубине; у цели `c`, где уровни входят последовательно, то
//! же правило приходится держать особой ветвью диспетчера.
//!
...
//!
//! Логика под-моделей печатается в общий `always_comb` в порядке вызова такта и
//! внутри ветви `case` родителя: они работают, только пока родитель в этом
//! состоянии. Вынос наружу изменил бы модель - под-модели продолжали бы работать
...
//! `(<sub>_state_next == <SUB>_END)`. Чтение регистра дало бы значение предыдущего
...
pub(crate) struct Fsm {
    ...
    /// отдаёт их вызывающему - печатает CLI. Печать `eprintln!` прямо из библиотеки не
    /// глушится `--quiet` и расходится с общим форматом.
...
///
/// У корня - просто `state` (форма ); у под-модели - с префиксом её уникального имени,
...
impl Fsm {
    ...
    pub(crate) fn build(
        ...
        // Ветвь сброса - то самое "до первого такта": у цели `sv` иного места нет, там
        // же живут стартовые состояния.
...
pub(crate) fn emit_ff(
    ...
    p.down();
    // `en` ограничивает только защёлкивание: сброс выше проверяется первым и от `en` не
    // зависит - иначе модуль нельзя сбросить, пока он не разрешён. Неподключённый порт
    // даёт `en == 1`, то есть шаг каждый такт.
