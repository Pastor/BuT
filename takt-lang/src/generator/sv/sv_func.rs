...
pub(crate) fn emit_functions(
    ...
    for (_, model_rc) in blocks {
        ...
        for func in model.functions.values() {
            ...
            // и распакованная размерность потеряется: тело присвоит массив скалярному
            // результату, verilator ответит "Illegal assignment", yosys - "Insufficient
            // number of array indices", всё при нулевом коде возврата `taktc`. Форма
            // выбрана прогоном обоих инструментов: возврат именованного типа принимают
            // оба, а безымянную размерность в заголовке функции - ни один.
            ...
            p.print(&body_text);
            // Присваивание поглотителя частично прочитанного параметра идёт после тела:
            // чтение до записи verilator встречает `ALWCOMBORDER`.
            for param in &narrow_params {
