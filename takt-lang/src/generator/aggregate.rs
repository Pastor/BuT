...
pub(crate) struct Place {
    /// Суфисправление к базе: `[0]` для массива, `.x` для поля структуры.
...
/// Индекс из суфисправления вида `[N]`, который построил [`places`].
...
/// многомерный массив в IEC адресуется одной парой скобок (`[i, j]`), и общий суфисправление
/// там был бы неверен.
