...
/// Публичная точка входа цели `sv-mmio` - вынесена из `lib.rs` (лимит размера);
/// реэкспортируется как `takt_lang::compile_to_sv_mmio`.
mod compile_sv_mmio;
...
///
/// Ниша та же - прошивка МК, - но дефекты отображения цели `c` здесь не воспроизводятся
/// конструктивно: `[u8;4]` -> `[u8; 4]` (а не `uint4_t`, дефект [0029]), `bit` ->
/// `bool` (а не `int`), `Rational` -> `f64` (а не `float`, что делает сверку с
/// симулятором достижимой). `void *userdata` заменён параметром типа `H: Hal`, а
/// `unsafe` в порождаемом коде запрещён атрибутом `#![forbid(unsafe_code)]`.
///
/// ## Границы
///
...
///
/// [0029]:
/// https://github.com/Pastor/Takt/blob/main/docs/features/0029-c-type-mapping.md
...
///
/// Часть дефектов отображения цели `c` здесь не воспроизводится конструктивно: `bit` ->
/// `logic` (а не `int`, дефект [0029]), а синтетического `INIT`-состояния нет вовсе -
/// стартовое состояние стоит в ветви сброса, поэтому сдвиг такта равен нулю на любой
/// глубине вложенности (контракт []) без единой правки.
///
/// ## Границы
///
...
/// [0029]:
/// https://github.com/Pastor/Takt/blob/main/docs/features/0029-c-type-mapping.md []:
/// https://github.com/Pastor/Takt/blob/main/../../docs/features/0033-init-tick-alignment.md#архитектура-adr
...
/// + 0068: проверяет LTL-свойство модели методом model checking.
///
...
/// молчаливое "ложно".
///
/// Условия переходов абстрагированы (любой `ref` - возможный переход), а данные в ядре
/// 0068 полностью недетерминированы, поэтому [`Verdict::Holds`] надёжен, а
...
/// сигнатуры.
///
/// Порядок результатов детерминирован (обход `BTreeMap` моделей - проверка 0048).
pub fn verify_all(
    model: std::rc::Rc<std::cell::RefCell<semantic::ModelNode>>,
) -> Vec<PropertyResult> {
...
/// /0051: то же, что [`verify_all`], но с явной областью и трассой.
///
...
fn verify_all_inner(
    ...
    drop(borrowed);
    for (key, m) in nested {
        // R3: у импортированного узла поддерево отсекается целиком. Проверять `origin`
