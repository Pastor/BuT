...
///
/// Без него правка, сводящая к операнду всё подряд, прошла бы тесты выше и сломала
...
///
/// Утверждение "поля структур не транслируются, отказ
...
start Main = M;",
    ...
    // ровно то, что тест обязан фиксировать.
    assert_eq!(
        code, "RS-020",
