...
//! его поднимает во временную свой проход. Разворот, этого не различающий, ломает
//! разворот у уже работающего входа - `iec2c` отвечает "';' missing" на `got[0] := pair(k)[0];`.
...
/// макрос верен, и бит-вектор (упакованный скаляр) в объём не входит.
#[test]
