...
///
/// Имя, взятое только из фикстуры, держало бы весь проект на
...
/// Скалярные типы отображаются по нормативной таблице.
///
...
///
/// Закреплять здесь `ST-004` ("локация IEC адресует скаляр") нельзя: это было бы
...
///
/// Диагностика приходит из слоя адресов **без изменений** - переиспользование, а не
...
/// срабатывание сломало бы валидную модель (`remaining` - имя переменной примера,
/// `left`).
#[test]
