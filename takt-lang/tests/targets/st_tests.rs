...
/// Компилирует исправлениетуру в ST и возвращает текст порождённого файла.
...
/// Каталог вывода теста - уникальный **по тесту**, а не по исправлениетуре.
///
/// Прежде имя строилось только из исправлениетуры, и это держало весь проект на
...
/// режиме имени нет (тест идёт в главном потоке), и ключом остаётся исправлениетура - там
/// гонок и не бывает.
...
///
/// Не перечислимым типом IEC: MatIEC отвергает явные значения вариантов (проба П4).
...
///
/// Исправлениетура называется `sequence`, а не `concat`, намеренно: имя модели берётся из
...
///
/// Пара с тестом выше: цели **асимметричны**, и это исправлениеируется с обеих сторон.
#[test]
...
///
/// Заодно исправлениеируется `SE-050`: карта перекрывает inline-адрес `btn` - наложение по
/// замыслу.
#[test]
...
/// **T1/A1, T2/A2 - тест исправления [](../fixes/-st-fn-dedup-silent.md), Tier 1.**
///
/// Две модели с одноимённой `fn helper`, но разными телами (`x+1` и `x+2`). До
/// преисправленияции цель `st` дедуплицировала их по голому имени: эмитилась **одна**
...
///
/// Ожидаемое (эталон - цель `c`): **две разные** функции с преисправлением модели- владельца,
...
fn test_st_same_named_fns_get_distinct_prefixed_functions() {
    ...
    // Голого `helper` (без преисправления) не осталось - склейки нет.
    assert!(
...
///
/// Нужен с: комментарии автора модели переносятся в вывод, и исправлениетура, объясняющая
...
/// она красна из-за собственного пояснения (тот же класс, что у сверок исправлениетур
/// `conformance_fixed_*`).
