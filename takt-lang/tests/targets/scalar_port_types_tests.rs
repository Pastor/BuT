...
///
/// Эту ветвь носитель 0488 пропустил, и `st-at` отвечал `ST-004` там, где прочие семь
...
///
/// Первая редакция контроля ждала отказа `RS-016`/`ST-004` на массиве и структуре - и
...
/// `[bit;N>64]`), проверяет модульный тест `rust_port`. Здесь тестытся другое:
/// перечисление не сломало разворот - обе формы переводятся, как и прежде.
#[test]
