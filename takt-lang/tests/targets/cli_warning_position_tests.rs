...
//!    именно это.
//! 3. **Форма преисправления та же, что у ошибки** - формат диагностики есть её
...
/// Файл с семантической ошибкой (`SE-034`) - эталон формы преисправления.
const BROKEN: &str = "\
model M {
    var v: NoSuchType := 1;
    start S {
...
/// Разбирает преисправление `путь:строка:колонка: ` в тройку.
///
/// Возвращает `None`, если преисправления нет вовсе - ровно то состояние, в котором
...
///
/// Проверяются все три составляющие преисправления: путь называет файл, строка и колонка
...
/// **Форма преисправления - та же, что у ошибки.**
///
...
/// на тест 0081 (тот гоняет цель `c`).
#[test]
