...
/// **T1.** Ни один пример корпуса не даёт двух `break;` подряд.
///
...
/// **T2.** Ребро, записанное после безусловного, целью `c` не печатается.
///
...
/// **T3.** У условного ребра `break;` остаётся - он выходит из `switch`
...
/// **T4.** Терминальное состояние по-прежнему уходит в `_END` и
