...
//! объявлений операндов: тащить координату через сорок вариантов узла ради
...
//! строку объявления - чужая верная координата хуже отсутствующей.
//!
...
/// Отказ указывает на строку употребления, а не объявления.
#[test]
...
/// Отказ вне оператора берёт свою координату, а не чужую.
///
...
/// ложь, от которой уходит носитель позиции.
#[test]
...
/// Позиция не "залипает": следующий оператор объявляет своё место.
///
...
/// Слой объявления снимается: тело чужой координаты не занимает.
///
...
/// Отказ о ячейке называет место обращения.
///
...
/// Заголовок цикла без инициализатора называет своё место.
///
/// Позиция у `StatementNode::For` своя: взятая у объявления в
...
/// Формула в теле блока называет своё место.
///
