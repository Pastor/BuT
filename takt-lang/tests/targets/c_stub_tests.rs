...
//! значения, затем они зафиксированы здесь.
...
/// Непереводимое условие -> ошибка, а не "успех" с мёртвым автоматом.
#[test]
...
/// Сообщение называет **целевое состояние** - иначе в модели с десятком
...
/// Причина сохранена: и код исходной диагностики, и её текст.
///
...
/// Позиция указывает на `ref` в **исходнике**, а не на `Codegen`.
///
...
/// При ошибке артефакты не остаются на диске.
///
...
/// Контрпример: переводимое условие компилируется, переходы на
...
/// Цель `c-hal` даёт **тот же** отказ: обе цели делят
...
/// Защита корпуса от возврата заглушек.
///
