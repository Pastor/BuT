...
//!
//!
//! **Разворот стал возможен после трёх смежных починок**, каждая из которых нашлась
//! прогоном инструментов целей, а не чтением кода: 0409 (локальный массив в аргументе у
//! `st`), 0410 (лишний `mut` и `needless_late_init` у `rust`), 0411 (`E0381` на
...
///
/// Первая редакция брала `#slice1`, и `cc` отвечал "expected identifier", а `iec2c` -
/// "invalid variable(s) declaration": имя, невыразимое в цели, превращает починку в
/// новый отказ.
#[test]
