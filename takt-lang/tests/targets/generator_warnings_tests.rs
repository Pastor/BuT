...
/// Цель `st` **возвращает** предупреждение об охранной формуле.
#[test]
...
/// Заглушка внешней функции - тоже возвращается.
#[test]
...
/// Предупреждение **тела блока** тоже доезжает.
///
...
/// Цель `sv` возвращает предупреждение о переменном делителе.
#[test]
...
/// У `sv-mmio` больше **одна** судьба у всей диагностики - предупреждение
...
/// `--quiet` глушит предупреждение **цели**, как и предупреждение компилятора.
#[test]
...
/// Без флага предупреждение печатается - и **общим** форматом, тем же, каким
