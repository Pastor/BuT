...
///
/// Возведение в степень здесь не годится: его переводят обе цели, и проверка теряет
...
///
/// Фикстура **пользуется** `res`: неиспользуемое объявление штатный фильтр выбрасывает
/// из вывода, и тест, мерящий отсутствие отказа вместо
/// его координаты.
#[test]
