...
/// Строит модель фикстуры и разрешает адреса против внешней карты (текст) и `--define`.
...
// -- Библиотечный уровень: печать формата map --------------------------------
...
// -- Библиотечный уровень: круговой рейс --------------------------------------
...
// -- Библиотечный уровень: формат json ----------------------------------------
...
// -- Библиотечный уровень: мёртвый порт  ---------------------------------

/// Порт без адреса. В `map` записи нет и `0x0` **не появляется** (главный
...
// -- Библиотечный уровень: плоский ключ  ---------------------------------

/// Одноимённые порты разных под-моделей в **плоской** выгрузке
...
/// круговой рейс `.ld` перестал бы быть тождеством. Тест фиксирует эту
/// границу формата.
#[test]
fn flat_key_collision_last_wins() {
    let res = resolve_fixture("collide.takt", None, &[]);
    // Карта с квалифицированными ключами содержит оба порта - это и есть исправление:
    assert_eq!(
...
/// Достижимый порт без адреса -> `SE-052`, ненулевой код, нет выгрузки.
#[test]
...
/// Неизвестный формат `--emit svd` -> ошибка, ненулевой код, упоминание что SVD
