...
/// **T2 - главная проверка: порождённый C компилируется.**
///
...
///
/// Тест живой: он вызывает настоящий `cc`. Снапшот тут бесполезен - он заисправлениеировал бы
...
/// A9: порождённый C для композиции `f -> g` внутри одной модели собирается `cc
...
/// A14: C для `invariant P = C;` идентичен C для пары `cond P = C; : [Guard] P;` -
