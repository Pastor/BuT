...
pub(crate) enum Touch {
    ...
    ///
    /// Её область - прогоны из этого состояния: верификатор десахаризует
    ...
    /// этого состояния, и проверка обязана стоять в его ветви. Стой она в
    ...
    ///
    /// Без общего печатника этот вход даёт пять ответов, а у цели `c` - ссылку на неопределённый
    ...
    LoopForNoInit,
    /// Цикл `while` - синоним `loop` с условием.
    LoopWhile,
...
impl Touch {
    ...
    fn declarations(self, kind: Kind) -> String {
        match self {
            ...
            // (контракт частоты, `SE-069` и `SE-070`).
...
pub(crate) fn extra_flags(touch: Touch) -> Vec<String> {
    match touch {
        ...
        // `SE-069` и `SE-070`. Это часть случая, а не настройка.
