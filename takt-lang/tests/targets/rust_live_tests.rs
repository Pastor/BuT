...
//! (политика цели) означают **отказ сборки** порождённого кода при нулевом коде
...
/// сравнений.
///
/// Без вердикта здесь печатается `let mut t: u8 = 0;` - мёртвое значение, то есть
...
///
/// Без него печатается `let mut i: u8 = 0;` перед `{ i = 0; while ... }` - то же
/// `unused_assignments`.
#[test]
