...
//
// Литерал шире `i64` ронял бы компилятор: разбор шёл бы через `unwrap()`
...
fn u64_max_literal_lexes_after_0157() {
    // Носитель `i64` дал бы здесь `LE-009` - значение в него не влезает.
    assert!(
...
fn out_of_range_literal_does_not_cascade_into_syntax_error() {
    // Одна причина - одна ошибка. Иначе токен "исчезает", парсер
...
//
// Показатель степени не отбрасывается ни у одного вида литерала: границы
...
fn rational_exponent_is_kept_in_text() {
    ...
    // Срез, оборванный перед показателем, дал бы для `2.5e3` значение 2.5.
