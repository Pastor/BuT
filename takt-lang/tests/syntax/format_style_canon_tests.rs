...
fn comment_between_brace_and_else_survives() {
    ...
    // Место комментария наследуется от прежнего поведения; тестым сохранность и
...
// комментариев. Теста исправлениетурные по той же причине - хвостовых комментариев у
...
fn empty_struct_keeps_its_comment() {
    ...
    // перечисления, - и печатается прежними двумя строками; тестым ровно то, что
