...
//!
//! Форматтер печатает **всё** дерево, поэтому его нельзя проверить парой фикстур: любой
...
//! - **семантическая нейтральность:** `parse(fmt(x))` структурно равен
...
//! Список каталогов данных обязан быть полным: пропавший каталог
...
fn a1_a3_corpus_report() {
    ...
    for path in &files {
        ...
        // Невалидные фикстуры (`data/semantic/invalid/`) могут не разбираться - это не
        // дефект форматтера.
    ...
    //
    // Список **пуст** - замер: на 279 файлах корпуса непокрытых узлов
...
/// дошёл до фикстур `takt-sim/tests/data/`. Инвариант не должен зависеть от того, лежит
...
fn fmt_rewrites_file_in_place() {
    ...
    // Повторный прогон ничего не меняет и остаётся каноничным - через CLI.
...
fn inline_formula_guard_and_ltl_are_printed() {
    ...
    // форму. Это нормализация раскладки, а не смена смысла: сверка `parse(fmt(x)) ==
