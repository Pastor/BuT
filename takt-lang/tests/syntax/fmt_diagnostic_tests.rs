...
//!
//! Со склейкой `Vec<Diagnostic>` в строку **внутри** библиотеки `format_source`
...
/// Отказ `fmt` печатается тем же текстом, что и
...
/// Две ошибки разбора дают **две** строки, а не одну.
///
/// Разбор накапливает диагностики; печать вектора одним `Debug`-блоком отдаёт
...
/// Внутреннее представление наружу не выходит.
///
...
/// Коды возврата не изменились: отказ - это `1`.
///
...
/// Узел `formula` печатается, а не отвергается.
///
...
/// У `--stdin` пути нет, поэтому нет и префикса позиции - но код и
/// текст на месте.
///
