...
/// этого исправлениетура могла бы незаметно стать корректной, и тест проверял бы не то, что
/// заявлено.
#[test]
...
/// `--stdin`: предупреждение печатается, пути нет - преисправление позиции не выдумывается (то
