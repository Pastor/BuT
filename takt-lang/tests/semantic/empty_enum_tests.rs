...
/// Перечисление без вариантов даёт `SE-105`.
#[test]
...
/// Позиция - объявление перечисления (его имя), а не закрывающая скобка.
#[test]
...
/// Текст называет имя, правило и способ исправить.
#[test]
...
/// Накопление - каждое пустое перечисление высказывается само за себя, включая
...
/// При использовании пустого перечисления автор видит **обе** записи - `SE-105`
...
/// Мусор внутри скобок по-прежнему отвергается **разбором** - ветвь восстановления
...
/// Форматтер печатает пустую форму и делает это **идемпотентно** - запись доезжает
...
/// Диагностика доезжает до **редактора** - тем же входом `validate_model_all`,
...
/// Перечисление с единственным вариантом законно, и `SE-105` на нём
/// не возникает.
#[test]
