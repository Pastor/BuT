...
/// Составное имя состояния даёт C, который собирает `cc`.
///
/// Одно имя, две функции.
#[test]
...
/// Порт-тёзка состояния переводится целью `c`, и вывод собирает `cc`.
///
/// Отказ на эту форму (`CC-022`) снят решением проекта:
...
/// Состояние-тёзка дочерней модели отвергается **семантикой**,
...
/// Состояние-тёзка **соседней** модели законно и переводится.
///
/// Граница правила, а не перестраховка: проверка без этого условия
