...
//!
//! Отдельно - параметр там, где нужна величина, известная при генерации (`after
...
// --- Контр-примеры (параметр остаётся переменной) ------------------------

/// Присваивание из **вложенного** оператора: параметр - поле, не константа.
///
/// Тот самый пропуск, которого требует контрпример: разбор, не спускающийся в тело
...
/// требование "вывод корпуса байт-в-байт", из-за которого квалификацию тогда получили
/// **только** константы-параметры. Исключение снято: имя константы
...
///
/// квалифицировано владельцем у всех целей.
#[test]
...
// --- Параметр в позиции compile-time величины -----------------------
