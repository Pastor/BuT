...
/// `q(8, 8) sat` разбирается и даёт формат с признаком насыщения.
#[test]
...
/// Умолчание не изменилось - `q(8, 8)` по-прежнему перенос.
#[test]
...
/// Слово, отличное от `sat`, отвергается `SE-104`.
///
...
/// Смешение `sat` и не-`sat` - ошибка (правило "формат обязан совпасть").
///
/// Код - существующий `SE-059`, а не новый: правило одно, и разделение его надвое дало
/// бы два места, обязанные давать один вердикт.
#[test]
...
/// Явное приведение к насыщающему формату законно.
#[test]
