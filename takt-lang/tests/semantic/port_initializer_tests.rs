...
/// Голый адрес на `bit`-порту не даёт `SE-035`.
#[test]
...
/// `bit := 0` / `bit := 1` на порту валидны (адрес 0/1).
#[test]
...
/// `bit`-порт и `u8`-порт с голым адресом принимаются одинаково (устранена
/// асимметрия u8/bit).
#[test]
...
/// Контрпример - не-порт `var: bit := N` (N∉{0,1}) сохраняет `SE-035`.
#[test]
...
/// Контрпример - `const: bit := N` (N∉{0,1}) тоже сохраняет `SE-035`.
#[test]
