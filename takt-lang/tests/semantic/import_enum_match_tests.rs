...
//!
//!
//! **Поэтому здесь стоит тест, а не правка.** Корпус класс не покрывает:
...
/// Выборочный импорт: обе ветви `match` доезжают до вывода **разными**
/// номерами вариантов.
#[test]
...
/// Импорт целым файлом - та же запись, другой путь переноса.
#[test]
...
/// **Контр-пример.** Несуществующий вариант - по-прежнему ошибка.
///
/// Без него тест доказывал бы лишь то, что образец **что-нибудь** принимает:
/// разрешение "всякого имени в образце" прошло бы соседние тесты.
#[test]
...
/// Условие ребра на том же варианте - путь, который карточка называла
/// работающим.
///
