...
/// Индексация не-массива: код `SE-117` и позиция имени.
#[test]
...
/// Локальная функция без тела: код `SE-118` и позиция объявления.
#[test]
...
/// **Устройство: конверсии `From<&str> for Diagnostic` больше нет.**
///
...
/// **Устройство: в семантике не осталось `Err(\"...\".into())`.**
///
/// Второй слой: конверсию могут вернуть под другим именем, а этот тест смотрит на
