...
/// **Перечисление стадий в проекте одно.**
///
...
/// **`SE-068` не зависит от того, чей это файл.**
///
...
/// **`SE-067` ловит конфликт частот через границу импорта.**
///
...
/// **`clock` подключённого файла - контракт всей сборки.**
///
...
/// **`specialize` на импортированной модели без аргументов даёт валидный C.**
///
/// Проверяется не факт компиляции `taktc` (он возвращает ноль и на дефекте), а приёмка
/// вывода **чужим** инструментом: без правки `cc` отвечает "no member named 'limit' in
/// 'struct ...Tuner'".
#[test]
...
/// **Специализация импортированной модели с аргументами работает.**
///
/// Отказ `SE-120` здесь закреплять нельзя - это утверждение неверно:
