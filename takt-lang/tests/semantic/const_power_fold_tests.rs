...
/// (неиспользуемое объявление цели не печатают - названная ловушка).
...
/// Значение нормируется по типу приёмника - обёрткой переполнения.
///
...
/// каким проверяется константная выдержка). Сверка трасс на одном входе такого
/// утверждения не даёт.
#[test]
