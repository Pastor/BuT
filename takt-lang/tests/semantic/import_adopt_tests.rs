...
/// **Импортированное объявление - переменная корня, а не под-модели.**
///
...
/// **Частичный импорт не молчит.** Модель импортирована без объявлений, на
...
/// **Область видимости не расширилась.** Переменная импортёра библиотечной
...
/// **Импорт файла целиком не отказывает.** Без усыновления цель `c` падала
