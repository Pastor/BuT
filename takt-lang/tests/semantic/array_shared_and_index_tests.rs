...
/// предупреждения идут отдельным каналом. Первая редакция теста звала его - и проходила
...
/// A5 (тест класса): порождённый Rust с переменной-индексом **компилируется**.
///
/// Это не косметика, а починка другой цели. До фичи переменная, использованная только
