...
//! тестыла **счётом**. Тест T3 проверяет это напрямую.
...
/// значило бы тестыть устройство вместо ответа пользователю. Цель `st` выдаёт по
...
/// них - и счёт, которым тестыт, это поймал бы, а координата нет. Теперь позиции
