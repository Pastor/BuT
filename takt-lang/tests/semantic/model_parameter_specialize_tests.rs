...
//! конвейеру идёт обычная модель. Проверяется **текст** вывода (критерий A5 анализа:
...
fn different_values_produce_different_models() {
    ...
    // после `_init` нет. `gain` в исправлениетуре не присваивают, поэтому выводит его
    // константой - макросом на специализацию (прежде тест ждал здесь `model->gain =
...
/// **число**, критерий A5 - иначе тест прошёл бы и без дедупа).
#[test]
...
fn sv_accepts_different_sets_via_specialization() {
    ...
    // **свой** `localparam` (прежде это были регистры со своими ветвями сброса:
...
/// в дереве (`CC-004`, урок adopt 0184).
#[test]
