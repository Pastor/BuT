...
/// Ссылка на константу-длительность протягивает тип.
#[test]
...
/// Порядок имён не важен: источник может стоять **позже** по алфавиту.
///
...
/// Цепочка длины три сходится целиком.
#[test]
...
/// Обычная переменная - тот же случай, что константа.
#[test]
...
/// Арифметика длительностей - вторая причина, независимая от ссылок.
///
...
/// Таблица доезжает до **рекурсивных** ветвей вывода.
///
...
/// Взаимная ссылка не зацикливает вывод.
///
...
/// Порядок трёх шагов цел: свёртка по-прежнему после вывода типов.
///
...
/// Форма из фикстуры переводится **всеми** целями.
///
...
/// Тип доезжает до объявления в порождённом C.
///
