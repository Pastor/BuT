...
/// **R1:** цель `sv` отвергает не-ASCII имя своей диагностикой.
#[test]
...
/// **R4:** цели `c` и `rust` тот же вход **переводят**.
///
...
/// **R4:** вывод цели `c` на не-ASCII именах принимается настоящим `cc`.
///
...
/// **R3:** отказ приходит на **каждый** вид объявления, а не только на порт.
///
/// Условие живёт в самой проверке имени, а не в её вызовах - иначе осталась бы дыра,
/// как у направления порта до 0188. Тест держит это свойство: каждая фикстура несёт
