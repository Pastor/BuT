...
/// Файл с тремя ошибками разбора даёт **три** сообщения, а не первое.
#[test]
...
/// Сообщения идут по возрастанию позиции в тексте.
///
...
/// Два прогона дают побайтно один и тот же список.
#[test]
...
/// Точный повтор не доходит до пользователя дважды.
#[test]
...
/// Проверки высказываются все, а не только первая.
///
...
/// Ошибка вложенной модели попадает в тот же список.
#[test]
...
/// Диагностика несёт **реальную** позицию, а не начало файла.
///
...
/// Языковой сервер и компилятор сообщают об одних и тех же ошибках.
#[cfg(feature = "lsp")]
#[test]
