...
//! теле (признак `mutated`), даёт домен из одного значения - своего умолчания.
//!
//! Параметр, которому в теле присваивают, остаётся переменной: умолчание признака -
//! "изменяемый", и не размеченный параметр обязан остаться свободной переменной.
...
/// Предикат с параметром проверяется - как с константой.
#[test]
...
/// Вердикт содержателен: `G Below` над тем же входом нарушается.
///
/// Без этой проверки соседний тест доказывал бы лишь то, что задача уложилась в потолок.
#[test]
...
/// Параметр и константа дают один вердикт - в этом предмет фичи.
#[test]
...
/// **Контроль:** параметр, которому присваивают, остаётся переменной.
///
