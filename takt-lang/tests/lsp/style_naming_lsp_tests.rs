...
//!    у одного вопроса, и здесь он был бы особенно тих (кто сверяет вывод
...
//!    Без него редактор здесь молчит, а `fmt` печатает.
...
///
/// Без этого редактор здесь молчит, и автор, правящий имя в файле, который ещё не
...
///
/// Фикстура несёт неиспользуемую переменную `Unused` (`SE-036`) **и** ошибку `SE-026`.
...
/// **Паритет с `fmt` на файле, который не собирается.**
///
