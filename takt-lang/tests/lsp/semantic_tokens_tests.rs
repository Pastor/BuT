...
//!
//! Продуктивный код не трогается (R9) - только тесты и исправлениетура.
...
fn decode(source: &str, tokens: &lsp_types::SemanticTokens) -> Vec<Tok> {
    ...
    for t in &tokens.data {
        ...
        // символам строки (для ASCII исправлениетуры совпадает с байтами).
        let text = lines
            .get(line as usize)
            .map(|l| {
                l.chars()
...
/// Зонд: печатает реальную классификацию токенов исправлениетуры (правило проекта - "сперва
