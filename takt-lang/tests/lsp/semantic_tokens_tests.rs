...
/// Каждая категория **идентификатора** получает свой
...
/// Не-идентификаторы получают свои типы - ключевое слово, число, комментарий,
/// оператор.
#[test]
...
/// Строковый литерал -> STRING. В самодостаточной модели строк нет, поэтому
...
/// Непарсящийся исходник -> токены отдаются, паники нет,
...
/// Пустой ввод -> пустой поток, без паники.
#[test]
fn semantic_tokens_empty_source() {
    assert!(semantic_tokens("").data.is_empty());
}

/// Длина кириллического идентификатора - в кодовых единицах UTF-16, не байтах.
#[test]
