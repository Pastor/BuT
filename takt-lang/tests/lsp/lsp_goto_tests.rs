...
/// Кросс-файловый переход к декларации.
///
/// Исправлениетуры - `tests/data/goto56/`, пара "угадывание сработало бы / не сработало бы".
...
mod goto_exact_file {
    ...
    //
    // Исправлениетуры - `tests/data/goto56/`, пара "угадывание сработало бы / не сработало
    ...
    /// диапазон накрывает смещение курсора. Без этой проверки правка исправлениетуры
...
mod goto_state_in_condition {
    ...
    //
    // Прежде здесь стояла исправлениетура `ref B: x = Done;` - сравнение переменной с именем
