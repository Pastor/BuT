...
//!
//! Тесты перехода к декларации: файл отделён от общих тестов LSP по предмету.
...
mod goto_exact_file {
    ...
    /// Каталог фикстур перехода к декларации.
    ...
    /// Узел под курсором - из **текущего** файла.
    ///
    ...
    /// Переход открывает импортированный файл на объявлении модели.
    #[test]
    ...
    /// Диапазон в чужом файле считается по **его** тексту.
    ///
    /// Смещения чужого файла к своему тексту не относятся: наложи их на текущий
    /// документ - получишь мусорный диапазон.
    #[test]
    ...
    /// Переход внутри своего файла не изменился - URI пуст.
    ///
    ...
    /// Угадывание удалено. Каталог целиком: слой LSP разделён на модули, и проверка
    ...
    /// Сервер зовёт кросс-файловый вариант.
    ///
    /// Иначе проверять нечего: код без потребителя молча гниёт, а весь кросс-файловый переход
...
///
/// Соседний случай: позицию использования получает `ConditionNode::Model` (`Ping`),
/// здесь - имя **состояния** в условии. Без разбора этой формы goto на нём отвечает `None` (узел
...
mod goto_state_in_condition {
    ...
    /// Кросс-модельный `S(Ping) = Done`, курсор на `Done` -> декларация в Ping.
    #[test]
    ...
    /// Курсор на `Ping` в `S(Ping)` - это ссылка на
    ...
    //
    // Фикстура `ref B: x = Done;` здесь не годится - сравнение переменной с именем
...
/// `ConditionNode::State` игнорирует use-site позицию в равенстве.
///
