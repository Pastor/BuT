...
//! класс сложности: обходы переведены с рекурсии на итерацию (падение на
//! 2800 состояниях), а потолок вершин `1_000_000` введён потому, что
...
/// честно отвечает `Unsupported`. Замер по такому входу мерил бы
...
fn data_model(vars: usize) -> String {
    ...
    for i in 0..vars {
        ...
        // замер по нему показывает 87.5 мс против 87.7 мс.
...
fn bench_data_properties(c: &mut Criterion) {
    ...
    //
    // Одна точка здесь не годится: она объясняла бы, почему
