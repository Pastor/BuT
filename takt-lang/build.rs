...
/// Строит каталоги сообщений (фича 0532) из `messages/*.txt`.
///
/// Языков **открытый список** (решение заказчика 2026-09-06): третий язык — это
...
/// отказом компиляции.
///
/// ⚠️ Паритет каталогов тут НЕ проверяется — это предмет гейта
